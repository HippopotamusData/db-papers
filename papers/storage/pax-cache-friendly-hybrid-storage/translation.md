...
Anastassia Ailamaki（Carnegie Mellon University，natassa@cs.cmu.edu）、David J. DeWitt（University of Wisconsin–Madison，dewitt@cs.wisc.edu）、Mark D. Hill（University of Wisconsin–Madison，markhill@cs.wisc.edu）、Marios Skounakis（University of Wisconsin–Madison，marios@cs.wisc.edu）

Anastassia Ailamaki 的这部分工作在 University of Wisconsin–Madison 任职期间完成。

原文许可声明允许免费复制本文全部或部分内容，但副本不得为直接商业利益而制作或分发，且须注明 VLDB 版权声明、出版物标题与日期，并说明复制已获 Very Large Data Base Endowment 许可；其他复制或再出版须付费和/或取得 Endowment 特别许可。本文发表于第 27 届 VLDB Conference（2001 年，意大利罗马）。
...
CPU 与二级存储之间的通信，即 I/O，传统上被视为数据库性能的主要瓶颈。为了优化与大容量存储之间的数据传输，关系 DBMS 长期使用 NSM，把记录组织在带槽磁盘页中。NSM 从每个磁盘页开头开始连续存储记录，并在页尾使用偏移量（slot）表定位每条记录的起始位置 [27]。

问题在于，大多数查询只使用每条记录的一小部分。为减少不必要的 I/O，研究者在 1985 年提出 DSM [10]。DSM 把一个 n 属性关系垂直划分为 n 个子关系，每个子关系只在对应属性被需要时访问。然而，涉及同一关系多个属性的查询必须花费大量额外时间把参与的子关系连接回来。除 Sybase-IQ [33] 外，当时通用关系 DBMS 基本使用 NSM 作为通用数据放置方法 [20,29,32]。

近期研究显示，现代数据库负载（例如决策支持系统和空间应用）经常受处理器和内存子系统相关延迟约束，而不再只是受 I/O 约束 [20,5,26]。在现代处理器上运行商业数据库系统时，缓存层次中未命中的数据请求是关键内存瓶颈 [1]。并且，从内存传入缓存的数据中只有一部分对查询有用，因为查询处理算法请求的数据项与内存到处理器之间的传输单位通常大小不同。把无用数据装入缓存会浪费带宽、污染缓存，并可能替换未来需要的信息，从而引入更多延迟。
...
我们在 Shore 存储管理器 [7] 上实现并评估 PAX、NSM 和 DSM，使用数值数据上的谓词选择查询以及 TPC-H 数据集上的多种查询。实验改变选择率、投影度、谓词数量、投影属性与谓词属性之间的距离以及关系宽度。结果显示，PAX 比 NSM 少 50-75% 的二级缓存数据未命中；范围选择和更新快 17-25%；涉及 I/O 的 TPC-H 查询快 11-42%。与 DSM 相比，PAX 更快，并且当查询涉及更多属性时性能保持稳定，而 DSM 由于记录重构成本上升而变慢。

PAX 的其他优势包括：在使用 NSM 的 DBMS 中实现 PAX 只需要修改页级数据操作代码；存储管理器可以按关系决定是否使用 PAX；PAX 同时具备垂直分区和页内局部性，因此有利于页级压缩 [13]；PAX 只作用在页级，因此可与 affinity graph-based partitioning [11] 等其他存储决策正交组合。

## 2. 相关工作

多项近期工作负载特征研究报告称，在现代平台上，数据库系统会遭受较高的内存相关处理器延迟；关于这些研究的详细综述见 [1,34]。已有研究大体一致认为，数据缓存未命中造成的停顿时间占总内存相关停顿时间的 50-70%（OLTP [19]）到 90%（DSS [1]）。即使在执行 OLTP 负载时指令缓存未命中率通常更高的架构上，数据缓存未命中仍然是主要停顿来源 [21]。

计算机体系结构、编译器和数据库系统领域都研究过面向缓存性能的数据放置优化。编译器导向方法会 profile 程序，并用启发式算法寻找优化缓存利用率的数据放置方案 [6]。程序员也可手工对基于指针的数据结构使用 clustering、compression、coloring 等技术来提升缓存性能 [8]。在 DBMS 中，属性聚簇能够改善压缩效果 [13] 和关系查询处理性能 [29]。
...
为了求值谓词，扫描算子 [14] 需要从关系中每条记录取出 `age` 属性。假设 NSM 页已经在主存中，且缓存块大小小于记录大小，那么扫描算子几乎每条记录都会产生一次缓存未命中。除了所需的 `age` 值，每次缓存未命中还会把记录中相邻的其他值带入缓存，浪费缓存空间并引入不必要的主存访问。
...
DSM 的问题在于：当查询涉及每个参与关系的多个属性时，性能会显著下降。系统必须基于 surrogate 连接参与的子关系来重构被分解的记录，连接耗时会随结果关系涉及的属性数增加而增加。另一类方法基于属性亲和图分区关系 [11]，但其性能高度依赖查询属性是否落在同一片段中，因此能力受限。
...
我们在 Shore 存储管理器 [7] 中实现 NSM、PAX 和 DSM。Shore 提供现代存储管理器的特性，包括 B-tree、R-tree [16]、ARIES 风格恢复 [22]、分层锁和带提示的 clock-hand buffer manager。通常 Shore 把记录存为连续字节序列。
...
Shore 支持把记录插入第一个能够容纳它的页，并不要求总是追加到关系末尾。NSM 插入算法先把各记录值拼成图 5 所示的字节序列，由 Shore 添加 tag 后存储。PAX 则计算每个属性值在页内的位置，存入该值，并相应更新 presence bitmap 与 offset array。

**更新。** NSM 使用 Shore 底层基础设施在记录内更新属性值。变长属性更新可能使记录伸缩，进而需要页重组和 slot table 更新；若记录增长超过页内空闲空间，则移动到其他页。PAX 通过计算属性值在相应 minipage 中的偏移来更新该值。变长属性更新只需在 V-minipage 内重组，移动的也只是被更新属性的值；如果新值长于 V-minipage 的可用空间，该 V-minipage 会向相邻 minipage 借用空间。若相邻 minipage 也没有足够空间，才把记录移到其他页。

**删除。** NSM 使用 slot array 标记删除记录，后续插入可填充空闲空间。PAX 在页首使用 bitmap 跟踪删除记录，并通过位运算判断记录是否已删。删除时，PAX 重组 minipage 内容以填补空洞，尽量减少会影响缓存利用率的碎片。计算属性值偏移时，系统把 record id 转换为页内 record index，并把已删除记录计入转换。对于删除密集型负载，可以只标记删除并延后重组，再通过周期性文件重组维持缓存性能；原文计划未来实现这一算法，并允许按文件选择更适合应用的方案。本文实验实现的是更彻底的即时重组方案。

### 4.3 查询算子

**扫描算子。** 我们在 Shore 上实现支持 sargable predicates 的 scan operator [28]。NSM 查询调用一个扫描算子，逐条读取记录并提取谓词相关属性。PAX 为查询涉及的每个属性调用一个扫描算子，每个算子顺序读取对应 minipage 中的值。对满足条件记录的投影属性通过计算偏移从对应 minipage 取出。DSM 则为谓词涉及的每个属性调用一个子关系扫描算子，生成满足条件的 record id 列表，再通过 record id 上的 B-tree 索引从对应子关系取回投影属性。

**连接算子。** 我们实现了 adaptive dynamic hash join [23]；DB2 也使用该算法 [20]。算法按连接属性把左表分区为主存哈希表；主存耗尽时，除一个 bucket 外其余 bucket 写到磁盘。随后以类似方式分区右表，并用右表连接属性动态探测左表在主存中的部分。算法只使用查询需要的属性构建包含子记录的哈希表。连接算子从两个扫描算子接收输入，每个扫描算子读取一个关系；输出还可通过作为参数传入的函数过滤。
...
实验平台是 Dell 6400 PII Xeon/MT，运行 Windows NT 4.0，处理器为 400MHz Pentium II Xeon，512MB 主存，100MHz 系统总线。处理器包含分离的 16KB L1 数据/指令缓存，以及统一的 512KB L2 缓存。两级缓存都是 non-blocking，缓存块大小 32 字节[^1]。我们使用 Xeon 的硬件计数器和既有方法 [1] 获得实验结果。

负载使用一个关系和如下范围选择查询 (1) 的变体：
...
该查询足以考察在顺序或按 record id 随机访问记录时，各数据布局的净效应。除非另有说明，关系 $R$ 包含 8 个 8 字节数值属性，并装入 120 万条记录。为使实验可预测且便于验证正确性，属性大小被选为每条 32 字节缓存行恰好容纳 4 个值，记录大小则使记录边界与缓存行边界对齐。实验改变投影度、选择谓词中的属性数、这些属性的相对位置以及记录属性数。谓词属性的取值沿用 TPC 决策支持基准 `Lineitem.l_partkey` 的数据偏斜与均匀分布 [15]。

PAX 的目标主要是优化数据缓存行为，并不改变 I/O 性能。在 I/O 延迟主导的负载中，PAX 与 NSM 的性能最终会收敛；PAX 保证的是数据页从磁盘可用之后的高数据缓存性能，并与其他 I/O 优化正交。因此，本节其余实验研究主存驻留关系上的数据放置影响。
...
PAX 减少数据访问相关缓存延迟，从而更快执行查询。L1 数据缓存 miss 的惩罚约为 10 个处理器周期，不显著影响总时间；L2 miss 每次代价约 70-80 cycles[^2]。PAX 将总体 L2 数据 miss 惩罚降低约 70%，因此总体处理器停顿时间下降约 75%。使用 NSM 时，内存相关惩罚占执行时间 22%；使用 PAX 时降为 10%。
...
本节在 TPC-H 决策支持负载上比较 PAX 与 NSM。决策支持应用通常是内存和计算密集型 [20]，关系一般不完全驻留主存，查询包含投影、选择、聚合和连接。结果显示，在该负载的所有 TPC-H 查询上，PAX 都优于 NSM。

### 6.1 设置与方法

实验仍使用第 5 节平台，buffer pool 为 128MB，hash join heap 为 64MB。负载包括 TPC-H 数据库和多种查询。数据库和查询使用 TPC 发布的 dbgen 与 qgen 生成；数据库含整数、浮点数、日期、定长字符串和变长字符串属性。实验包括：

- **Bulk-loading**：NSM 对每条插入记录执行一次内存间复制并顺序存储；PAX 的复制次数等于元组中的值数；DSM 创建并填充与属性数相同数量的关系。实验比较三种布局加载 100MB、200MB、500MB TPC-H 数据集的耗时，三种实现均开启完整恢复。
- **Range selections**：在最大表 `Lineitem` 上执行范围选择，并把投影属性写到输出关系，以尽量向系统施压。`Lineitem` 有 16 个不同类型的属性，其中 3 个是变长属性。表上没有索引，因为商用系统中的 TPC-H 实现大多使用聚簇索引，其访问行为与顺序扫描相似 [2]；实验改变投影度和谓词涉及的属性数。
- **TPC-H queries**：在 Shore 上实现 Q1、Q6、Q12、Q14。Q1 和 Q6 是带多个聚合和谓词的范围选择；实现把谓词下推到 Shore，对满足条件的记录计算聚合，并对结果分组和排序。Q12 和 Q14 是带额外谓词并计算条件聚合的等值连接；实现使用第 4 节所述 adaptive dynamic hash join。
- **Updates**：在 `Lineitem` 属性上执行如下更新 (2)，并改变更新字段数、谓词字段数和选择率：
...
其中 `a_p`、`a_q` 和 `b` 都是 `Lineitem` 的数值属性。对每个查询，实验既测量事务执行时间（包括把更新页刷新到磁盘），也测量实际主存更新时间，以确定更新算法对内存层次的影响。
...
第 5 节解释过，随着投影度增加、查询访问记录更大部分，PAX 的性能提升会下降。在 `Lineitem` 上的范围选择中，PAX 相对 NSM 的平均加速[^4]在 100MB、200MB、500MB 数据集上分别约为 14%、13%、10%。
...
NSM 和 PAX 的更新算法思想相同：尽量原地更新属性值，并在必要时重组。差异主要在变长属性更新。替换为更长变长值时，PAX 平均只需移动 V-minipage 一半数据；NSM 平均要移动页内一半数据，因为它移动的是包含无关属性的完整记录。较少情况下，两种方案都需要页重组。总体而言，大规模更新变长字段并不常见；TPC-C 唯一的更新查询只在 Payment 事务中更新客户信用历史，每次仅影响一条记录，且 history 字段最大长度为 500 字节。
...
缓存层次中的数据访问是现代数据库负载的重要性能瓶颈 [1]。商业 DBMS 使用 NSM 而不是 DSM 作为通用数据放置方法，是因为 DSM 常引入高记录重构成本。本文显示 NSM 会负面影响数据缓存性能，并提出 PAX，一种关系 DBMS 的新数据页布局。
...
PAX 相对 NSM 或 DSM 都没有存储惩罚。它只在每个页内重组记录，因此可与其他存储方案正交使用，并对 DBMS 其他部分透明。最后，由于每个 minipage 内类型一致、页内值域更小，页级压缩算法也可能在 PAX 上取得更好效果 [13]。
...
[^1]: 其他系统使用更大的缓存块，尤其是 L2 中的 64-128 字节。此时 PAX 的空间局部性会带来更高的缓存和带宽利用率；更大缓存块的结果见 [3]。
[^2]: Pentium 4 上相应延迟超过 300 个周期；原文注明来源为 Intel Corporation。
[^3]: Xeon 流水线可同时 issue（读取）3 条指令、execute 5 条此前周期 issue 的指令，并 retire（把结果写入寄存器和缓存）3 条此前 issue 的指令。
[^4]: 原文在此处以脚注给出文献 [17] 并定义该加速；为保证公式可读性，译文将公式展开在正文中。
...
32. http://technet.oracle.com/docs/products/oracle8i/doc_index.htm
