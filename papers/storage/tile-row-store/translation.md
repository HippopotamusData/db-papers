...
作者：Joy Arulraj、Andrew Pavlo、Prashanth Menon。

机构：Carnegie Mellon University；联系邮箱：`jarulraj@cs.cmu.edu`、`pavlo@cs.cmu.edu`、`pmenon@cs.cmu.edu`。

本文发表于 SIGMOD ’16（2016 年 6 月 26 日至 7 月 1 日，美国加利福尼亚州旧金山），DOI：<https://doi.org/10.1145/2882903.2915231>。
...
为了更具体地展示这些问题，我们做了一个动机实验。数据库只有表 $R(a _ 0, a _ 1, \ldots, a _ {500})$，包含 1000 万个元组。每个属性 $a _ k$ 都是区间 $[-100, 100]$ 中的随机整数。应用包含两种查询。事务查询 Q1 向表 R 插入一个元组：

```sql
INSERT INTO r VALUES (a0, a1, ..., a500);
```

分析查询 Q2 从表 R 中满足 $a _ 0 \lt \delta$ 的所有元组投影出属性 $a _ 1, a _ 2, \ldots, a _ k$：
...
tile-based DBMS 的主索引和二级索引都可以使用任何保序主存索引，例如 B+tree。索引 key 由元组属性组成，value 是最新元组版本的逻辑位置。系统不存裸指针，因为重组 tile group 布局时必须更新这些指针。若算子通过索引遇到对当前事务不可见的元组版本，就沿 `PreV` 字段遍历版本链，找到对该事务可见的最新版本。这样 DBMS 不必在表和索引中重复维护版本信息。
...
考虑包含 $m$ 条查询的查询负载 $Q$，以及一个包含 $n$ 个属性的表。对于每个查询 $q \in Q$，朴素分区算法都需要检查 $2^n$ 个属性组，随后分析该表所有可能的属性划分；这类划分的数量称为 Bell number。根据 Bell number 的渐近界，该算法的复杂度为 [19, 23]：

$$
\mathcal{O}(e^{n \ln n} + mn2^n), \quad \Theta(2^n)
$$

cluster 均值会随时间向近期样本漂移。若 $c _ j$ 是第 j 个 cluster 的代表查询， $c _ 0$ 是初始均值，老样本权重为 $w$，加入 $s$ 个样本后当前均值为：

$$
c _ j = (1-w)^s c _ 0 + w \sum _ {i=1}^{s}(1-w)^{s-i}Q _ i
$$

权重 $w$ 控制算法遗忘旧查询样本的速度。每轮时间复杂度为 $\mathcal{O}(mnk)$，空间复杂度为 $\mathcal{O}(n(m+k))$，比朴素分区算法更高效。得到 representative queries 后，系统用贪心算法生成表布局：按 cluster 权重降序遍历 representative queries，把该查询访问的属性组合到同一 tile，直到所有属性都分配到某个 tile。
...
ADAPT 数据库含一张窄表和一张宽表。每张表的元组都有主键 $a _ 0$ 和 $p$ 个 4 byte 整数属性 $a _ 1, \ldots, a _ p$。窄表 $p=50$，元组大小约 200 B；宽表 $p=500$，元组约 2 KB。所有实验都先向每张表装载 1000 万个元组。
...
本附录形式化定义 logical tile algebra 算子的语义。对每个算子，均给出输入参数与输出的类型，以及从输入到输出的代数变换。记可含重复元组的多重集为 $\langle\ldots\rangle$，所有元素唯一的集合为 $\lbrace\ldots\rbrace$， $\lvert S\rvert$ 是多重集或集合 $S$ 的大小。表中 $\mathrm{LT}$ 表示 logical tile， $\mathrm{PT}$ 表示 physical tile， $T$ 表示表， $I$ 表示索引。

表 1：Logical Tile Algebra。每个算子给出类别、输入参数、输出及代数变换。

| 类别 | 算子 | 输入参数 | 输出 / 代数变换 |
| --- | --- | --- | --- |
| Bridge Operators | SEQUENTIAL SCAN | $T\thinspace X$，谓词 $P$ | $\lbrace\mathrm{LT}\rbrace \equiv \lbrace\mathrm{LT}\thinspace w \mid w \equiv \langle x \mid x \in X \land P(x)\rangle\rbrace$ |
| Bridge Operators | INDEX SCAN | $I\thinspace X$，谓词 $P$ | $\lbrace\mathrm{LT}\rbrace \equiv \lbrace\mathrm{LT}\thinspace w \mid w \equiv \langle x \mid x \in X \land P(x)\rangle\rbrace$ |
| Bridge Operators | MATERIALIZE | $\mathrm{LT}\thinspace X$ | $\mathrm{PT}\thinspace Y$ |
| Metadata Operators | SELECTION | $\mathrm{LT}\thinspace X$，谓词 $P$ | $\mathrm{LT} \equiv \langle x \mid x \in X \land P(x)\rangle$ |
| Metadata Operators | PROJECTION | $\mathrm{LT}\thinspace X$，属性 $C$ | $\mathrm{LT} \equiv X' \mid \mathrm{schema}(X') = C$ |
| Mutators | INSERT | $T\thinspace X$， $\mathrm{LT}\thinspace Y$ | $T \equiv X \Rightarrow X \lor Y _ p$ |
| Mutators | DELETE | $T\thinspace X$， $\mathrm{LT}\thinspace Y$ | $T \equiv X \Rightarrow X \setminus Y _ p$ |
| Mutators | UPDATE | $T\thinspace X$，属性 $C$，表达式 $E$ | $T \equiv X \Rightarrow (X \setminus Y _ p) \lor Z _ p$ |
| Pipeline Breakers | JOIN | $\mathrm{LT}\thinspace X$， $\mathrm{LT}\thinspace Y$，谓词 $P$ | $\mathrm{LT} \equiv \langle x \mathbin{\Vert} y \mid x \in X \land y \in Y \land P(x,y)\rangle$ |
| Pipeline Breakers | UNION | $\mathrm{LT}\thinspace X$， $\mathrm{LT}\thinspace Y$ | $\mathrm{LT} \equiv \lbrace z \mid z \in X \lor z \in Y\rbrace$ |
| Pipeline Breakers | INTERSECTION | $\mathrm{LT}\thinspace X$， $\mathrm{LT}\thinspace Y$ | $\mathrm{LT} \equiv \lbrace z \mid z \in X \land z \in Y\rbrace$ |
| Pipeline Breakers | DIFFERENCE | $\mathrm{LT}\thinspace X$， $\mathrm{LT}\thinspace Y$ | $\mathrm{LT} \equiv \lbrace z \mid z \in X \land z \notin Y\rbrace$ |
| Pipeline Breakers | UNIQUE | $\mathrm{LT}\thinspace X$ | $\mathrm{LT} \equiv \lbrace z \mid z \in X\rbrace$ |
| Pipeline Breakers | COUNT | $\mathrm{LT}\thinspace X$ | $\mathrm{int} \equiv \lvert X\rvert$ |
...
**Mutators。** 这些算子直接修改表数据，故表 1 用 $\Rightarrow$ 表示变换。Insert 接收 logical tile $Y$，将其表示的元组 $Y_p$ 追加到表 $X$。Delete 从 $X$ 删除 $Y_p$。Update 先把 $X$ 中的 $Y_p$ 标记为死亡，再在 $Y_p$ 上计算属性 $C$ 对应的表达式 $E$，构造新版本 $Z_p$，最后将 $Z_p$ 追加到表。Mutator 与事务存储管理器协作，控制元组生命周期。

**Pipeline breakers。** Join 接收 $X$、 $Y$，在它们上计算 $P(x,y)$。表 1 中的 $\mathbin{\Vert}$ 表示把匹配元组拼接起来，而 $\mid$ 表示后续条件。Join 先拼接 $X$ 和 $Y$ 的 schema 构造输出，再遍历每一对元组，将满足 $P$ 的元组对拼接并追加。图 14 中输出 $Z$ 就是拼接 $X$ 与 $Y$ 中匹配元组对所得。

集合算子同样是 pipeline breaker。例如 union 在遍历子节点的所有输入 logical tile 时记录所观察元组，最后把重复元组标记为不可见后返回这些 tile。Count、sum 等聚合算子检查子算子的全部 logical tile，构造聚合元组。它们与集合算子的区别是，会构造新 physical tile 存聚合结果，再构造表中用 $[\ldots]$ 表示的 passthrough logical tile 包裹新 tile，每次一个 logical tile 地向计划树上层传递。
