...
Gui Huang、Xuntao Cheng、Jianying Wang、Yujie Wang、Dengcheng He、Tieying Zhang、Feifei Li、Sheng Wang、Wei Cao、Qiang Li（Alibaba Group）
...
![图 9：MySQL 使用三种存储引擎处理不同点查询、范围扫描、更新、插入比例时的吞吐；三个子图 y 轴相同。](assets/xengine-fig09-ecommerce-workload-throughput.png)
...
![图 12：经 SQL 接口访问时，三种存储引擎在点查询、插入和更新上的吞吐。](assets/xengine-fig12-sql-throughput.png)

![图 13：经 SQL 接口访问时，三种存储引擎在点查询、插入和更新上的响应时间。](assets/xengine-fig13-sql-response-time.png)
...
![图 17：处理电子商务负载时，X-Engine 在无/有增量缓存替换情况下的 block cache 命中率和 QPS；两图坐标轴相同。](assets/xengine-fig17-cache-hit-rates.png)
