...
¹ 慕尼黑工业大学；² Snowflake Computing；³ 荷兰国家数学与计算机科学研究中心（Centrum Wiskunde & Informatica）
...
近年出现了一类专门面向 OLAP 工作负载的新数据库架构。这些系统以压缩列式格式存储数据，并通过提升 CPU 执行效率，相比传统行存数据库获得超过一个数量级的查询性能提升。这种效率提升通常来自向量化执行：不再逐元组解释查询表达式，而是对一批值执行操作。块级操作的虚函数一次处理数千个元组，降低了解释开销；函数内部针对块的循环也能受益于循环优化和自动 SIMD 指令生成。IBM BLU [30]、SQL Server Column Store Index [18]、SAP HANA [12] 和 Vectorwise [39] 都属于这一类。

另一种近期用于加速查询求值的方法是把 SQL 查询 JIT 编译为可执行代码。它完全避免查询解释及其开销。Drill、HyPer [16, 25] 和 Impala [34] 都使用了 JIT。
...
**图 1：本文提出的新型 Data Block 格式既允许在压缩数据上高效扫描和点查，也通过向量化解决了在编译式逐元组查询引擎中集成多种存储布局组合的难题。**
...
一种做法是把关系划分为读优化分区和写优化分区 [17]。更新只发生在写优化分区中，然后周期性地合并到读优化的压缩分区。但合并过程复杂度为 $O(n)$，其中 $n$ 是关系基数，并且需要重新压缩整个关系，因此非常昂贵。本文提出另一种策略：在 HyPer 中，关系被划分为固定大小的 chunk；当某个 chunk 被识别为冷数据后，就按该 chunk 的最优压缩方案独立压缩为只读不可变 Data Block。一个 chunk 被打包为 Data Block 后，内部数据被冻结。更新仍然可行，但会把冷记录置为无效，并把新版本插入热区；内部等价于删除后插入。

为了通过元组位置高效访问单条记录，Data Blocks 只采用轻量压缩方案。许多 OLAP 系统使用 bit-packing 提高压缩率，但本文有意避免。实验表明，即使使用近年的 SIMD bit-packed 算法 [27]，扫描也只有在早期过滤极端选择性、或整个块全部命中时才很快；若早期过滤得到稀疏位置集合，bit-unpacking 扫描列的代价会掩盖早期过滤收益。Data Blocks 的设计目标是让 OLTP 的压缩数据位置访问保持低成本，同时让扫描型 OLAP 负载受益于早期过滤。

为加速 Data Block 扫描，我们提出 Small Materialized Aggregates [23] 的一种新型“位置式”变体 Positional SMA。PSMA 是一种轻量索引，即使块无法依据最小值/最大值完全跳过，也可以缩小块内扫描范围。

![子图 (a)：每个块中的每一列都选择最优压缩技术。](assets/figure-02a-compression-techniques.png)

![子图 (b)：Data Blocks 不仅存储属性的最小值和最大值以跳过整个块，还存储称为 Positional SMA（PSMA）的轻量索引，按属性缩小扫描范围。](assets/figure-02b-positional-sma-overview.png)

**图 2：压缩 Data Blocks 概览。**
...
但按 chunk 选择不同压缩方案会给 JIT 编译的逐元组引擎带来挑战：每个压缩块可能有不同内存布局，扫描所需编译代码路径数量会指数级增长，导致临时查询和事务的编译时间不可接受。此时向量化扫描的优势出现了：扫描可以保持解释式并预编译；向量化扫描还易于利用 SIMD，并可表达自适应算法。我们最终展示了如何将 JIT 编译与向量化融合：解释型向量化扫描子系统把结果送入 JIT 编译的逐元组查询管线。对于向量化谓词求值，我们还给出了新的 SSE/AVX2 SIMD 算法。总体而言，我们表明，这种新型查询引擎与 Data Blocks 格式结合后，可以在大幅节省主内存的同时，仍支持最高性能的 OLTP 与 OLAP。

## 2. 相关工作

与 Data Blocks 最接近的是 IBM DB2 BLU [4, 30] 的存储格式。BLU 也把一段元组的所有列以压缩列式形式放在块中，这与 PAX [3] 概念类似，并且能在扫描中早期求值范围过滤。主要差异在于：HyPer 避免 bit-packing 以保持位置访问快速，并引入 PSMA 进一步提升早期选择求值。

近年关于高级 bit-packing [22, 13] 及其 SIMD 实现 [27] 的研究强调早期过滤收益，但往往忽略 bit-unpacking 的高逐元组成本，或把这类格式定位为辅助存储结构。本文选择字节对齐存储，主要是为了避免这一惩罚，使早期过滤能在更广泛的查询负载中受益。

Vectorwise [39] 提出把数据 chunk 解压到 CPU cache 中并在缓存中处理。Vectorwise 不在扫描中做早期过滤，而是完全解压所有扫描列范围，因为它认为位置式解压过于低效。HyPer 的高效位置访问则允许在中等选择性场景下也受益于扫描内部 SARGable 谓词的早期求值。取决于选择性，HyPer 的早期过滤可比 Vectorwise 快数倍；代价是字节对齐压缩平均增加不超过 1.25 倍空间。

Abadi 等人 [1] 系统评估了列式数据库中的压缩方案，结论是轻量压缩优于重量压缩，因为可以直接在压缩数据上操作。本文只考虑压缩数据上的过滤操作和点查，不把压缩数据传递给连接等更复杂算子。Oracle Database [28] 使用块级压缩，但压缩方案较少，也不是为现代 CPU 上的高效处理设计。SAP HANA [12] 面向混合负载优化，但采用另一条路线：整体关系优化内存与扫描性能，更新发生在单独分区后周期性合并 [17]。Microsoft Hekaton [10] 的 Siberia 框架 [11] 用于管理冷数据，但主要通过把冷数据驱逐到磁盘来降低 RAM 使用，与 Data Blocks 不同。

关于 JIT 与向量化结合，Vectorwise 曾尝试 JIT 编译投影和 hash join [31]，但收益有限，随后也一直仅作为实验特性存在。Impala [34] 的 JIT 方式也不同于 HyPer 的逐元组 JIT。Impala 为每个物理算子提供 C++ 模板类，预先编译为 LLVM [19] 中间代码；查询编译时只编译处理元组和表达式的方法并链接到模板类中。不同算子没有融合，算子之间仍通过 tuple buffer 通信，因此某种意义上仍是向量化算子。这样做丢失了 JIT 的主要优势：减少指令数、通过寄存器传递数据、避免 load/store；其唯一真正的收益是模板化执行引擎更易理解和测试。向量化则方便使用 SIMD，在数据库中已用于选择扫描 [37, 35, 26]、bit unpacking [35, 27]、批量加载 [24]、排序 [6] 和图上的广度优先遍历 [32] 等。
...
Data Blocks 还面向未来的 NVRAM 和字节可寻址 flash 设计 [8, 33]。存放在这类设备上的 Data Block 可以直接寻址和读取，无需把整个块读入 RAM。

在 HyPer 中，Data Blocks 用作冷数据的压缩内存存储格式，也用于持久化。识别冷 chunk 是正交问题，已有工作对此进行了研究 [15, 9]。一旦记录被打包到 Data Block 中，内部数据不可变；只允许删除操作，即用标志标记冷记录。更新会转换为删除冷记录并向关系的热未压缩 chunk 插入新记录。

Data Blocks 具有如下性质：

- 每个 chunk 中每个属性根据实际值分布选择最优压缩方法，因此能获得较高压缩率。
- 只使用字节可寻址压缩格式，拒绝 BitWeaving [22] 等 sub-byte 编码，以支持 OLTP 和 OLAP 所需的高效点查。
- SARGable 扫描限制，如 `=`, `is`, `<`, `<=`, `>`, `>=`, `between`，可在压缩表示上通过 SIMD 方法求匹配。Data Block 中多数压缩数据为 1、2 或 4 字节整数，因此 SIMD 加速甚至可能高于未压缩数据。
- Data Blocks 包含每个属性的 Small Materialized Aggregates [23]（SMA），即最小值和最大值，可用于扫描时跳过整个 Data Block。
...
Data Block 的第一个值是记录数。通常一个 Data Block 最多存储 $2^{16}$ 条记录。记录数之后是每个属性的信息：所用压缩方法，以及到该属性 SMA [23]、字典、压缩数据向量和字符串数据的偏移。属性信息后面是实际数据，从第一个属性的 SMA 和 PSMA 开始。由于 Data Blocks 用列式布局存储数据，但同一个块中可以包含一个元组的所有属性，因此它类似于 PAX [3] 存储格式。
...
**图 3：包含 $n$ 个属性的 Data Block 布局。**

### 3.2 Positional SMAs

Data Blocks 为每个属性保存 SMA。基础 SMA 是该属性在块中的最小值和最大值。类似 ORC 文件格式 [14]，HyPer 使用这些域知识在 SARGable 谓词不落入块的 min/max 范围时排除整个 Data Block。min/max SMA 对有序属性效果尤其好，例如日期或递增 key。若 TPC-H 的 `lineitem` 按 `l_shipdate` 排序，Q6 对 `l_shipdate` 的高选择性谓词就只需访问少量 Data Blocks。
...
为进一步缩小扫描范围，我们扩展每个属性的基础 min/max 信息，加入 PSMA。PSMA 内部是一个紧凑查找表，在冷 chunk 冻结为 Data Block 时计算。每个表项包含一个扫描范围 $[b,e)$，指向 Data Block 压缩数据中可能匹配的元组。对固定大小类型，查找表为数据类型每个字节提供 $2^8$ 个表项。例如 4 字节整数属性的查找表包含 $4 \times 2^8=1024$ 项。

![图 4：Positional SMA 查找表示例。查询值先转成相对最小值的 delta，再映射到查找表项和扫描范围。](assets/figure-04-psma-lookup.png)

**图 4：探测 Positional SMA 索引（PSMA）查找表的示例。**

多个值可能映射到同一个查找表项。若 $n$ 个值映射到索引 $i$，且对应范围为 $[b _ 0,e _ 0),\ldots,[b _ {n-1},e _ {n-1})$，则表项 $i$ 存储合并后的范围：

$$
\left[\min _ {0\le j\lt n} b _ j,\ \max _ {0\le j\lt n} e _ j\right)
$$
...
等值查找过程如下：先计算探测值 $v$ 与 SMA 最小值之间的差值 $\Delta=v-\min$；然后以 $\bar{\Delta}$ 表示 $\Delta$ 的最高非零字节，以 $r$ 表示剩余字节数，计算查找表索引：

$$
i=\bar{\Delta}+r\times256
$$

等值谓词只需一次查表。对于 `between a and b` 这样的非等值谓词，HyPer 分别求边界 $a$、 $b$ 的查找表索引 $i _ a$、 $i _ b$，并合并其间非空表项的范围：

$$
\mathrm{range}=\left[\min _ {i _ a\le i\le i _ b} b _ i,\ \max _ {i _ a\le i\le i _ b} e _ i\right)
$$

构建查找表是 $O(n)$ 操作。先初始化空范围，然后扫描列 $C=(v _ 0,\ldots,v _ {n-1})$，对每个值 $v _ i$ 找到对应表项；如果表项为空，则设置为 $[i,i+1)$，否则把范围结束位置更新为 $i+1$。每个属性单独保存查找表。若多个属性上存在 SARGable 谓词，分别查询其查找表并求返回范围交集，可进一步缩小扫描范围。

PSMA 空间占用很小。每个表项是两个 4 字节无符号整数。当索引 $n$ 字节值时，查找表共有 $n\times2^8$ 项，因此 1、2、4 字节整数的典型占用分别为 2 KB、4 KB、8 KB。它远小于针对一个 $2^{16}$ 值 Data Block 的树形索引。由于 PSMA 只是限制扫描范围，访问路径仍与全扫描相同，因此相比传统索引查找更稳健；即使潜在命中范围很大，也不会带来明显惩罚。
...
Data Blocks 的压缩方案首要要求是压缩数据仍保持字节可寻址，以支持高效点查。sub-byte 编码（例如 BitWeaving [22]）可获得更高压缩率，但会让低选择性扫描和点查成本提升数个数量级。我们认为以下方案在压缩率、扫描性能和点查性能之间较适合 Data Blocks：
...
由于 Data Blocks 不可变，有序字典压缩无需支持新值插入或更新。这使 HyPer 可以使用保持顺序的字典压缩：若未压缩值 $k \lt k'$，则对应字典编码也满足 $d _ k \lt d _ {k'}$。不可变性也免除了引用计数和空闲空间管理。字典 key 的字节宽度根据不同 key 数量选择：最多 256 个 key 使用 8 位整数，最多 $2^{16}$ 个使用 16 位，最多 $2^{32}$ 个使用 32 位。保持顺序和使用字节截断 key 的优点是可以直接用 SIMD 优化算法在压缩数据上找匹配；由于只需比较截断值，可能比未压缩数据更快。

截断压缩通过计算每个值与属性最小值之间的 $\Delta$ 来降低内存占用。给定未压缩数据 $A=(a_0,\ldots,a_m)$，其压缩数据为：

$$
\Delta(A)=\left(a_0-\min(A),\ldots,a_m-\min(A)\right)
$$
...
块级压缩也有缺点。例如字典压缩字符串时，相同字符串若出现在多个 chunk 中，需要存入多个字典，带来冗余。但块级压缩可为每个块的每列选择最适合的方案，从而摊薄这类开销；取决于值分布，它甚至可能比整体压缩关系获得更好的压缩率。系统还可根据 OLTP 访问频率在压缩和未压缩表示间迁移 chunk。
...
HyPer 使用以数据为中心的编译（data-centric compilation），把关系代数树通过 LLVM 编译成高效本地机器码。常见查询从 LLVM IR 到优化后机器码的编译时间通常为毫秒级。与传统执行模型不同，HyPer 为整个查询管线生成代码。一个管线从物化状态加载元组，执行无需中间物化的所有算子逻辑，最后把输出物化到下一个 pipeline breaker。扫描是向初始查询管线供给数据的叶算子。与传统解释执行相比，元组不是从输入算子逐级拉取，而是被推送给消费算子。

为便于阅读，下面以 C++ 而非 LLVM IR 展示：当关系采用未压缩列式格式时，抽取两个属性的扫描代码如下。
...
为简化示例，这里省略了多版本并发控制检查。若要对不同压缩技术形成的存储布局组合执行同一扫描，第一种办法是在每个被抽取的属性上设置跳转表，跳到相应的解压方法：
...
虽然跳转表分支的结果在一个 chunk 内保持不变，CPU 通常能够正确预测，分支未命中不会很多；但新增指令仍会增加扫描最热内层循环的延迟，实践中使扫描代码比无这些分支时几乎慢 3 倍。

另一种办法是不在内层循环添加分支，而是“展开”所有存储布局组合，为每种组合生成代码，并对每个 chunk 用 computed goto 选择相应的扫描代码：
...
然而，展开组合要求查询引擎为每种实际使用的存储布局生成代码路径。若 $n$ 个属性各有 $p$ 种表示，路径数便是 $p^n$；例如仅两个属性、六种表示就会生成 36 条代码路径。即使并非所有组合都会实际出现，少量组合也会显著增加代码体积和编译时间。本文提出的压缩 Data Blocks 在 TPC-H SF100 的 `lineitem` 关系上就使用了 50 多种不同布局。图 5 展示了一个扫描 8 个属性的简单 `select *` 查询，在基础关系存储布局组合数量变化时的编译时间。

![图 5：扫描 8 个属性时，基础关系存储布局组合数量增加会使 JIT 生成代码的编译时间快速上升；解释型向量化扫描避免了这类代码路径爆炸。](assets/figure-05-compile-times.png)

**图 5：在 HyPer 中，扫描 8 个属性的查询计划随基础关系存储布局组合数量变化的编译时间。**

鉴于编译时间会爆炸，我们转而调用预编译的解释型向量化扫描代码，每次处理例如 8K 个元组；返回的元组再由生成代码逐个消费并推入后续算子：
...
使用这种预编译的解释型向量化扫描后，无论扫描多少种存储布局组合，编译时间都能保持较低。SARGable 谓词还可以下推到扫描算子中，对元组向量求值。

### 4.1 在 HyPer 中的集成

本文的 JIT 编译查询引擎集成在完整的主内存数据库系统 HyPer 中；HyPer 支持 SQL-92+ 查询处理和 ACID 事务。HyPer 的向量化扫描在热未压缩 chunk 和压缩 Data Blocks 上共享同一接口；JIT 编译查询管线不感知底层存储布局。执行过程如下：
...
**图 6：Data Blocks 在查询引擎中的集成。左侧对 Data Blocks 和未压缩 chunk 的向量化扫描共享同一接口，并用 SSE/AVX2 SIMD 指令对记录向量求值 SARGable 谓词；匹配项逐元组推入查询管线。右侧原有 JIT 编译扫描则在查询管线中求值谓词。**
...
虽然向量化扫描会复制更多数据，但评估显示大部分情况下复制成本可忽略，向量化谓词求值可超过逐元组求值。TPC-H Q1 和 Q6 代表两个极端。Q1 中大多数元组满足扫描限制，向量化扫描几乎复制所有扫描数据，因此性能恶化、运行时间增加近 50%。需要说明的是，在没有谓词时，若一个向量中的所有元组都命中，HyPer 的向量化扫描会用一种避免复制数据的优化，性能不会下降；但受限属性的值均匀分布，谓词下推为 SARG 时该优化无法奏效。Q6 中则只有小比例元组命中，未压缩数据上的向量化谓词求值最多可比 JIT 扫描快 2.3 倍。使用压缩 Data Blocks 后，Q6 运行时间提升 6.7 倍，22 个 TPC-H 查询的几何平均运行时间提升 1.27 倍。

### 4.2 使用 SIMD 指令查找匹配项

向量化处理允许用 SIMD 指令求值 SARGable 谓词。求值阶段会把命中元素的位置或偏移存入向量，本文称为 match positions；这一概念也称 selection vector [5]。若对属性向量 $A^T=(0,1,5,2,3,1)$ 应用谓词 $P(a):3\le a\le5$，得到的匹配位置为 $M^T=(2,4)$。
...
我们使用预计算表把 bit-mask 映射到位置，`movemask` 指令提供表偏移。每个表项对应一种 $n$ 路 SIMD 比较结果。例如 8 路比较中从左向右计数的第 0、3、4、6 个 lane 命中，相应 lane 的全部位为 1，其余为 0；`movemask` 抽取每个 lane 的最高位，得到 $10011010 _ 2=154 _ {10}$。以该值作为查表偏移可取得本地匹配位置 $(0,3,4,6)$，再加上全局扫描位置后写入 match vector。

每个 SIMD lane 都可能命中或不命中，因此 $n$ 路比较需要覆盖 $2^n$ 种结果，表可能极大。例如用 256 位 AVX2 寄存器并行比较 32 个 8 位整数时，完整表会有 $2^{32}$ 项，约 512 GB。我们把表限制为 $2^8$ 项，必要时执行多次查表。每个表项最多保存 8 个匹配位置，每个位置为 32 位无符号整数，整个表大小为 $2^8\times8\times4 \mathrm{B}=8 \mathrm{KB}$，足以放入现代 Intel CPU 的 32 KB L1 cache。每个表项还保存匹配元素数（图中未画出）；该数值存放在低位，以减小内存占用并避免 cache pollution。因此，抽取实际位置时需要对预计算位置向量额外右移，而匹配数则用于递增全局 match vector 的写偏移。实现细节见附录 C。

额外限制的应用与初始匹配不同：访问元素不再连续，因为只读取已满足第一个谓词的元素；且已有 match vector 需要收缩。我们仍使用类似方法，但从相应内存位置直接 gather 到 SIMD 寄存器，查找表项则用作 shuffle 控制掩码。图 7b 中，谓词作用于位置 $(17,18,20,21,25,26,29,31)$ 的属性，SIMD lane $(0,2,4,5)$ 命中；匹配位置按表项重排，最终把 $(17,20,25,26,-,-,-,-)$ 写回 match vector，并按 4 个匹配元素递增写偏移。破折号表示无关值，将在下一次迭代中被覆盖。

![子图 (a)：初始 SIMD 限制求值得到初始匹配位置向量；掩码对应的位置预存在查找表中，以避免昂贵计算。](assets/figure-07a-initial-simd-restriction.png)

![子图 (b)：追加限制缩减初始匹配向量；查找表项用作 shuffle 控制掩码。](assets/figure-07b-additional-simd-restriction.png)

**图 7：使用 SSE/AVX2 SIMD 指令在压缩与未压缩数据上求值谓词。**

gather 指令只支持 32 位和 64 位类型。对 8 位和 16 位值，我们也使用 32 位 gather，因此并行度分别降低 2 倍或 4 倍。在 Haswell 桌面 CPU 的微基准中，between 谓词、20% 选择性时，SSE 加速接近 4 倍，AVX2 超过 5 倍。底层类型位宽越小，并行度和收益越高；64 位整数上 AVX2 约 1.5 倍，SSE 并行度不足以带来明显收益。

初始匹配实现由于预计算位置表，对选择性变化不敏感；性能主要只随写入 position vector 的匹配位置数变化。本实验一次处理的元组数固定为 16K，因此写入数量没有显著性能影响。收缩 match vector 的实现也对追加谓词本身的选择性不敏感，但因非连续访问模式而高度依赖前置谓词选择性；实验中的匹配元组均匀分布。对最多 32 位整数，随着前置谓词选择性提高，AVX2 相比无分支标量代码可获得 1.0 到 1.25 倍收益；64 位整数上，高选择性时 SIMD 甚至可能更慢。由于 8 位和 16 位值缺乏原生 gather 指令，其他底层数据类型的性能收益几乎相同。真正关键的是命中元组的分布：即使谓词选择性很高，只要数据具有自然顺序，也可能得到 1.25 倍加速。

![图 8：选择性为 20% 时，SIMD 谓词求值在不同整数位宽上的加速比。](assets/figure-08-simd-speedup.png)

**图 8：选择性为 20% 时， $l \le A \le r$ 型 SIMD 谓词求值的加速比。**

![图 9：第二个谓词选择性固定为 40% 时，前置谓词选择性对追加限制求值成本的影响。](assets/figure-09-additional-restriction-selectivity.png)

**图 9：第二个谓词选择性固定为 40% 时，追加限制在不同首个谓词选择性下的表现。**

这些 SIMD 算法只适用于整数数据。其他数据类型和非 SARGable 谓词回退到标量实现。不过 Data Blocks 会把数据压缩为字节可寻址整数，包括字符串属性，因此当 OLAP 负载访问大量 Data Blocks 时能显著受益。

## 5. 评估

我们在 HyPer 查询引擎中评估了解释型向量化扫描、SIMD 优化谓词求值和压缩 Data Blocks。该查询引擎属于完整的主内存数据库系统 HyPer，后者支持 SQL-92 查询和 ACID 事务。实验还比较了字节可寻址压缩与 SIMD 优化水平 bit-packing，以解释为何拒绝 sub-byte 编码。

除特别说明外，实验运行在一台 4 路 Intel Xeon X7560 NUMA 机器上：主频 2.27 GHz（最高睿频 2.67 GHz），每颗 CPU 有 8 核、16 个硬件上下文和 24 MB 共享 L3 cache，机器配备 1 TB DDR3 主内存（每颗 CPU 256 GB），运行 Linux 3.19。针对 AVX2 ISA 的实验使用 Intel Haswell i5-4670T：主频 2.3 GHz（最高睿频 3.3 GHz），16 GB DDR3 内存和 6 MB 共享 L3 cache。

### 5.1 压缩率

我们使用 TPC-H scale factor 100、IMDB 最大关系 `cast_info`，以及 1987 年 10 月至 2008 年 4 月美国所有商业航班的到达/起飞明细数据集评估压缩率（数据来源：`http://stat-computing.org/dataexpo/2009/`）。Vectorwise 是 MonetDB/X100 [5] 的商业版本，其轻量压缩方案通过称为 patching 的技术处理不可压缩值（离群值）；PFOR（Frame of Reference）、PFOR-DELTA（基于 PFOR 的 delta 编码）和 PDICT（字典编码）中的 “P” 即指 patching [40, 38]，适用处还使用 run-length encoding。相比 HyPer，Vectorwise 平均节省约 25% 更多空间，但查询性能较差。Vectorwise 的压缩方案主要面向数据库不能放入内存时加速磁盘加载；内存驻留压缩数据的处理速度虽可接受，却可能慢于未压缩数据 [2]。在 SF10 上，Vectorwise 压缩存储执行 TPC-H Q1 和 Q6 分别比未压缩存储慢 18% 和 38%。Vectorwise 的点访问总是以扫描完成，只是扫描范围会由索引缩小。

**表 1：TPC-H、IMDB `cast_info` 与航班明细数据库在 HyPer 和 Vectorwise 中的空间占用。**
...
Data Blocks 则假设大部分数据在主内存中，性能目标是压缩后查询和事务处理至少不慢于未压缩内存存储。图 10 考察从 $2^{11}$ 条记录到默认 $2^{16}$ 条记录的不同块大小。实验显示，Data Block 太小时，块元数据开销会恶化压缩率；默认每块 $2^{16}$ 条记录在压缩率和查询处理速度之间取得良好平衡，因为多数值仍能压缩为小整数类型。

![图 10：Data Block 大小变化时，TPC-H、IMDB cast_info 和航班数据集的压缩率。](assets/figure-10-compression-ratio.png)

**图 10：TPC-H（scale factor 100）、IMDB `cast_info` 和航班到达/起飞明细数据集在不同 Data Block 大小下的压缩率（相对于未压缩存储）。**

### 5.2 查询性能

TPC-H scale factor 100 实验比较了 HyPer 中未压缩存储上的 JIT 扫描、未压缩存储上的向量化扫描，以及压缩 Data Block 上的向量化扫描。64 个硬件线程参与，运行时间取多次测量中位数。

结果表明，用向量化扫描替代 JIT 扫描不会显著改变查询性能；把 SARGable 谓词下推到向量化扫描也类似。但向量化扫描的编译时间几乎只有 JIT 扫描代码的一半。仅使用 Data Blocks 的压缩方面时，查询性能也基本不变。若把 SARGable 谓词和 SMA 下推到 Data Block 扫描中，TPC-H 几何平均运行时间提升约 26%。本文的 SARGing 实现按设计依赖 SMA 的块跳过，因此无法单独测量没有 SMA 块跳过时 SARG 谓词的影响；不过 TPC-H 中的值均匀分布，实验实际没有跳过任何块，收益完全来自对压缩数据向量执行高效的 SIMD 谓词求值。加入 PSMA 后，在默认 TPC-H 上没有显著额外收益。另与 Vectorwise 的未压缩和压缩存储比较：数据驻留内存时，Vectorwise 的未压缩格式比压缩格式约快 8%，表明即使用轻量压缩也很难仅凭压缩获得加速。
...
**图 11：TPC-H Q6（scale factor 100）在块内排序数据（+SORT）上的加速比。**

**表 2：TPC-H SF100 在 HyPer 与 Vectorwise 的未压缩、压缩数据库上采用不同扫描类型时的运行时间。括号内为相对 JIT 的加速比。**
...
| Vectorwise 未压缩存储 | 2.336s | 74.4s | — |
| Vectorwise 压缩存储 | 2.527s | 78.5s | 0.92x（相对其未压缩存储） |

### 5.3 OLTP 性能

为了衡量访问 Data Block 中单条记录相对未压缩 chunk 的开销，我们在压缩的 TPC-H SF100 `customer` 关系（1500 万条记录）上做随机点查实验。若存在主键索引，访问 Data Block 中记录约有 60% 开销。若没有主键索引，所有查找都以扫描完成；此时 Data Block 扫描可能快于未压缩 chunk，因为 SMA 和 PSMA 可以缩小扫描范围。若 `customer` 按 `c_custkey` 保持 `dbgen` 生成顺序，SMA/PSMA 有帮助；若打乱顺序，则不能缩小扫描范围。因此，SMA/PSMA 并不是 OLTP 负载中传统索引结构的通用替代。Vectorwise 没有传统索引结构，其随机点查吞吐仅为每秒 17 次。

**表 3：TPC-H SF100 上随机点查 `select * from customer where c_custkey = randomCustKey()` 的吞吐（lookups/s）；比较有无 `c_custkey` 主键索引、JIT/向量化未压缩扫描，以及有无 PSMA 的 Data Block 扫描。**
...
我们还运行了 5 warehouse 的 TPC-C。第一组实验只压缩旧的 `neworder` 记录，这符合冷数据使用场景：未压缩存储吞吐 89,229 tps，冷 `neworder` 记录存 Data Blocks 时为 88,699 tps。开销来自每次访问时判断 chunk 是否压缩。第二组实验只执行只读 TPC-C 事务 `order status` 和 `stock level`，并比较完整未压缩 TPC-C 数据库与完全存入 Data Blocks 的数据库：未压缩为 119,889 tps，Data Blocks 为 109,649 tps，差异 9%。这说明即使单记录查找吞吐下降 60%，在真实事务负载中，如果冷压缩数据访问比例有限，总体开销仍较低。由于 Data Blocks 主要面向 OLAP 工作负载，本文省略混合负载 CH-benCHmark [7]；TPC-H 与 TPC-C 的完整评估覆盖了 CH-benCHmark 的两个极端，原文预计混合结果位于其间。

### 5.4 字节可寻址性的优势

sub-byte 编码（例如 BitWeaving [22]）可获得更高压缩率，但会伤害点查和低选择性扫描。我们构造三列 $A$、 $B$、 $C$，各含 $2^{16}$ 个整数值，其中 $A$、 $B$ 的值域为 $[0,2^{16}]$， $\mathrm{dom}(C)=[0,2^8]$。这使水平 bit-packing 可分别压成 17 位或 9 位；这些值域有意比 2 字节和 1 字节截断范围多一位，对 Data Blocks 是最差情况，因为它被迫使用 4 字节和 2 字节代码。此时 bit-packing 压缩率几乎高一倍。

工作负载是对 $A$ 求值 $l\le A\le r$，并把匹配元组解包到输出缓冲区。我们使用 Haswell i5-4670T 和 AVX2，水平 bit-packing 代码来自高度 SIMD 优化的实现 [27]；唯一扩展是增加一个按给定位置解包单条元组的函数。
...
![子图 (a)：水平 bit-packing 与 Data Blocks 的 SARGable 谓词求值成本对比。](assets/figure-12a-bit-packing-sargable-predicate.png)

**子图 (a)：在不同选择性下求值 $l \le A \le r$ 型 SARGable 谓词的成本。**
...
Data Blocks 在几乎所有情况下都优于 bit-packing，只有 100% 元组都命中时 bit-packing 约快 9%。当命中率低于 20% 时，bit-packing 的位置访问表现较好；超过 20% 时，全解包再过滤因 SIMD 更好。中等选择性下，bit-packing 的解包成本显著更高，因为大量非命中元组也被解包；与 SARGing 相比，解包在所有情况下都占主导。例如 10% 元组命中时，Data Blocks 和 bit-packing 分别只把 10% 和 5% 的周期花在谓词求值，而分别有 90% 和 95% 的时间用于解包。选择 10% 均匀分布元组时，Data Block 提取所选元组超过 3 倍更快，选择本身也快 1.8 倍。

![子图 (b)：解包匹配元组（3 个属性）的成本。](assets/figure-12b-bit-packing-unpacking.png)

**图 12：水平 bit-packing 与采用字节可寻址压缩方案的 Data Blocks 对比。**

当 SARGable 谓词选出稀疏元组集合时，只有单个元组能够快速按位置访问和解压，早期选择才会带来收益；反之，命中集合越大、越稠密，早期谓词求值就越没有意义。

由于 HyPer 必须支持快速访问单条压缩记录，并在中等选择性扫描上保持稳健性能，我们决定不在 Data Blocks 中使用 bit-packing。本文有意不展示 vertical bit-packing 的结果，因为它以更高的解压成本换取更快的 SARGing，更不适合该高性能 OLTP/OLAP 系统。不过 Li 等人 [21] 的近期工作显著改善了 vertical bit-packed 数据上的 SARGing，可能为其带来新的用途，例如作为辅助索引。
...
**图 13：TPC-H 100 查询运行时间的几何平均值随向量大小的变化。**
...
向量化扫描向 JIT 编译管线供给数据后，打开了更多优化空间。一个研究方向是在向量化扫描中加入除 SARGable 谓词求值外的更多 SIMD 处理；逐元组 JIT 引擎生成的编译循环通常无法进行 SIMD 处理，而向量化扫描提供了这种可能。

一种思路是为向量化扫描加入 eager aggregation [36]：只在一个 chunk 内计算算术表达式并求聚合，所得预聚合数据再由真正的聚合算子重新聚合；对于非整体聚合，其变换方式与分布式、并行及 eager aggregation 通常采用的方式相同。该优化主要针对只依赖扫描、预计分组很少的聚合，例如 TPC-H Q1、Q6。原文将未压缩 chunk 和压缩 Data Blocks 的向量化扫描早期聚合列为后续实现方向。

另一机会是提前探测 JIT 编译查询管线上游的 hash join。向量化扫描的 probe 端开始运行时，JIT 管线中 hash join 的 build 端已经物化。数据仓库负载中这类 join 常常选择性很高，例如事实表探测受限制的维表；若在向量化扫描内部提前探测 probe 端，就能避免访问那些随后会被 join 消除的事实表记录，也不必扫描和解压其余属性。Vectorwise 使用 bloom filter 实现这种探测 [29]。

探测 bloom filter 比完整 hash 查找便宜有两个原因。第一，bloom filter 比完整 hash table 小得多，通常能装入较低层级的 CPU cache。第二，JIT 编译的 hash 查找本身虽只需很少指令，但所在循环还包含同一查询管线后续所有算子的代码；代码路径很长，CPU 即使能在停顿时深度推测执行，也不大可能跨越整条复杂路径一直推测到下一次循环迭代，即下一条输入元组。因此，JIT 编译 bloom filter 通常一次只查一个元组；若 filter 大于 cache，同时只能有一个未完成内存加载，无法利用现代硬件并发处理多个 cache miss 的能力，也就无法用满内存带宽。相比之下，向量化扫描中的 bloom filter 探测可实现为没有元组间依赖的简单布尔查找循环，容易产生足够多的并行内存加载以饱和带宽。

HyPer 在 hash table pointer 中内建了一种与 bloom filter 类似的早期探测形式，即 tagged hash table pointer [20]，并已用于 JIT 编译管线的逐元组早期探测。把 key 向量的这种探测直接放到向量化扫描中、紧接 SARGable 谓词求值之后（见图 14），初步实验在 TPC-H Q3–Q5、Q7–Q8、Q10–Q14、Q21–Q22 上获得显著收益，查询运行时间几何平均提升 1.2 倍；但对扫描中的所有 join 都执行早期测试也使相当一部分其他查询变慢。

这引出了第三个机会：让查询处理自适应。Vectorwise 的 Micro Adaptivity [29] 会在运行时尝试特定向量化函数的不同实现（“flavors”）并监控性能。由于一个查询内同一向量化函数可能调用数百万次，自适应表达式求值器可以实验不同 flavor，并在大多数时间坚持使用当前表现最好的版本，从而使查询性能更稳健。在逐元组 JIT 编译管线中，每个二元决策点都会使可能的代码路径数翻倍，重新导致较长编译时间，因此无法采用这种自适应算法；向量化扫描则可以。实验以 micro-adaptivity 决定是否在扫描中启用早期 join 测试，从而避免了性能惩罚。

![图 14：对 Data Blocks 返回的匹配 key 向量执行早期探测；hash table 中的 pointer tags 用于过滤不在任一 bucket 中的 key。](assets/figure-14-early-probing.png)

**图 14：对 Data Blocks 返回的匹配 key 向量执行早期探测；hash table 中的 pointer tags 用于过滤不属于任何 bucket 的 key（参见 [20] 的 tagged hash table pointers）。**

图 14 的 pointer tags 对应 tagged hash table pointer [20]。

### F. TPC-H 结果

**表 4：TPC-H SF100 各查询的运行时间与编译时间（括号内）。依次比较：(i) 未压缩数据上的 JIT 编译逐元组扫描；(ii) 未压缩数据上的向量化扫描；(iii) 加 SARGable 谓词求值的未压缩向量化扫描；(iv) 压缩 Data Block 向量化扫描；(v) 加 SARGable 谓词求值和 SMA；(vi) 再加 Positional SMA 索引。**
