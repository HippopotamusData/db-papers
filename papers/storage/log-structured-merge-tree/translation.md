...
LSM-tree 的优势来自两个相乘的因素：多页块 I/O 的单位成本 `COST_π/COST_P` 低于随机单页 I/O，以及批量合并让每个条目的成本再缩小约 `1/M`。本节建立磁盘成本模型，比较 B-tree 与 LSM-tree 的插入成本，再说明何时需要三个或更多组件以及如何确定各组件大小。

这里的类比之所以直接，是因为 LSM-tree 与 B-tree 都在叶层按排序顺序为每个被索引行保存条目，并以高层目录把访问沿页大小节点路径引到叶层。B-tree 的叶页按五分钟法则通常不会在短暂缓冲期间再次收到第二次插入：每次都是读页、插一条、再写页，没有批处理效应。LSM-tree 则不同。例如 16 字节条目在填满的 4 KB 节点中约可放 250 条；若 `C_0` 大小为 `C_1` 的 `1/25`，一次 `C_1` 节点 I/O 平均可接收约 10 个新条目。滚动合并正是同时获得多页顺序访问与批量摊销的关键。
...
多页块 I/O 的容量更高，因此用于滚动合并时应把 `COST_P` 换成 `COST_π`。IBM 在 1989 年分析 3380 磁盘上的 DB2 实用程序性能时估计：随机读一个页面约需 20 ms（10 ms 寻道、8.3 ms 旋转延迟、1.7 ms 读取）；顺序预取 64 个连续页面约需 125 ms（10 ms 寻道、8.3 ms 旋转延迟、106.9 ms 读取），即每页约 2 ms。因此 `COST_π/COST_P` 约为 `1/10`。较新的 SCSI-2 磁盘也给出相同比例：随机读取一个 4 KB 页约需 16 ms（9 ms 寻道、5.5 ms 旋转延迟、1.2 ms 读取），连续读取 64 个 4 KB 页约需 95 ms，即每页约 1.5 ms。
...
**例 3.1。** 例 1.1 的 Account 表有 1 亿个 100 字节行，共 10 GB；达到 1000 TPS 时，需要 `H=2000` IOPS。磁盘容量成本约 10,000 美元，I/O 能力成本为 50,000 美元，故数据温度为 `0.2` IOPS/MB，属于温数据；按 I/O 配置的磁盘只使用约五分之一容量，经济瓶颈是磁盘臂而不是介质容量。例 1.2 的 20 天 `Acct-ID||Timestamp` 索引情形相似：叶层条目为 9.2 GB，而增长中的 B-tree 仅约 70% 填满，整棵树约需 13.8 GB，但仅插入就具有与 Account 表相同的 I/O 率，因此温度相近。

#### 多页块 I/O 的优势

滚动合并按键序读写连续的多页块，磁盘寻道一次后可传输许多页面。若多页访问的每 IOPS 成本只有随机访问的十分之一，那么即便合并会读写比单条 B-tree 插入更多的页面，其磁盘臂成本仍可能更低。下一节还会看到，`C_0` 中的一批条目共同分摊这些页面，形成第二重收益。
...
这一分析假定连续插入随机落在叶层。键值持续递增、总在最右端插入是一个不满足该假设的常见特例；B-tree 本身已能高效处理这种情形，因为树不断向右增长、I/O 很少，这也正是 B-tree 批量装载的基本情形。另有一些结构专门处理按递增值为日志记录建索引 [8]。

**定义 3.2.1（批量合并量 `M`）。** 令 `S_e` 为一个索引项的字节数，`S_p` 为页大小，`S_0`、`S_1` 分别为 `C_0`、`C_1` 的叶层条目字节数。一次 `C_1` 页经过合并游标时，平均从 `C_0` 合入的条目数称为批量合并量 `M`：
...
若 `COST_π/COST_P≈0.1` 且 `M≈5`，两项因素相乘可让插入成本比 B-tree 低近两个数量级。
...
上述式子也显示极限：`C_0` 过小会使 `M` 变小；若 `M<K_1(COST_π/COST_P)`，多页磁盘读取的批处理优势甚至会被抵消，此时普通 B-tree 的插入成本反而更低。双组件结构必须用较贵的内存保持足够大的 `C_0`，才能降低合并游标在 `C_1` 上的巡回频率。`C_0` 越大，插入 I/O 越低，但内存成本越高；存在一个使内存成本与 `C_1` 的介质/磁盘臂成本之和最小的规范大小。

### 3.3 多组件 LSM-tree

设总叶条目大小 `S=S_0+S_1` 大致稳定，新条目以每秒 `R` 字节进入 `C_0`。为保持 `C_0` 接近阈值，同速率的数据必须通过滚动合并迁出。缩小 `C_0` 会提高游标从最小键到最大键的巡回频率，并增加 `C_1` 多页块 I/O；概念极端中若 `C_0` 只有一个条目，每插入一个条目都要扫过 `C_1` 全部块，合并反而成为沉重负担。增大 `C_0` 会降低 I/O，却提高内存成本。
