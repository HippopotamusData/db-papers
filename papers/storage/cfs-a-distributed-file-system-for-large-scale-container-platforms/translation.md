...
算法 1 给出伪代码。Resource manager 先提前把 meta partition 的 inode 范围截断到上界 `end`，其值大于当前已使用的最大 inode id `maxInodeID`；然后向 meta node 发送拆分请求：把原 meta partition 的 inode id 范围更新为 $[1,end]$，并创建范围为 $[end+1,\infty)$ 的新 meta partition。这样两个 partition 的范围分别为 $[1,end]$ 和 $[end+1,\infty)$。再创建文件时，其 inode id 将取原 partition 中的 $maxInodeID+1$，或新 partition 中的 $end+1$。

每个 meta partition 的 `maxInodeID` 由 resource manager 与 meta node 的周期通信获得。
