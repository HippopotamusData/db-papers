...
Tim Kraska¹[^author-google]，Alex Beutel²，Ed H. Chi²，Jeffrey Dean²，Neoklis Polyzotis²<br>
...
arXiv:1712.01208v3 [cs.DB]，2018 年 4 月 30 日。

[^author-google]: 本工作在第一作者任职 Google 期间完成。

## 摘要

索引就是模型：B 树索引可视为把键映射到有序数组中记录位置的模型，哈希索引是把键映射到无序数组中记录位置的模型，位图索引则是判断某条数据记录是否存在的模型。本文是一项探索性研究：从这一前提出发，我们认为所有现有索引结构都可以由其他类型的模型替代，包括我们称为“学习型索引”的深度学习模型。其核心思想是，模型可以学习查找键的排序规律或结构，并用这些信号有效预测记录的位置或存在性。我们从理论上分析学习型索引在何种条件下优于传统索引结构，并说明设计学习型索引结构的主要挑战。我们的初步结果表明，通过使用神经网络，我们能够在若干真实数据集上达到比缓存优化的 B 树最多快 70% 的速度，同时节省一个数量级的内存。更重要的是，我们相信，用学习模型替换数据管理系统的核心组件会对未来系统设计产生深远影响，而本文仅展示了这种可能性的一角。
...
下文我们首先以 B 树介绍学习型索引的一般思想；第 4 节把它扩展到哈希表，第 5 节扩展到 Bloom 过滤器，各节均有独立评估；第 6 节讨论相关工作，第 7 节总结。
...
为提高效率，通常不会索引每条有序记录，而只索引每第 $n$ 条记录，也就是每页的第一个键。此处我们只考虑连续内存区域（单个数组）上的定长记录和逻辑分页，不考虑位于不同内存区域的物理页；物理页与变长记录见附录 D.2。只索引每页首键能显著减少索引保存的键数，而性能几乎不受影响。因此，B 树是一个模型；用 ML 术语说，它是回归树：把键映射到位置，并带有最小、最大误差（最小误差为 0，最大误差为页大小），保证键若存在便能在该区域找到。只要其他 ML 模型也能给出类似的最小、最大误差强保证，我们就能用包括神经网络在内的模型替代索引。

乍看之下，其他 ML 模型很难给出相同保证，实际上却很简单。第一，B 树只对已存储的键提供强误差保证，而非对所有可能键。加入新数据时，B 树需要重新平衡——用 ML 术语说就是重新训练——才能维持保证。对于单调模型，我们只需在每个键上执行模型，记录位置预测的最大高估与低估，即可算出最小、最大误差。[^2] 第二，更重要的是，其实我们并不需要强误差界。为支持范围查询，数据本来就必须排序；任何误差都可通过预测位置附近的局部搜索（如指数搜索）纠正，因此我们甚至可以允许非单调模型。于是，我们可以用线性回归、神经网络等任意回归模型替换 B 树，如图 1(b) 所示。
...
考虑一棵索引 1 亿条记录、页大小为 100 的 B 树。我们可以把每个节点看作划分空间、减小“误差”并缩小数据所在区域的一种方式，因此我们说每个节点带来 $1/100$ 的精度增益，总共需要遍历 $\log _ {100}N$ 个节点：第一层把 1 亿缩到 100 万，第二层把 100 万缩到 1 万，依此类推，直至我们找到记录。用二分搜索遍历单个 B 树页约需 50 个周期，而且很难并行化。[^3] 相比之下，现代 CPU 每周期可完成 8–16 次 SIMD 运算。因此，只要模型每 $50\times8=400$ 次算术运算取得的精度增益优于 $1/100$，就会更快。该估算还假定所有 B 树页都在缓存中；一次缓存未命中另需 50–100 个周期，因而允许更复杂的模型。
...
索引输入一个键并预测记录位置。点查询不关心记录顺序，而范围查询要求数据按查找键排序，以便高效取出一个范围内的全部项。这导出一个有趣观察：给定键、预测其在有序数组中位置的模型，实际上是在逼近累积分布函数（CDF）。我们可以用数据的 CDF 预测位置：

$$
p=F(\mathrm{Key})\times N. \qquad \text{(1)}
$$

其中 $p$ 是位置估计， $F(\mathrm{Key})$ 是数据的估计累积分布函数，即观测到小于等于查找键的键的概率 $P(X\leq\mathrm{Key})$； $N$ 是键总数，见图 2。

这一观察开启了多条方向。第一，索引从字面上说就是学习数据分布；B 树通过构建回归树“学习”分布，线性回归则通过最小化线性函数的平方误差学习分布。第二，估计数据分布是研究已久的问题，学习型索引可以利用数十年成果。第三，正如我们将在后文说明的那样，学习 CDF 也在优化其他索引结构和潜在算法时扮演关键角色。第四，理论 CDF 与经验 CDF 的逼近程度有长期研究积累，可作为理解我们方法理论收益的基础 [28]。我们在附录 A 给出高层次的扩展性分析。
...
在该设置下，我们每秒约完成 1,250 次预测，即仅执行 TensorFlow 模型、尚不计从预测位置找到真实记录的搜索时间，就约需 80,000ns。相比之下，同一数据上的 B 树遍历约需 300ns，整个数据上的二分搜索约需 900ns。仔细分析后，我们发现朴素方法有三项主要限制：
...
LIF 可视为索引合成系统：给定索引规格，自动生成、优化并测试不同配置。LIF 可即时学习线性回归等简单模型，复杂模型（如神经网络）则依赖 TensorFlow；但推理阶段从不使用 TensorFlow。它从训练好的 TensorFlow 模型抽取全部权重，并按模型规格生成高效 C++ 索引结构。我们的代码生成专为小模型设计，去除了 TensorFlow 管理大模型所需的不必要开销与监测逻辑；这里我们借鉴 [25] 避免 Spark 运行时开销的思想。因此，我们能在约 30ns 内执行简单模型。
...
这里我们用 $f _ {\ell-1}(x)$ 表示递归执行前一阶段。我们逐阶段以 $L _ \ell$ 训练即可构建完整模型。每个模型对键的位置作出带误差的预测，该预测选择负责相应键空间区域的下一个模型，使后者作出误差更小的预测。但 RMI 不必是树：同一阶段的不同模型可以选择下一阶段的同一模型；每个模型覆盖的记录数也不必像 B 树页那样相同。[^4] 依所用模型而定，阶段间输出甚至不一定应解释为位置估计，更适合解释为选择对某些键更了解的专家 [62]。
...
RMI 的另一项优势是我们可以混合模型。顶层可能以小型 ReLU 网络最合适，因为它能学习复杂分布；底层则可使用数千个空间和执行成本都低的线性回归模型。若某段数据尤其难学，我们甚至可以在底层使用传统 B 树。
...
> **译者注：** 原文第 9 行将预测位置直接除以下一阶段模型数。若沿用第 3.2 节中模型输出位于 $[0,N)$ 的位置这一设定，该写法与同节给出的模型选择公式不一致；以算法中的索引记号表示，下一阶段模型索引应计算为 $\left\lfloor \mathrm{index} _ {i,j}(r.\mathrm{key}) \cdot \mathrm{stages} _ {i+1} / N \right\rfloor$，其中下标 $(i,j)$ 对应代码中的 `index[i][j]`。此处仍按原文保留算法代码。
...
- **模型偏置搜索：** 我们的默认策略，仅把传统二分搜索的第一个中点设为模型预测值。
- **偏置四分搜索：** 每轮使用三个分点，希望硬件一次预取三处数据，在数据不在缓存时取得更好性能。在我们的实现中，我们把初始三个中点设为 $pos-\sigma$、 $pos$、 $pos+\sigma$，即我们先假设大多数预测准确，把我们的注意力集中在位置估计附近，之后继续传统四分搜索。

在我们的所有实验中，我们都使用最小、最大误差定义搜索区域：在每个键上执行 RMI，并按末级模型保存最大高估与低估。这保证能找到全部已存在的键，但若 RMI 非单调，对不存在的键可能返回错误的上下界。一个办法是像 [41, 71] 所研究的那样强制我们的 RMI 单调。另一办法是对非单调模型自动调整搜索区域：若找到的上（下）界键位于误差定义的搜索边界，我们就逐步扩大区域；还可使用指数搜索。若误差服从正态分布，指数搜索平均表现应与其他策略相当，且无须存储最小、最大误差。

### 3.5 字符串索引

前文主要索引实数键，但许多数据库依赖字符串索引，而机器学习界已有大量字符串建模研究。我们同样需要设计既高效又有表达力的字符串模型，这带来独特挑战。

首要问题是如何把字符串转换为模型特征，即分词。为简单高效，我们把长度为 $n$ 的字符串视为特征向量 $\mathbf{x}\in\mathbb{R}^n$，其中 $x_i$ 是 ASCII 十进制值（或依字符串使用 Unicode 十进制值）。多数 ML 模型在输入等长时更高效，因此我们设最大输入长度为 $N$。由于数据按字典序排序，我们先把键截断为 $N$ 再分词；若字符串长度 $n\lt{}N$，则对 $i\gt{}n$ 令 $x_i=0$。

为提高效率，我们总体沿用实数输入时的建模方式，学习小型前馈网络的层次结构，只是输入从一个实数 $x$ 变为向量 $\mathbf{x}$。线性模型 $\mathbf{w}\cdot\mathbf{x}+b$ 的乘加次数随 $N$ 线性增长；即使只有一层、宽度为 $h$ 的前馈网络，也需要 $O(hN)$ 次乘加。我们相信字符串学习型索引还有大量优化空间；例如，我们可以采用 NLP 中把字符串分成更有用片段的分词方法（如机器翻译中的 wordpiece [70]）、把后缀树思想与学习型索引结合，或探索循环、卷积网络等复杂架构。
...
训练（即装载）时间不是我们的重点，但无论浅层神经网络还是简单线性/多元回归，训练都相对很快。简单网络可用随机梯度下降高效训练，在随机化数据上少于一次到数次遍历便可收敛；线性多元模型（包括零层网络）有闭式解，可单次扫描有序数据完成训练。对 2 亿条记录，简单 RMI 的训练只需数秒左右，具体取决于自动调优量；神经网络则依复杂度每个模型约需数分钟。通常也不必让顶层模型遍历全部数据，因为它往往在对全部随机数据扫描一次之前便已收敛。原因之一是我们使用简单模型，而且不太在意精度末几位，因为它对索引性能影响很小。ML 社区改善学习时间的研究 [27, 72] 同样适用于我们的场景，我们预计该方向会有大量后续工作。
...
第一项实验中，我们比较两阶段 RMI（第二阶段分别有 10k、50k、100k、200k 个模型）与多种页大小的读优化 B 树，并使用两个真实数据集和一个合成数据集：（1）Weblogs，（2）Maps [56]，（3）Lognormal。Weblogs 含某大型大学网站多年间每次请求的 2 亿条日志，我们以唯一请求时间戳为索引键。课程安排、周末、节假日、午休、院系活动和学期间歇形成复杂时间模式，因而几乎是学习型索引的最坏情况。对 Maps 数据集，我们索引全球约 2 亿个用户维护地物（道路、博物馆、咖啡店等）的经度；位置经度较为线性，异常少于 Weblogs。最后，为测试重尾分布，我们从 $\mu=0$、 $\sigma=2$ 的对数正态分布采样 1.9 亿个唯一值，缩放为最大 10 亿的整数；其 CDF 高度非线性，神经网络更难学习。所有 B 树实验中，我们使用 64 位键和 64 位负载/值。

作为基线，我们使用一份生产质量 B 树实现，它与 `stx::btree` 相似，但进一步做了缓存行优化、使用 100% 填充的稠密页，性能很有竞争力。我们用网格搜索调优两阶段学习型索引的神经网络，搜索 0–2 个隐藏层、每层 4–32 个节点。总体而言，我们发现第一阶段以简单（零隐藏层）到中等复杂（两层、宽 8 或 16）的模型最佳；第二阶段则以简单线性模型最佳，因为最后一英里通常不值得执行复杂模型，而线性模型能最优学习。

**学习型索引与 B 树。** 我们在图 4 中给出主要结果。B 树页大小表示每页键数而不是字节数；我们给出的指标包括 MB 大小、总查找时间（ns），以及模型执行时间（B 树遍历或 ML 模型，以 ns 和总时间百分比表示）。我们还在大小与查找列括号中给出相对页大小 128 的 B 树的加速和空间节省；我们选择它作为固定参照，是因为它的 B 树查找性能最佳。颜色表示相对参照更快/更慢或更小/更大。

几乎所有配置中，学习型索引都占优：快 1.5–3 倍，体积最多小两个数量级。B 树当然可用压缩换取解压 CPU 时间，但多数优化彼此正交，也同样甚至更适用于神经网络。例如可用 4/8 位整数而非 32/64 位浮点数表示参数，即量化，从而进一步获益。第二阶段大小显著影响索引体积和查找性能。第二阶段使用 10,000 个或更多模型尤其印证第 2.1 节：我们的第一阶段模型取得的精度跃迁远大于 B 树单节点。我们未报告这些数据集上的混合模型和二分搜索以外策略，因为收益不显著。
...
- **查找表：** 固定大小、固定结构的层次查找表（如每层 64 槽）可用 AVX 高度优化。我们比较三阶段表：每取第 64 个键放入带填充的数组，使其长度为 64 的倍数，再对该数组重复一次但不填充，总计两个数组。查找时我们对顶层二分搜索，再用 AVX 优化的无分支扫描 [14] 搜索第二层和数据本身；该配置优于扫描顶层或对第二层、数据二分搜索等变体。
- **FAST：** FAST [44] 是高度 SIMD 优化的数据结构，我们使用 [47] 的代码进行比较。为使用无分支 SIMD，它必须按 2 的幂分配内存，索引可能明显更大。
- **定高 B 树与插值搜索：** 依 [1]，我们构造固定高度、采用插值搜索的 B 树，使总大小为 1.5MB，与我们的学习模型相近。
- **去除框架开销的学习型索引：** 对我们的学习型索引，我们使用两阶段 RMI，顶层为多元线性回归，底层为简单线性模型。我们自动为顶层创建并选择 `key`、 $\log(key)$、 $key^2$ 等特征。多元线性回归只用少数操作就能拟合非线性模式，是神经网络之外的有趣选择。为公平比较，我们在基准框架之外实现该索引。

![图 5：替代基线](assets/figure-5.png)

**图 5：替代基线。**

比较中我们使用对数正态数据和 8 字节指针负载，结果见图 5。在公平条件下，学习型索引总体性能最好，并显著节省内存；FAST 体积大源于对齐要求。不过，我们绝不声称学习型索引在大小或速度上总是最佳。它提供的是思考索引的新方式，其完整影响仍需大量研究。
...
字符串上的加速不如整数明显，部分原因是模型执行成本较高，而 GPU/TPU 可消除这一问题。字符串搜索本身也更昂贵，所以更高精度往往值得；混合索引用 B 树替换表现差的模型，因而有帮助。由于搜索成本高，不同策略差异也更大：一隐藏层网络配偏置二分搜索需 1102ns，我们采用同一模型的偏置四分搜索只需 658ns。二者表现更好，是因为利用了模型误差。
...
作为键 $K$ 的哈希函数。若 $F$ 完美学习键的经验 CDF，就不会发生冲突。该函数与具体哈希表架构正交，可同分离链接法或任何其他类型结合。对于模型，我们仍可利用上一节的递归架构，索引大小与性能之间同样存在由模型和数据集决定的权衡。
...
我们在上一节三个整数数据集上评估冲突率。我们使用两阶段 RMI 作为模型哈希，第二阶段有 100k 个模型且无隐藏层；我们使用类似 MurmurHash3 的简单函数作为基线，比较槽位数与记录数相同的表。图 8 显示，学习经验 CDF 可把冲突最多减少 77%，而模型执行代价合理，与图 4 相同，约 25–40ns。

![图 8：冲突减少比例](assets/figure-8.png)

**图 8：冲突减少比例。**

考虑模型执行时间后，减少冲突是否划算，取决于哈希表架构、负载等因素。我们的附录 B 实验表明，对于 20 字节记录的分离链接哈希表，学习型哈希可减少最多 80% 的浪费空间，延迟仅比随机哈希增加 13ns。增加量不是 40ns，是因为冲突减少往往也减少缓存未命中。很小的负载下，标准哈希加 Cuckoo hashing 可能仍是最佳；但正如我们在附录 C 中所示，负载较大时，学习型哈希加链接法可快于 Cuckoo hashing 和/或传统随机哈希。

最后，我们认为潜力最大的场景是分布式系统。例如 NAM-DB [74] 通过哈希函数定位远程机器上的数据，再用 RDMA 读取；每次冲突都要多发一次微秒级 RDMA 请求，因此模型时间可忽略，冲突率即使略降也会显著改善总体性能。总之，学习型哈希函数独立于哈希表架构，其复杂度是否值得由具体架构决定。
...
Bloom 过滤器虽极省空间，仍会占用可观内存：10 亿条记录约需 1.76GB；若我们要求 0.01% 的假阳性率（FPR），将需要约 2.23GB。已有多项工作尝试改进 [52]，但总体观察不变。若集合内外之间存在可学习结构，则可能构造更高效的表示。数据库中的存在性索引与我们前面看到的延迟、空间权衡不同：访问磁盘甚至磁带等冷存储延迟很高，因而我们可承受更复杂的模型；首要目标是减少索引空间和假阳性。下面我们给出两种学习型存在性索引。

### 5.1 学习型 Bloom 过滤器

范围与点索引学习键分布，而存在性索引要学习把键与其他一切分开的函数。换言之，点索引的好哈希函数应尽量减少键之间的冲突；Bloom 过滤器的好哈希函数则应让键之间、非键之间冲突很多，却让键与非键之间冲突很少。下面我们说明如何学习这样的 $f$ 并将其纳入存在性索引。

传统 Bloom 过滤器对事先选定的任意查询集保证零假阴性率（FNR）和特定 FPR [22]；我们则希望对特别是现实查询提供特定 FPR，同时维持零 FNR。也就是说，像评估 ML 系统一样，我们在留出的查询集上测量 FPR [30]。两种定义不同，但我们认为，在许多应用、尤其数据库中，我们可以从历史日志观察查询分布。[^5]

传统存在性索引不利用键分布以及键与非键的差异，学习型 Bloom 过滤器可以。例如，若我们的数据库恰含所有满足 $0\le x\lt{}n$ 的整数，只需计算 $f(x)\equiv\mathbf{1}[0\le x\lt{}n]$，便能以常数时间和近乎零内存判断存在性。
...
我们可把存在性索引表述为二元概率分类任务：学习模型 $f$，预测查询 $x$ 是键还是非键。对字符串，我们可训练 RNN 或 CNN [64, 37]，数据集为
...
作为二元分类器，我们的网络用 sigmoid 输出概率，并训练以最小化对数损失：

$$
L=\sum _ {(x,y)\in\mathcal D}y\log f(x)+(1-y)\log(1-f(x)).
$$

> **译者注：** 原文称这里要最小化 log loss，但所列式子没有前导负号；按通常的二元交叉熵定义，最小化目标应为该和式的负值。此处保留原式。

 $f(x)$ 可解释为 $x$ 是我们数据库键的概率。我们选择阈值 $\tau$，若输出高于阈值便认为键存在。我们的模型与 Bloom 过滤器不同，很可能同时有非零 FPR 和 FNR，而且降低 FPR 会提高 FNR。为保持存在性索引无假阴性的约束，我们建立溢出 Bloom 过滤器：

$$
\mathcal K^- _ \tau=\lbrace{}x\in\mathcal K\mid f(x)\lt{}\tau\rbrace{}
$$

我们把它视为 $f$ 的假阴性键集合，并为其建立一个 Bloom 过滤器。随后我们按图 9(c) 运行我们的存在性索引：若 $f(x)\ge\tau$，认为键存在；否则检查溢出过滤器。

问题是如何设置阈值，使我们的学习型 Bloom 过滤器达到目标 FPR $p^{\ast}$。我们在留出的非键集合 $\widetilde{\mathcal U}$ 上定义我们的模型 FPR：

$$
\mathrm{FPR} _ \tau=\frac{\sum _ {x\in\widetilde{\mathcal U}}\mathbf 1(f(x)\gt{}\tau)}{|\widetilde{\mathcal U}|}.
$$

我们把溢出 Bloom 过滤器的假阳性率记为 $\mathrm{FPR} _ B$，则我们的系统整体为 [53]：[^6]

$$
\mathrm{FPR} _ O=\mathrm{FPR} _ \tau+(1-\mathrm{FPR} _ \tau)\mathrm{FPR} _ B.
$$

为简单起见，我们取 $\mathrm{FPR} _ \tau=\mathrm{FPR} _ B=p^{\ast}/2$，于是 $\mathrm{FPR} _ O\le p^{\ast}$，并在 $\widetilde{\mathcal U}$ 上调节 $\tau$。学习模型相对数据可很小；Bloom 过滤器大小随键集合大小增长，因此溢出过滤器会随 FNR 缩放。实验中我们将看到，该组合可减小存在性索引内存。此外，模型计算能受益于 ML 加速器，而传统 Bloom 过滤器高度依赖内存随机访问延迟。

#### 5.1.2 采用模型哈希的 Bloom 过滤器

另一方案是，我们学习哈希函数，最大化键内部与非键内部冲突，同时最小化键与非键冲突。我们仍可使用上述概率分类模型：把 $f$ 的输出缩放并映射到大小为 $m$ 的位数组，形成

$$
d=\lfloor f(x)\times m\rfloor.
$$

我们像普通 Bloom 过滤器一样把 $d$ 用作哈希函数。训练后的 $f$ 会把大多数键映射到位数组高端、非键映射到低端（图 9(b)）。附录 E 详述此方法。

### 5.2 结果

为了实验检验，我们考察用存在性索引记录钓鱼 URL 黑名单这一应用。我们把 Google 透明度报告中的 170 万个唯一 URL 作为键集合；我们的负集合混合随机有效 URL 与可能被误判为钓鱼页面的白名单 URL，并把它随机划分为训练、验证和测试集。我们训练字符级 RNN（具体为 GRU [24]）判断 URL 属于哪个集合，在验证集上设置 $\tau$，并报告测试集 FPR。

目标 FPR 为 1% 的普通 Bloom 过滤器需 2.04MB。我们采用 16 维 GRU，每个字符有 32 维嵌入，模型大小 0.0259MB。构建可比较的学习型索引时，我们在验证集把 $\tau$ 设到 0.5% FPR，此时 FNR 为 55%；测试集 FPR 为 0.4976%，验证了阈值选择。我们的溢出 Bloom 过滤器大小随 FNR 缩放，我们发现模型与它合计 1.31MB，节省 36%。若我们要求整体 FPR 为 0.1%，FNR 为 76%，总大小从 3.06MB 降至 2.59MB，节省 15%。我们在图 10 中展示这一一般关系，也可看到不同大小模型对精度与内存的权衡不同。

![图 10：不同 FPR 下学习型 Bloom 过滤器的内存占用](assets/figure-10.png)

**图 10：学习型 Bloom 过滤器在广泛 FPR 范围内改善内存占用。 $W$ 为 RNN 宽度， $E$ 为每字符嵌入大小。**

我们还简要考虑查询分布存在我们的模型尚未处理的协变量偏移这一情形。只用随机 URL 作为验证和测试集时，我们发现相对 FPR 为 1% 的 Bloom 过滤器可节省 60%；只用白名单 URL 时，我们发现可节省 21%。负集合的选择取决于应用，协变量偏移也可更直接地处理；这些实验旨在展示方法如何适应不同情况。

我们的模型越准确，Bloom 过滤器节省越多。我们的模型无须使用与 Bloom 过滤器相同的特征，例如已有大量研究用 ML 预测网页是否为钓鱼页 [10, 15]；加入 WHOIS 数据或 IP 信息可提高精度、减小过滤器，同时仍保持零假阴性。我们在附录 E 给出第 5.1.2 节方法的更多结果。

## 6 相关工作

学习型索引建立在机器学习与索引技术的广泛研究之上，下面我们概述最重要的相关领域。

**B 树及其变体。** 几十年来提出了多种索引 [36]，如磁盘系统的 B+ 树 [17]，内存系统的 T 树 [46]、平衡树/红黑树 [16, 20]。早期内存树缓存行为差，于是出现 CSB+ 树等缓存感知变体 [58]，以及利用 SIMD 的 FAST [44] 和 GPU 方案 [44, 61, 43]。许多内存索引还以偏移代替节点指针，减少存储。文本索引则包括 trie/radix tree [19, 45, 31]，以及结合 B 树与 trie 的结构 [48]。这些方法都没有学习数据分布来压缩表示或提升性能，因而与学习型索引正交；也可像我们的混合索引一样，把硬件感知策略与模型更紧密结合。
...
**为 ANN 索引学习哈希函数。** 学习哈希函数已有大量研究 [49, 68, 67, 59]，尤其是为近似最近邻（ANN）索引学习局部敏感哈希。文献 [66, 68, 40] 探索神经网络哈希，[69] 甚至尝试保持多维输入空间的顺序。但 LSH 的一般目标，是把相似项放入同一桶以支持最近邻查询，通常在高维空间用汉明距离的变体学习近似相似度。既有方法无法直接适配我们所考虑的基础数据结构，也不清楚能否改造。

**完美哈希。** 完美哈希 [26] 与我们在哈希表中使用模型的做法密切相关；与我们的 CDF 模型一样，完美哈希也试图避免冲突。但据我们所知，既有方案没有考虑学习技术，函数大小也随数据规模增长；学习型哈希可以与规模无关。例如，用线性模型映射 0 到 2 亿之间每隔一个的整数，不会冲突，模型大小也不随数据增长。完美哈希还不能用于 B 树或 Bloom 过滤器。

**Bloom 过滤器。** 我们的存在性索引直接建立在 Bloom 过滤器研究 [29, 11] 上，但视角不同：用分类模型增强 Bloom 过滤器，或把模型用作特殊哈希函数，其优化目标也不同于我们为哈希表创建的哈希模型。
...
**CDF 建模。** 我们的范围与点索引模型都与 CDF 模型紧密相关。估计 CDF 并不简单，ML 社区已有研究 [50]，也用于排序等任务 [42]；怎样最有效地建模仍是开放问题。

**专家混合。** 我们的 RMI 沿袭了为数据子集建立专家的长期研究 [51]，在神经网络发展后更常见并显示出更大价值 [62]。在我们的场景中，我们看到它恰好能解耦模型大小与计算量，使我们可以使用更复杂却不增加单次执行成本的模型。
...
**致谢。** 我们感谢 Michael Mitzenmacher、Chris Olston、Jonathan Bischof 以及 Google 的许多同事在论文准备过程中提出宝贵意见。
...
把学习型范围索引表述为对数据累积分布函数（CDF）建模的一项优势，是我们可以利用关于 CDF 建模的长期研究。大量研究考察了理论 CDF $F(x)$ 与从 $F(x)$ 采样得到的数据的经验 CDF 之间的关系。我们考虑从某分布独立同分布采样 $N$ 个数据点，集合为 $Y$；我们将经验累积分布函数记为：

$$
\widehat F_N(x)=\frac{\sum _ {y\in Y}\mathbf 1 _ {y\le x}}{N}. \qquad \text{(2)}
$$

学习型索引的一项理论问题是：它如何随数据规模 $N$ 扩展？在我们的设置中，我们学习模型 $F(x)$ 来逼近我们的数据分布 $\widehat F_N(x)$。这里我们假定已知生成数据的分布 $F(x)$，并分析数据从该分布采样所固有的误差。（注：学习 $F(x)$ 可能改善也可能恶化误差；对某些应用，例如数据以随机哈希为键时，我们把它作为合理假设。） 也就是说，我们考察数据分布 $\widehat F_N(x)$ 与我们的分布模型 $F(x)$ 的误差。因为 $\widehat F_N(x)$ 是均值为 $F(x)$ 的二项随机变量，我们发现我们的数据与模型之间的期望平方误差为：

$$
\mathbb E\negthinspace{}\left[\left(F(x)-\widehat F_N(x)\right)^2\right]
=\frac{F(x)(1-F(x))}{N}. \qquad \text{(3)}
$$

在我们的应用中，查找时间随有序数据中的平均位置误差增长，也就是我们关注我们的模型 $NF(x)$ 与键位置 $N\widehat F_N(x)$ 的误差。对式（3）稍作变换，我们可得预测位置的平均误差以 $O(\sqrt N)$ 增长。对固定大小模型而言，这种次线性误差扩展优于固定大小 B 树的线性扩展。这初步解释了我们方法的可扩展性，也说明把索引表述为学习 CDF 很适合理论分析。

## 附录 B 分离链接哈希表

我们用分离链接哈希表评估学习型哈希函数：记录直接存于数组，仅发生冲突时才把记录接到链表上。无冲突时最多一次缓存未命中；多个键映射到同一位置时才可能增加未命中。我们选择这一设计，是因为即使负载较大，它也有最佳查找性能。我们还测试了商用级稠密哈希表，它以桶内原地溢出减少开销 [2]。该技术占用可更小，但我们发现其速度是分离链接法的约一半；内存利用率达到 80% 或更高时，性能进一步下降。还有许多正交优化可做，我们并不声称实现的内存或 CPU 效率最优，目标只是展示学习型哈希的一般潜力。
...
对于哈希表，我们存储完整记录：64 位键、64 位负载，以及用于删除标志、版本号等的 32 位元数据，因此记录定长 20 字节；我们的链接哈希表另加 32 位指针，每槽共 24 字节。

![图 11：模型哈希表与随机哈希表](assets/figure-11.png)

**图 11：模型哈希表与随机哈希表。**

图 11 列出平均查找时间、空槽 GB 数，以及相对随机哈希函数的空间比例。与 B 树实验不同，这里我们计入数据大小，因为要实现一次缓存未命中的查找，数据本身必须放入哈希表；前文我们只计有序数组之外的索引开销。

总体上，模型哈希的性能相近而内存利用更好。例如 Maps 数据集上，模型哈希只在空槽浪费 0.18GB，相比随机哈希减少近 80%。当我们把表扩大到槽位比记录多 25% 时，随机哈希也能更好地分散键，节省自然缩小。令人意外的是，当我们把槽位降到键数 75% 时，生日悖论仍然存在，学习型哈希仍有优势。
...
- **采用学习型哈希的原地链接哈希表：** 分离链接需要额外链表内存。作为替代，我们实现采用两遍算法的链接哈希表：第一遍用学习型哈希把项放进槽，槽已占用则跳过；随后我们对跳过项使用分离链接，但以剩余空槽保存项，并用偏移作指针。因此，由于我们不考虑插入，利用率可达 100%；学习型哈希质量只影响性能，不影响大小——冲突越少，缓存未命中越少。我们使用单阶段多元模型作为哈希函数；为公平比较，我们在基准框架之外实现哈希表与模型。
...
与附录 B 相同，我们的记录长 20 字节，由 64 位键、64 位负载和 32 位元数据组成。对于所有架构，我们都尽量提高利用率并使用完整记录；只有 AVX Cuckoo 还测量了 32 位值。我们使用对数正态数据和同前硬件。结果见表 1。
...
正文中，我们聚焦只读内存数据库索引；本附录中，我们概述学习型索引结构未来可如何扩展。

### D.1 插入与更新

乍看之下，模型训练成本可能使插入成为学习型索引的阿喀琉斯之踵；但对某些工作负载，学习型索引也可能显著占优。我们把插入分为两类：（1）追加；（2）中间插入，例如在订单表上更新按客户 ID 建立的二级索引。

先看追加。对于我们前述实验中的 Web 日志时间戳索引，大多数乃至全部插入很可能是时间戳递增的追加。再让我们假定，我们的模型能泛化，学到的模式对未来数据仍成立，那么更新索引就是 $O(1)$：简单追加即可，无须改变模型；B 树却需 $O(\log n)$ 次操作保持平衡。中间插入也可作类似论证，但可能需要移动数据或预留空间，以便把新项放入正确位置。
...
正文中，我们假设真实记录或 `<key, pointer>` 对存储在连续块中。但磁盘数据索引常把数据划分为大页，存于不同磁盘区域。此时 $pos=\Pr(X\lt{}\mathrm{Key})\times N$ 不再成立，我们关于模型学习 CDF 的观察也被破坏。我们考虑以下方案：

- **利用 RMI 结构：** RMI 已把空间划分为多个区域；我们稍改训练过程便可减少各模型覆盖区域的重叠，也可复制可能被多个模型访问的记录。
...
我们在第 5.1.2 节提出另一种学习型 Bloom 过滤器：离散化分类器输出，并把它作为传统 Bloom 过滤器中的额外哈希函数。初步结果在一些情况下优于第 5.2 节，但结果依赖离散化方案，值得进一步分析。下面我们介绍这些额外实验。

与前文一样，我们仍假设已有模型 $f(x)\to[0,1]$。我们为位图 $M$ 分配 $m$ 位，并对每个插入键 $x\in\mathcal K$ 设置：

$$
M[\lfloor m f(x)\rfloor]=1.
$$

我们可以通过观察验证集非键映射到值为 1 的位图位置的比例，得到：
...
此外，我们还使用假阳性率为 $\mathrm{FPR} _ B$ 的传统 Bloom 过滤器。只有当 $M[\lfloor f(q)m\rfloor]=1$ 且 Bloom 过滤器也返回存在时，我们才把查询 $q$ 判为键。因此整体假阳性率为 $\mathrm{FPR} _ m\times\mathrm{FPR} _ B$；若系统目标为 $p^{\ast}$，我们把传统过滤器取为：

$$
\mathrm{FPR} _ B=\frac{p^{\ast}}{\mathrm{FPR} _ m}.
$$

我们仍用 Google 透明度报告数据测试我们的学习型 Bloom 过滤器，并使用宽度 16、字符嵌入 32 维的同一字符 RNN。我们扫描不同 $m$，计算模型、学习型位图与传统 Bloom 过滤器的总大小。目标 $p^{\ast}=0.1\verb0%0$ 时，我们发现 $m=1{,}000{,}000$ 的总大小为 2.21MB，内存减少 27.4%，优于第 5.1.1/5.2 节的 15%；目标 $p^{\ast}=1\verb0%0$ 时，我们得到总大小 1.19MB，减少 41%，也优于第 5.2 节的 36%。
...
[^3]: FAST [44] 等 SIMD 优化索引能把控制依赖转换为内存依赖，但通常仍明显慢于对简单缓存内数据依赖执行乘法；我们的实验也表明，FAST 等结构没有显著更快。
[^4]: 我们当前采用逐阶段训练，而非完全端到端；端到端训练可能更好，留作未来工作。
[^5]: 我们感谢 Michael Mitzenmacher 就这些定义之间的关系进行宝贵讨论，并以富有洞见的意见改进本章。
[^6]: 我们再次感谢 Michael Mitzenmacher 在审阅我们的论文时指出这一关系。
