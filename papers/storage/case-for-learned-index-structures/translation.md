...
实验仍采用 Google 透明度报告数据，以及宽度 16、字符嵌入 32 维的同一字符 RNN。扫描不同 $m$，计算模型、学习型位图与传统 Bloom 过滤器的总大小。目标 $p^{\ast}=0.1\unicode{x25}$ 时， $m=1{,}000{,}000$ 的总大小为 2.21MB，内存减少 27.4%，优于第 5.1.1/5.2 节的 15%；目标 $p^{\ast}=1\unicode{x25}$ 时总大小 1.19MB，减少 41%，也优于第 5.2 节的 36%。
