...
![图 1](assets/ceph-fig01-architecture.png)

图 1：系统架构。客户端通过直接与 OSD 通信执行文件 I/O。每个进程既可以直接链接到一个客户端实例，也可以与已挂载的文件系统交互。
...
![图 2](assets/ceph-fig02-dynamic-subtrees.png)

图 2：Ceph 根据当前工作负载，把目录层次结构的子树动态映射给元数据服务器。只有单个目录成为热点时，才会按哈希把它分散到多个节点。
...
![图 3](assets/ceph-fig03-data-placement.png)

图 3：文件被条带化到多个对象上，对象分组为 placement group（PG），再通过专用副本放置函数 CRUSH 分发给 OSD。
...
![图 4](assets/ceph-fig04-write-protocol.png)

图 4：写入应用到复制该对象的所有 OSD 的 buffer cache 后，RADOS 回复 `ack`；只有安全提交到磁盘后，才向客户端发送最终 `commit` 通知。
...
![图 5](assets/ceph-fig05-osd-write-throughput.png)

图 5：单 OSD 写入性能。水平线表示物理磁盘施加的上限。复制对单 OSD 吞吐量影响很小；但 OSD 数固定时，n 路复制要求把数据写入 n 个 OSD，因此总有效吞吐量降为原来的 1/n。
...
![图 6](assets/ceph-fig06-ebofs-filesystems.png)

图 6：EBOFS 与通用文件系统的性能比较。原型中的粗粒度锁影响小写入，但写入超过 32 KB 后，EBOFS 几乎使磁盘饱和。EBOFS 在大增量写入时按大 extent 布局数据，因此读取性能显著更好。
...
![图 7](assets/ceph-fig07-write-latency.png)

图 7：不同写入大小和复制级别下的写入延迟。复制更新并发进行，因此小写入增加到两个以上副本只产生很小额外成本。大型同步写入受传输时间支配；写入超过 128 KB 时，客户端取得排他锁并异步刷写数据，可掩盖部分延迟。
...
![图 8](assets/ceph-fig08-osd-scaling.png)

图 8：OSD 写入性能随 OSD 集群规模线性扩展，直到 24 个 OSD 时交换机达到饱和。更多 PG 降低 OSD 利用率方差，因此能提高 CRUSH 和哈希的性能。
...
![图 9](assets/ceph-fig09-metadata-latency.png)

图 9：本地磁盘避免第一次网络往返，因而降低写入延迟。读取受益于缓存；`readdirplus` 或放宽一致性则消除 `readdir` 之后执行 `stat` 时与 MDS 的交互。(a) 有无本地磁盘的 MDS 元数据更新延迟，零表示不写 journal；(b) 遍历文件系统的累计耗时。
...
![图 10](assets/ceph-fig10-mds-scaling.png)

图 10：不同工作负载和集群规模下的单 MDS 吞吐量。集群增长到 128 个节点时，大多数工作负载的效率相对完美线性（水平）扩展下降不超过 50%，性能仍远高于现有系统。
...
![图 11](assets/ceph-fig11-mds-latency-throughput.png)

图 11：不同集群规模下的平均延迟与单 MDS 吞吐量，使用 `makedirs` 工作负载。
