...
联系邮箱：`{melnik,andrey,jlong,gromer,shiva,mtolton,theov}@google.com`

## 出版信息

本文发表于 *Proceedings of the VLDB Endowment*，Vol. 3, No. 1（2010），ISSN 2150-8097；本卷论文曾在 2010 年 9 月 13–17 日于新加坡举行的第 36 届 International Conference on Very Large Data Bases 上报告。
...
要执行大规模交互式数据分析，需要非常高的并行度。例如，在一秒内读取 1 TB 压缩数据，按今天的商品磁盘需要数万块磁盘。类似地，CPU 密集型查询可能需要运行在数千 core 上才能在数秒内完成。Google 使用商品机器组成的共享集群来完成大规模并行计算 [5]。集群通常承载大量共享资源的分布式应用，不同应用的工作负载差异很大，机器硬件参数也不同。分布式应用中的某个 worker 可能比其他 worker 花费更长时间完成给定任务，也可能因为集群故障或管理系统抢占而永远无法完成。因此，处理 straggler 和 failure 对快速执行和容错至关重要 [10]。
...
本文描述 Dremel[^1] 系统，它支持在 Google 共享集群上对超大数据集进行交互式分析。与传统数据库不同，Dremel 能够直接在原地嵌套数据上操作。原地意味着可以访问数据所在位置，例如 GFS [14] 等分布式文件系统，或 Bigtable [8] 等其他存储层。传统上，对这类数据执行查询常常需要一系列 MapReduce (MR [12]) 作业，而 Dremel 只需其中一小部分执行时间。Dremel 并不是为了替代 MR；它经常用于分析 MR pipeline 的输出，或快速原型化更大的计算。

[^1]: Dremel 是一种主要依靠转速而非扭矩的电动工具品牌；原文脚注说明，该名称只用于内部项目。
...
本文结构如下：第 2 节说明 Dremel 如何与其他数据管理工具结合用于数据分析；第 3 节介绍其数据模型；第 4–8 节展开上述主要贡献；第 9 节讨论相关工作，第 10 节给出结论。
...
$$
\tau = \mathit{dom} \mid \langle A_1 : \tau[\ast|?], \ldots, A_n : \tau[\ast|?] \rangle
$$

其中 $\tau$ 是原子类型或记录类型。 $\mathit{dom}$ 中的原子类型包括 integer、floating point number、string 等。记录由一个或多个字段组成。记录中字段 $i$ 具有名称 $A_i$，并可带一个可选 multiplicity label。Repeated field (`*`) 可以在记录中出现多次，并被解释为值列表；字段在记录中出现的顺序是有意义的。Optional field (`?`) 可以从记录中缺失。否则，字段是 required，即必须恰好出现一次。
...
**Repetition level。** 以图 2 中的字段 `Code` 为例。它在 `r1` 中出现三次：`en-us` 和 `en` 都位于第一个 `Name` 中，`en-gb` 位于第三个 `Name` 中。为了区分这些出现，Dremel 为每个值附加 repetition level。Repetition level 表示字段路径中哪个 repeated field 的值发生了重复。字段路径 `Name.Language.Code` 包含两个 repeated field：`Name` 和 `Language`。因此，`Code` 的 repetition level 范围在 0 到 2 之间；level 0 表示新记录开始。自上而下扫描 `r1` 时，遇到 `en-us` 前尚未见到任何 repeated field，其 repetition level 为 0；遇到 `en` 时，`Language` 已重复，level 为 2；遇到 `en-gb` 时，最近重复的是 `Name`，而 `Language` 在该 `Name` 之后仅出现一次，因此 level 为 1。故 `r1` 中这三个 `Code` 值的 repetition level 依次为 0、2、1。
...
**Definition level。** 对路径为 `p` 的某个字段的每个值，尤其是每个 `NULL`，都有一个 definition level，用于说明路径中有多少个可能未定义的字段（即 optional 或 repeated 字段）在该记录中实际存在。以 `r1` 为例，它没有 `Backward` link，但字段 `Links` 已定义到 level 1；为保留该信息，Dremel 向 `Links.Backward` 列添加一个 definition level 为 1 的 `NULL` 值。`r2` 中缺失的 `Name.Language.Country` 携带 definition level 1；而该字段在 `r1` 中的两次缺失分别携带 definition level 2（位于 `Name.Language` 内）和 1（位于 `Name` 内）。
...
**Encoding。** 每列存储为一组 block。每个 block 包含 repetition level、definition level（下文统称 level），以及压缩字段值。`NULL` 不显式存储，因为它们由 definition level 决定：若 definition level 小于字段路径中 repeated 与 optional 字段的数量，就表示 `NULL`。对于始终有定义的值，不存储 definition level；同样，repetition level 也只在必要时存储。例如，definition level 0 蕴含 repetition level 0，因而后者可以省略。事实上，图 3 中 `DocId` 不存储任何 level。Dremel 将 level 紧凑打包为 bit sequence；例如，最大 definition level 为 3 时，每个 definition level 使用 2 bit。
...
该语言支持 nested subquery、inter- and intra-record aggregation、top-k、join、user-defined function 等；其中一些特性将在实验部分举例说明。附录 D 给出了 Dremel 上 select-project-aggregate 查询的求值算法。
...
表 $R_1^1,\ldots,R_n^1$ 是发送给 serving tree 第 1 层节点 $1,\ldots,n$ 的查询结果：
...
$T_j^i$ 是第 $i$ 层 server $j$ 处理的、由 $T$ 的 tablet 构成的不相交 partition。每个 serving level 执行类似重写。最终，查询到达 leaf，leaf 并行扫描 $T$ 中的 tablet。向上每一级 intermediate server 都执行 partial result aggregation。上述执行模型很适合返回小型和中型结果的聚合查询，这类查询是交互式查询中非常常见的一类。大型 aggregation 和其他查询类型可能需要基于同一框架的其他执行机制。
...
每个 server 都有内部 execution tree，如图 7 右侧所示。内部树对应于物理查询执行计划，包括 scalar expression evaluation。多数 scalar function 都会生成优化的、特定于类型的代码。Project-select-aggregate 查询的 execution plan 包含一组 iterator，它们以 lockstep 方式扫描输入列，发出带有正确 repetition level 和 definition level 标注的 aggregate 与 scalar function 结果，在查询执行期间完全绕过 record assembly。详情见附录 D。
...
**Scalability。** 下一个实验展示系统在 trillion-record table 上的可扩展性。图 13 所示 Query Q5 选择表 $T_4$ 中出现次数最多的 20 个 `aid` 及其出现次数；该查询扫描 4.2 TB 压缩数据：
...
- 对列数和 server 数都能实现近线性扩展；这一结果适用于包含数千节点的系统。
...
Dremel 从许多 Google 工程师和实习生的贡献中受益良多，特别是 Craig Chambers、Ori Gershoni、Rajeev Byrisetti、Leon Wong、Erik Hendriks、Erika Rice Scherpelz、Charlie Garrett、Idan Avraham、Rajesh Rao、Andy Kreling、Li Yin、Madhusudan Hosaagrahara、Dan Belov、Brian Bershad、Lawrence You、Rongrong Zhong、Meelap Shah 和 Nathan Bales。
...
- [10] G. Czajkowski. Sorting 1PB with MapReduce. Official Google Blog, Nov. 2008. Available at <http://googleblog.blogspot.com/2008/11/sorting-1pb-with-mapreduce.html>.
...
把 record 分解成 column 的算法见图 16。过程 `DissectRecord` 接收一个 `RecordDecoder` 实例，用于遍历二进制编码记录。`FieldWriter` 形成与输入 schema 同构的树状层级。每处理一条新 record，算法都接收 root `FieldWriter`，并把 repetition level 设为 0。`DissectRecord` 的主要任务是维护当前 repetition level；当前 definition level 则由当前 writer 在树中的位置唯一确定，即字段路径上 optional 与 repeated 字段数量之和。

算法的主循环（第 5 行）遍历给定 record 中的所有 atomic field 和 record-valued field。集合 `seenFields` 跟踪字段在该 record 中是否已出现，以确定最近重复的是哪个字段。Child repetition level 取最近重复字段的 level；若没有重复字段，则沿用 parent level（第 9–13 行）。对 nested record，过程在第 18 行递归调用自身。

第 4.2 节提到，`FieldWriter` 会累积 level，并将它们惰性传播给更低层的 writer。具体而言，每个 non-leaf writer 保存一串 `(repetition, definition)` level，并拥有一个 version number；每加入一个 level，version 就加一。Child 只需记住上次与 parent 同步时的 version。Child 获得自己的非 `NULL` 值时，先从 parent 取回新增 level、同步状态，然后再写入新数据。
...
在线上表示中，record 被布局为 field identifier 后接 field value 的 pair。Nested record 可视为具有 opening tag 和 closing tag，类似 XML（实际二进制编码可能不同，详见 [21]）。下文把写入 opening tag 称为“开始”record，把写入 closing tag 称为“结束”record。

过程 `AssembleRecord` 接收一组 `FieldReader`，并隐式接收 reader 间带状态转换的 FSM，最终返回重组后的 record。主过程中的 `reader` 保存当前 `FieldReader`；`lastReader` 保存上一个已将值追加到 record 的 reader，并由图 17 中三个过程共享。主循环（第 5 行）从当前 reader 取下一个值。若根据 definition level 判断该值不是 `NULL`，算法通过 `MoveToLevel` 将正在组装的 record 同步到当前 reader 对应的 record structure，再追加字段值；若值是 `NULL`，则只调整 record structure 而不追加值，这一步在存在空 record 时仍然必要。第 12 行使用 full definition level；普通 definition level 不计 required field，而 full definition level 把所有字段都计入。
...
图 18 展示构造 record assembly 所需 FSM 的算法。输入是要填入 record 的字段，排列顺序与它们在 schema 中的顺序一致。两个字段的 common repetition level 是其 lowest common ancestor 的 repetition level；例如，`Links.Backward` 与 `Links.Forward` 的 common repetition level 为 1。另一个概念是 barrier，即序列中当前字段之后的下一个字段。其直觉是逐个处理字段，直到触及 barrier、必须跳回此前出现过的字段。
...
图 19 展示 Dremel 中 select-project-aggregate 查询的求值算法。它处理查询可能引用 repeated field 的一般情形；对于只引用 required 与 optional field 的 flat-relational query，会使用更简单的优化版本。算法有两个隐式输入：查询中每个字段对应的 `FieldReader` 集合，以及包括 aggregate expression 在内的 scalar expression 集合。一个 scalar expression 的 repetition level 是该 expression 所用字段的最大 repetition level。
