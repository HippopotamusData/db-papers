...
内建 connector 只需指定 connector 名与数据源；没有时可按第 6.4 节扩展。假设并行访问 HBase 表 `sales`，关注 column family `details` 的 `store`、`price` 两个 qualifier；原文随后的建表示例将前者写作 `details:storeid`：
