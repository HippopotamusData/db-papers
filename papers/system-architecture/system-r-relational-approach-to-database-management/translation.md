...
1. **方法 1：** 使用匹配等值谓词的聚簇映像。取出全部结果元组的预期成本为 `R/(T * I)` 次页面访问，即 `R/I` 个元组除以每页 `T` 个元组。
2. **方法 2：** 使用匹配非等值谓词的聚簇映像。假定关系中一半元组满足谓词，预期成本为 `R/(2 * T)`。
3. **方法 3：** 使用匹配等值谓词的非聚簇映像。由于每个元组需要一次页面访问，预期成本为 `R/I`。
4. **方法 4：** 使用匹配非等值谓词的非聚簇映像。取出全部结果元组的预期成本为 `R/2`。
5. **方法 5：** 使用不匹配任何谓词的聚簇映像。扫描映像，并让每个元组接受全部谓词测试。预期成本为 `(R/T) + H * R * N`，其中 `N` 为查询谓词数。
6. **方法 6：** 使用不匹配任何谓词的非聚簇映像。预期成本为 `R + H * R * N`。
7. **方法 7：** 在该关系独占其段时执行关系扫描，并让每个元组接受全部谓词测试。预期成本为 `(R/T) + H * R * N`。
8. **方法 8：** 在段内还有其他关系时执行关系扫描。成本未知，但大于 `(R/T) + H * R * N`，因为有些被读取页面可能不含相关关系的元组。
...
示例 12 是连接查询的一例。其最一般形式包括限制、投影和连接：对关系 `R` 应用某个限制得到 `R1`，对关系 `S` 应用可能不同的限制得到 `S1`；连接 `R1` 和 `S1` 形成关系 `T`，再从 `T` 投影若干字段。我们考虑四种求值方法：
...
[l] 非文献标记：源 PDF 文本层把参考文献页的页眉圆点误识别为小写字母 `l`；可见页面中的正式文献清单仍为连续的 1–31。
