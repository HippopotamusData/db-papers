...
列组数据放在定长页中。区段（extent）是连续存储单元，含该表固定数量的页，也是存储分配单位；每个区段只保存一个列组。行（tuple）在列组上的投影称为小元组（Tuplet，即“mini-tuple”），可空列组的小元组至少有两列值。所有列组中的元组顺序相同。
...
一个列组各列字典分区的笛卡尔积，决定小元组的所有可能格式（即列宽组合），每种组合称为一个单元（Cell）。在常见的单列组中，单元就等同于字典分区。因为元组顺序由装载或插入顺序决定而与压缩编码无关，同组的所有单元都可能出现在同一页。页内属于相同单元或分区、且格式相同的小元组聚在一个区域（Region）中，这相当于按所属单元划分页内小元组。
...
区域再划分为存储带（Bank），即页中保存小元组值的连续区域。小元组的各列可以分开存入不同存储带，例如可空列及其 NULL 指示列可以位于两个存储带。多数存储带保存定长小元组。压缩小元组被紧密装入 128 或 256 位机器字，但加入填充，避免一个小元组跨越机器字边界；该机器字长度称为存储带宽度。`CountryCode` 分区 2 的 3 位编码放入 128 位存储带，每个机器字有 42 个值，余下 2 位作填充（图 1）。
...
其中 `x` 表示结果不关心的位。要把三个最低有效位聚为 `101...`，可乘以 $2^{10} + 2^{20} + 2^{30}$，也就是分别把第 1、2、3 个最低有效位左移 10、20、30 位。POWER7 上使用专用位排列指令；对于 128 位存储带，`Literal1 <= Column <= Literal2` 不论其中容纳多少小元组，约 12 条指令即可求值。
...
LCOL 从紧密排列的存储带中按目标偏移提取编码，必要时再解码。只装载少量存储带条目时直接使用掩码和移位；从存储带内的小元组序号换算到位位置原本需要除以每个存储带的小元组数，系统预先计算模逆（modular inverse）并改用乘法。装载存储带的大部分内容时，则用模板化逻辑把整个存储带解包到按机器字对齐的数组，再抽取目标编码。
...
DB2 BLU 支持内连接、左/右/全外连接、为每个外侧行返回首个匹配的提前连接（early join），以及使用不等谓词的内/左/右反连接。连接不要求是 N:1，也不假定连接列声明了引用完整性或键约束。被连接的表无需装入内存，超过可用内存堆空间时会溢写到磁盘。
...
本节用一个简单的双表连接说明该算法。优化器按代价模型指定哈希连接的构建侧（内侧）与探测侧（外侧），通常偏向把较小表以及可知或可推断的 N:1 连接放到内侧。构建阶段先扫描并分区内侧的连接键和连接载荷；内侧可以是基表，也可以是中间结果。每个线程扫描并分区彼此独立的行子集，之后每个线程接管一个分区，建立从键到载荷的哈希表。内侧扫描唯一的跨线程同步点位于分区阶段与构建阶段之间。扫描同样使用延迟物化和压紧；分区数经过谨慎选择，使每个分区尽量装入可行的某一级内存层次。
