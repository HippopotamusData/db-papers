...
联系方式：`{xiaowei.jxw, yuejun.huyj, yu.xiangy, guangran.jianggr, xiaojun.jinxj, chen.xiac, guobei.jwh, bob.yj, haitao.w, yuan.jiang, jihong.ma, lisu.sl, zengkai.zk}@alibaba-inc.com`
...
**PVLDB 引用格式：** Xiaowei Jiang, Yuejun Hu, Yu Xiang, Guangran Jiang, Xiaojun Jin, Chen Xia, Weihua Jiang, Jun Yu, Haitao Wang, Yuan Jiang, Jihong Ma, Li Su, Kai Zeng. Alibaba Hologres: A Cloud-Native Service for Hybrid Serving/Analytical Processing. *PVLDB*, 13(12): 3272–3284, 2020.

**DOI：** <https://doi.org/10.14778/3415478.3415550>

**许可：** 本文采用 Creative Commons Attribution-NonCommercial-NoDerivatives 4.0 International License（<http://creativecommons.org/licenses/by-nc-nd/4.0/>）。超出该许可范围的使用须通过 `info@vldb.org` 取得授权。版权由权利人持有，出版权授予 VLDB Endowment。本文发表于 *Proceedings of the VLDB Endowment* 第 13 卷第 12 期，ISSN 2150-8097。
...
本文认为，Hybrid Serving/Analytical Processing（HSAP）应在单个系统中统一处理。在阿里巴巴，我们构建了名为 Hologres 的云原生 HSAP 服务。作为新的服务范式，HSAP 面临与现有大数据栈截然不同的挑战（详见第 2.2 节）：（1）系统需要处理远高于传统 OLAP 系统的查询负载。这些负载是混合的，有不同延迟和吞吐权衡。（2）在处理高并发查询负载时，系统还必须跟上高吞吐数据摄取。摄取数据需要在数秒内可读，以满足服务和分析作业严格的新鲜度要求。（3）混合工作负载高度动态，通常会突然突发。系统必须具有高度弹性和可扩展性，能及时响应这些突发。
...
本文其余部分安排如下：第 2 节介绍 Hologres 的关键设计考虑与系统概览；第 3 节说明数据模型和存储框架；第 4 节介绍调度机制与查询处理细节；第 5 节给出并讨论实验结果；第 6 节讨论相关研究；最后第 7 节总结全文。
...
尽管上述过程非常重要，它仍只是完整 pipeline 的一小部分。推荐系统背后还有整套监控、验证、分析和精细化流程，包括对收集事件执行持续 dashboard 查询（图 1 中的 ③），以监控关键性能指标并进行 A/B 测试；还包括周期性 batch 查询（④），用于生成 BI 报表。数据科学家也不断对收集的数据执行复杂交互式分析，以获得实时业务决策洞察、进行因果分析并改进模型。例如，在双 11 购物节期间，传入的 OLAP 请求可达到每秒数百个查询。

图 1 展示的是一个高度复杂的 HSAP 场景：从实时摄取（ⓐ）到 bulk load（ⓑ），从 serving workload（①）、continuous aggregation（③）到 interactive analysis（④），再到 batch analysis（②）。没有统一系统时，上述场景必须由多个孤立系统共同服务：Hive 等系统负责 batch analysis，Cassandra 等系统负责 serving workload，Druid 等系统负责 continuous aggregation，Impala 或 Greenplum 等系统负责 interactive analysis。

### 2.2 HSAP 服务的挑战

作为一种新的大数据服务范式，HSAP 服务带来了在几年前还不突出的挑战。
...
现有 OLAP 系统通常采用基于进程或线程的并发模型，即为每个查询使用独立进程 [5] 或线程 [6]，并依靠操作系统调度并发查询。这种设计导致昂贵的上下文切换，对系统并发设置了硬上限，已不再适合 HSAP；它还使系统缺少足够的调度控制，难以满足不同查询的 SLO。
...
**可定制的调度策略。** HOS 将调度策略从基于 execution context 的调度机制中解耦。HOS 把不同查询的 execution contexts 分组到 scheduling groups，每个组有自己的资源 share。HOS 负责监控每个 scheduling group 已消耗的 share，并在组之间实施资源隔离和公平性。

### 2.5 系统概览

图 2 给出 Hologres 系统概览。Frontend nodes（FEs）接收客户端提交的查询并返回查询结果。对每个查询，FE 节点中的 query optimizer 生成查询计划，并将其并行化成 fragment instances 的 DAG。Coordinator 把查询计划中的 fragment instances 分发到 worker nodes，每个 worker node 再把 fragment instances 映射为 work units（第 4.1 节）。Worker node 是物理资源单元，例如 CPU cores 和 memory。每个 worker node 可以为一个数据库持有多个 table group shards 的 memory tables（第 3.2 节）。在 worker node 中，work units 作为 EC pool 中的 execution contexts 执行（第 4.2 节）。HOS scheduler 按预配置调度策略（第 4.5 节），在系统线程之上调度 EC pool（第 4.3 节）。

![图 2：Hologres 架构](assets/hologres-fig02-architecture.png)

*图 2：Hologres 架构。*

Resource manager 管理 table group shards 在 worker nodes 之间的分布。Worker node 中的资源被逻辑划分为 slots，每个 slot 只能分配给一个 table group shard。Resource manager 也负责向 Hologres 集群添加或移除 worker nodes。Worker nodes 周期性向 resource manager 发送 heartbeat；worker node 故障或集群出现负载突发时，resource manager 会动态向集群添加新的 worker nodes。Storage manager 维护 table group shards（见第 3.1 节）的目录及其元数据，例如物理位置和 key ranges。每个 coordinator 缓存这些元数据的本地副本，以便分发查询请求。

Hologres 允许单个查询跨 Hologres 和其他查询引擎执行（第 4.2.3 节）。例如，当 fragment instances 需要访问不存储在 Hologres 中的数据时，coordinator 会把它们分发到存储所需数据的其他系统。我们设计并实现了一组统一 API，使在 Hologres 中执行的 work units 可以与 PostgreSQL [12] 等其他执行引擎通信。非 Hologres 执行引擎有自己独立的查询处理和调度机制。
...
例如，考虑一个只含一张表的 TGS：该表有一个唯一 secondary index（ $k _ 1\rightarrow v _ 1$ ）和一个非唯一 secondary index（ $k _ 2\rightarrow v _ 2$ ），base data 同时存为 row tablet 和 column tablet。此时 base-data tablets 的 key 都是 $\langle\text{row locator}\rangle$，唯一索引 tablet 的 key 是 $\langle k _ 1\rangle$，非唯一索引 tablet 的 key 则是 $\langle k _ 2,\text{row locator}\rangle$。
...
Tablets 统一按 LSM tree 管理。每个 tablet 包含 worker node 内存中的 memory table，以及持久化在分布式文件系统中的一组不可变 shard files。Memory table 会周期性 flush 为 shard file。Shard files 被组织成多个 levels： $\mathrm{Level} _ 0,\mathrm{Level} _ 1,\ldots,\mathrm{Level} _ N$。在 $\mathrm{Level} _ 0$，每个 shard file 对应一次 flush 的 memory table。从 $\mathrm{Level} _ 1$ 开始，该 level 中所有记录按 key 排序，并按 key 分区到不同 shard files，因此同一 level 中不同 shard files 的 key ranges 不重叠。 $\mathrm{Level} _ {i+1}$ 可以容纳比 $\mathrm{Level} _ i$ 多 $K$ 倍的 shard files，每个 shard file 最大大小为 $M$。Row tablet 和 column tablet 的更多细节分别见第 3.3 节和第 3.4 节。

Tablet 还维护 metadata file，用于存储 shard files 状态。该 metadata file 以类似 RocksDB [13] 的方式维护，并持久化到文件系统。

由于记录被 versioned，TGS 中的读写完全解耦。在此基础上，Hologres 使用 lock-free 方法：只允许 WAL 有单个 writer，但允许任意数量 reader 在 TGS 上并发。由于 HSAP 场景的一致性要求弱于 HTAP，Hologres 只支持 atomic write 和 read-your-writes read，以获得读写的高吞吐与低延迟。下面详细说明读写过程。
...
**Single-shard write。** 如图 3 所示，收到 single-shard ingestion 后，WAL manager（1）为写请求分配 LSN，LSN 由 timestamp 和递增序列号组成；（2）创建新的 log entry 并将其持久化到文件系统。Log entry 包含重放该写所需的信息。Log entry 完全持久化后，写被提交。随后，（3）写请求中的操作应用到相应 tablets 的 memory tables 中，并对新的读请求可见。不同 tablet 上的更新可并行化（见第 4.1 节）。Memory table 满后，（4）被 flush 为文件系统中的 shard file，并初始化新的 memory table。最后，（5）shard files 在后台异步 compact。Compaction 或 memory table flush 结束时，tablet 的 metadata file 会相应更新。
...
当前实现中，一个 TGS 的 writer 和所有 readers 被 colocate 在同一个 worker node 中，以共享该 TGS 的 memory tables。如果 worker node 正经历负载突发，Hologres 支持把一些 TGS 从过载 worker nodes 迁移出去（见第 4.4 节）。

Hologres 正在实现位于 writer 远端的 TGS read-only replicas，以进一步均衡并发读，并计划支持两类副本：（1）fully-synced replica 维护 TGS 的最新 memory table 和 metadata file，可服务所有读请求；（2）partially-synced replica 只维护最新 metadata file，只能服务读取已经 flush 到文件系统的数据。系统可根据读版本把 TGS 读取分发到不同副本。两类只读副本都不必复制 shard files；需要时直接从分布式文件系统加载。

对于 TGS 故障，storage manager 从 resource manager 请求可用 slot，同时向所有 coordinators 广播 TGS-fail 消息。恢复 TGS 时，系统从最新 flush LSN 起重放 WAL logs 来重建 memory tables；所有 memory tables 完整重建后恢复才结束。随后 storage manager 得到确认，并广播包含新位置的 TGS-recovery 消息；coordinators 在恢复前临时 hold 访问失败 TGS 的请求。
...
**Row tablet 写入。** Row tablet 中的 insert 或 update 包含 key、column values 和 `LSN_write`。Delete 包含 key、特殊删除标记和 `LSN_write`。每个写会被转换为 row tablet 的 key-value pair。Insert 和 update 的 `del_bit` 为 0。Delete 的 column fields 为空且 `del_bit` 为 1。Key-value pairs 先追加到 memory table。Memory table 满后，flush 到文件系统中作为 $\mathrm{Level} _ 0$ shard file，并可能在 $\mathrm{Level} _ i$ 满时触发从 $\mathrm{Level} _ i$ 到 $\mathrm{Level} _ {i+1}$ 的级联 compaction。
...
其中 `value_cols` 是非 key 列，`LSN` 是相应写 LSN。在 column LSM tree 中，memory table 用 Apache Arrow [2] 格式存储。记录按到达顺序持续追加到 memory table。Shard file 中记录按 key 排序，并在逻辑上划分为 row groups。Row group 的每一列存为独立 data block，同一列的 data blocks 在 shard file 中连续存储，以方便顺序扫描。Meta block 维护每列和整个 shard file 的元数据，以加速大规模数据读取：对每列记录 data block offsets、各 data block 的 value ranges 和 encoding scheme；对 shard file 记录 compression scheme、总行数、LSN 和 key range。为了按给定 key 快速定位行，index block 还保存各 row group 已排序的首 key。

Delete map 是一个 row tablet，key 是 column LSM tree 中的 shard file ID（memory table 被视作特殊 shard file），value 是 bitmap，表示该 shard file 中哪些 records 在相应 LSN 新近被删除。Delete map 使 column tablet 能够大规模并行执行顺序扫描。

**Column tablet 读取。** 每个读取包含目标列和 `LSN_read`，结果通过扫描 memory table 和全部 shard files 得到。扫描 shard file 前，系统比较其 LSN range 与 `LSN_read`：（1）若最小 LSN 大于 `LSN_read`，跳过该文件；（2）若最大 LSN 小于等于 `LSN_read`，整个文件对该读版本可见；（3）否则，只有部分记录可见，系统扫描文件的 LSN column，生成标示可见行的 LSN bitmap。为了过滤 shard file 中被删除的行，系统以 shard file ID 为 key、在 `LSN_read` 版本读取 delete map（见第 3.3 节），并把所有 candidate bitmaps 做 union。所得删除 bitmap 与 LSN bitmap 相交，再与目标 data blocks join，以过滤掉被删除和对该读版本不可见的行。与 row tablets 不同，column tablet 中每个 shard file 可独立读取，无需与其他 levels 的 shard files consolidate，因为 delete map 能高效给出一个 shard file 中截至 `LSN_read` 的所有删除行。

**Column tablet 写入。** Insert 包含 key、一组 column values 和 `LSN_write`。Delete 指定待删除行的 key，系统据此快速找到包含该行的 file ID 及其 row number。Delete 会在 delete map 中以 `LSN_write` 版本插入：key 为 file ID，value 为要删除行的 row number。Update 通过先 delete、再 insert 实现。向 column LSM tree 和 delete map 插入都可能触发 memory table flush 和 shard file compaction。
...
**TGS 迁移。** 当前实现中，read/write fragment instances 总是分发到托管 TGS 的 worker nodes。如果某个 TGS 成为热点，或 worker node 过载，Hologres 支持把一些 TGS 从过载 worker nodes 迁移到更有可用资源的节点。迁移 TGS 时，系统在 storage manager 中将该 TGS 标为 failed，然后按标准 TGS 恢复流程（见第 3.2.3 节）在新的 worker node 中恢复。如第 3.2.3 节所述，read-only replicas 完成后，也可把对某 TGS 的 read fragment instances 均衡到位于多个 worker nodes 的只读副本。
...
为隔离摄取和查询负载，HOS 将 data-bound ECs 和 query ECs 放入不同 SG。Data-bound ECs 处理所有查询共享且需要同步的关键操作，主要服务摄取负载（read-sync WU 通常非常轻量）；Hologres 把所有 data-bound ECs 归入一个 data-bound SG。相反，不同查询的 query ECs 被放入独立 query SG。Data-bound SG 被分配足够大的 share 以处理所有摄取负载。默认情况下，所有 query SGs 被分配相同 share，以实施公平资源分配。SG share 可配置。

给定 SG，它在一个时间区间中分配给 ECs 的 CPU time 受两个因素影响：（1）其 share；（2）上一时间区间中已占用的 CPU time。SG 的 share 会根据其 ECs 在上一时间区间中的状态调整，具体如下。EC 只有处于 runnable 状态时才可调度。令 $\mathrm{ECshare} _ i$ 表示 EC $i$ 的 share，用 $\mathrm{ECshare} _ {\mathrm{avg},i}$ 表示 EC $i$ 在某时间区间中的实际 share，SG $i$ 的实际 share 是其 ECs share 之和：

$$
\mathrm{ECshare} _ {\mathrm{avg},i}
= \mathrm{ECshare} _ i \times
\frac{\Delta T _ {\mathrm{run}}}
{\Delta T _ {\mathrm{run}}+\Delta T _ {\mathrm{spd}}+\Delta T _ {\mathrm{blk}}}
$$

$$
\mathrm{SGshare} _ {\mathrm{avg},i}
= \sum _ {j=1}^{N} \mathrm{ECshare} _ {\mathrm{avg},j}
$$

其中 $\Delta T _ {\mathrm{run}}$、 $\Delta T _ {\mathrm{spd}}$ 和 $\Delta T _ {\mathrm{blk}}$ 分别表示 EC $i$ 处于 runnable、suspended 和 blocking 状态的时间区间。

对 SG $j$ 中的 EC $i$，系统维护 Virtual Runtime，用于反映其历史资源分配状态。令 $\Delta\mathrm{CPUtime} _ i$ 表示 EC $i$ 在上一时间区间中获得的 CPU time，则 EC $i$ 的 Virtual Runtime 增量为：

$$
\mathrm{ECvshare} _ i
= \frac{\mathrm{ECshare} _ i \times \mathrm{SGshare} _ j}
{\mathrm{SGshare} _ {\mathrm{avg},j}}
$$

$$
\Delta\mathrm{vruntime} _ i
= \frac{\Delta\mathrm{CPUtime} _ i}{\mathrm{ECvshare} _ i}
$$
...
本节评估 Hologres 性能。首先分别在 OLAP 和 serving 工作负载上，将 Hologres 与先进的专用 OLAP 系统、serving 系统比较（第 5.2 节）；结果表明，即使与这些专用系统相比，Hologres 也具有更优性能。随后从三方面评估 Hologres 处理混合 serving/analytical processing 工作负载的能力：

1. 分别考察只处理 analytical workload 或 serving workload 时，Hologres 的并行化与资源扩展能力，包括增加工作负载与计算资源的实验（第 5.3 节）；
2. 考察 HOS 能否在混合负载下实施资源隔离与公平调度，以及能否迅速响应突发负载（第 5.4 节）；
3. 考察存储设计的效率，包括高速数据摄取对读性能的影响，以及维护多个索引时的写延迟与写吞吐（第 5.5 节）。

### 5.1 实验设置

**工作负载。** 使用 TPC-H benchmark [15]（1TB）模拟典型 analytical workload。使用 YCSB benchmark [17] 模拟典型 serving workload；其表有 1 亿条记录，每条记录 11 个字段，每个字段 100 bytes。在第 5.4.1 节测试同一数据上的混合 serving/analytical workload 时，系统使用 TPC-H 数据集，并把 TPC-H 查询与 `lineitem` 表上的 synthetic serving queries（point lookup）混合。为研究混合读写请求，我们模拟阿里巴巴生产工作负载 PW。PW 有一张购物车表，包含 6 亿行、每秒 $10^6$ 次 update；每条记录 16 个字段、大小 500 bytes。实验期间会重放这些 update。

**系统配置。** 默认集群由 8 台物理机器组成，每台有 24 个虚拟核（超线程）、192GB 内存和 6TB SSD；除非特别说明，TPC-H 和 YCSB 都使用该配置。由于当时没有现成 HSAP 系统，分析处理与 Greenplum 6.0.0 [5] 比较，serving 与 HBase 2.2.4 [6] 比较：

- Greenplum 集群共有 48 个 segments，均匀分布在 8 台物理机器上，每个 segment 分配 4 cores。该设置遵循官方文档 [11] 对 intra-query 和 inter-query 并发的建议。Greenplum 使用本地磁盘，并以 column format 存储数据。
- HBase 集群有 8 个 region servers，每台物理机器部署一个。HBase 把数据文件存入使用本地磁盘配置的 HDFS，并以 row format 存储数据。
- Hologres 集群有 8 个 worker nodes，每个 worker node 独占一台物理机器。为与 Greenplum 和 HBase 公平比较，Hologres 同样使用本地磁盘，并同时以 row 和 column formats 存储数据。

PW 实验另在云环境中进行，共使用 1,985 cores 和 7,785GB memory，以 Pangu 远程分布式文件系统存储数据。购物车表的 base data 使用 column format，另有一个以 row format 存储的 index。

**实验方法。** 所有实验先预热 20 分钟。每个报告的数据点都重复 5 次并给出平均值。YCSB 数据实验使用标准 YCSB client；TPC-H 和 PW 数据实验使用一个类似 YCSB 的自实现 client。Client connections 异步提交查询，可配置单个连接能够提交的最大并发查询数，记为 $W$；多个连接还可并发提交请求。除非特别说明，实验均令 $W=100$。

### 5.2 整体系统性能

本组实验分别研究 Hologres 在分析与 serving 工作负载上的性能，并与专用 OLAP 和 serving 系统比较。

**Analytical workload。** Hologres 与 Greenplum 在 TPC-H 数据集上比较。为准确测量查询延迟，实验使用单个 client，并令 $W=1$。图 6(a) 报告 22 个查询的平均端到端延迟。Hologres 在全部 TPC-H 查询上都优于 Greenplum，平均延迟只有 Greenplum 的 9.8%；在 Q1 上快 42 倍。原因包括：

1. HOS 为查询执行提供灵活的高 intra-operator parallelism，读并行度最高可达到表中 shard files 的数量；Greenplum 的并行度则由 segment 数决定，无法在所有查询上充分利用 CPU，例如 Q1。
2. Column tablet 布局支持高效 encoding 和 indexes；当 filter 可下推到 data scan 时，这些布局优化能显著改善性能，例如 Q13。
3. Hologres 使用高效向量化执行，并支持 AVX-512 [8]，可进一步加速从向量化中获益的查询，例如 Q15。[^1]
4. Hologres 可利用 join dynamic filter 等优化生成更优计划，例如 Q20。

为验证以上四项技术，图 6(b) 对每项各选一个代表查询并比较开关功能后的延迟：Q1-DOP 把并行度限制为 Greenplum segment 数；Q13-Storage 关闭 dictionary encoding；Q15-AVX512 使用不含 AVX-512 的构建；Q20-Plan 关闭 dynamic filter。四项技术分别带来 1.2–7.6 倍的性能提升。

论文还用 TPC-H 100GB 做了单机 microbenchmark，将 Hologres 与 Vectorwise（Actian Vector 5.1 [1]）比较。机器有 32 cores 和 128GB memory。Hologres 跑完全部 22 个查询需要 84 秒，而 Vectorwise 需要 27 秒。这说明 Hologres 的单机性能仍有改进空间；Vectorwise 中的优化技术也可用于 Hologres，Hologres 团队计划在未来集成这些技术。

**Serving workload。** 实验使用 YCSB 比较 Hologres 与 HBase，把查询吞吐从 100K QPS 逐步提高到 1600K QPS，并对每个吞吐报告平均、P95 和 P99 延迟。P99 latency SLO 设为 100ms，超过 SLO 的数据点不再报告。

HBase 无法扩展到 1000K QPS 以上，因为查询延迟超过 SLO；Hologres 即使在 1600K QPS 时，P99 延迟仍低于 6ms，P95 延迟甚至低于 1.18ms。吞吐低于 1000K QPS 时，Hologres 的平均、P95 和 P99 延迟平均分别比 HBase 好 10 倍、22 倍和 57 倍。HBase 的 thread-based concurrency model 在高并发 serving workload 下产生显著上下文切换开销；Hologres 的 execution contexts 很轻量，可用极低切换开销协作调度，因而延迟也更稳定。例如在 800K QPS 时，HBase 的 P99 延迟是其平均延迟的 10.5 倍，而 Hologres 中这一倍数只有 1.8。
...
上述实验表明，借助新的存储和调度设计，Hologres 在分析与 serving 两类负载上都持续优于先进的专用系统。

### 5.3 Hologres 的并行性和可扩展性

下面分别研究 Hologres 处理 analytical workload 与 serving workload 时的并行性和可扩展性。

**Analytical workload。** 实验选择顺序扫描大量数据的 TPC-H Q6，考察查询并行化与增加计算资源后的扩展性。第一组实验使用默认集群（8 个 worker nodes，每个 24 cores），由单个 client 提交查询，把并发查询数 $W$ 从 1 增加到 32。图 7(a)(b) 显示，随着并发查询数增加，吞吐保持稳定，说明单个分析查询已经能充分利用硬件并行度；由于并发查询平均共享资源，查询延迟近似线性增加。

第二组实验固定 $W=8$，仍使用 8 个 worker nodes，但把每个 worker 的 cores 从 3 增加到 24，也就是总 cores 从 24 增加到 192。图 7(c)(d) 显示吞吐线性提高、查询延迟随 cores 增加而下降，再次表明高 intra-operator parallelism 能自动饱和硬件并行度。

**Serving workload。** 实验仍使用 8 个 worker nodes，把每个 worker 的 cores 从 3 增加到 24。每种配置下都由 8 个 clients 持续提交查询，并逐步增加吞吐，直到 P99 延迟超过 2ms SLO。图 7(e)–(h) 显示最大吞吐随 cores 近似线性增长；192 cores 时的最大吞吐是 24 cores 时的 8 倍。在达到最大吞吐前，平均、P95 和 P99 延迟都保持稳定；例如 192 cores 时，即使吞吐增长，三项延迟也只缓慢增加。这说明 Hologres 能完全控制用户态 execution contexts 的调度。
...
本节考察 HOS 的两项能力：混合 serving/analytical workload 下的资源隔离，以及突然突发时的调度弹性。

#### 5.4.1 混合工作负载下的资源隔离

实验研究延迟敏感 serving 查询是否会受资源密集分析查询影响。混合负载包含两部分：（1）background：持续提交谓词不同的 TPC-H Q6，并把后台并发查询数 $W$ 从 0 增加到 16；（2）foreground：提交 point lookup serving queries，并令 $W=1$ 以准确测量延迟。每个后台负载设置收集 50,000 个前台延迟数据点并绘制 CDF。

图 8(a) 显示，后台查询从 0 增加到 1 时，serving 查询延迟小幅上升；继续从 1 增加到 16 则不再增加。这表明 HOS 很好地隔离了不同查询的资源，因为不同查询的 execution contexts 被放入不同 scheduling groups。因此，分析与 serving 查询可共存于同一系统，并同时满足各自的 latency SLO。

#### 5.4.2 突发工作负载下的调度弹性

实验在时间 0 同时提交 Q1 和 Q2；时间 5 提交 Q3–Q7，它们约在时间 30 完成；时间 40 提交 Q8；Q1 和 Q2 约在时间 50 完成；最后在时间 60 提交 Q9 和 Q10，并让 Q8–Q10 运行到结束。所有查询优先级相同。图 8(b) 显示各查询沿时间线占用的 CPU 比例。

时间 5 后，HOS 迅速调整资源分配，让 7 个查询获得相同 CPU share；时间 30，Q3–Q7 完成后，HOS 立即在仍运行的 Q1 和 Q2 之间平均重新分配 CPU。时间 40、50 和 60 也出现同样的及时调整。这说明 HOS 能根据系统中的实时并发负载动态、迅速改变调度，并始终保证公平共享。
...
本组实验评估读写分离对查询延迟的影响，并研究 Hologres 在维护索引时的写性能。

#### 5.5.1 分离读写操作

为研究写对查询延迟的影响，实验在 PW 上生成混合读写负载。背景部分重放 PW tuple writes，模拟 20 分钟后台负载，并把 write clients 数从 1 增加到 32 以改变写吞吐；写均匀分布到各 TGS。以 32 个 write clients 为例，实验每 10 秒采样一次写吞吐，并在图 9(b) 报告所有 TGS 的平均、最小和最大写吞吐。前台部分使用 16 个 clients 提交 OLAP 查询，每个 client 令 $W=1$，并在图 9(a) 报告每种吞吐设置下的平均查询延迟。

图 9(a) 显示，后台写吞吐增加时，前台 OLAP 查询延迟保持稳定。高吞吐写对查询延迟影响很小，原因是 Hologres 分离了读写路径，versioned tablets 保证读不会被写阻塞。

#### 5.5.2 写性能

实验使用 YCSB 研究索引维护下的写性能，为 YCSB 表创建 0–10 个 secondary indexes。每种设置都把系统推到最大写吞吐，并报告写延迟的 P95 和 P99。图 9(c)(d) 显示，随着索引数量增加，写延迟和写吞吐都相当稳定。与没有 secondary index 相比，维护 10 个 secondary indexes 只让写延迟增加 25%、写吞吐下降 8%。

索引维护开销很小有三个原因：（1）Hologres 让一个 TGS 中所有 index tablets 共享 WAL，增加索引不会增加 log flush；（2）每次写 TGS 时，每个索引由独立 write-apply WU 并行更新；（3）memory table flush、file compaction 等操作被积极并行化，并卸载到 background EC pool。只要计算资源充足，该设计就能消除性能瓶颈。
...
[^1]: Hologres 主要在以下操作中使用 AVX-512：（1）加、减、乘、除、相等和不等判断等算术表达式；（2）filtering；（3）bitmap operations；（4）hash value computation；（5）batch copy。
...
MPP 数据库如 Greenplum [5] 通常把数据分区为大 segments，并把 data segments 与计算节点 colocate。系统扩展时，MPP 数据库通常需要 reshard 数据。相反，Hologres 以 TGS 管理数据，TGS 是比 segment 小得多的单元。Hologres 动态把 TGS 映射到 worker nodes，并可在 worker nodes 之间灵活迁移而无需 reshard 数据。Worker node 只需在内存中保存它所托管 TGS 的 memory tables；TGS shard files 则按需从远程文件系统获取。多租户调度方面，Greenplum 等系统在不同进程中处理不同请求并依赖 OS 调度，容易给查询并发设置硬上限。Hologres 则在一组用户态线程上 multiplex 并发查询，取得更好的查询并发。

Quickstep [31] 和 morsel-driven parallelism [29] 等研究分析工作负载的高并行查询处理机制，把查询分解为小任务，并在固定到物理 cores 的线程集合上调度。Hologres 也采用高并行方法，但使用分层调度框架；work unit 抽象降低了多租户场景中调度大量任务的复杂度和开销。Execution contexts 与 scheduling groups 提供了跨租户资源隔离机制。[19] 研究多租户 Database-as-a-Service 中用于性能隔离的 CPU sharing，强调绝对 CPU 预留；Hologres 只需要相对 CPU 预留，就足以防止分析查询延迟 serving queries。

**HTAP 系统。** 随着实时分析需求增加，研究界和工业界对大数据集上的 HTAP 方案产生大量兴趣。[33] 研究混合 row/column format 如何改善不同数据访问模式下的查询性能；后续 SAP HANA [21]、MemSQL [9]、HyPer [23]、Oracle Database [25] 和 SQL Server [20, 28] 等系统同时支持事务和分析处理。它们通常用 row format 处理 OLTP、column format 处理 OLAP，但需要在两种格式之间转换。由于这些转换，新提交数据可能无法立即反映到 column stores 中。Hologres 则可把表同时存储在 row 和 column tablets 中，对表的每次写会同时更新两类 tablets，并行化这些写以获得高写吞吐。

此外，HSAP 场景摄取率远高于 HTAP 场景事务率，例如用户通常会产生数十次 page-view event 才发生一次购买事务；但 HSAP 的一致性要求通常较弱。Hologres 有意只支持 atomic write 和 read-your-write read，通过避免复杂并发控制获得更高读写吞吐。[32] 研究 HTAP 高并发负载的任务调度：对大量使用同步的 OLTP tasks，它自适应调整并发度以饱和 CPU；Hologres 则采用 latch-free 方法，避免频繁阻塞。对 OLAP workload，[32] 用 concurrency hint 调整 task granularity，这一方法也可集成到 Hologres 的 execution-context 调度中。

**NewSQL。** Hologres 中采用的 sharding 机制类似 BigTable [16] 和 Spanner [18]。BigTable 使用 table tablet 抽象支持对排序数据的范围搜索。Spanner 是支持强一致性的全球分布式 key-value store，其 data shard 是在存在分布式数据复制时维护一致性的基本单元。与主要用于 OLTP 的 Spanner 不同，Hologres 为 HSAP 场景有意选择较弱一致性模型以追求更好性能。
