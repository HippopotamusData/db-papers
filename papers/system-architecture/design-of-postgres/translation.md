...
POSTGRES 的第一个目标是更好地支持复杂对象 [LORI83, STON83c]。考虑一个地理数据库，其中有三种对象：多边形、圆和直线。可以用三个关系表示它们：
...
第二个目标是让系统可扩展。传统 DBMS 只有少量内置数据类型和访问方法。CAD/CAM 可能需要几何类型，地图应用可能需要经纬度位置类型；用内置类型模拟它们会产生冗长而费解的查询，也可能性能很差。关于 box 类型的简单示例见 [STON86]。用户应当能够增加新的数据类型、新运算符和新的访问方法。例如，多维点数据可能适合 K-D-B 树 [ROBI81]，多边形数据可能适合 R-tree [GUTM84]。关键是，非数据库专家也应当能实现这些扩展，所以所有用户可编写代码的接口都必须易于使用。DeWitt 与 Carey 等研究者也在追求类似目标 [DEWI85]。系统不能把类型系统、运算符语义和访问路径永久写死在核心代码中。

第三个目标是支持主动数据库和规则。许多应用采用 alerter 和 trigger 最容易编写。例如，错误报告系统一类表单流应用会把主动表单从一位用户传给另一位用户 [TSIC82, ROWE82]。在这类应用中，如果分配给程序员的一项高优先级错误未能在指定日期前修复，系统就需要用数据库 alerter 向程序维护组经理发送消息并提醒其注意。Trigger 则可传播数据库更新以维持一致性；例如，删除 `DEPT` 关系中的部门元组时，可以触发更新，删除 `EMP` 关系中该部门的全部员工。
...
7. 教师可以有最多 2 个课堂接触小时的结余或亏欠。

这些规则会频繁改变；休假状态、课程安排以及系主任和排课负责人等行政任职也都经常变化。最自然的做法是把上述规则存入 DBMS，再据此推导每位教师的实际教学负担；如果把教学负担当作普通数据保存，再试图用一组复杂完整性约束强制执行这些规则，就会困难得多。
...
它表示先求出员工的 `hobbies`，再从该对象中取出 `battingavg`。这种多重点号表示法在 [STON84a] 中提出，最初由 Zaniolo 在 GEM [ZANI83] 中建议。

过程也是一种数据类型。过程可以用带嵌入式数据操作命令的通用程序设计语言编写，例如 EQUEL [ALLM76] 或 Rigel [ROWE79]，并存入关系。假设有：
...
此时 `display` 字段可以改用 POSTQUEL 类型，因为 `OBJECT` 中保存的已不是过程本身，而只是执行该过程的命令。`display-list` 过程可以再执行对象中保存的 POSTQUEL：
...
迭代查询是计算传递闭包所必需的 [GUTM84, KUNG84]。在命令后加 `*` 可以表示迭代执行。考虑员工关系：
...
每当 Bill 的员工记录发生变化时，结果就会通过 portal 返回给发出命令的程序。[^1]

Trigger [ASTR76] 则是持续存在的更新命令。例如，下面的规则在某个部门没有员工时删除该部门：
...
迭代查询执行到不再产生效果便终止，而 alerter 和 trigger 会无限期存在。这样的 `always` 命令实现正向链式推理：一次更新会唤醒一组 alerter 和 trigger，它们又可能唤醒其他命令；直到不再有新命令被唤醒，过程才终止。
...
会由标准查询修改算法 [STON75] 的扩展转换成递归命令，或转换成作用于存储关系的一系列命令，从而推导 Bill 的所有祖先；查询优化器也必须扩展，才能处理这些命令。
...
Portal 可以由 `Retrieve-portal` 或 `Execute-portal` 命令定义。下面的命令建立一个名为 `P` 的 portal：
...
Portal 缓冲区中的数据，就是浏览器当前显示的数据。用户在终端输入的命令会被前端浏览器转换成数据库命令，改变缓冲区内容后再重新显示。例如，先取出 20 条记录后，依次执行 `move P forward 10` 和 `fetch 20 into P`，会把当前位置移到 portal 的第 11 条元组，并取回第 11 至第 30 条元组，从而让屏幕向前滚动半页。向后滚动半页则依次使用 `move P backward 10` 与 `fetch 20 into P`。
...
- P1：传递性。如果 $A \lt B$ 且 $B \lt C$，则 $A \lt C$。
- P2：非对称性。如果 $A \lt B$，则不可能有 $B \lt A$。
- P3：三歧性。 $A \lt B$、 $A = B$ 和 $A \gt B$ 三者中恰有一个成立。
- P4： $A \le B$ 等价于 $A \lt B$ 或 $A = B$。
- P5：等号具有对称性，即 $A = B$ 等价于 $B = A$。
- P6： $A \gt B$ 等价于 $B \lt A$。
- P7： $A \ge B$ 等价于 $B \le A$。

任何服从 P1-P7 的运算符集合都可以使用 B-tree。定义新运算符的协议与 ADT-INGRES [STON83c] 相似；建立索引时，用户只需声明要采用的运算符集合，具体语法见 [STON86]。这些信息使优化器能够规范化谓词、识别矛盾、交换参数并选择可用索引，而无需在代码中写死某种类型。
...
的限制谓词，运算符定义会按照 [SELI79] 的方法提供要访问的页面数和要检查的元组数。对可参与 join 的运算符，目录还保存 join 选择率和可用的 join 策略。嵌套循环总是可用；merge join 还必须说明能否使用，以及分别用什么运算符对两个关系排序；hash join 也只在定义声明可用时才能采用。整个机制由目录表驱动，因此加入新类型、运算符和访问方法时，不必修改优化器主体。
...
现有系统通常算出答案后立即丢弃，只用特殊代码使访问计划失效并重算 [ASTR76]。POSTGRES 则计划同时缓存计划和答案。后台 daemon 利用空闲时间或空闲处理器编译计划并预计算答案。插入过程值时，运行时也记录提交用户；编译过程代表该用户检查保护权限，执行过程字段时仍会检查调用者是否获准。只有执行用户与定义用户相同时，才能走 fast-path，这样就无需在运行时访问系统目录，也不会用缓存计划绕过访问控制。最理想时，过程值已经缓存，访问可以立即完成；更常见时，至少已有可复用的访问计划。

缓存对象必须在其依赖数据变化时失效：schema 不适当地改变时，编译计划失效；被访问的数据改变时，缓存答案失效。我们比较过多种实现 [STON85c]，这里采用持久的 `I`（invalidate-me）锁。命令编译或答案预计算时，POSTGRES 在期间访问的全部数据库对象上设置 `I` 锁。锁可细到元组甚至字段，也可以向更粗粒度升级，必须正确检测 phantom [ESWA75]；[STON85a] 建议把这类锁直接放入数据记录。这样，即使建立缓存的进程已经退出或系统崩溃，依赖关系仍然存在。
...
对标识符位于 `T1` 与 `T2` 之间的事务，需要区分三种状态：`0` 表示 abort，`1` 表示 commit，`2` 表示仍在 progress。`T2` 与 `T3` 之间不可能再出现 progress 状态，因此每个事务只需一个状态位；早于 `T3` 的事务已归档，其中止事务的更新也已被 vacuum 丢弃，因而不再需要日志记录。
...
设 $M = 1000$，缓冲区大小为 $B$ 位，事务失败概率为 $F$，每个 `XACT` 位图平均有 $X$ 位被置位。选择：
...
$$
\frac{Q F \cdot 1000}{X}
$$

假设：

$$
\frac{F \cdot 1000}{X} \ll 1
$$

直接缓存 `LOG` 时，缺页概率为：

$$
\mathrm{FAULT}(\mathrm{LOG}) = 1 - \frac{B}{Q}
$$

使用 `XACT` 时，一个目标位图不在缓冲区中的概率为：

$$
P(\mathrm{XACT}) = 1 - \frac{B X}{Q F \cdot 1000}
$$

即使位图在缓冲区中，观察到 1、因而仍需查 `LOG` 的概率也是 $X/1000$。因此，当：

$$
B \lt \frac{Q F \cdot 1000}{X}
$$

时：

$$
\mathrm{FAULT}(\mathrm{XACT}) = P(\mathrm{XACT}) + \frac{X}{1000}
$$

若 $B$ 更大，accelerator 不再占用全部缓冲区，剩余空间可以缓存 `LOG`，所以 $\mathrm{FAULT}(\mathrm{XACT})$ 会继续下降。

两种方案的缺页概率之差为：

$$
\delta = \mathrm{FAULT}(\mathrm{LOG}) - \mathrm{FAULT}(\mathrm{XACT})
$$
...
时，收益最大。图 7 给出了采用该 `X` 时两种系统在不同缓冲区大小下的预期缺页数：只有缓冲区小得微不足道，或已接近足以容纳整个 `LOG` 时，accelerator 才处于劣势。此时：

$$
\mathrm{size}(\mathrm{XACT}) = \sqrt{F} \cdot \mathrm{size}(\mathrm{LOG})
$$

如果 $B = \mathrm{size}(\mathrm{XACT})$，缺页概率可以从 $1-\sqrt{F}$ 降到 $\sqrt{F}$。当 $F = .01$ 时，`XACT` 的大小只有 `LOG` 的十分之一，缺页概率从 $.9$ 降到 $.1$；当 $F = .1$ 时，它约使用三分之一的缓冲空间，并把缺页概率减半。
...
有些用户可能希望最近更新的记录继续保留在磁盘上。为支持这种调节，我们提出允许用户按如下方式指定 vacuum 条件：
...
在这种情况下，vacuum daemon 只有在条件变为真之后，才会从 `rel-name` 的磁盘表示中移除记录。
...
光盘可以有效地用作归档介质；POSTGRES 还有一组后台 daemon，可以有效利用空闲处理器。定制硬件可以提供稳定主存、支持 `LOG` 关系，并在运行时检查元组有效性。因此，前述目标也为未来硬件留下了直接的利用路径。
