...
电子邮件：pmenon@cs.cmu.edu、tcm@cs.cmu.edu、pavlo@cs.cmu.edu

**PVLDB 引用格式：** Prashanth Menon、Todd C. Mowry、Andrew Pavlo，*Relaxed Operator Fusion for In-Memory Databases: Making Compilation, Vectorization, and Prefetching Work Together At Last*，PVLDB 11(1): 1-13，2017。DOI：10.14778/3136610.3136611。该卷论文获邀在 2018 年 8 月于巴西里约热内卢举行的第 44 届 VLDB 大会上报告成果。
...
| 优化 | 修改 | 说明 |
|---|---|---|
| O1 | $P_1\Rightarrow(\mathrm{LineItem}\to\sigma\to\Xi\to\Gamma)$ | 对谓词 $\sigma$ 应用 SIMD。 |
| O2 | - | 使用 $\Xi$ 为 $\Gamma$ build 阶段预取 bucket。 |
...
| 优化 | 修改 | 说明 |
|---|---|---|
| O1 | $P_1\Rightarrow(\mathrm{Customer}\to\sigma_1\to\Xi_1\to\bowtie_1)$ | 对 `Customer` 的谓词 $\sigma_1$ 应用 SIMD。 |
| O2 | $P_2\Rightarrow(\mathrm{Orders}\to\sigma_2\to\Xi_2\to\bowtie_1\to\bowtie_2)$ | 对 `Orders` 的谓词 $\sigma_2$ 应用 SIMD。 |
| O3 | - | 使用 $\Xi_2$ 为 $\bowtie_1$ probe 阶段预取 bucket。 |
| O4 | $P_2\Rightarrow(\mathrm{Orders}\to\sigma_2\to\Xi_2\to\bowtie_1\to\Xi_3\to\bowtie_2)$ | 使用 $\Xi_3$ 为 $\bowtie_2$ build 阶段预取 bucket。 |
| O5 | $P_3\Rightarrow(\mathrm{LineItem}\to\sigma_3\to\Xi_4\to\bowtie_2\to\mathrm{Sort})$ | 对谓词 $\sigma_3$ 应用 SIMD。 |
| O6 | - | 使用 $\Xi_4$ 为 $\bowtie_2$ probe 阶段预取 bucket。 |

图 12 是 Q13 case study。Q13 使用文献 [28] 的 group-join 算子。Orders 的 predicate 不能用 SIMD 直接处理，但在 predicate 后建立 stage boundary，仍能通过预取 group-join 的 hash bucket 提升性能。
...
| 优化 | 修改 | 说明 |
|---|---|---|
| O1 | $P_2\Rightarrow(\mathrm{Orders}\to\sigma\to\Xi_1\to\bowtie\to\Gamma)$ | 使用 $\Xi_1$ 为 $\bowtie$ probe 阶段预取 bucket。 |
| O2 | $P_2\Rightarrow(\mathrm{Orders}\to\sigma\to\Xi_1\to\bowtie\to\Xi_2\to\Gamma)$ | 使用 $\Xi_2$ 为 $\Gamma$ build 阶段预取 bucket。 |
...
| 优化 | 修改 | 说明 |
|---|---|---|
| O1 | $P_1\Rightarrow(\mathrm{LineItem}\to\sigma\to\Xi\to\bowtie)$ | 对 `LineItem` 的谓词 $\sigma$ 应用 SIMD。 |
| O2 | - | 使用 $\Xi$ 和 `Part` 为 join $\bowtie$ 期间预取 bucket。 |
...
| 优化 | 修改 | 说明 |
|---|---|---|
| O1 | $P_2\Rightarrow(\mathrm{LineItem}\to\sigma_1\to\Xi_1\to\bowtie\to\sigma_2\to\Gamma)$ | 对 `LineItem` 的谓词 $\sigma_1$ 应用 SIMD。 |
| O2 | - | 使用 $\Xi_1$ 为 $\bowtie$ probe 阶段预取 bucket。 |
| O3 | $P_2\Rightarrow(\mathrm{LineItem}\to\sigma_1\to\Xi_1\to\bowtie\to\Xi_2\to\sigma_2\to\Xi_3\to\Gamma)$ | 在每对算子之间插入 staging point。 |
