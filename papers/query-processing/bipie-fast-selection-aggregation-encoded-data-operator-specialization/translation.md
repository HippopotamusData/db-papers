...
MemSQL。Akash Katipally 的这项工作完成于其在 MemSQL 任职期间。
...
硬件技术进步让系统能更快处理数据，使企业可以把分析查询直接发送到数据库，对实时数据做决策。这些查询通常是即席的，过滤条件复杂，并且很少能从预建索引中受益。因此，分析查询往往需要扫描大量数据。为了节省磁盘和内存带宽，数据通常会用更少字节进行编码。本文把利用这些机会的一组增强技术统称为 Business Intelligence ProcessIng on Encoded Data（BIPie）。注 1：MemSQL 工程师也非常喜欢 pie（馅饼）。
...
本文先介绍 MemSQL 与 BIPie 所优化的实时分析负载（第 2 节），再给出 BIPie 架构概览（第 3 节）以及选择、聚合策略的细节（第 4、5 节）；随后在广泛参数范围内比较所有策略组合，并以 TPC-H Query 1 对照其他数据库引擎（第 6 节）。
...
查询计划使用 LLVM [14] 编译为机器码，并缓存以加速后续执行。编译后的查询计划不预先指定参数值，因此同一结构的后续查询可以用不同参数快速运行。MemSQL 同时追求广泛硬件适配和性能优先，因此会使用本文这类先进查询执行技术。
...
MemSQL 的列式查询处理遵循 batch processing。扫描 columnstore 表时使用固定行数的移动窗口，MemSQL 中最多 4096 行。窗口内行形成一个 batch。系统完整处理一个 batch 后才进入下一个 batch，并且不会回访之前 batch。这个技术此前已用于 MonetDB/X100 [7]。

### 2.2 简化假设

为便于讲解算法，论文做了若干简化假设。第一，只有一个 group-by 列，且唯一值不超过 256 个。该列使用字典编码和 bit packing。所有不同值被分配从 0 开始的连续整数标识，即 group id。每行的 group-by 值替换为对应 group id，再用能够表示最大字典索引的最小固定 bit 数无间隙顺序存储。这种格式允许使用以 group id 为索引的数组更新每组聚合结果，直到输出聚合结果时才需要访问字典。
...
直接操作编码数据提供三类收益：避免完全或部分解码；避免物化解码值；利用编码附带信息。例如，字典编码已提供从列值到小整数的单射映射，可作为该列的完美哈希函数。避免物化解码值还让系统可以在简单循环中处理输入向量，重复可预测指令序列，并在独立数据元素上运行 [7]。这有利于 CPU 流水线和 SIMD 并行。
...
本文以下重点讨论 Vector Toolbox 中供 Aggregate Processor 使用的部分：多种 SIMD 方法各自只覆盖特定输入范围，并非通用替代品，但能在常见且有用的查询情形中显著加速。
...
多个 `SUM` 可以按列逐个完成，也可逐行更新所有聚合。原文实验中，聚合结果数组采用面向行的布局，逐行更新全部 sum 的 row-at-a-time 方法更快；展开遍历所有聚合列的内层循环还能进一步改善它。图 3 在 32 组和不同 sum 数量下比较了 column-at-a-time、row-at-a-time 与 loop-unrolled row-at-a-time 三种实现，单位为 cycles/row/aggregate。

### 5.2 基于排序的 SUM 聚合

Sort-Based 方法把算法分为重新分组与求和两步。第一遍统计每个 group id 的行数，据此把输出索引数组切成每组一个连续子区间；若查询本来就含 `COUNT(*)`，该计数只计算一次并复用。第二遍把每行的索引追加到对应组的区间。组数很少时，相邻行更新同一 bucket counter 仍可能产生与标量方法相似的写冲突；实现为每个 bucket 分配两个 counter，分别处理偶数行和奇数行。得到索引数组后，对每个组连续遍历索引并对输入列求和，并用 SIMD gather 根据索引取得聚合列值。

聚合时直接对编码输入执行解码或 bit unpacking，把解码、选择和聚合放在同一优化单元中。gather 或 compaction selection 会在排序前排除过滤行；special group assignment 则在排序过程中排除它们。
...
图 6 中字母表示聚合列，数字表示行位置；每一行代表一个 SIMD 寄存器。若一行的所有聚合装得下，只需一次向量装载、一次加法和一次存储；装不下时使用多个寄存器。表 4 给出 32 组、不同聚合数和元素大小组合的示例，单位为 cycles/row/sum。
...
实验使用 TPC-H SF=100，扫描表约 6 亿行，查询使用的编码列可装入测试机内存。`LINEITEM` 按查询没有使用的 `l_orderkey` 排序并分片，因此实验没有利用行顺序。每个 batch 为 4096 行。过滤使用 SIMD 整数比较求值，结果存为 byte vector。两个字符串 group-by 列的整数字典 id 被 bit unpack 为一字节元素向量，再合成范围 0 到 5 的单个整数值。虽然查询输出 4 个组，元数据可判断理论上有 6 个可能组。再通过 Special Group Selection 把 group index byte vector 与过滤得到的 selection byte vector 组合，引入第 7 个组。
...
为比较不同数据库系统的 Q1 性能，我们用执行时间乘标称 CPU 时钟频率与物理核心数，再除以表的总行数，把已发表结果统一成 cycles/row。该归一化并不完全公平，因为忽略了 scale factor 对最大列值的影响、内存速度和 CPU 微架构等差异；但 Q1 的跨核同步很少、CPU 压力高于内存带宽，且对比工作都使用较新的 Intel CPU，因此仍可作为执行进展的概览。BIPie 达到 8.6 CPU cycles/row，比 Gubner 等近期发表的手写实现 [11] 成本低约 2 倍，比最快已发表数据库引擎实现 Hyper 低约 3.3 倍。
...
GPU 数据库 [9]，例如 MapD [19]、Kinetica 和 BlazingDB，通过 GPU 大规模并行性提升分析性能。Boral 和 DeWitt [8] 曾指出，数据库采用专用硬件并非好策略，因为通用硬件发展更快；GPU 究竟属于专用还是通用硬件尚不明确。BIPie 表明，通过针对现代 CPU 架构优化算法，可以在不限制部署选项的情况下达到接近 GPU 数据库的性能。
