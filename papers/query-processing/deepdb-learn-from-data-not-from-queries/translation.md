...
- Benjamin Hilprecht，德国达姆施塔特工业大学，benjamin.hilprecht@tu-darmstadt.de
- Andreas Schmidt，卡尔斯鲁厄理工学院（KIT）与卡尔斯鲁厄应用技术大学（Hochschule Karlsruhe），andreas.schmidt@kit.edu
- Moritz Kulessa，德国达姆施塔特工业大学，moritz.kulessa@tu-darmstadt.de
- Alejandro Molina，德国达姆施塔特工业大学，alejandro.molina@tu-darmstadt.de
- Kristian Kersting，德国达姆施塔特工业大学，kristian.kersting@tu-darmstadt.de
- Carsten Binnig，德国达姆施塔特工业大学，carsten.binnig@tu-darmstadt.de

**PVLDB 引用信息：** Benjamin Hilprecht 等，*DeepDB: Learn from Data, not from Queries!*，*Proceedings of the VLDB Endowment*，13(7):992–1005，2020。DOI：https://doi.org/10.14778/3384345.3384349
...
由于我们的模型捕获的是数据本身的信息，因此它不只服务于某一项特定任务，还可支持从查询回答、基数估计到 DBMS 内机器学习推断等更复杂的任务。有人可能认为，这必然要付出精度代价，因为工作负载驱动方法能够获得比我们的纯数据驱动方法更多的信息。但正如我们在实验中证明的那样，事实并非如此：我们的方法实际上优于多种最先进的工作负载驱动方法，而且泛化能力更好。

不过，我们并不声称数据驱动模型是解决 DBMS 所有任务的“银弹”。相反，我们认为在适当场景中，数据驱动模型应与工作负载驱动模型结合。例如，学习型查询优化器的工作负载驱动模型可以把我们的模型给出的基数估计作为输入特征。数据驱动模型与工作负载驱动模型的结合是一条很有意义的未来研究路线，但超出了本文范围。
...
第 2 节中，我们首先概述 DeepDB；第 3、4 节分别介绍我们的模型及查询编译细节；随后，我们在第 5 节说明 DeepDB 的进一步扩展，再在第 6 节通过广泛实验，将 DeepDB 在多项任务上与最先进方法比较；最后，我们在第 7 节回顾相关工作，并在第 8 节总结全文。

## 2. 概览与应用

### 概览

如图 1 所示，DeepDB 的核心思想是在离线阶段学习数据的表示。需要强调的是，我们并不打算在 DeepDB 中用模型替换原始数据。相反，DeepDB 中的模型像索引一样扩充数据库：既可加速查询并提供额外的查询能力，也让我们仍可在原始数据上运行标准 SQL 查询。
...
RSPN 在离线阶段建立后，可在运行时用于多种应用：既包括面向用户的任务，例如快速给出 SQL 查询的近似答案；也包括系统内部任务，例如提供基数估计。为支持这些任务，DeepDB 提供一种新的概率查询编译过程，把给定任务转换为 RSPN 上的期望与概率求值。下面，我们概述 DeepDB 查询编译引擎目前支持的应用。

### 基数估计

DeepDB 支持的第一项任务是为查询优化器进行基数估计。基数估计不仅用于给出代价估计，也用于在查询优化期间选择正确的连接顺序。与现有学习型基数估计方法 [16, 42] 相比，DeepDB 的一项突出优势是我们不必创建专门训练数据，即查询与基数的配对。RSPN 捕获与工作负载无关的数据特征，因此我们能够支持任意连接查询，而不必针对某种工作负载训练模型。与传统直方图方法相似，RSPN 还能以较低成本保持最新；工作负载驱动的学习型方法 [16, 42] 则需要重新训练。

### 近似查询处理（AQP）

我们目前在 DeepDB 中支持的第二项任务是近似查询处理（Approximate Query Processing，AQP）。AQP 旨在为大数据集快速给出近似答案。单表查询在 DeepDB 中的基本执行思想很简单：例如，带有条件 $C$ 的聚合查询 `AVG(X)` 等于条件期望 $E(X\mid C)$，可以用 RSPN 近似。我们在 DeepDB 中实现了更一般的 AQP 过程，利用 RSPN 对多表连接的支持。与 [25, 44] 等其他学习型 AQP 方法相比，DeepDB 的主要区别仍在于支持即席查询，因而不受训练集所覆盖查询类型的限制。
...
本节中，我们介绍关系和积网络（RSPN）。我们用它学习数据库表示，并通过我们将在下一节介绍的查询引擎回答查询。我们首先回顾和积网络（SPN），然后介绍 RSPN；最后，我们说明如何创建 RSPN 集成来编码含多张表的数据库。

### 3.1 和积网络

和积网络（SPN）[35] 学习数据集中变量 $X_1,X_2,\ldots,X_n$ 的联合概率分布 $P(X_1,X_2,\ldots,X_n)$。它很有吸引力，因为任意条件的概率都能高效计算；我们稍后会把这些概率用于我们的 AQP 和基数估计等应用。
...
SPN 的学习 [10, 29] 递归地把数据拆分为不同的行簇（引入和节点），或拆分为相互独立的列簇（引入积节点）。行聚类可使用 KMeans 等标准算法，也可按随机超平面拆分。为避免对底层分布作强假设，方法使用随机依赖系数（Randomized Dependence Coefficient，RDC）检验不同列之间的独立性 [23]。一旦某簇的行数低于阈值 $n _ {min}$，就假设所有列相互独立。[35, 28] 指出，一般 SPN 的大小是多项式级，推断时间相对于节点数是线性的。不过，对于我们在实验中使用的配置，我们甚至可以把 SPN 大小限制为相对于数据集列数的线性复杂度，因为我们令 $n _ {min}=n_s/100$（即相对于样本量）；实践证明该配置较为稳健。

有了 SPN，我们就能计算任意列条件的概率。直观上，我们先在每个相关叶节点上计算条件，再自底向上求值。例如在图 2(d) 中，为估计来自欧洲且年龄小于 30 岁的客户数，我们先在蓝色地区叶节点计算欧洲客户概率（80% 和 10%），再在绿色年龄叶节点计算年龄小于 30 岁的概率（15% 和 20%）。我们在上方积节点处分别相乘，得到 12% 和 2%；最后在根部和节点结合簇权重，得到 $12\verb0%0\times0.3+2\verb0%0\times0.7=5\verb0%0$。再乘以表中行数，我们便得到约 50 名年龄小于 30 岁的欧洲客户。
...
第一项也是最重要的扩展，是为 `AVG`、`SUM` 等查询计算期望。例如，回答某列平均值的 SQL 聚合查询就需要期望。我们让 RSPN 对叶节点上的变量计算期望，从而回答这些聚合。若还需应用过滤谓词，我们就在过滤属性的叶节点上仍计算概率，并把期望与概率一并向上传播。我们在积节点把来自子节点的期望和概率相乘，而在和节点计算加权平均。

图 3 展示了我们如何计算欧洲客户的平均年龄。两项之比给出正确的条件期望：

$$
E(c\verb0_0age\mid c\verb0_0region=\text{'EU'})=
\frac{E(c\verb0_0age\cdot \mathbf{1} _ {c\verb0_0region=\text{'EU'}})}{P(c\verb0_0region=\text{'EU'})}
=\frac{16.5}{31\verb0%0}.
$$

![图 3：计算条件期望的过程。](assets/figure-03-conditional-expectation.png)

**图 3：计算 $E(c\verb0_0age\mid c\verb0_0region=\text{'EU'})$ 的过程。**
...
3. **非键属性之间的函数依赖。** SPN 不能很好地捕获 $A\rightarrow B$。我们可以简单地忽略这一依赖，并让 RSPN 同时学习属性 $A$ 与 $B$，但这往往会为了实现二者独立而把数据拆成大量小簇，造成很大的 SPN。因此，我们允许用户随表模式一起定义函数依赖。若定义 $A\rightarrow B$，我们会在 RSPN 的独立字典中保存从 $A$ 值到 $B$ 值的映射，并在学习 RSPN 时省略列 $B$；运行时把针对 $B$ 的过滤谓词转换为针对 $A$ 的过滤谓词。

#### 可更新性

RSPN 相比 SPN 的最后一项重要扩展是模型可直接更新。底层数据库表变化后，模型可能不再准确。例如，若我们向图 2(a) 的表中插入更多年轻欧洲客户，图 2(d) 计算出的概率就会偏低，需要更新 RSPN。
...
为支持即席连接查询，正如我们将在第 4 节讨论的那样，一种朴素做法是每张表学习一个 RSPN。但这种做法可能丢失表间相关性，产生不准确的近似。因此，我们在为多表数据库学习 RSPN 集成时，会考虑模式中的表是否相关。

下面，我们介绍为给定数据库模式构造“基础集成”的过程。对于每条外键→主键关系，若两表属性相关，我们就在它们的全外连接上学习一个 RSPN；否则分别为单表学习 RSPN。例如，若模式由图 4 所示的 `Customer` 与 `Order` 表组成，我们可以学习两个独立 RSPN（每表一个），也可以在全外连接上学习联合 RSPN。
...
基础集成只捕获两张表之间的相关性。在我们的实验中，我们看到这已能得到很准确的答案，但非直接相邻表间也可能存在相关性；学习这些相关性有助于进一步提高跨越两张以上表的查询准确性。例如，若还有一张可与 `Order` 连接的 `Product` 表，并且产品价格与客户地区相关，基础集成不会考虑这种相关性。因此，我们在第 5.3 节扩展我们的基础集成创建过程，以纳入多表依赖。

## 4. 查询编译

我们的概率查询编译过程面临的主要挑战，是把输入查询转换为针对 RSPN 集成的推断过程。DeepDB 当前支持的 SQL 查询类别如下：
...
最重要的是，DeepDB 即席支持这些查询：RSPN 集成只学习一次，此后即可用我们的概率查询编译过程回答上述形式的任意查询。下面，我们先介绍无分组 `COUNT` 查询的处理，它足以支持基数估计；再说明如何扩展到更广泛的 AQP 聚合查询，包括 `AVG`、`SUM` 以及分组。

### 4.1 简单 COUNT 查询

本节中，我们说明如何转换带或不带过滤谓词的单表 `COUNT` 查询，以及多表内等值连接上的 `COUNT` 查询。这些查询已经足以用于基数估计，也覆盖部分 AQP 聚合查询。我们把简单 `COUNT` 查询映射到 RSPN 时区分三种情形：
...
我们可用图 4(a) 的 `CUSTOMER` RSPN 回答：

$$
|C|\cdot E(\mathbf{1} _ {c\verb0_0region=\text{'EU'}})=3\cdot\frac{2}{3}=2.
$$

 $\mathbf{1} _ C$ 表示条件 $C$ 成立时为 1 的随机变量，因此 $E(\mathbf{1} _ C)=P(C)$。过滤谓词的合取由 RSPN 直接支持，析取可用容斥原理实现。
...
$$
|C\bowtie O|\cdot P(o\verb0_0channel=\text{'ONLINE'}\cap c\verb0_0region=\text{'EU'})
=4\cdot\frac14=1.
$$

不过，多表联合 RSPN 是在全外连接上学习的。我们用全外连接保留原表中的全部元组，而不只是至少有一个连接伙伴的元组，因此也能用联合 RSPN 回答单表查询，正如我们将在情形 2 看到的那样。
...
$$
|C\mathbin{\Join _ {\mathrm{full}}}O|\cdot
P(o\verb0_0channel=\text{'ONLINE'}\cap c\verb0_0region=\text{'EU'}\cap N_O=1\cap N_C=1)
=5\cdot\frac15=1.
$$

#### 情形 2：存在更大的 RSPN

第二种情形是，我们必须使用建在一组连接表上的 RSPN，而查询只需要其中一个子集。假设我们用图 4(b) 那样在客户与订单全外连接上学习的 RSPN，近似只询问欧洲客户的 $Q_1$。有多个订单的客户会在连接中出现多次，从而被重复计数。全外连接中的欧洲客户比例是 $3/5$，但数据集中实际有三位客户、其中两位来自欧洲。

为解决这个问题，对于表 $P$ 与 $S$ 之间的每条外键关系 $S\leftarrow P$，我们给表 $S$ 增加一列 $F _ {S\leftarrow P}$，表示一个元组拥有多少个对应连接伙伴，并把它称为**元组因子**。例如图 4(a) 中第一位客户有两个订单，所以其元组因子为 2。对每对可通过外键连接的表，元组因子只需计算一次；我们在 DeepDB 创建 RSPN 时计算它们，我们的更新过程也会相应修改这些值。RSPN 把元组因子作为普通附加列学习；我们在连接中使用时将它记作 $F' _ {S\leftarrow P}$。由于我们使用外连接， $F'$ 的值至少为 1。

于是，我们可将统计欧洲客户的查询表示为：

$$
|C\mathbin{\Join _ {\mathrm{full}}}O|\cdot
E\left(\frac{1}{F' _ {C\leftarrow O}}\cdot
\mathbf{1} _ {c\verb0_0region=\text{'EU'}}\cdot N_C\right)
...
$$
|J|\cdot E\left(
\frac{1}{F'(Q,J)}\cdot \mathbf{1} _ C\cdot
\prod _ {T\in Q}N_T
\right).
$$

为简化记号，下文我们把查询 $Q$ 所需的因子写作 $F(Q)$。定理 1 中的期望 $E(F(Q))$ 可由 RSPN 计算，因为所有相关列都已学习。

#### 情形 3：组合多个 RSPN

最后一种情形是我们处理跨越多个 RSPN 的 `COUNT` 查询。我们先考虑两个 RSPN，再把过程扩展到 $n$ 个。我们可把查询拆成分别对应两个 RSPN 的子查询 $Q_L$ 和 $Q_R$；二者还可能有重叠，我们记为 $Q_O$，即对公共表的连接。我们首先用第一个 RSPN 估计 $Q_L$ 的结果，再把该结果乘以 $Q_R$ 中元组数相对于重叠部分 $Q_O$ 的比例。直观上，该比例表示不在 $Q_L$ 中的缺失表会把查询结果的 `COUNT` 放大多少。

例如，图 4(a) 分别有 `Customer` 与 `Order` 的 RSPN。 $Q_2$ 被拆成分别在客户 RSPN、订单 RSPN 上执行的 $Q_L$ 和 $Q_R$，此时 $Q_O$ 为空。结果可写为：

$$
\underbrace{|C|\cdot E(\mathbf{1} _ {c\verb0_0region=\text{'EU'}}\cdot F _ {C\leftarrow O})} _ {Q_L}
\cdot
\underbrace{E(\mathbf{1} _ {o\verb0_0channel=\text{'ONLINE'}})} _ {Q_R}
=3\cdot\frac{2+0}{3}\cdot\frac24=1.
$$

左侧 $Q_L$ 计算欧洲客户的订单数，右侧计算全部订单中在线订单所占比例。

现在，我们处理重叠不为空的更一般情形：若 $Q_O$（也在 $Q_L$ 中）的表 $S$ 与 $Q_R$ 中但不在 $Q_L$ 中的表 $T$ 存在外键关系 $S\leftarrow T$，我们就利用左侧 RSPN 中的元组因子 $F _ {S\leftarrow T}$。此时，我们估计的不只是 $Q_L$，而是 $Q_L$ 与表 $T$ 的连接，因此重叠扩大了，我们将其记为 $Q' _ O$。

**定理 2。** 设在给定 $Q_O$ 的过滤谓词后， $Q_L\setminus Q_O$ 与 $Q_R\setminus Q_O$ 的过滤谓词和元组因子条件独立。设我们要连接的关系为 $S\leftarrow T$，其中 $S$ 属于 $Q_L$， $T$ 属于 $Q_R$。则 $Q$ 的结果等于：

$$
|J_L|\cdot E(F(Q_L)\cdot F _ {S\leftarrow T})\cdot
\frac{E(F(Q_R))}{E(F(Q' _ O))}.
$$

这种跨 RSPN 独立性通常成立，因为我们在第 3 节介绍的集成创建过程会优先把相关表放入同一个 RSPN。

或者，我们也可以从 $Q_R$ 开始执行。仍以我们的示例查询 $Q_2$ 为例，如果从订单表开始，我们可从左侧期望中去掉元组因子 $F _ {C\leftarrow O}$，但随后必须用元组因子归一化 $Q_L$，以正确计算欧洲客户比例：

$$
|O|\cdot E(\mathbf{1} _ {o\verb0_0channel=\text{'ONLINE'}})\cdot
\frac{E(\mathbf{1} _ {c\verb0_0region=\text{'EU'}}\cdot F _ {C\leftarrow O}\mid F _ {C\leftarrow O}\gt{}0)}
{E(F _ {C\leftarrow O}\mid F _ {C\leftarrow O}\gt{}0)}.
$$
...
因此，运行时我们贪心选择当前所处理过滤谓词的两两 RDC 之和最高的 RSPN。我们还试验过枚举多种概率查询编译方案并取预测中位数，但并不优于我们基于 RDC 的策略。用于决定学习哪些 RSPN 的 RDC 已预先算好，故运行时策略也很高效。若连接跨越两个以上 RSPN，我们只需多次应用定理 2。

### 4.2 其他聚合查询

到目前为止，我们只讨论了不带分组的 `COUNT` 查询。下面，我们先说明如何扩展查询编译以支持 `AVG` 和 `SUM`，最后再说明分组和外连接。

#### AVG 查询

我们仍先考虑 RSPN 与查询表集合完全匹配的情况，随后再讨论其他情形。`AVG` 聚合可写成条件期望。例如：
...
在图 4(a) 的集成上，原文将其写为 $|C|\cdot E(c\verb0_0age\mid c\verb0_0region=\text{'EU'})$。

若 RSPN 比查询跨越更多表，我们就不能直接使用该条件期望，否则拥有多个订单的客户权重更高，仍需按元组因子归一化。对于图 4(b) 跨越客户与订单的 RSPN，我们为 $Q_3$ 使用：

$$
\frac{E\left(\frac{c\verb0_0age}{F' _ {C\leftarrow O}}\mid c\verb0_0region=\text{'EU'}\right)}
{E\left(\frac{1}{F' _ {C\leftarrow O}}\mid c\verb0_0region=\text{'EU'}\right)}
=\frac{20/2+20/2+50}{1/2+1/2+1}=35.
$$

一般而言，若我们要在全外连接 $J$ 的 RSPN 上回答连接查询 $Q$ 中、带过滤谓词 $C$ 的属性 $A$ 平均值，就使用：
...
最后，若查询需要多个 RSPN，我们只使用一个包含 $A$ 的 RSPN，并忽略不在该 RSPN 中的部分过滤谓词。只要 $A$ 与这些属性独立，结果就是正确的；否则只是近似。选择 RSPN 时，我们仍优先处理 $A$ 与谓词 $P$ 之间更强相关性的模型，并以 RDC 衡量。RDC 也可检测近似是否忽略了与 $P$ 中缺失属性的强相关性。

#### SUM 查询

处理 `SUM` 查询时，我们分别运行一个 `COUNT` 查询和一个 `AVG` 查询，两者相乘即得正确的 `SUM` 结果。

#### GROUP BY 查询

我们通过为每个组增加过滤谓词、执行多个单独查询来处理分组查询。对于 $n$ 个组，我们需要计算的期望数是对应无分组查询的 $n$ 倍。我们在实验评估中表明，只要在模型上执行查询，这不会造成实际性能问题。

#### 外连接

我们也可以很容易地把查询编译扩展到左、右、全外连接。我们的思路是：只有对所有内连接，我们才过滤没有连接伙伴的元组（第 4.1 节情形 1、2）；对外连接则根据外连接语义决定是否过滤。此外在情形 3 中，为满足相应外连接语义，取值为 0 的元组因子 $F$ 必须按 1 处理。

## 5. DeepDB 扩展

下面，我们介绍对前述基本框架的几项重要扩展。

### 5.1 支持置信区间

置信区间对 AQP 尤其重要，但 SPN 本身并不提供置信区间。经过概率查询编译后，查询被表示为多个期望的乘积。下面，我们先说明如何估计每个因子的不确定性，再说明如何推导最终估计的置信区间。

首先，我们把期望拆成概率与条件期望的乘积。例如：

$$
E(X\cdot\mathbf{1} _ C)=E(X\mid C)\cdot P(C).
$$

这样我们便可把针对过滤谓词 $C_i$ 的全部概率视作一个二项随机变量，其概率为

$$
p=\prod_i P(C_i),
$$

试验次数为 RSPN 的训练数据量 $n _ {samples}$。原文把相应方差写为

$$
\sqrt{n _ {samples}p(1-p)}.
$$

对于条件期望，我们使用柯尼希—惠更斯公式：

$$
V(X\mid C)=E(X^2\mid C)-E(X\mid C)^2.
$$

RSPN 也能计算平方因子，因为平方运算可下推到叶节点。至此，我们就得到了结果中每个因子的相应方差。

组合这些因子时，我们需要两个简化假设：（i）各期望与概率估计相互独立；（ii）最终估计服从正态分布。我们在实验评估中表明，尽管存在这些假设，我们的置信区间仍与典型的基于采样的方法相吻合。

在独立性假设下，我们可递归应用独立随机变量乘积的标准公式，近似乘积方差：

$$
V(XY)=V(X)V(Y)+V(X)E(Y)^2+V(Y)E(X)^2.
$$

由于我们已知整个概率查询编译结果的方差，并假设估计服从正态分布，因此我们可以给出置信区间。

### 5.2 支持更新

我们的更新算法的直觉是把 RSPN 看作索引。与索引相似，插入和删除只影响部分子树，可以递归完成。更新元组递归遍历整棵树，期间调整和节点权重及叶节点分布。我们的方法支持插入和删除；更新操作则映射为一次删除加一次插入。
...
算法 1 是递归算法，我们需要分别处理和节点、积节点与叶节点。在和节点处（第 4 行），我们需要判断插入或删除元组属于哪个子节点，从而确定应提高或降低哪个权重。和节点的子节点表示学习阶段由 KMeans 找到的行簇 [29]，因此我们可计算最近的簇中心（第 5 行），提高或降低其权重（第 6 行），再把元组向该子树传播（第 7 行）。

积节点（第 8 行）拆分的是列集合，因此我们不是把元组传给某一个子节点，而是拆分元组，把各片段传给相应子节点（第 9—11 行）。到达叶节点时，元组只剩下一列，此时我们根据列值更新叶分布（第 2 行）。

这种做法不改变 RSPN 结构，只调整权重和直方图值。插入产生的新依赖不会体现在 RSPN 中。正如我们在第 6.1 节的真实数据实验中所示，即使增量学习比例高达 40%，这种情况通常也不会发生。但如果确实产生新依赖，就必须重建 RSPN。为此，系统周期性检查数据库：在积节点的列拆分处，按第 5.3 节的方法计算两两 RDC，检测依赖是否变化。若发现变化，则重新生成受影响的 RSPN；与传统索引相同，我们可在后台执行该过程。

### 5.3 集成优化

前文说明了我们如何为数据库创建 RSPN 集成。基础集成包含单表 RSPN；若外键相连的两张表相关，也包含跨两表的 RSPN。跨两张以上表的相关性此前被忽略，因为它们会造成更大的模型和更长的训练时间。本节中，我们扩展集成创建过程：用户给出训练预算（时间或空间）后，DeepDB 选择还应创建哪些更大的 RSPN。

为量化表间相关性，我们先计算模式中所有属性对的 RDC。对每对表，我们定义两列之间的最大 RDC

$$
\max _ {c\in T_i,\ c'\in T_j}rdc(c,c')
$$

为依赖值。依赖值指示哪些表应该、哪些不应该出现在同一个 RSPN 中；我们的目标是让每个 RSPN 获得较高的两两最大 RDC 均值，从而只合并两两相关性较高的表。

额外 RSPN 的选择受预算约束，即相对于基础集成允许增加的训练时间。我们把最大额外学习代价定义为相对基础集成学习代价 $C _ {Base}$ 的系数 $B$。 $B=0$ 表示只创建基础集成； $B\gt{}0$ 时还会学习覆盖更多表的 RSPN。若我们假设所有可能且互异的 RSPN 集合为 $R$，某个 RSPN $r$ 的代价为 $C(r)$，则我们可把优化目标表述为最小化
...
$$
\sum _ {r\in E}C(r)\le B\cdot C _ {Base}.
$$

但构建 RSPN 的真实代价 $C(r)$（即时间）很难估计，因此我们无法直接求解该优化问题。作为替代，我们估计相对代价，选择 RDC 均值最高且相对创建代价最低的 RSPN。由于 RDC 是两两计算的，我们假设代价随列数 $cols(r)$ 的平方增长，并随行数 $rows(r)$ 线性增长。因此，在不超过最大训练时间的前提下，我们按高平均 RDC 和低代价
...
本节中，我们证明 DeepDB 在基数估计和 AQP 两项任务上都优于最先进系统。我们在所有实验中使用的 RSPN 都以 Python 实现为 SPFlow [30] 的扩展。我们使用 0.3 的 RDC 阈值，以及输入数据 1% 的最小实例切片作为超参数，以决定聚类粒度；此外，我们使用 0.5 的预算系数，即较大 RSPN 的训练时间大约比基础集成多 50%。我们通过网格搜索确定这些超参数，它们使我们在不同数据集上取得最佳结果。

### 6.1 实验 1：基数估计

#### 工作负载与设置

与 [16, 19] 相同，我们为所有方法（DeepDB 与基线）使用 JOB-light 基准。该基准基于真实 IMDb 数据库，包含 70 个查询。我们还定义了包含 200 个查询的合成查询集：在 IMDb 数据集上均匀生成涉及 3—6 张表连接、1—5 个过滤谓词的查询。我们用该查询集比较学习型方法的泛化能力。

基线包括以下学习型和传统方法：

- 我们训练一个多集合卷积网络（Multi-Set Convolutional Network，原文记作 MCSN）[16] 作为学习型基线。该专用深度神经网络以连接路径、表和过滤谓词为输入。
- 我们以小波 [5] 作为基于概要的代表方法，每表构建一个小波，并直接在小波表示上执行连接等查询算子。我们选择它，是因为它与 DeepDB 一样，无需预先知道查询会连接哪些表。
- 我们实现 `Perfect Selectivities`，由预言机返回单表真实基数。它可视为通过组合“完美”单表概要来支持即席查询的最佳情形。
- 最后，我们使用 PostgreSQL 11.5 的标准基数估计，以及在线随机采样和基于索引的连接采样（Index-Based Join Sampling，IBJS）[20]。IBJS 与 DeepDB 一样会考虑跨表相关性。

除非另有说明，我们为 DeepDB 使用前述超参数，并用 1000 万个样本构建 RSPN。

#### 训练时间与存储开销

与其他学习型基数估计方法 [16, 42] 不同，我们不需要为 DeepDB 准备专门训练数据，而只需学习数据表示。训练基础集成需 48 分钟，其中包括采样和计算第 4.1 节元组因子的数据准备时间。相比之下，MCSN [16] 需要执行 10 万个查询来收集基数；以 PostgreSQL 执行时，训练数据准备耗时 34 小时，随后在 NVIDIA V100 GPU 上训练神经网络约 15 分钟。由此可见，我们的训练时间显著更短，因为我们无需收集工作负载训练数据；数据库修改后，我们也不必重跑查询，而是用第 3.2 节给出的高效算法更新 RSPN。

在存储占用方面，IBJS 和随机采样等采样方法没有离线存储开销，但可用样本数受查询延迟限制。其他方法在 JOB-light 使用的 3.7 GB IMDb 数据库上只需数 KB 到数 MB：DeepDB 为 28.9 MB，MCSN 为 2.6 MB；PostgreSQL 默认仅使用 100 个桶的直方图，占 60 KB，但正如我们下一节所示，其精度最低。对于小波方法，我们使用 2 万个小波系数，使存储量与标准 DeepDB 大致相同。

此外，我们还把每个 RSPN 的样本数降到 10 万，构建存储优化版 DeepDB，使其存储占用接近 MCSN。增大 MCSN 的存储开销（例如增加隐藏层）不会提升性能，因为我们已经采用 [16] 的优化超参数；正如我们下一节所示，存储优化版 DeepDB 仍明显优于包括 MCSN 在内的所有基线。尽管已有一系列研究优化 DNN 的存储占用，目前仍没有可直接用于 SPN 的可比压缩方法，但我们相信未来研究会进一步降低 DeepDB 的存储需求。即使如此，我们认为，对数 GB 数据库使用数 MB 存储以获得更准确的基数估计，也仍是可以接受的。
...
表 1 给出我们的方法与其他基线在 JOB-light 上的中位数、90 百分位、95 百分位和最大 q-error；我们还给出仅使用基础集成且每个 RSPN 只有 10 万样本的存储优化版 DeepDB 的 q-error。可以看到，DeepDB 及其存储优化版通常以数个数量级的优势超过最佳竞争者。IBJS 的中位 q-error 较低；MCSN 的优势在于高百分位误差比传统方法低数个数量级，因而更稳健。DeepDB 不仅中位数优于 IBJS，而且 95 百分位 q-error 仅为 3.16，而 MCSN 为 143，稳健性进一步提高。PostgreSQL 与随机采样在中位数和高百分位上都明显更差；小波受维数灾难影响，误差最高，正如我们稍后在图 12 中展示的那样。使用预言机的 Perfect Selectivities 虽优于小波，仍因不考虑跨表相关性而不如 DeepDB。

#### 合成数据

为进一步研究各方法的权衡，我们为 IMDb 模式实现了合成数据生成器，以便继续运行 JOB-light。我们首先生成均匀且无相关性的数据，然后改变现实中令基数估计困难的特征，即使用偏斜分布与列间相关性。随后，我们仍用前述方法估计原有 70 个 JOB-light 查询，并报告结果非零查询的平均 q-error（结果为零时 q-error 无定义）。

![图 5：合成数据上的平均估计误差。](assets/figure-05-synthetic-qerror.png)

**图 5：合成数据上的平均估计误差。** 上图改变偏斜程度，下图改变属性相关程度；纵轴为对数尺度。

从图 5 可以看到，DeepDB 和存储优化版均优于全部基线。在均匀、独立的数据上，DeepDB 如预期般相对于随机采样或 PostgreSQL 等简单方法没有明显优势；随着偏斜或相关性增强，DeepDB 则超过其他方法。随机采样、IBJS 和 PostgreSQL 都显著退化；MCSN 更能处理偏斜和相关性，但仍会退化，我们仍把原因归于训练查询的覆盖范围。小波受维数灾难影响，在所有配置上精度最低，与原文图 1 所指的真实数据情形相似。

#### 泛化能力

泛化对学习型方法尤其重要，即模型在此前未见查询上的表现。MCSN 默认只用至多三路连接的查询训练，否则生成训练数据过于昂贵 [16]。类似地，在我们的方法中，DeepDB 集成也只有少量跨大连接的 RSPN，否则训练同样昂贵；不过两种方法都能为未见查询给出基数估计。

我们在 IMDb 数据集上随机生成涉及 4—6 张表、1—5 个选择谓词的查询。在图 6 中，我们比较 DeepDB 与 MCSN [16] 的中位 q-error。

![图 6：不同连接规模与过滤谓词数下的中位 q-error。](assets/figure-06-generalization-qerror.png)

**图 6：不同连接规模（4、5、6）与过滤谓词数（1—5）下的中位 q-error（对数尺度）。**

对更大的连接，DeepDB 的中位 q-error 低数个数量级。我们还观察到，MCSN 在过滤谓词较少时反而更不准确。一种可能解释是，此类查询会命中更多元组，需要估计更大的基数；而训练数据最多只连接三张表，因此很可能没有覆盖这些较高基数值。由此，RSPN 在未见查询上表现出更强的泛化能力。

#### 更新

在本实验中，我们展示 RSPN 的更新能力。RSPN 易于且高效地更新，是 DeepDB 相对于基于深度学习的基数估计方法 [16, 42] 的明显优势。为评估更新质量，我们先在 IMDb 全量数据的某一部分上训练集成，再用其余元组更新数据库。为贴近真实情况，我们按影片制作年份划分数据，即较新的影片后插入。我们为基础 RSPN 集成使用预算系数 0，借此说明即使只有基础集成，更新后也能保持良好估计；这也解释了误差为何与表 1 略有不同。
...
我们在表 2 中的结果表明，即使更新比例提高到 40.1%，更新后的 RSPN q-error 也没有明显变化，精度只略微下降。原因在于更新只改变 RSPN 参数而不改变树结构；若更新导致数据分布或相关性变化，原结构可能不再最优。若精度下降超过阈值，DeepDB 仍可基于新数据在离线阶段重建 RSPN。

#### 参数探索

最后，我们研究 DeepDB 集成训练时间与预测质量的权衡。我们先把集成选择的预算系数从 0（只学习基础集成，每个双表连接一个 RSPN）变化到 $B=3$（较大 RSPN 的训练约为基础集成的三倍），每个 RSPN 使用 $10^7$ 个样本；再用所得集成评估 200 个涉及 3—6 张表、1—5 个选择谓词的查询。

![图 7：预算系数和样本量变化时的 q-error 与训练时间。](assets/figure-07-budget-sample.png)

**图 7：预算系数和样本量变化时的 q-error 与训练时间（秒）。**

图 7 显示，预算增大时平均质量改善，但在 $B=0.5$ 已趋于饱和，因为更大的连接中不存在基础集成尚未捕获的强相关性。此外，我们还评估了采样对缩短训练时间的作用：在本实验中，我们把样本量从 1000 变化到 1000 万。我们观察到，样本量增大时训练时间增长，而中位预测误差从 2.5 改善到 1.9。因此，若可接受轻微质量妥协，可以显著降低训练时间。

若目标更偏重最短训练时间，我们还可以退而只为所有单表学习 RSPN，完全不学习连接，集成训练时间可降至 5 分钟。即使这种廉价策略也具有竞争力：JOB-light 中位 q-error 为 1.98，90 百分位为 5.32，95 百分位为 8.54，最大值为 186.53。它在高百分位上仍优于最先进基线，只有基于索引的连接采样在中位数上略胜，再次体现 RSPN 的稳健性。

#### 延迟

DeepDB 当前基数估计延迟在微秒至毫秒量级，对通常运行数秒的大型数据集复杂连接查询已经足够。跨多列的复杂谓词或更多连接表会增加延迟。图 8 对这一影响作了更详细的研究：我们同时报告 RSPN 推断延迟，以及包含我们的概率查询编译把查询转换为期望与概率之开销的总延迟。

![图 8：不同连接规模与过滤属性数下的 DeepDB 延迟。](assets/figure-08-cardinality-latency.png)

**图 8：不同连接规模（4、5、6）与过滤属性数（1—5）下的 DeepDB 延迟。**

RSPN 推断高效，是因为系统会像 [40] 那样为训练好的 RSPN 自动编译 C++ 代码。可以看到，复杂谓词和连接虽会提高延迟，最坏情况仍约 3 ms，简单查询则在微秒范围。未来，我们计划不仅优化 RSPN 推断，还会降低查询转换开销，使总延迟更接近纯 RSPN 推断延迟。

### 6.2 实验 2：AQP

#### 工作负载与设置

我们同时在合成数据集和真实数据集上评估各方法。对于合成数据，我们采用比例因子 500 的星型模式基准（Star Schema Benchmark，SSB）[32] 及其标准查询（记作 S1.1—S4.3）。对于真实数据，我们采用 Flights 数据集 [1]，查询选择率从 5% 到 0.01%，覆盖多种分组属性，以及 `AVG`、`SUM`、`COUNT` 查询（记作 F1.1—F5.2）；我们使用 IDEBench [9] 把数据集扩展到 10 亿条记录。

我们使用 VerdictDB [33]、Wander Join/XDB [21] 和 PostgreSQL 的 `TABLESAMPLE`（随机样本）作为基线。VerdictDB 是可配合任意数据库系统使用的中间件，通过为事实表创建分层样本和均匀样本提供近似查询。对于 Flights 数据集，我们使用 VerdictDB 默认的全量数据 1% 样本。SSB 上默认设置会造成很高延迟，因此我们改为选取使平均查询时间约 2 秒的样本量。

Wander Join 是利用二级索引快速生成连接样本的连接采样算法。为公平比较，我们也把时间上限设为 2 秒，并只在包含连接的数据集上评估该算法；为此，我们为连接及谓词创建全部二级索引。对于 `TABLESAMPLE`，我们同样选择使查询平均耗时 2 秒的样本量。为 DeepDB 构建 RSPN 时，我们在 Flights 上使用 1000 万样本，在 SSB 上使用 100 万样本。
...
AQP 关注两个维度：近似质量和查询运行时间。为报告近似质量，我们使用相对误差

$$
\frac{|a _ {true}-a _ {predicted}|}{a _ {true}},
$$

其中 $a _ {true}$ 和 $a _ {predicted}$ 分别是真实与预测聚合结果。对分组查询，先为多个组分别计算聚合，再对各组相对误差取平均。

![图 9：Flights 数据集的平均相对误差与延迟。](assets/figure-09-flights-error-latency.png)

**图 9：Flights 数据集的平均相对误差与延迟。**

从图 9 可以观察到，DeepDB 始终具有最低的平均相对误差。低选择率查询中尤其如此：基于采样的方法只有很少元组满足选择谓词，近似因而很不准确；DeepDB 则对数据分布建模，并利用学到的表示作估计。例如选择率为 0.5% 的查询 11 中，VerdictDB 与 `TABLESAMPLE` 的平均相对误差分别为 15.6% 和 13.6%，DeepDB 仅为 2.6%。
...
本实验中，我们评估 DeepDB 预测置信区间的准确性。为此，我们使用相对置信区间长度：

$$
\frac{a _ {predicted}-a _ {lower}}{a _ {predicted}},
$$

其中 $a _ {predicted}$ 是预测值， $a _ {lower}$ 是置信区间下界。我们把该长度与基于采样方法的置信区间比较：抽取 1000 万个样本，与我们的模型在本实验中学习所用样本数一致；我们估计平均值、计数与求和聚合，再用标准统计方法计算置信区间。这些置信区间长度可视为真实值，并在图 11 中与我们系统的置信区间比较。满足过滤谓词的样本不足 10 个时，标准差估计本身方差过高，因此我们排除相应查询。
...
下面，我们把 DeepDB 与要求预先了解工作负载，或能利用这些先验信息提高精度的方法比较。

首先，我们把 DeepDB 与前述使用过的小波方法 [5] 比较。它本身不要求先验信息，但小波无法扩展到大量维度，因此可利用先验信息优化：若预先知道所有查询所需的列组合，我们就能为每个查询单独构造最优、最小的小波。即使如此，图 13 表明 DeepDB 仍优于小波。我们进一步研究了这一现象，发现即使维数不高，小波精度也会显著下降，如图 12 右侧所示。

![图 12：非即席方法的微基准。](assets/figure-12-non-adhoc-microbenchmarks.png)

**图 12：非即席方法的微基准。** 左：DBEst 查询延迟随数值列过滤属性数变化；右：小波平均相对误差随维数变化。

其次，我们还把 DeepDB 与另外两种需要先验信息的方法比较：（1）BlinkDB [2] 使用的分层采样，可减轻偏斜影响；（2）近期学习型 AQP 方法 DBEst [25]。二者不同于 DeepDB，不能回答先验信息没有覆盖的列组合所形成的即席查询；DeepDB 则能用我们在第 4 节介绍的概率查询编译组合多个 RSPN。

![图 13：DeepDB 与使用先验知识方法的相对误差。](assets/figure-13-prior-knowledge.png)

**图 13：DeepDB 与使用先验知识方法的相对误差。**

下面，我们讨论预先给出 Flights 全部查询信息时的结果。图 13 显示，分层采样可达到与 DeepDB 相当的精度。但若查询在某个分层列上带有高选择性过滤条件，如 F5.2 或 F4.2，DeepDB 仍更优。我们还注意到，DBEst 的精度虽与 DeepDB 相当，但部分查询需要长达 20 秒。我们在图 12 左侧进一步研究这一现象，发现其延迟随数值列过滤条件数量呈指数增长；由于该方法依赖对数值列域进行积分，这一点不可避免。
