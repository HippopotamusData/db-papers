...
图 2 把不同执行模式放到 compilation time 与 execution time 的对数坐标中。LLVM IR 解释启动最快但执行最慢；optimized machine code 执行接近手写代码，但编译成本高；bytecode 与 unoptimized machine code 位于中间。图中的手写版本没有实现溢出检查，因此运行时间略快。自适应执行正是利用这个空间，在运行中选择收益最高的点。
...
自适应执行（adaptive execution）的目标是组合两种执行模式的优点：
...
`queryStart` 只运行一次，负责初始化 C++ 状态并启动各 pipeline，因而不值得编译；数据相关的工作位于 worker 函数中。每个 worker 接收共享状态和一个描述输入范围的 morsel。多个线程以不重叠 morsel 调用同一 worker，并通过 work stealing 负载均衡。系统通常每次分配约 10,000 个元组，既避免线程失衡，也自然提供了进度采样点。每处理完一个 morsel，线程本就要访问调度结构；本文只需附加少量计时与计数，并在 pipeline 开始时记录总工作量，即可得到已完成比例和处理速率。
...
系统连续比较三种选择：维持当前模式、编译未优化机器码、编译优化机器码。维持解释时，剩余时间由当前每线程的 morsel 处理率和剩余元组数直接外推。选择编译时，还必须加入预计编译时间、预计加速比，并扣除编译期间其余 `w-1` 个线程仍可解释处理的元组。编译时间和加速比用历史实验拟合；TPC-H/TPC-DS 中 LLVM 指令数与编译时间近似线性相关。自适应策略不要求预测十分精确，只需足以区分明显有利和无利的切换。

![LLVM 指令数与编译时间的关系](assets/precise-manual-figure-06-llvm-compilation-time.png)

图 6 说明 query plan 生成的 LLVM instruction 数与机器码编译时间高度相关，因此系统可以用已观测数据估算 unoptimized/optimized compilation 的成本和速度收益。
...
本文算法允许把某个值的生命周期保守扩展到“包含其全部使用的最内层循环”边界，以换取线性复杂度。TPC-DS Q55 若完全不复用槽位需 36 KB，固定窗口贪心策略需 21 KB，本文算法仅需 6 KB。

![变量活性范围的控制流示例](assets/precise-manual-figure-10-liveness-cfg.png)

图 10 说明变量 `v` 在控制流中的活性范围。虽然 `v` 在 block 2 定义、在 block 5 使用，但 block 5 位于从 block 3 开始的循环内，因此仅用 `[2,5]` 表示生命周期会低估实际需要覆盖的路径。
...
算法先按逆后序标号基本块并构建支配树，再以支配树的前序/后序区间常数时间判断祖先关系。若跳转 `B -> B'` 中 `B'` 支配 `B`，则 `B'` 是循环头。每个块关联到最近的支配循环头，每个循环记录首尾块、外层循环和嵌套深度。对值 `v`，取包含定义和使用的块集合，求能容纳这些块的最内层公共循环 `C_v`；位于 `C_v` 内层循环中的使用被提升到相应外层循环的完整区间。φ 节点需按控制流边处理：参数在前驱块末读取，φ 值随后写入，并在 φ 所在块读取。
