...
允许为个人或课堂使用免费制作本文全部或部分内容的数字或纸质副本，条件是副本不得用于营利或商业利益，并须在首页载明本声明及完整引文。其他复制、再版、发布到服务器或转发到邮件列表的行为，须事先取得特定许可并/或付费。
...
**译者注：** 原文此处将 $(T_i.C_p\ge c)$ 写成更新“上界”，并将 $(T_i.C_p\gt{}c)$ 写成 $(T_i.C_p\le c+\Delta)$；这与上下界定义、后续补偿谓词和示例相冲突。按通常的区间语义，二者应分别更新下界，并把后者写作 $(T_i.C_p\ge c+\Delta)$。上段按原文可见内容保留，没有静默改写。
...
**译者注：** 原文末句写作“视图的每个输出列都能由视图输出计算”；结合本节正在验证的目标和前述测试，应为“查询的每个输出列”，译文据此显式更正。
...
如果查询的 `group-by` 列表与视图的相同，就无须进一步聚合，因此第四个要求自动满足。如果查询列表是严格子集，就必须在视图之上添加补偿性 `group-by`。分组表达式与查询相同。因为它们是视图分组表达式的子集，而视图的所有分组表达式都必须属于视图输出，所以它们总能在视图输出中获得。因此，第四个要求会自动满足。

测试第五个要求是否满足，几乎与 SPJ 查询部分的讨论完全相同。唯一的细微差别出现在聚合表达式上。如果查询指定 `count(*)`，而视图是聚合视图，就必须把 `count(*)` 替换为对视图 `count_big(*)` 列求 `SUM`。如果查询输出包含 `SUM(E)`，其中 $E$ 是标量表达式，就要求视图含有与之精确匹配的输出列（考虑列等价关系）。`AVG(E)` 转换为 `SUM(E)/count_big(*)`。

**译者注：** 原文在上述两处分别写作“第三个要求”和“第四个要求”；对照本节开头列出的五项要求，上下文对应的应为第四项和第五项，译文已显式更正。
...
**译者注：** 原文可见 SQL 在 `count_big(*) as cnt` 后缺少逗号；译文为保持语法完整补入了该逗号。
...
**译者注：** 原文在递归检查处写作“目标节点是否为 `AB` 的子集”，但本段的搜索目标、遍历方向和返回节点均表明这里是在检查 `AB` 的超集；译文按该上下文显式更正。
...
**译者注：** 原文首句末尾写作“该列在视图中不受范围约束”，与同一句前半和紧随其后的正式条件自相矛盾；后文明确要求检查查询是否约束该列，译文据此显式更正为“查询没有在该列上施加范围约束”。
