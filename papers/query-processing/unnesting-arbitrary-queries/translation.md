...
Thomas Neumann and Alfons Kemper<br>
...
因此，现有系统使用若干启发式来去嵌套这些查询，也就是对它们去相关。这些去嵌套技术可以显著加速查询处理，但通常仅限于特定类别的查询。据我们所知，没有现有系统能在一般情形下对查询去相关。我们提出一种用于去嵌套任意查询的通用方法。结果是，去相关后的查询可以用更简单且更高效的方式求值。

## 1. 引言

SQL 查询中经常使用子查询来简化查询表达。作为我们的贯穿示例，考虑如下模式：

- $\mathit{students}:\lbrace{}[\mathit{id},\mathit{name},\mathit{major},\mathit{year},\ldots]\rbrace{}$
- $\mathit{exams}:\lbrace{}[\mathit{sid},\mathit{course},\mathit{curriculum},\mathit{date},\ldots]\rbrace{}$
...
据我们所知，没有现有系统能去嵌套这样的查询。事实上，去嵌套这个查询很难：标准去嵌套技术依赖这样一个事实，即查询中已有属性可用于替代由外层查询决定的自由变量。这里并非如此，例如 $s.\mathit{year}$ 不能被替代。

显然，这类复杂相关查询会比简单子查询更昂贵。然而，如我们将展示的那样，即便这样的查询也确实可以去嵌套。我们必须额外推导 $s.\mathit{year}$ 和 $s.\mathit{major}$ 的值，但可以不使用依赖连接做到这一点。我们需要付出的额外代价受依赖连接代价约束。多数查询在去相关形式下会显著更高效；最坏情况下，我们会有相同的连接工作量。也就是说，我们的去嵌套方法绝不会比直接嵌套循环求值产生更高代价，并且在大多数情况下会显著提升性能，往往提升几个数量级。此外，即使最坏情况也很可能获益，因为消除依赖连接允许使用更高效的连接实现。
...
本文余下结构如下：我们首先在第 2 节定义本文使用的记法。然后，第 3 节给出代数去嵌套变换。第 4 节讨论适用于特殊情形的进一步优化规则，例如可以推断函数依赖时的规则。第 5 节给出一个“粗略”的性能评估，将若干知名 DBMS 与我们的 HyPer 系统进行分析对比；该系统包含本文所述的去嵌套技术。最后，我们综述相关工作并总结全文。

## 2. 预备知识

在讨论去嵌套技术之前，我们先简要回顾关系代数的一些定义，因为基础算子之外的记法并不标准化。

首先，我们有常规内连接，它可简单定义为笛卡尔积后接选择：

$$
T _ 1 \bowtie _ p T _ 2 := \sigma _ p(T _ 1 \times T _ 2).
$$

它计算 $T _ 1$ 和 $T _ 2$ 中所有匹配项的组合。大多数 SQL 查询都使用它，但在存在相关子查询时，这一定义并不足够。子查询必须为外层查询的每个元组求值，因此我们定义依赖连接为：

$$
T _ 1 \mathbin{\mathrm{DepJoin}} _ p T _ 2
:=
\lbrace{}
t _ 1 \circ t _ 2
\mid
t _ 1 \in T _ 1
\land t _ 2 \in T _ 2(t _ 1)
\land p(t _ 1 \circ t _ 2)
\rbrace{}.
$$

这里，右侧会为左侧的每个元组求值。我们用 $A(T)$ 表示表达式 $T$ 产生的属性，用 $F(T)$ 表示表达式 $T$ 中出现的自由变量。要对依赖连接求值，必须有 $F(T _ 2) \subseteq A(T _ 1)$，即 $T _ 2$ 需要的属性必须由 $T _ 1$ 产生。

注意，我们在本文中有时会在连接谓词中显式提到自然连接，以简化记法。我们假设查询中出现的所有关系都有唯一属性名，即使它们引用同一个物理表，因此 $A \bowtie B \equiv A \times B$。不过，如果我们显式两次引用同一关系名，并要求自然连接，则同名属性列会被比较，重复列会被投影掉。例如：

$$
(A \bowtie C)
\bowtie _ {p \land \mathrm{natural\ join}\ C}
(B \bowtie C).
$$

这里，最上层连接既检查谓词 $p$，也比较来自两侧的 $C$ 的列，并消除 $C$ 的两份列副本中的一份。

对于半连接 $\mathrm{SemiJoin}$、反连接 $\mathrm{AntiJoin}$ 和外连接 $\mathrm{LeftOuterJoin}$、 $\mathrm{FullOuterJoin}$，我们相应地定义其依赖变体 $\mathrm{DepSemiJoin}$、 $\mathrm{DepAntiJoin}$、 $\mathrm{DepLeftOuterJoin}$、 $\mathrm{DepFullOuterJoin}$；同样，右侧会为左侧的每个元组求值。

除连接算子外，我们还有另一个重要算子，即 group by 算子：
...
除此之外，我们还需要常规关系代数算子，包括 $\sigma$、 $\times$、 $\Pi$、 $\rho$、 $\cup$、 $\cap$、 $\setminus$。使用这些算子，可以把 SQL 查询翻译为关系代数。

下面我们经常需要比较属性集合。作为简写，我们定义属性比较算子 $={} _ A$ 为：

$$
t _ 1 = _ A t _ 2
:=
\forall a \in A : t _ 1.a = t _ 2.a.
$$
...
如前所述，从性能角度看，这些依赖连接非常不理想，我们希望消除它们。基本上，我们操纵代数表达式，直到右侧不再依赖左侧，从而把依赖连接转换为常规连接。我们用两种技术实现这一点，并将在下文讨论。首先，我们尝试简单去嵌套，它处理那些仅因语法原因产生依赖的情形。如果这不足以去嵌套查询，我们就使用可处理任意复杂查询的一般去嵌套框架。
...
$$
l _ 1
\mathbin{\mathrm{DepSemiJoin}}
\left(
\sigma _ {l _ 1.\mathit{okey}=l _ 2.\mathit{okey}}(l _ 2)
\right).
$$
...
$$
l _ 1
\mathbin{\mathrm{SemiJoin}} _ {l _ 1.\mathit{okey}=l _ 2.\mathit{okey}}
(l _ 2).
$$
...
&\equiv
T _ 1
\bowtie _ {p \land T _ 1 = _ {A(D)} D}
\left(
D \mathbin{\mathrm{DepJoin}} T _ 2
\right),\\
\text{其中}\qquad
D
&:=
\Pi _ {F(T _ 2)\cap A(T _ 1)}(T _ 1).
\end{aligned}
$$

乍看之下，该变换似乎没有太大改善查询计划，因为我们用一个常规连接和另一个依赖连接替换了一个依赖连接。然而再看就会发现，这个变换非常有用：原始表达式中，我们必须为 $T _ 1$ 的每个元组求值 $T _ 2$，这可能达到数百万次。因此，在第二个表达式中，我们先计算所有变量绑定的域 $D$，只为每个不同变量绑定求值一次 $T _ 2$，然后用常规连接把结果匹配回原始 $T _ 1$ 值。如果有大量重复，这已经能显著减少 $T _ 2$ 的调用次数。

我们可以用我们在引言中的第一个示例查询说明这个收益。该查询为每个学生确定最好考试。直接求值会为学生曾参加的每门考试计算该学生的最佳成绩：

$$
\sigma _ {e.\mathit{grade}=m}
\left(
\left(
\mathit{students}\ s
\bowtie _ {s.\mathit{id}=e.\mathit{sid}}
\mathit{exams}\ e
\right)
\mathbin{\mathrm{DepJoin}}
\Gamma _ {\emptyset; m:\min(e _ 2.\mathit{grade})}
\left(
\sigma _ {s.\mathit{id}=e _ 2.\mathit{sid}}
(\mathit{exams}\ e _ 2)
\right)
\right).
$$

该等价规则允许把最佳成绩计算限制为每个学生一次，而不是为每个 $(\mathit{student},\mathit{exam})$ 对冗余计算。因此，依赖连接只在学生 ID 的投影上执行：

$$
\ldots
\Pi _ {d.\mathit{id}:s.\mathit{id}}
\left(
\left(
\mathit{students}\ s
\bowtie _ {s.\mathit{id}=e.\mathit{sid}}
...
\Gamma _ {\emptyset; m:\min(e _ 2.\mathit{grade})}
\left(
\sigma _ {d.\mathit{id}=e _ 2.\mathit{sid}}
(\mathit{exams}\ e _ 2)
\right)
\right).
$$

图 1 展示了对我们的示例查询应用“下推”规则时的完整查询求值计划。从某种意义上说，这构成了从外层左侧连接参数到内层右侧参数的侧向信息传递，用于消除求值冗余。因此，投影必须按真正的、会消除重复的集合语义实现，而不能使用 SQL 的保留重复的多重集语义。

**图 1：依赖连接“下推”的示例应用。** 原始计划把 $\mathit{students}\ s \bowtie \mathit{exams}\ e$ 与聚合子查询做依赖连接；转换后先投影得到无重复的 $d.\mathit{id}$ 域，把该域传给聚合子查询，再通过常规连接把结果接回外层。

![图 1](assets/figure-01-precise2.png)

更重要的是，我们把一个通用依赖连接转换为一个集合上的依赖连接，即左侧是无重复关系。知道 $D$ 不含重复，有助于把依赖连接进一步下推到查询中。下面我们假设任何名为 $D$ 的关系都是无重复的，并且下面的等价关系只考虑左侧为集合的依赖连接。不过我们强调，这种嵌套查询优化技术保留 SQL 多重集语义。所有重复项，包括基表中包含的重复项以及查询产生的重复项，都会保留在优化后的计划中；只有约束嵌套子查询求值工作的集合 $D$ 是无重复的。如果查询因为 `DISTINCT` 子句需要去重，我们还可以进一步利用这一点，把重复消除下推到查询求值计划中。

我们下推依赖连接的最终目标，是达到右侧不再依赖左侧的状态：

$$
D \mathbin{\mathrm{DepJoin}} T
\equiv
D \bowtie T
\qquad
\text{若 } F(T)\cap A(D)=\emptyset.
$$

在这种情况下，我们仍需执行一个连接，但至少我们可以执行常规连接，而不是极低效的依赖连接。而且，如我们将在后文看到的那样，我们总能达到这种状态。更理想的目标是，得到的常规连接可由已有属性替代，从而完全消除连接。我们将在第 4 节讨论这一点。

解释了我们下推依赖连接的起点和目标后，我们现在讨论各个算子。对选择而言，下推非常简单：

$$
D \mathbin{\mathrm{DepJoin}} \sigma _ p(T _ 2)
\equiv
\sigma _ p
\left(
D \mathbin{\mathrm{DepJoin}} T _ 2
...
$$
D \mathbin{\mathrm{DepJoin}} (T _ 1 \bowtie _ p T _ 2)
\equiv
\begin{cases}
\left(D \mathbin{\mathrm{DepJoin}} T _ 1\right)\bowtie _ p T _ 2,
& F(T _ 2)\cap A(D)=\emptyset,\\
T _ 1\bowtie _ p\left(D \mathbin{\mathrm{DepJoin}} T _ 2\right),
& F(T _ 1)\cap A(D)=\emptyset,\\
\left(D \mathbin{\mathrm{DepJoin}} T _ 1\right)
\bowtie _ {p\land\mathrm{natural\ join}\ D}
\left(D \mathbin{\mathrm{DepJoin}} T _ 2\right),
& \text{其他情况}.
\end{cases}
$$

如果依赖连接提供的值只在一侧需要，我们就把它下推到对应一侧；否则我们必须把它复制到两侧。注意，这个下推规则过于保守，我们通常可以简化连接下方的部分（见第 4 节），但我们暂时先坚持基本下推规则。如果我们把依赖连接下推到两侧，就必须扩展连接谓词，使两侧在 $D$ 的值上匹配。还要注意，相比原始表达式，这种复制并不是性能惩罚，因为两种情况下 $T _ 1$ 和 $T _ 2$ 都会被求值 $|D|$ 次。

对于外连接，如果内侧依赖它，我们总是必须复制依赖连接，否则无法跟踪外侧未匹配元组：

$$
D \mathbin{\mathrm{DepJoin}}
\left(T _ 1 \mathbin{\mathrm{LeftOuterJoin}} _ p T _ 2\right)
\equiv
\begin{cases}
\left(D \mathbin{\mathrm{DepJoin}} T _ 1\right)
\mathbin{\mathrm{LeftOuterJoin}} _ p T _ 2,
& F(T _ 2)\cap A(D)=\emptyset,\\
\left(D \mathbin{\mathrm{DepJoin}} T _ 1\right)
\mathbin{\mathrm{LeftOuterJoin}} _ {p\land\mathrm{natural\ join}\ D}
\left(D \mathbin{\mathrm{DepJoin}} T _ 2\right),
& \text{其他情况}.
\end{cases}
$$
...
$$
D \mathbin{\mathrm{DepJoin}}
\left(T _ 1 \mathbin{\mathrm{FullOuterJoin}} _ p T _ 2\right)
\equiv
\left(D \mathbin{\mathrm{DepJoin}} T _ 1\right)
\mathbin{\mathrm{FullOuterJoin}} _ {p\land\mathrm{natural\ join}\ D}
\left(D \mathbin{\mathrm{DepJoin}} T _ 2\right).
$$
...
$$
D \mathbin{\mathrm{DepJoin}}
\left(T _ 1 \mathbin{\mathrm{SemiJoin}} _ p T _ 2\right)
\equiv
\begin{cases}
\left(D \mathbin{\mathrm{DepJoin}} T _ 1\right)
\mathbin{\mathrm{SemiJoin}} _ p T _ 2,
& F(T _ 2)\cap A(D)=\emptyset,\\
\left(D \mathbin{\mathrm{DepJoin}} T _ 1\right)
\mathbin{\mathrm{SemiJoin}} _ {p\land\mathrm{natural\ join}\ D}
\left(D \mathbin{\mathrm{DepJoin}} T _ 2\right),
& \text{其他情况}.
\end{cases}
$$
...
$$
D \mathbin{\mathrm{DepJoin}}
\left(T _ 1 \mathbin{\mathrm{AntiJoin}} _ p T _ 2\right)
\equiv
\begin{cases}
\left(D \mathbin{\mathrm{DepJoin}} T _ 1\right)
\mathbin{\mathrm{AntiJoin}} _ p T _ 2,
& F(T _ 2)\cap A(D)=\emptyset,\\
\left(D \mathbin{\mathrm{DepJoin}} T _ 1\right)
\mathbin{\mathrm{AntiJoin}} _ {p\land\mathrm{natural\ join}\ D}
\left(D \mathbin{\mathrm{DepJoin}} T _ 2\right),
& \text{其他情况}.
\end{cases}
$$
...
$$
D \mathbin{\mathrm{DepJoin}}
\left(\Gamma _ {A; a:f}(T)\right)
\equiv
\Gamma _ {A\cup A(D); a:f}
\left(D \mathbin{\mathrm{DepJoin}} T\right).
$$
...
$$
\begin{aligned}
D \mathbin{\mathrm{DepJoin}} (T _ 1\cup T _ 2)
&\equiv
\left(D \mathbin{\mathrm{DepJoin}} T _ 1\right)
\cup
\left(D \mathbin{\mathrm{DepJoin}} T _ 2\right),\\
D \mathbin{\mathrm{DepJoin}} (T _ 1\cap T _ 2)
&\equiv
\left(D \mathbin{\mathrm{DepJoin}} T _ 1\right)
\cap
\left(D \mathbin{\mathrm{DepJoin}} T _ 2\right),\\
D \mathbin{\mathrm{DepJoin}} (T _ 1\setminus T _ 2)
&\equiv
\left(D \mathbin{\mathrm{DepJoin}} T _ 1\right)
\setminus
\left(D \mathbin{\mathrm{DepJoin}} T _ 2\right).
...
这个方法的一个潜在担忧是， $D$ 可能非常大，因为它是嵌套子查询所有变量绑定的集合。但幸运的是，情况并非如此。注意，如果原始嵌套连接是 $T _ 1 \mathbin{\mathrm{DepJoin}} T _ 2$，则 $|D| \le |T _ 1|$。因此，如果在对子查询去相关后，最上层连接是把 $T _ 1$ 存入哈希表的哈希连接，那么通过计算 $D$，该连接的内存消耗最多翻倍。这是绝对最坏情形。如果我们知道来自 $T _ 1$ 的值无重复，例如因为它们包含键，甚至可以避免物化 $D$，改为读取连接哈希表，从而消除任何开销。收益方面，我们把一个 $O(n^2)$ 操作转换为一个理想情况下的 $O(n)$ 操作，这完全值得内存开销。
...
\mathit{exams}\ e
\right)
\bowtie _ {s.\mathit{id}=d.\mathit{id}}\\
&\left(
\Pi _ {d.\mathit{id}:s.\mathit{id}}
\left(
\mathit{students}\ s
...
$$
\Gamma _ {d.\mathit{id}; m:\min(e _ 2.\mathit{grade})}
\left(
\Pi _ {d.\mathit{id}:s.\mathit{id}}(\ldots)
\mathbin{\mathrm{DepJoin}}
\sigma _ {d.\mathit{id}=e _ 2.\mathit{sid}}
(\mathit{exams}\ e _ 2)
\right).
...
\left(
\Pi _ {d.\mathit{id}:s.\mathit{id}}(\ldots)
\bowtie _ {d.\mathit{id}=e _ 2.\mathit{sid}}
\mathit{exams}\ e _ 2
\right).
$$

在许多情况下，本例也如此，我们可以完全消除与域 $D$ 的连接：如果域的所有属性都与已有属性进行等值连接，我们就可以改为从已有属性推导该域。在本例中，我们知道连接后 $d.\mathit{id}=e _ 2.\mathit{sid}$ 成立，因此可以用一个 map 算子把 $d.\mathit{id}$ 替换为 $e _ 2.\mathit{sid}$，如图 8 所示。该替代的好处是，查询的各部分完全独立，不再留下原始嵌套的痕迹。
...
\sigma _ {d.\mathit{id}=e _ 2.\mathit{sid}}
\left(
\chi _ {d.\mathit{id}:e _ 2.\mathit{sid}}
(\mathit{exams}\ e _ 2)
\right)
\right).
$$

不过，替代会创建原始元组的超集！至少一般如此；在本例中，参照完整性很可能会防止这一点。但一般来说，丢弃与域的连接并改用替代，可能导致更大的中间结果，因为连接的过滤效果也被移除了。这不影响最终结果，因为过滤仍会在更高位置发生，也就是在原始依赖连接的位置发生，但它可能影响查询运行时间。因此，是否移除连接必须由查询优化器决定（见第 4 节）。

注意，替代后尝试移除 $\sigma _ {d.\mathit{id}=e _ 2.\mathit{sid}}$ 时需要谨慎。很容易认为该选择可以直接丢弃，因为 $d.\mathit{id}$ 是从 $e _ 2.\mathit{sid}$ 推导出来的。但只有在 $e _ 2.\mathit{sid}$ 不可为空时这样才安全。如果 $e _ 2.\mathit{sid}$ 可以取 NULL 值，则必须保留该选择；事实上，它只是一个非 NULL 检查。

### 3.4 示例查询 Q2 的优化

对于我们的动机查询 Q2，结果计划见图 9。注意，这里 $D$ 不能消除，因为存在使用 $D$ 中值的非等值连接，阻止了替代。这里无法解耦嵌套子查询求值，因为 $s.\mathit{year}$ 没有与 $e _ 2.\mathit{date}$ 做等值连接。因此，外层查询的域必须被侧向传递给嵌套查询求值。不过要注意，所有依赖连接都已经消失，并被高效的常规代数算子替换。
...
$$
\begin{aligned}
\Pi _ {s.\mathit{name}, e.\mathit{course}}
(
&\left(\mathit{students}\ s\ \text{（按专业过滤）}\right)
\bowtie _ {s.\mathit{id}=e.\mathit{sid}}
\mathit{exams}\ e\\
&\bowtie _ {e.\mathit{grade}\gt{}m+1
\land
\left(
d.\mathit{id}=s.\mathit{id}
\lor
\left(
d.\mathit{year}\gt{}e.\mathit{date}
\land
e.\mathit{curriculum}=d.\mathit{major}
\right)
\right)}
\Gamma _ {d.\mathit{id},d.\mathit{year},d.\mathit{major};
m:\mathrm{avg}(e _ 2.\mathit{grade})}
(
\Pi _ {d.\mathit{id}:s.\mathit{id},
d.\mathit{year}:s.\mathit{year},
d.\mathit{major}:s.\mathit{major}}
(\ldots)\\
&\qquad\bowtie _ {d.\mathit{id}=e _ 2.\mathit{sid}
\lor
\left(
d.\mathit{year}\gt{}e _ 2.\mathit{date}
\land
e _ 2.\mathit{curriculum}=d.\mathit{major}
\right)}
\mathit{exams}\ e _ 2
)
).
...
显然，不能把这个查询翻译为依赖连接；但可以否定谓词，并使用后续未编号图左侧所示的依赖反连接。去嵌套优化过程中，该依赖反连接可以转换为图右侧的“普通”反连接。转换可写为：

$$
R
\mathbin{\mathrm{DepAntiJoin}} _ {X\ne Y}
\left(
\sigma _ {S.B=R.A}(S)
\right)
\quad\Rightarrow\quad
R
\mathbin{\mathrm{AntiJoin}} _ {(X\ne Y)\land(S.B=R.A)}
S.
$$
...
一般而言，如果我们可以用子树中已有的值替代 $D$，就可以消除 $D$。这在等值连接中很常见。例如，查询包含表达式 $D\bowtie _ {D.a=R.b}R$，我们就可以通过检查 $R.b$ 的值，得知哪些 $D.a$ 的可能值能够到达原始依赖连接。句子中强调的部分很重要：当然， $D$ 可以包含不存在于 $R$ 中的值，但这些值永远找不到连接伙伴，因此永远不会到达原始依赖连接。我们可以忽略它们。

为了决定是否替代，我们必须先分析查询树，找出由连接和过滤条件诱导的等价类。例如，过滤条件 $\sigma _ {a=b}$ 意味着 $a$ 和 $b$ 位于同一个等价类中。我们知道最终结果中 $a$ 和 $b$ 具有相同值，因此可以用 $b$ 替代 $a$。计算这些等价类相对直接。一个潜在问题来源是外连接，它可能导致上例中的 $a$ 和 $b$ 不相等；但由于 $D$ 上最顶层的连接已知是 NULL 拒绝的，这里不是问题。

识别出等价类 $C$ 后，我们可以按如下方式判断可能的替代：
...
\chi _ {A(D):B}(T)
\qquad
\text{若存在 } B\subseteq A(T) :
A(D)\equiv _ C B.
$$

因此，与其和 $D$ 连接，我们不如扩展 $T$（使用 map 算子），并用等价属性计算来自 $D$ 的隐含属性值。注意，这只有因为 $D$ 是集合才成立。还要注意，替代可能增加中间结果大小，两种表达式之间的关系不是等号，而是子集关系。这种基数增加由失去与 $D$ 连接的潜在剪枝能力造成。原本我们只用 $T$ 中能在 $D$ 中找到连接伙伴的每个元组来求值形成的相关子树；替代后我们则用 $T$ 中的每个元组求值。那些没有连接伙伴的元组只会在树的更高位置，即原始依赖连接执行时被消除，但中间结果可能更大。

因此，只有当与 $D$ 的连接选择性不高时，替代才划算。查询 Q1 中就是这种情况，所以使用替代是个好主意；但一般而言，查询优化器必须比较两种替代方案的代价，并选择更便宜的一种。对于选择性高的连接，保留 $D$ 从而尽早消除元组更好。

## 5. 评估

去嵌套相关子查询可以带来几乎任意大的收益，因为它可以把 $O(n^2)$ 操作转换为理想情况下的 $O(n)$ 操作。例如，用精心构造的查询展示 100 倍提升会很容易。但这些示例以及得到的因子都会有些任意。因此，我们首先比较我们的技术与其他系统的表达能力，然后给出 TPC-H 的一些性能数字。所有实验均在配备 64GB 主内存的 Intel i7-3930K 上运行。

我们在自己的 HyPer [KN11] 系统中实现了该去嵌套技术，并将其与其他方法比较。首先，我们研究了若干数据库系统的去嵌套能力。在商业系统中，SQL Server 似乎拥有最好的去嵌套引擎；根据 MS SQL Server 团队的论文 [GJ01]，这是可以预期的。不幸的是，SQL Server 2014 的许可条款禁止发布运行时间数字，因此我们只定性描述结果。我们也在 PostgreSQL 9.1 上运行实验，因为它允许我们发布数字。对于我们的两个示例查询的测试数据，我们生成了 1,000 个学生元组和 10,000 个考试元组，即每个学生 10 门考试。这是一个小数据集，但去嵌套的效果非常极端，因此我们不想增加数据集大小。粗略地说，我们方法的收益会随关系大小按二次方增长，因此通过增加数据集大小可以展示任意大的收益。
...
TPC-H 中的查询并不那么难去嵌套，所有大型商业系统都能去嵌套它们。事实上，这是绝对必要的；以下来自我们的 HyPer 系统的性能数字说明了这一点，该系统允许“打开和关闭”去嵌套。即使在 $\mathit{SF} = 1$，Query 4 在去嵌套时运行时间为 7ms，不去嵌套时为 157,616ms。相差几个数量级，突出说明去嵌套绝对必要。其他查询也受影响。例如，Query 17 去嵌套时需要 9ms，不去嵌套时需要 4,664ms。当然，每个厂商都会确保众所周知的 TPC-H 查询能在其系统中正确去嵌套，但这些数字说明性能影响非常大，因此系统应当能够去嵌套任意查询，正如我们提出的那样。
...
Brantner、May 和 Moerkotte [BMM07] 使用带 bypass 算子的代数 [KMPS94] 优化了一个特定查询模式，即存在析取时的标量子查询。Akinde 和 Böhlen [AB03] 讨论了涉及大型事实表的查询去嵌套这一重要特殊情形。他们认为外连接在拥有非常大事实表的 OLAP 环境中过于昂贵，因此提出了一个广义多维连接算子 GMDJ。
...
过去已经发表了许多关于 SQL 查询去嵌套的方案。然而，到目前为止，它们的实现都不能全面覆盖所有可能模式；这一点从既有甚至近期文献仍在优化特殊情形即可看出。此外，我们对商业和开源数据库系统的分析也表明，它们的优化只覆盖特殊模式，而这些模式可以说是查询嵌套中最重要的用例。我们在本文中开发了一种代数变换方法，覆盖所有类型的嵌套子查询。代数等价关系允许把初始使用的依赖连接完全替换为“常规”连接算子。为了减少嵌套查询求值工作的范围，我们使用侧向信息传递，把独立子查询求值计划限制为那些与外层查询相关的元组集合。该方法已经完整实现在我们的主内存数据库系统 HyPer 中，并可通过我们的 Web 界面 `www.hyper-db.de` 实验；该界面不仅显示交互式查询运行时间，也显示优化后的查询求值计划。把去嵌套变换集成到我们的基于代价的优化器中，确保我们不会过于急切地做某些事，例如把内层查询与外层查询解耦却引入更高代价；优化器只会在存在代价收益时这样做，而在许多实际相关场景中，该收益非常显著。
...
[BAW+09] Srikanth Bellamkonda, Rafi Ahmed, Andrew Witkowski, Angela Amor, Mohamed Zaït, and Chun Chieh Lin. Enhanced Subquery Optimizations in Oracle. PVLDB, 2(2):1366-1377, 2009.
...
[CB05] Bin Cao and Antonio Badia. A Nested Relational Approach to Processing SQL Subqueries. In Fatma Özcan, editor, Proceedings of the ACM SIGMOD International Conference on Management of Data, Baltimore, Maryland, USA, June 14-16, 2005, pages 191-202. ACM, 2005.
