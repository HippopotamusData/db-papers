...
**联系邮箱：** `{pmenon,angom,lin.ma,tcm,pavlo}@cs.cmu.edu`
...
我们提出 PCQ，用以弥合 JIT 编译与 AQP 之间的鸿沟。PCQ 允许 DBMS 修改已经编译的查询，既不需要重新编译，也不要求在查询开始前包含全部可能变体。PCQ 在查询代码中设置间接层，使 DBMS 甚至能在查询运行过程中改变计划。我们在一个内存 DBMS 中实现 PCQ，并用微基准将其与非自适应计划比较，又与先进分析型 DBMS 比较。我们的评估表明，PCQ 相对静态计划可获得超过 4 倍的性能，在分析基准上相对其他 DBMS 可获得超过 2 倍的性能。
...
为弥合刚性编译与细粒度适应性之间的鸿沟，我们提出一种面向 JIT DBMS 的系统架构和 AQP 方法，以支持 PCQ。PCQ 的关键是我们提出的新颖“只编译一次”方法：我们不反复调用编译器，也不预编译多个物理计划，而是只编译一个物理查询计划，并把它组织成日后无需大量重编译即可排列的形式。例如五个合取过滤项有 120 种顺序，我们的 PCQ 框架只编译每项一次，就能按观测选择率在 120 种顺序间切换。我们的方法融合两项关键技术：第一，我们使用结合预编译向量原语 [9] 与 HyPer 风格流水线 JIT [28] 的混合查询引擎；第二，我们在查询的底层执行代码中嵌入轻量钩子，用于观察且在运行中修改流水线。一条流水线的指标也可在其他流水线编译前用于优化它们。

我们在编译式内存 DBMS NoisePage [4] 中实现了我们的 PCQ 方法。我们的实验结果表明，对于商业优化器生成的计划，PCQ 最高可提升 4 倍；我们还将它与 Tableau HyPer [3] 和 Actian Vector [1] 等现有内存 OLAP 系统比较，并表明启用 PCQ 的 NoisePage 最高快 2 倍。
...
我们生成每列数据并选择过滤常量 $\delta_i$，使查询总体选择率固定，而各谓词项的选择率在不同数据块间变化；我们的实验环境详见第 5 节。我们首先测量 DBMS 使用最佳“静态”计划执行上述查询的时间，即采用优化器选择的固定求值顺序。我们还执行预先知道每个数据块最佳过滤顺序的“最优”计划；它相当于编译了查询的全部可能流水线，因此代表理论执行时间下界。最后，我们还使用 DBMS 根据选择率重排的 PCQ 过滤器执行查询。

图 1(a) 表明，选择率较低时，静态计划最多比最优计划慢 4.4 倍；选择率升高后，需要处理的元组更多，差距逐渐缩小。我们的第二个观察是，PCQ 会依据实时数据分布周期性重排谓词，在全部选择率下始终保持在最优执行时间的 10% 以内。
...
PCQ 让 JIT DBMS 在运行中改变已编译查询策略，同时不重启、不重复工作、不预编译替代流水线。它与主动式重新优化 [7] 都无需回到优化器或重复处理元组；区别是 PCQ 不预计算全部子计划，也不预设切换阈值，而在运行时探索能改善延迟或资源利用率的策略。本节我们以图 2 的示例查询概述 PCQ。正如我们下文将讨论的，查询生命周期分为三个阶段。我们虽然为 NoisePage 的 LLVM 环境设计了该框架，但它也适用于其他支持查询编译的 DBMS 执行引擎。
...
优化器生成物理计划后，翻译器把它分解为流水线，并转换成名为 TPL 的数据库 DSL。TPL 结合 VectorWise 风格预编译原语 [9] 与 HyPer 以数据为中心的代码生成 [28]，比 C/C++ 更易应用数据库专用优化。此外，正如我们下文所述，TPL 的编译延迟很低。

翻译器加入两类 PCQ 构造。第一类是可开关的指标钩子，按策略收集流水线底层操作的运行时性能。第二类是参数化运行时结构，以间接层替换执行策略。算子逻辑可分查询无关与查询特定部分；后者由 DBMS 生成不同版本并用指针切换。我们定义两级间接性：一级中算子无需知道查询特定实现，二级要求运行时与代码生成器协作。图 2 的过滤数组每项指向生成函数，排列只需交换指针。
...
下面我们介绍 PCQ 支持的优化类别。如前所述，DBMS 生成查询执行代码时，会让流水线内可能具有不同性能的操作可以在运行时重新排列或选择性启用。操作既可以是单个谓词这类短时、细粒度步骤，也可以是连接这类代价较高的关系算子。各项优化彼此独立，不会改变同一流水线或查询其他流水线中其他优化的行为。对每个类别，我们分别说明优化器是否需要修改物理计划、Translator 如何组织代码以支持运行时排列；我们还讨论 DBMS 如何采集策略决策所需的指标。
...
**运行时排列。** 可排列过滤器收到输入批次后，先由策略决定是否重新采集每个过滤组件的统计信息；采集频率和具体指标均可配置。我们采用的一个简单策略是，以固定概率 $p$ 随机采样选择率和运行时间。我们在第 5 节研究 $p$ 的影响。
...
下面我们用图 4 中的查询介绍 PCQ 自适应聚合如何利用分组键倾斜：
...
**准备与代码生成。** Translator 先创建处理热键的专用函数 `aggregateHot`。它接收一批输入元组和一个包含 $N$ 个热键聚合 payload 的数组；数组中的每个元素都保存分组键和持续累积的聚合值， $N$ 的大小由策略决定。为便于说明，我们选择抽取两个 heavy-hitter key。由于 $N$ 是查询编译期常量，Translator 会生成 $N$ 个条件分支：匹配热数组的元组按查询语义更新聚合值，其余元组落入“冷”键路径。
...
**运行时排列。** 聚合的常规流程基本不变，区别在于 DBMS 为一批分组键计算哈希值时，还使用 HyperLogLog（HLL）[15] 跟踪近似 distinct key 数。HLL 表示紧凑，与复杂的聚合逻辑相比，计算开销很小。只有当 HLL 估计该批次的不同分组键少于 $N$ 时，我们才采用优化流水线。

在优化路径中，DBMS 分配聚合值数组，并用当前批次中最热的键初始化。识别热键的方法由策略决定：简单策略取批次中前 $N$ 个唯一键，更复杂的策略则随机采样，直至找到 $N$ 个唯一键。在本文中，我们采用前一种策略，因为我们发现它能提供最佳的性能与成本权衡。初始化后，DBMS 调用专用热键聚合函数，返回时再用合并函数把部分结果写回哈希表。
...
PCQ 从两方面优化 hash join：依据运行时信息选择哈希表实现，以及在右深或左深查询计划中重排连接的应用顺序。下面我们先讨论数据结构专门化，再说明代码生成与运行时实现连接重排所需的步骤。我们约定 hash join 的左输入为构建侧、右输入为探测侧。
...
**准备与代码生成。** 优化器在包含连续连接的右深计划中启用可排列连接，并指定一张与其余一张或多张表连接的“驱动”表。根据访问方法，系统可以使用 hash join 或 index join；无论是 inner join 还是 outer join，都能以任意顺序应用，因为每个驱动元组彼此独立，中间迭代状态也只在一个元组批次内短暂存在。图 5(b) 中，A 可以先与 B 连接，也可以先与 C 连接，最佳顺序可能随查询处理的数据块分布变化。我们在 NoisePage 中的实现另有一项要求：驱动表必须包含全部连接所需的所有键列。
...
下面我们分析 PCQ 方法及其系统架构。我们在 NoisePage DBMS [4] 中实现了我们的 PCQ 框架和执行引擎。NoisePage 是兼容 PostgreSQL 的 HTAP DBMS，在 Apache Arrow 内存列式数据 [25] 上采用 HyPer 风格的 MVCC [29]，并使用 LLVM 9 把我们的字节码 JIT 编译为机器码。

我们在一台配有 2 颗 10 核 Intel Xeon Silver 4114 CPU（2.2 GHz、每核 25 MB L3 cache、支持 AVX512）和 128 GB DRAM 的机器上进行评估。我们用 `numactl` 确保 DBMS 把整个数据库载入同一 NUMA region。我们使用 Google Benchmark [2] 库实现微基准；该库会把每项实验运行足够多次，以获得统计稳定的执行时间。我们先介绍评估中使用的工作负载，然后测量 PCQ 改善已编译查询性能的能力。首批实验使用单线程，以尽量消除调度干扰。最后，我们用多线程查询把启用 PCQ 的 NoisePage 与两个先进 OLAP DBMS 比较。

### 5.1 工作负载

我们先介绍评估中使用的三类工作负载。

**微基准。** 我们建立了一个合成基准，用于隔离并测量 DBMS 运行时行为的不同方面。数据库包含 A 至 F 六张表，每张表都有六个 64 位有符号整数列 `col1` 至 `col6`、300 万个元组，占用 144 MB。在使用该基准的每项实验中，我们都会改变列值的分布和相关性，以突出某个具体组件。工作负载包含三类查询，分别对应第 4 节的一项优化：含三个谓词的扫描查询、带分组的聚合查询，以及多路连接查询。

**TPC-H。** 该决策支持工作负载模拟 OLAP 环境 [37]，包含 3NF schema 中的八张表。我们使用 scale factor 10（约 10 GB），并采用倾斜版 TPC-H 生成器 [5]，使数据更接近真实应用。我们选择九条查询，覆盖 TPC-H choke point 分类 [8] 中从计算密集型到内存/连接密集型的类别；因此，我们预期结果也能推广到基准中的其余查询。
...
我们首先评估 PCQ 在数据分布变化时优化并排列过滤条件的能力。我们使用微基准工作负载，其中的 `SELECT` 查询对一张表执行顺序扫描：
...
> **译者注：** 以上 SQL 按 `source.pdf` 原样保留，其中第二、第三个谓词都引用 `col3`。但紧随其后的实验说明称最具选择性的谓词会在 `col1`、`col2` 等三个谓词之间轮换；两者存在不一致。这里不据此擅自修改源 SQL 的列名。

`WHERE` 子句常量使实验的数据生成器能够指定目标选择率。

**随时间变化的性能。** 在第一项实验中，我们改变各谓词选择率，以评估 PCQ 过滤器在表扫描期间的性能。我们让其中一个谓词的选择率约为 2%，另两个各为 98%，并在表中互不重叠的区段轮换最有选择性的谓词：前 500 个元组块由 `col1` 上的谓词最有选择性，接下来 500 个块则由 `col2` 上的谓词最有选择性，如此使每个谓词只在表的三分之一范围内最优。

我们使用采用 10% 采样率策略的 PCQ 可排列过滤器执行该查询，即 DBMS 对每个数据块以 10% 概率收集指标。我们还使用三种分别把不同谓词放在首位的“静态”顺序执行查询，代表现有不可排列的 JIT 编译式 DBMS。

![图 6：随时间变化的性能。图中比较顺序扫描期间三种静态过滤顺序与我们的 PCQ 过滤器的每数据块执行时间。](assets/manual-figure-06-performance-over-time.png)

**图 6：随时间变化的性能。顺序扫描期间三种静态过滤顺序与我们的 PCQ 过滤器的每数据块执行时间。**
...
**改变谓词选择率。** 接下来，我们分析可排列过滤器在一系列选择率下的表现，以检验其稳健性。在该实验中，我们改变上述查询的数据分布，让过滤器的总体选择率从 0%（没有元组入选）变化到 100%（全部元组入选）。PCQ 仍使用 10% 采样率。我们把优化器根据已有统计信息选择的顺序作为“静态”配置；我们还执行“最优”配置，为 DBMS 预先提供每个数据块的最佳过滤顺序，代表不计重新采样开销时可达到的性能上界。
...
图 7 表明，在全部选择率范围内，PCQ 都保持在最优配置的 20% 以内；我们的第二个观察是，只要选择率低于 100%，PCQ 和最优配置也始终优于优化器给出的静态顺序。选择率为 0% 时，PCQ 和最优配置分别比静态顺序快 2.7 倍和 3.6 倍，因为二者都能把最有选择性的谓词放在首位。选择率增加后，DBMS 必须处理更多元组，各配置耗时都会增长；到 100% 时自适应已无必要，PCQ 反而因采样开销最慢。所有配置在 100% 时又比 90% 时快，是因为 DBMS 会对满向量启用专用优化。

**过滤排列开销。** 第 4.1 节指出，指标采集频率与运行性能之间存在权衡。为量化这一关系，接下来我们用不同重采样频率执行该 `SELECT` 查询，把频率从 0.0（从不采样）变化到 1.0（每访问一个数据块都采样并重新排序）。我们把过滤器总体选择率固定为 2%，并像图 6 那样在第 500 和第 1000 个数据块处改变最有选择性的谓词。查询从图 6 中总体性能最好的 `Order-3` 静态顺序开始；我们分别测量采集性能指标和实际执行所花的时间。
...
图 8 表明二者并非线性关系。完全禁用采样虽然没有指标开销，却无法响应分布波动，因此比可排列过滤器慢约 1.7 倍。每个数据块都采样会在执行时间上增加 15% 开销。0.1 采样率表现最佳，因此我们在后续实验中均采用这一设置。

### 5.3 聚合适应性

下面我们评估 PCQ 利用倾斜加速 hash aggregation 的能力。本节实验在没有另行说明时，都使用微基准中对单表执行以下哈希聚合的查询；我们修改工作负载的数据生成器，让分组键 `col1` 呈现指定倾斜：
...
**改变聚合键数量。** 我们首先测量 PCQ 聚合在改变基准表中唯一聚合键总数时的性能。我们从随机分布中为分组键列 `col1` 取值，以控制每次试验的唯一键数；聚合函数使用的 `col2` 至 `col4` 则从各自取值域中均匀随机生成。

我们把 PCQ 框架配置为抽取五个 heavy-hitter key；该数量在 DBMS 中可调，但本实验不改变。我们还执行一个按数据中心式方法把表扫描与聚合融合的静态计划 [28]，代表现有 JIT DBMS 的执行方式。

![图 9：改变 aggregate 数量。我们改变唯一聚合键总数时，自适应聚合在 hot key 数较少时收益明显，在 key 数扩大后仍能避免静态计划的高开销。](assets/manual-figure-09-varying-number-aggregates.png)

**图 9：改变聚合数量。我们改变唯一聚合键总数时，自适应聚合的性能。**
...
**改变聚合倾斜度。** 抽取更多 heavy-hitter key 会增加分支预测失败的可能性，也会生成更大的函数、延长编译时间，因此 DBMS 必须谨慎选择抽取数量。为研究这一关系，我们把唯一分组键总数固定为 200k，使用倾斜的 Zipfian 分布生成 `col1`，并比较抽取 0、1、2、4、8 个热键的 PCQ 配置。我们同时测量查询执行时间和输入元组命中热键条件分支的比例。
...
下面我们评估 PCQ 根据数据分布优化 hash join 的能力。每项实验都构造右深连接树，在独立流水线中建立哈希表，再在最终流水线中探测；数据生成器会为每对表的连接键指定选择率，并控制总体选择率。
...
我们针对上述连接属性定制数据生成，使其达到从 0%（没有元组找到连接伙伴）到 100%（全部元组找到伙伴）的目标选择率。我们使用反映 DBMS 所给连接顺序的静态配置，并采用 HyPer JIT 引擎会生成的融合一次一元组模型 [28]。我们还使用 10% 重采样策略执行 PCQ 连接，并从与静态配置相同的初始连接顺序开始。
...
**改变连接数量。** 下面我们评估 PCQ 执行多步连接时的性能。在该实验中，我们把查询中的 hash join 数量从一个增加到五个，同时把总体选择率保持在 10%。单连接本不需要排列，但我们仍将它列入图中，以便完整比较；NoisePage 在这种情况下会消除可排列连接。我们执行与前述实验类似的 `SELECT` 查询，并在其后追加连接子句、投影全部表列。
...
最后，我们把启用和禁用 PCQ 的 NoisePage 与两个先进内存 DBMS 比较：Actian Vector 9.2 [1] 和 Tableau HyPer 5.1 [3]。Vector 是基于 MonetDB/X100 [9] 的列式 DBMS，执行引擎由 SIMD 优化的向量原语构成；我们调整其配置，使其充分利用系统内存和 CPU 线程。HyPer 用 LLVM 生成可解释或 JIT 编译的一次一元组计划；我们使用的 HyPer 版本也支持 SIMD 谓词求值。依 Tableau 工程师建议，我们不修改 HyPer 配置。

本节我们使用 TPC-H 和 SSB 基准评估这些系统。载入数据后，我们在各系统中执行所需的统计收集和优化操作；我们用工作负载预热一次，再报告连续五次运行的平均执行时间。我们人工检查计划并尽力确保各 DBMS 执行等价计划，但我们注意到，这些系统仍各自包含其他系统没有的优化。对于 NoisePage，我们使用 HyPer 优化器生成的查询计划。

#### 5.5.1 倾斜 TPC-H 逐查询分析

我们首先使用 Microsoft 倾斜数据生成器 [5] 评估 TPC-H，并将 skew 设为 2.0。图 13 给出整体结果；我们还用表 1 拆分每项优化的作用。表中每格表示在此前 PCQ 优化基础上再启用本项的相对加速；接近 1.0 表示影响小，空白表示不适用。

- **Q1：** 四个分组键上计算五个聚合。最热键对接收的更新从无倾斜时 49% 增至高倾斜时 86%，触发聚合优化并提升 1.7 倍。PCQ NoisePage 比 Vector 快 4.8 倍、比 HyPer 慢 1.2 倍；我们认为原因在于 HyPer 的定点运算快于 NoisePage 的浮点运算。
...
- **Q6：** `LINEITEM` 上过滤选择率 0.05%。我们注意到，倾斜增加并不改变谓词顺序，固定概率重采样反使 PCQ 比基线慢 4%；更高级采样策略可避免。各系统 SIMD 过滤性能接近。
- **Q7：** HyPer 选择的 bushy 连接计划比右深计划慢 4 倍。虽无元组进入最终聚合，PCQ 翻转 `l_shipdate` 范围谓词仍提升 1.2 倍。
- **Q11：** 五个连接均不可排列；两个聚合基数不触发优化；向量化谓词都只有单项。我们将它纳入比较，以表明 PCQ 的空载开销可忽略，并展示优化器可在数据倾斜下帮助识别更好计划的场景。NoisePage 与 HyPer 接近，比 Vector 快 4 倍。
- **Q16：** 以 `PARTSUPP` 为驱动的右深流水线，`PART` 上有多项过滤和哈希聚合。过滤重排提升近 1.2 倍；按构建表大小重排连接以使用 SIMD gather 又提升 1.2 倍。总体比 HyPer 快 7.4 倍、比 Vector 快 3 倍；HyPer 高倾斜下错选左反连接而非右反连接。我们认为，更好的计划会改善 HyPer 的性能。
- **Q18：** 以 `ORDERS` 为驱动的右深流水线；聚合基数超过阈值。PCQ 重排连接，使小表使用 SIMD gather，提升 1.19 倍。HyPer 错用右半连接而非左半连接，比 PCQ 慢 2.6 倍。
- **Q19：** `PART` 与 `LINEITEM` 内连接后接复杂析取过滤和静态聚合。重排 `LINEITEM` 谓词提升 1.2 倍。HyPer 分别比 NoisePage、Vector 快 1.2 倍和 2.5 倍；我们把 NoisePage 的性能下降归因于一次一元组与向量过滤并用时“已选元组”内部表示间的高成本转换。
...
本实验在 Star Schema Benchmark [30] 上评估全部系统。十三条查询分四组，组内结构等价，只是过滤和聚合项不同；因此我们按组讨论结果，因为同组中一条查询的行为可以推广到其他查询。与前面的 TPC-H 实验不同，我们让启用 PCQ 的 NoisePage 从随机初始计划开始，以展示我们的方法带来的收益；无 PCQ 版本则使用 HyPer 生成的最优计划。

- **Q1.*：** 最小表与最大表之间只有一个连接，两表都有选择性多项过滤。连接不可排列，过滤重排只带来小幅收益。HyPer 在压缩数据上执行 SIMD 向量过滤，CPI 更好，分别比 NoisePage 和 Vector 快 1.7 倍、3.7 倍。
- **Q2.*：** 三个连接和一个聚合。PCQ 从随机连接顺序出发，按选择率和运行条件排列，平均比基线快约 1.5 倍。我们观察到，过滤探索有轻微负收益，因为最佳顺序始终不变；我们认为，更复杂、能调整采样频率的自适应策略可以避免该问题。总体比 HyPer 快 1.4 倍、比 Vector 快 2.2 倍。
- **Q3.*：** 与 Q2 类似但替换一张基表，仅 Q3.4 触发聚合优化。连接探索平均提升 1.3 倍；最后，我们注意到 Q3.4 会在最佳排名稳定后仍探索其他顺序，因而出现探索开销大于收益的退化。我们认为，更好的策略设计可以缓解该问题。总体比基线和 HyPer 快 1.3 倍、比 Vector 快 3.2 倍。
...
与我们的 PCQ 方法最相关的是参数化优化 [11, 16] 和主动式重新优化 [7]。Volcano 为一条流水线生成多个计划，以 `choose-plan` 算子按观测基数选择。Rio 在流水线内加入 `switch` 算子并收集运行统计 [7]。Plan Bouquets [14] 生成运行时切换且有最坏情况界的“参数化最优计划集”。所有这些方法都与我们的方法相似，但它们面向解释式 DBMS，只在执行前粗粒度选择不可排列子计划；PCQ 可在流水线执行中切换策略。Perron 等也表明现代代价优化器对部分查询仍会失效 [33]，PCQ 在执行期处理许多相同问题。

IBM 的 AQP 可动态重排流水线连接 [24]，但针对 OLTP，难以推广到分析查询。SkinnerDB 用强化学习近似最优连接顺序 [38]，却需为所有索引预计算哈希表，且只支持单线程。HyPer 的自适应编译 [20] 包含许多我们用来构建支持 PCQ 的 DBMS 的基础部件：它与 NoisePage 一样先解释专用字节码，但其字节码源自 LLVM IR；HyPer 只在解释/编译模式间切换，不修改高层计划，也不执行我们在第 4 节描述的流水线内底层优化。
...
我们为三类关系算子提出了 PCQ 优化。对于扫描，我们提出自适应过滤器，高效发现能缩短执行时间的过滤顺序。对于基于哈希的聚合，我们提出一种动态优化，用于识别并利用 grouping key 的倾斜，把重键抽入哈希表之外的专用路径。最后，我们为左深或右深 hash join 提出一种优化，使 DBMS 可以在运行时重排多个连接的应用顺序，以最大化性能。我们的评估表明，NoisePage 启用 PCQ 后，在合成工作负载上最高获得 4 倍性能提升，在 TPC-H 和 SSB 基准上最高获得 2 倍性能提升。
