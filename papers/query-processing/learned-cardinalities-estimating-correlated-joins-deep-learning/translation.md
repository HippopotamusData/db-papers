...
$$
\text{表模块：}\quad w_T=\frac{1}{|T_q|}\sum_{t\in T_q}\mathrm{MLP}\relax_T(v_t)
$$

$$
\text{连接模块：}\quad w_J=\frac{1}{|J_q|}\sum_{j\in J_q}\mathrm{MLP}\relax_J(v_j)
$$

$$
\text{谓词模块：}\quad w_P=\frac{1}{|P_q|}\sum_{p\in P_q}\mathrm{MLP}\relax_P(v_p)
$$

$$
\text{合并并预测：}\quad w_{\text{out}}=\mathrm{MLP}\relax_{\text{out}}([w_T,w_J,w_P])
$$
...
查询表示由多个集合组成，因此 MSCN 对每个集合 $S$ 学习一个集合专用的逐元素神经网络 $\mathrm{MLP}\relax_S(v_s)$，即分别作用于每个元素 $s\in S$ 的特征向量 $v_s$。¹ 集合的最终表示 $w_S$ 是各元素变换后表示的平均：²

$$
w_S=\frac{1}{|S|}\sum_{s\in S}\mathrm{MLP}\relax_S(v_s)
$$
...
$$
w_{\text{out}}=\mathrm{MLP}\relax_{\text{out}}([w_{S_1},w_{S_2},\ldots,w_{S_N}])
$$
