...
$$
\mathrm{COST} = \text{PAGE-FETCHES} + W \times (\text{RSI CALLS})
$$

这个成本是 I/O（取页次数）和 CPU 使用量（执行的指令）的加权度量。 $W$ 是 I/O 与 CPU 之间可调的权重因子。 $\text{RSI CALLS}$ 是预计从 RSS 返回的元组数。由于 System R 的 CPU 时间大部分消耗在 RSS 中，RSI 调用数是 CPU 使用量的良好近似。因此，选择处理查询的最小成本路径，目的在于尽量减少所需的总资源。
...
- $\mathrm{NCARD}(T)$：关系 $T$ 的基数。
- $\mathrm{TCARD}(T)$：段中包含关系 $T$ 元组的页面数。
- $P(T)$：段中包含关系 $T$ 元组的数据页比例。

$$
P(T) = \frac{\mathrm{TCARD}(T)}{\text{段中非空页面数}}
$$

对关系 $T$ 上的每个索引 $I$：

- $\mathrm{ICARD}(I)$：索引 $I$ 中不同键的数量。
- $\mathrm{NINDX}(I)$：索引 $I$ 的页面数。
...
| 谓词 | 选择率因子与说明 |
| --- | --- |
| $\mathrm{column} = \mathrm{value}$ | 若 $\mathrm{column}$ 上有索引， $F = 1 / \mathrm{ICARD}(\text{column index})$；这里假定元组在索引键值之间均匀分布。否则 $F = 1/10$。 |
| $\mathrm{column1} = \mathrm{column2}$ | 若两列都有索引， $F = 1 / \mathrm{MAX}(\mathrm{ICARD}(\text{column1 index}), \mathrm{ICARD}(\text{column2 index}))$；这里假定基数较小的索引中的每个键值都能在另一索引中找到匹配值。若只有 $\text{column-i}$ 上有索引，则 $F = 1 / \mathrm{ICARD}(\text{column-i index})$；否则 $F = 1/10$。 |
| $\mathrm{column} \gt \mathrm{value}$（或其他任何单端开放的比较） | 如果 $\mathrm{column}$ 是算术类型，且访问路径选择时 $\mathrm{value}$ 已知，则 $F = (\text{high key value} - \mathrm{value}) / (\text{high key value} - \text{low key value})$，即按该值在键值范围内的位置作线性插值。否则 $F = 1/3$。这个数本身没有特殊意义，只是它比无索引等值谓词的猜测选择性更弱，并且小于 $1/2$；我们假设很少有查询使用会被半数以上元组满足的谓词。 |
| $\mathrm{column}\ \mathrm{BETWEEN}\ \mathrm{value1}\ \mathrm{AND}\ \mathrm{value2}$ | 如果 $\mathrm{column}$ 是算术类型，且访问路径选择时 $\mathrm{value1}$ 与 $\mathrm{value2}$ 都已知，则 $F = (\mathrm{value2} - \mathrm{value1}) / (\text{high key value} - \text{low key value})$，也就是用 BETWEEN 值域与整个键值范围之比作为选择率因子。否则 $F = 1/4$。这个选择本身也无特殊意义，只是位于等值谓词与范围谓词的默认选择率因子之间。 |
| $\mathrm{column}\ \mathrm{IN}\ (\text{list of values})$ | $F = (\text{列表中的项数}) \times (\mathrm{column} = \mathrm{value}\text{ 的选择率因子})$，但最大不超过 $1/2$。 |
| $\mathrm{columnA}\ \mathrm{IN}\ \mathrm{subquery}$ | $F = (\text{子查询结果的预期基数}) / (\text{子查询 FROM 列表中所有关系基数的乘积})$。推导见表后正文。 |
| $(\mathrm{pred1})\ \mathrm{OR}\ (\mathrm{pred2})$ | $F = F(\mathrm{pred1}) + F(\mathrm{pred2}) - F(\mathrm{pred1}) \times F(\mathrm{pred2})$。 |
| $(\mathrm{pred1})\ \mathrm{AND}\ (\mathrm{pred2})$ | $F = F(\mathrm{pred1}) \times F(\mathrm{pred2})$；这里假定列值相互独立。 |
| $\mathrm{NOT}\ \mathrm{pred}$ | $F = 1 - F(\mathrm{pred})$。 |
...
$$
F' = \frac{\text{子查询基数}}{\text{所有可能子查询答案的基数}}
$$

稍微乐观一点，可以把这一推理扩展到子查询本身为连接的情况，也可以扩展到 `columnB` 被含有列名的算术表达式替代的情况，于是得到表 1 中给出的公式。

查询基数 $\mathrm{QCARD}$ 等于查询块 FROM 列表中每个关系的基数之积，再乘以该查询块所有布尔因子的选择率因子之积。预期 RSI 调用数 $\mathrm{RSICARD}$ 等于关系基数之积乘以可作为搜索参数的布尔因子的选择率因子，因为这些布尔因子会被放入搜索参数，在元组跨 RSS 接口返回之前就将其滤除。
...
如果查询没有 GROUP BY 或 ORDER BY 子句，就不存在感兴趣次序，直接选择最便宜的访问路径。如果存在 GROUP BY 或 ORDER BY 子句，则必须把产生该感兴趣次序的成本，与“最便宜无序路径的成本加上把 $\mathrm{QCARD}$ 个元组排序成适当次序的成本”作比较，选择其中最便宜的方案作为查询块的计划。
...
| --- | --- |
| 与等值谓词匹配的唯一索引 | $1 + 1 + W$ |
| 聚簇索引 $I$ 与一个或多个布尔因子匹配 | $F(\mathrm{preds}) \times (\mathrm{NINDX}(I) + \mathrm{TCARD}) + W \times \mathrm{RSICARD}$ |
| 非聚簇索引 $I$ 与一个或多个布尔因子匹配 | $F(\mathrm{preds}) \times (\mathrm{NINDX}(I) + \mathrm{NCARD}) + W \times \mathrm{RSICARD}$；如果结果能放入 System R 缓冲区，则为 $F(\mathrm{preds}) \times (\mathrm{NINDX}(I) + \mathrm{TCARD}) + W \times \mathrm{RSICARD}$ |
| 聚簇索引 $I$ 不与任何布尔因子匹配 | $(\mathrm{NINDX}(I) + \mathrm{TCARD}) + W \times \mathrm{RSICARD}$ |
| 非聚簇索引 $I$ 不与任何布尔因子匹配 | $(\mathrm{NINDX}(I) + \mathrm{NCARD}) + W \times \mathrm{RSICARD}$；如果结果能放入 System R 缓冲区，则为 $(\mathrm{NINDX}(I) + \mathrm{TCARD}) + W \times \mathrm{RSICARD}$ |
| 段扫描 | $\mathrm{TCARD} / P + W \times \mathrm{RSICARD}$ |
...
系统还用一条启发式规则减少需要考虑的连接顺序排列。只要可能，就只考虑这样一些连接顺序：内关系至少有一个连接谓词把它同已经参与连接的其他关系联系起来。也就是说，连接关系 $t _ {1},t _ {2},\ldots,t _ {n}$ 时，只检查满足下述条件的顺序 $t _ {i _ {1}},t _ {i _ {2}},\ldots,t _ {i _ {n}}$：对于所有 $j\ (j = 2,\ldots,n)$，或者

1. $t _ {i _ {j}}$ 至少有一个连接谓词把它同某个 $t _ {i _ {k}}$ 联系起来，其中 $k \lt j$；或者
2. 对所有 $k \gt j$， $t _ {i _ {k}}$ 与 $t _ {i _ {1}},t _ {i _ {2}},\ldots,t _ {i _ {j-1}}$ 中任何关系都没有连接谓词。

这意味着所有需要笛卡尔积的连接都尽可能放到连接序列的后面。例如，若 $T _ {1},T _ {2},T _ {3}$ 是查询块 FROM 列表中的三个关系， $T _ {1}$ 与 $T _ {2}$ 之间有连接谓词，而 $T _ {2}$ 与 $T _ {3}$ 之间也有连接谓词，且后一个连接用的是不同于 $T _ {1}-T _ {2}$ 连接的列，则不考虑以下排列：
...
令 $\text{C-outer}(\mathrm{path1})$ 为通过 $\mathrm{path1}$ 扫描外关系的成本，令 $N$ 为满足可应用谓词的外关系元组基数。 $N$ 的计算如下：

$$
N = \left(\prod _ {\text{到目前为止参与连接的所有关系 }T}\mathrm{NCARD}(T)\right) \times \left(\prod _ {\text{所有可应用谓词}}F\right)
$$

令 $\text{C-inner}(\mathrm{path2})$ 为扫描内关系并应用所有可应用谓词的成本。注意，在合并扫描连接中，这意味着扫描内关系中与外关系某个连接列值相对应的连续元组组。于是，嵌套循环连接的成本为：

$$
\text{C-nested-loop-join}(\mathrm{path1},\mathrm{path2}) = \text{C-outer}(\mathrm{path1}) + N \times \text{C-inner}(\mathrm{path2})
$$

合并扫描连接的成本可以分成实际执行合并的成本，以及在必要时对外关系或内关系排序的成本。实际执行合并的成本为：

$$
\text{C-merge}(\mathrm{path1},\mathrm{path2}) = \text{C-outer}(\mathrm{path1}) + N \times \text{C-inner}(\mathrm{path2})
$$

如果内关系被排序成临时关系，第 4 节的单关系访问路径公式都不适用。在这种情况下，内扫描类似段扫描，但合并扫描方法会利用内关系已排序这一事实，不必为了寻找匹配而扫描整个内关系。此时内扫描成本采用下式：

$$
\text{C-inner}(\text{sorted list}) = \frac{\mathrm{TEMPPAGES}}{N} + W \times \mathrm{RSICARD}
$$

其中 $\mathrm{TEMPPAGES}$ 是容纳内关系所需的页面数。这个公式假定合并过程中内关系的每个页面只取一次。

值得注意的是，嵌套循环连接与合并扫描的成本公式实质相同。合并扫描有时优于嵌套循环，是因为内扫描成本可能低得多。排序后，内关系在连接列上聚簇，这会尽量减少取页次数；而且不必针对外关系的每个元组都扫描整个内关系寻找匹配。

对关系排序的成本 $\text{C-sort}(\mathrm{path})$ 包括：通过指定访问路径检索数据、对数据排序（可能需要多趟），以及把结果放入临时列表。注意，在对内表排序之前，只能应用局部谓词。并且，如果必须对复合结果排序，就必须先把整个复合关系存入临时关系，再进行排序。排序前把复合元组插入临时关系的成本也包含在 $\text{C-sort}(\mathrm{path})$ 中。
...
上述结果以图 3 的形式保存到搜索树中。图中记号 $C(\mathrm{EMP.DNO})$ 或 $C(\mathrm{E.DNO})$ 表示通过 DNO 索引扫描 EMP，并应用在指定关系集合的元组已经取出后可以应用的所有谓词的成本。记号 $N _ i$ 表示不同部分结果的基数。
...
为了判断被引用列值是否唯一，OPTIMIZER 可以利用诸如 $\mathrm{NCARD} \gt \mathrm{ICARD}$ 这样的线索，其中 $\mathrm{NCARD}$ 是关系基数， $\mathrm{ICARD}$ 是被引用列上索引的索引基数。
