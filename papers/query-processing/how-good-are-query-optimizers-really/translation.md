...
注意，IBM DB2 允许用户显式指定谓词的选择率，从而对估计过程提供有限的控制。不过，选择率注入无法完整建模关系间的相关性，因此不如对任意表达式注入基数的能力通用。
...
我们将每个算子的内存限制 `work_mem` 设为 2 GB，使系统更常使用内存哈希连接，而不是外部排序归并连接，从而带来明显更好的性能；将缓冲池 `shared_buffers` 设为 4 GB；将 PostgreSQL 使用的操作系统缓冲缓存估计值 `effective_cache_size` 设为 32 GB。通常建议 PostgreSQL 同时使用操作系统缓冲和自身缓冲池，并将大部分内存留给操作系统侧。这三个设置的默认值都很低，以 MB 而非 GB 计，因此通常建议增大它们。最后，我们将 `geqo_threshold` 提高到 18，强制 PostgreSQL 对超过 12 个连接的查询也使用动态规划，而不是退回启发式算法。
