...
图 5 展示更大的 40 核系统上普通算法和分段算法的加速比。分段算法采用 2、5、10 三种线段数，每段大小为 $|S|/\unicode{x23}segments$。40 核系统包含 4 个处理器，每个处理器 10 核，因此有效并行化比 12 核系统更具挑战。四处理器设计可能增加同步与缓存一致性开销；此外，算法受通信制约，还要考虑内存带宽饱和问题。
