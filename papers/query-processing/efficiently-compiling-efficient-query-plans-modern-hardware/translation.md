...
随着主内存容量增长，查询性能越来越取决于查询处理本身的 CPU 成本。经典 iterator 风格的查询处理技术简单灵活，但在现代 CPU 上因为局部性差和频繁的指令误预测而性能较差。过去提出了批处理、向量化元组处理等方法，但这些技术仍经常慢于手写执行计划。在这项工作中，我们提出一种新的编译策略，使用 LLVM 编译框架把查询转换为紧凑高效的机器码。该策略强调良好的代码局部性和数据局部性，以及可预测的分支布局，因此生成代码常能接近手写 C++ 的性能。我们把这些技术集成到 HyPer 主内存数据库系统中，实验显示它能在编译时间适中的前提下获得优异的查询性能。
...
当数据驻留在内存中时，磁盘 I/O 不再掩盖这些 CPU 开销。手写程序甚至明显快于非常快的向量化系统，如图 1 所示 [16]。这未必令人意外，因为人可以使用 DBMS 不会采用的技巧；但图中的查询只是简单聚合，按理说合理的求值方式并不多，因此差距也说明现有通用执行方式仍然不够理想。代数算子模型很适合对查询进行推理，但在查询处理本身显式呈现算子结构却未必是好主意。
...
另一条路线是把查询编译为可执行形式。已有工作把查询逻辑编译成 Java bytecode 并交给 JVM [13]，但仍保留 iterator 接口，且运行时较重；HIQUE 用每算子模板生成 C [6]，虽然通过把结果物化内联到算子执行中消除了 iterator，但与我们的方案不同，算子边界仍然清晰可见，而且还要承担很高的 C 编译成本 [6]。其他局部优化包括在合取谓词中权衡分支数与实际求值谓词数 [14]，以及用 SIMD 加速表达式 [12, 15]。
...
我们为查询处理（相应也为查询编译）提出一种截然不同的架构。为了最大化查询处理性能，我们必须确保，我们也最大限度地提高数据局部性和代码局部性。为说明这一点，我们先对 pipeline breaker 给出比通常更严格的定义：若一个代数算子对某个输入侧把到达 tuple 移出 CPU 寄存器，它就是该侧的 pipeline breaker；若必须先物化该侧全部 tuple 才继续，则是 full pipeline breaker。该定义略显粗略，因为单个 tuple 可能已经大到无法放入可用 CPU 寄存器，但目前我们假定我们有足够多的寄存器容纳所有输入属性。我们将在第 4 节更详细地考察这一点。关键在于，我们把任何向内存 spill 数据的操作都视为打断 pipeline；查询处理期间，所有数据都应尽可能长时间地保留在 CPU 寄存器中。

现在的问题是：我们如何组织查询处理，使数据尽可能长时间地保留在 CPU 寄存器中？Volcano 模型的每个算子都把控制权交回调用者。跨越任意函数边界都会逐出寄存器内容；block model 虽减少调用，却产生超过寄存器容量的批次。以 iterator 暴露任意复杂算子的线性输出，本身就容易破坏 producer 的自然局部性；有些算子原本可能无需复制就能低成本地一次产生少量输出 tuple，线性 iterator 接口却无法利用这种能力。

#### 以数据为中心的执行思想

因此，我们反转数据流的控制方向。我们不再向上拉取 tuple，而是把它们推向 consumer 算子。在推送 tuple 时，我们持续推送，直到我们到达下一个 pipeline breaker。中间算子把属性留在寄存器中；复杂控制流通常移到 tight loop 之外，降低寄存器压力。由于 breaker 本来就必须物化，我们生成的执行计划实际上把内存访问压到不可避免的位置。
...
编译器内部采用 produce/consume 代码生成接口。每个算子支持两个方法：
...
到目前为止，我们只讨论了如何把代数表达式翻译成伪代码；但在实践中，我们希望把查询编译成机器码。我们最初尝试生成 C++、在运行时编译成共享库。这样易于直接访问我们的数据库系统的数据结构和代码（系统也用 C++ 编写），却有两项硬伤：优化 C++ 编译复杂查询要数秒，且 C++ 不能完全控制溢出 flag 等机器级细节。因此，我们改用 LLVM [7]：无限数量的 SSA 虚拟寄存器使我们可以假定，我们有一个 CPU 寄存器可用于我们的 tuple 中每个属性，并简化了属性映射；强类型 IR 暴露了我们原先的 C++ 文本生成中隐藏的错误。由此，我们实现了很低的查询编译时间：JIT 跨架构生成机器码通常只需数毫秒，而 C/C++ 编译器需要数秒（见第 6 节与文献 [6]）。

不过，并不适合用 LLVM assembler 实现完整的查询处理逻辑。第一，编写 assembler code 比使用 C++ 这类高级语言更繁琐；第二，数据库的许多逻辑（如 index structure）本来就用 C++ 编写。LLVM 与 C++ 很容易混合：C++ 方法可以直接从 LLVM 调用，反之亦然。对编译器而言，两类代码没有区别，因为二者最终都是 native machine code，而且都有 strongly typed prototype。
...
真实查询生成的汇编代码会迅速变得复杂，这使我们无法在此完整展示一个计划；但为说明机制，我们仍在图 7 中给出我们的贯穿示例中 $\Gamma _ {z;\mathrm{count}(\ast)}(\sigma _ {y=3}(R_2))$ 片段的简短 LLVM IR。右侧数字含义是：1 定位内存中的 tuple，2 遍历 tuple，3 过滤 $y=3$，4 计算 $z$ 的 hash，5 查 hash table，6 未找到时检查空间，7 空间不足时调用 C++ 分配或 spill。
...
按上述策略生成的 LLVM 代码本身已经很快：主要工作发生在遍历 tuple 的 tight loop 中，内存访问有良好的预取行为，分支也通常容易预测。代码很快之后，原先不显眼的细节会变成瓶颈。例如，在 TPC-H Q1——本质上只是扫描加基于哈希的聚合——中，我们初始计划的执行时间有一半以上耗在 hashing 上，虽然我们只需要对两个简单值做哈希。

分支布局同样关键。现代 CPU 只要分支几乎总是或几乎从不跳转，就能很好地预测；概率约为 50% 的分支则代价高昂。因此查询编译器必须生成有利于良好分支预测的代码布局。哈希表冲突链的直观写法把“检查当前 entry 是否存在”和“我们是否到达冲突链尾”混在同一个 `while` 条件里：
...
正如我们所预期的，第一个条件几乎总为真，因为哈希表通常已填充；第二个条件则几乎总为假，因为我们的冲突链很短。拆成下面的布局后，预测器更容易学习这两条路径：
...
当然，我们的代码使用 LLVM branch 而非 C++ loop，但控制流原则相同；在我们的实验中，仅这一改动就把 hash-table lookup 提升了 20% 以上。

如前所述，我们把所有 tuple 属性都保存在 LLVM 虚拟 CPU 寄存器中；对字符串，我们把长度和指针都保存在寄存器中。总体上，我们尽量晚地载入属性：在我们真正需要该属性时，或在我们反正能免费取得它时——因为我们必须访问相应内存。派生属性也遵循同样原则；但若 hash value 等派生值位于关键路径，就应比严格需要的时刻稍早计算，以隐藏计算延迟。分支布局同样必须服从可预测性。避免这些陷阱并不要求庞大的生成器：代码生成器上的一次投入会让此后的所有查询受益；在我们的实现中，覆盖 SQL-92 所需全部代数算子的代码生成约为 11,000 行，并不庞大。
...
在 OLAP 部分，我们把 TPC-CH 查询作为 prepared query 运行，并测量 warm execution time。VectorWise 的 Q2 触发系统 bug；DB X 为通用磁盘系统，即便这里数据在内存中，而且我们测量的是 warm execution time，它仍明显较慢。HyPer LLVM 对多数查询又快 2-4 倍。Q5 主要受 join 支配，因此 C++/LLVM 差异较小；Q1 只是 scan+aggregation，C++ 看似自然高效，仍无法像 LLVM 那样长期把值留在寄存器。编译时间本身不可接受，也是我们寻找 C++ 代码生成替代方案的部分原因。表中的编译时间包含从 SQL 文本到可执行代码的全部步骤：LLVM 为 16-41 ms，C++ 为 1.5-2.6 秒。因此，更换 backend 对 HyPer 显然很有收益：查询处理本身和编译时间两方面均从中受益。

### 6.2 代码质量

我们使用 Valgrind 3.6.0 的 Callgrind 工具运行了全部五个查询；Callgrind 通过 binary instrumentation 观测分支和 cache 行为。我们使用 callgrind control 将 profiling 限于查询处理本身。实验中，我们比较了 HyPer 的 LLVM 版本与 MonetDB。MonetDB 的 primitive 操作位于 tight loop 中，因而可预期具有较低的误预测率；比较旨在判断生成代码是否同样紧凑。
...
由于篇幅限制，我们在第 3 节只能高度概述算子翻译，这里给出更详细的讨论。我们在附录中集中讨论 Scan、Select、Project、Map 和 HashJoin；这些算子足以翻译很大一类查询。前四种算子较简单，适合说明基本的 produce/consume 交互，hash join 则复杂得多。
...
前四个算子较简单，因为大部分逻辑都由纯 LLVM 代码处理。Hash join 涉及 LLVM 与 C++ 之间的控制流往返，因此复杂得多。纯主存 hash join 可以完全用 LLVM 实现；但若需要在内存不足时 spill 到磁盘，就会调用很多与查询无关的方法，例如 I/O，我们的实现把这些部分写在 C++ 中。

我们先给出 C++ 代码，因为它定义了随后由 LLVM 片段补充的模板。C++ 代码把 build 侧读入主存，必要时 spill 到磁盘；若数据能放入主存，就直接与 probe 侧 join，否则也把 probe 侧写入分区，再逐分区 join。为简化讨论，我们在此只处理 inner join；非 inner join 还需额外记录哪些 tuple 已经匹配。
...
除主要实验外，我们还进行了一系列微基准，以更细致地研究不同查询处理方案的影响；同时实现了若干技术，并让它们在 HyPer 系统内运行。这样，所有方案都从完全相同的数据结构读取完全相同的数据，因而任何运行时差异都纯粹来自查询处理期间数据流和控制流的差异。除本文提出的以数据为中心的编译（data-centric compilation）外，我们还实现了经典 iterator model 的解释版本和编译为可执行代码的版本，以及大致对应 MonetDB/VectorWise 的 block-wise tuple processing [11]。我们这里只比较编译后的 block 模型，因为 MonetDB 与 VectorWise 都会预编译其 building block 以减少解释开销；实验数据采用 column-store 布局，所以访问列数越多，扫描的数据量也越大。

在第一项实验中，我们从一个运行在我们的 TPC-C 数据集上的简单聚合查询开始，并改变过滤条件的数量；所有过滤条件均不具选择性，即任意过滤条件组合的结果都相同：
...
由于我们关注数据传递开销，每个过滤条件都作为独立 selection 算子引入。图 8 中解释 iterator 因大量函数调用和差局部性最慢；编译 iterator 消除谓词虚调用后明显改善；block 模型在零过滤时因边界和 frame 设置略慢，从一个 selection 起才受益。以数据为中心的编译在零过滤时几乎为零，因为 LLVM 发现我们的 tuple-fragment loop 除递增计数器外不做任何工作，于是把它化简为 fragment size 的一次加法。三个过滤条件时性能看似略有回落，但实际上是两个过滤条件的结果因 cache effect 而偏快。三过滤查询以约 4.2 GB/s 的速率读取三个 tuple 属性，已接近我们所用机器的内存总线带宽，并且还要执行依赖这些数据读取的分支。我们或许能使用条件式 CPU 操作略微改善查询处理，但对通用 selection 代码而言，我们不能期待明显更快。

在第二项实验中，我们考察一个与我们的第一个查询“相反”的查询：前一查询过滤 tuple 而不执行计算；现在，我们取消 selection，改为根据属性执行计算：
...
为改变复杂度，我们分别以一个乘积项、前两个乘积项和全部三个乘积项运行该查询。图 9 中解释 iterator 仍最慢；编译 iterator 每 tuple 只剩一次虚调用，已大幅改善；block 更快但差距不大；我们的以数据为中心的方案始终最佳。三列时属性处理率约 6.5 GB/s，达到内存总线带宽。若不改变存储系统，我们不能期待更快；我们的查询处理已经快到基本成为以 RAM 访问为“I/O”的带宽受限状态。

## 附录 C. 优化设置

生成机器码时，优化设置会显著影响最终性能。因此，我们在此精确说明实验所用的优化设置。C++ 代码使用 `g++ 4.5.2` 生成机器码，并采用以下优化参数：
...
由于我们的 SQL compiler 已经生成相当合理的 LLVM 代码，我们主要依赖优化控制流的 pass；因此，与激进的 LLVM 优化级别相比，这里的优化时间相当低。
