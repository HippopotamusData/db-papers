...
联系邮箱：orestis@cs.columbia.edu、arun.raghavan@oracle.com、kar@cs.columbia.edu。

第一作者的部分工作在 Oracle Labs 完成。Kenneth A. Ross 获 NSF IIS-1422488 项目与 Oracle 赠款支持。

**出版信息：** SIGMOD ’15，2015 年 5 月 31 日至 6 月 4 日，澳大利亚维多利亚州墨尔本。DOI：[10.1145/2723372.2747645](https://doi.org/10.1145/2723372.2747645)。

**原文脚注 1：** `newsroom.intel.com/community/intel_newsroom/blog/2014/06/23/intel-re-architects-the-fundamental-building-block-for-high-performance-computing`

**原文脚注 2：** `software.intel.com/blogs/2013/avx-512-instructions`
...
实验把这些实现与相应的先进标量和向量技术比较，平台包括 Intel Xeon Phi 与最新主流 CPU。Xeon Phi 是当时唯一可获得的 MIC 硬件，也是唯一普遍可得、同时支持 gather 与 scatter 的硬件；最新 Haswell CPU 支持 gather。排序与连接实验还比较了 Xeon Phi 7120P（61 个 1.238 GHz P54C 核心，300 W TDP）与四颗高端 Sandy Bridge CPU（共 $4\times8$ 个 2.2 GHz 核心， $4\times130\ \mathrm{W}$ TDP）。二者性能相近，但 Phi 的能量消耗约为后者一半。
...
在算法 5 中，每轮让 $W$ 个 lane 各处理一个 probe key，并以 gather 访问哈希表。它没有先让固定的 $W$ 个键全部完成再加载下一组，而是在确认某个键不再有匹配时，立即 selective load 新键替换已完成 lane。因此，每个键的循环次数与标量实现相同；每次找到匹配，就用 selective store 写出相应 lane。偏移向量记录每个键已在表中前进多远；新键替换旧键时，其偏移清零。
...
构建同样以 selective load 动态复用已完成 lane，并用 gather 判断候选桶是否为空，再以 scatter 写入。多个 lane 可能选中同一个空桶，因此写入前必须检测冲突。我们先把 $[1,2,\ldots,W]$ 这样的 lane 唯一值 scatter 到候选位置，再用相同索引 gather 回来；若读回的标记等于本 lane 标记，它就可以安全写入。冲突 lane 下轮前进到下一桶。
...
上述双重哈希过程中， $m$ 标记已经探测过至少一个桶的 lane。若主哈希 $h _ 1$ 的值域为 $[0,|T|)$，碰撞哈希 $h _ 2$ 的值域为 $[1,|T|)$，且 $|T|$ 是素数，那么

$$
h = h_1 + N \cdot h_2 \pmod {|T|}
$$

在 $N\lt|T|$ 次碰撞内不会重复访问同一桶。为避免昂贵取模，只在 $h\ge|T|$ 时执行 $h-|T|$。
...
若多个 lane scatter 到同一计数器，最终仍只会增加 1。算法 11 因此复制直方图以隔离 lane：lane $j$ 更新 $H\verb0_0\mathrm{partial}[iW+j]$，最后把每个分区的 $W$ 个计数横向规约为一个结果。若复制后的直方图无法装入最快 cache，就使用 1 字节计数，并在溢出时 flush。
...
每轮把 lane 唯一值 scatter 到一个有 $P$ 个元素的临时数组，再用同一索引 gather 回来，冲突 lane 的序号加一并继续。即使最坏执行 $W$ 轮，访问的不同位置总数仍为 $W$；若第 $i$ 轮访问 $a _ i$ 个不同位置，则 $\sum a _ i=W$。
...
Hash partitioning 只需把数据拆成键域互不重叠的组，无须稳定。因此可以不做冲突串行化，只检测冲突 lane 并留到下一轮。若 $P\gt W$，每轮冲突很少，性能会略有提升。
...
| 核心数 × SMT | $61\times4$ | $4\times2$ | $(4\times8)\times2$ |
...
我们还配平两平台：把 SB 降频到 1.2 GHz，并把 Phi 的复制带宽减半至 40 GB/s；后者通过让代码访问两倍于实际处理量的字节实现。此时 Phi 的 radix sort 快 7%，hash join 慢 8%。同既有研究 [4, 14] 一致，hash join 胜过 sort-merge join：即使还要物化连接输出，连接 $2\times2\times10^8$ 个元组仍比单独排序 $4\times10^8$ 个元组快。
...
向量布尔操作产生 bitmask，在算法中写成标量变量。例如 $m\leftarrow x\verb0_0\mathrm{vec}\lt y\verb0_0\mathrm{vec}$ 得到掩码 $m$； $m\leftarrow\mathrm{true}$ 把 $m$ 的 $W$ 个 bit 全部置一。

向量作为数组下标时表示 gather 或 scatter。 $x\verb0_0\mathrm{vec}\leftarrow A[y]$ 是普通向量 load， $x\verb0_0\mathrm{vec}\leftarrow A[y\verb0_0\mathrm{vec}]$ 是 gather。Selective load/store/gather/scatter 在赋值上下标出 bitmask，例如 $x\verb0_0\mathrm{vec}\leftarrow[m]A[y]$ 为 selective load， $x\verb0_0\mathrm{vec}\leftarrow[m]A[y\verb0_0\mathrm{vec}]$ 为 selective gather。

$\mathrm{popcount}(m)$（原文写作 $|m|$）是掩码中置位 bit 的数量， $|T|$ 是数组 $T$ 的长度。 $x\verb0_0\mathrm{vec}\leftarrow m\thinspace?\thinspace y\verb0_0\mathrm{vec}:z\verb0_0\mathrm{vec}$ 表示 vector blend。向量表达式中的标量，要在主循环前预先广播成所有 lane 都等于该标量的向量；例如 $x\verb0_0\mathrm{vec}\leftarrow x\verb0_0\mathrm{vec}+k$ 与 $m\leftarrow x\verb0_0\mathrm{vec}\gt k$ 中的 $k$ 均如此。

## 附录 B：Gather 与 Scatter

最新主流 CPU 的 AVX 2 gather 以单条指令表达；当时主流 CPU 尚无 scatter。Xeon Phi 上，每次 gather/scatter 指令调用只访问一条 cache line，并重复调用直到 $W$ 个 lane 全部处理完。若索引涉及 $N$ 条不同 cache line，指令会发射 $N$ 次，且 $N\le W$。
...
下面代码假定 key 与 payload 都是 32 位，采用第 4 节的间接缓冲版本：把合格输入 rid 写入 cache-resident 缓冲，再 gather 数据。谓词为 $k\verb0_0\mathrm{lower}\le k\le k\verb0_0\mathrm{upper}$。缓冲必须足够小以常驻 L1；`_mm512_storenrngo_ps` 覆盖 cache line 而不先读回，因此输入输出需要 cache-line 对齐。
...
构建中的 scatter 冲突检测是向量化最常见的问题。未来 AVX 3 的 `_mm512_conflict_epi32` 等指令会比较所有 $i\lt j$ 的 lane 对，并为每个 lane 生成一个掩码，标出左侧相同值 lane；结果为零的 lane 无冲突，可省掉 gather/scatter 检测。
