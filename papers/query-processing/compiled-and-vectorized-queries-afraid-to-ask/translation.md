...
- Timo Kersten、Viktor Leis、Alfons Kemper、Thomas Neumann：Technische Universität München
- Andrew Pavlo：Carnegie Mellon University
- Peter Boncz：Centrum Wiskunde & Informatica

**联系邮箱：** `{kersten,leis,kemper,neumann}@in.tum.de`、`pavlo@cs.cmu.edu`、`boncz@cwi.nl`

**PVLDB 参考格式：** T. Kersten, V. Leis, A. Kemper, T. Neumann, A. Pavlo, P. Boncz. Everything You Always Wanted to Know About Compiled and Vectorized Queries But Were Afraid to Ask. PVLDB, 11 (13): 2209–2222, 2018. DOI: https://doi.org/10.14778/3275366.3275370

**出版信息：** Proceedings of the VLDB Endowment, Vol. 11, No. 13；ISSN 2150-8097；DOI: https://doi.org/10.14778/3275366.3275370

**许可：** 本文采用 Creative Commons Attribution-NonCommercial-NoDerivatives 4.0 International License（https://creativecommons.org/licenses/by-nc-nd/4.0/）。超出该许可范围的使用请联系 `info@vldb.org`。版权归版权所有者或本文作者所有，出版权许可给 VLDB Endowment。

## 摘要

现代数据库系统的查询引擎大多基于向量化执行（vectorization）或以数据为中心的代码生成（data-centric code generation）。这两种先进范式在系统结构和查询执行代码上有根本差异，也都能构建高速系统。但长期以来，人们并不清楚哪一种范式能带来更快的查询执行，因为具体实现选择会妨碍直接比较。我们在同一个测试系统中实现两种模型，使用相同的查询算法、数据结构和并行化框架，进行尽可能公平的比较。我们发现，两者都很高效，但强弱不同：向量化更善于隐藏 cache miss 延迟；以数据为中心的编译需要更少 CPU 指令，因此更有利于缓存驻留负载。除单线程原始性能外，我们还研究 SIMD、多核并行和不同硬件架构。最后，我们分析定性差异，为系统架构师提供选择依据。
...
两种模型都消除了传统引擎开销，但概念上不同：向量化采用拉取模型（从根到叶遍历）、一次处理一个向量并解释执行；以数据为中心的代码生成采用推送模型（从叶到根遍历）、一次处理一个元组并预先编译。正如我们在第 9 节所讨论的，已有其他设计混合或结合了两者的思想；本文中，我们聚焦这两个影响广泛、已被多个常用系统采用的具体模型。

这些差异会决定 DBMS 执行引擎源代码的组织方式和性能特征。更换模型意味着重写大量代码，设计者必须很早作出选择。从 Quickstep [33] 和 Peloton [26] 等近期 DBMS 发展中，我们看到两种选择都流行且合理；然而此前仍没有系统研究比较这两个先进模型。因此，我们进行深入实验，分析数据库架构师应在何种条件下偏好哪一种。

直接比较 HyPer 与 VectorWise 这类完整系统不能隔离执行模型：HyPer 的列存不使用亚字节压缩 [19]，VectorWise 使用更紧凑的压缩 [53]；与此相关，HyPer 在扫描中下推谓词，而 VectorWise 不这样做。两者的并行策略也不同：VectorWise 查询创建由操作系统调度的线程，并以优化时确定并行度的显式 exchange 算子控制并行 [3]；HyPer 则在每个核心运行一个线程，通过 NUMA 感知的无锁队列以 morsel-driven 方式显式调度任务。它们还使用不同的查询处理算法与数据结构、数据类型表示和查询优化器。这些选择会影响性能与可扩展性，却独立于查询执行模型。

为排除这些偶然差异，我们在同一测试系统中实现基于编译的关系引擎 Typer 和向量化引擎 Tectorwise（TW）[16]。两者采用相同算法与数据结构，唯一差别是向量化执行还是以数据为中心的编译执行。我们的实验结果表明，两者都非常高效，整体差距通常不大；基于编译的引擎在计算密集型查询中占优，向量化引擎则更善于在哈希连接等操作中隐藏 cache miss 延迟。

我们在第 2 节详细介绍两种模型，在第 3 节说明实验方法，并在第 4 节对内存 OLAP 工作负载做微架构分析；随后，我们在第 5 节研究数据并行操作（SIMD），在第 6 节讨论多核 CPU 上的查询内并行，并在第 7 节比较 Intel、AMD 和 Xeon Phi 平台。第 8 节中，我们在定量 OLAP 比较之后讨论 OLTP 与编译时间等因素；第 9 节讨论混合处理模型。最后，我们在第 10 节总结结果，为系统设计者提供指导。

## 2. 向量化查询与编译查询

向量化执行的核心，是在列式数据表示上进行批处理 [30]：每个操作数据的 primitive 不处理单个数据项，而处理表示多个元组的一批值。它尽量把解释决策的成本摊销到操作数据的方法内部，例如一次为数千个值计算哈希、比较数千对字符串、更新数千个聚合值，或从数千个地址取值。以数据为中心的编译则为 SQL 查询生成低层代码，把同一查询流水线中相邻的非阻塞算子融合成一个紧凑循环。为理解两者生成代码的性质，理解每种变体的代码结构很重要。因此，本节中我们给出算子实现示例，说明为什么要采用这些实现方式，并讨论它们的一些性质。

本文实验中的 Typer 把以数据为中心的代码生成到 C++；HyPer 则生成 LLVM IR。我们在本文中不计编译时间，因此这一差别只影响被排除在测量之外的编译阶段，不影响本文比较的执行时间。

### 2.1 向量化算法

Typer 通过运行生成代码执行查询，因此开发者可以按需要组织算子实现。图 1a 的函数扫描所有行，在每次迭代中同时判断颜色是否为绿色、轮胎数是否为 4。Tectorwise 尽可能采用同一算法，但任何向量化函数都受两个约束：它只能处理一种数据类型，并且必须一次处理多个元组。理论上也能创建同时处理多种类型的 primitive，但类型组合数会呈指数增长，实践中不可行。生成代码可以把两个判断写在同一个 `if` 中，但这违反单一数据类型约束，因此 Tectorwise 必须像图 1b 那样拆成两个函数。解释控制逻辑先调用第一个函数按颜色选择元素，再调用第二个函数按轮胎数继续筛选；批量处理又满足了第二项约束。

所有 Tectorwise 算子都面对这一约束，因而被拆成同时满足两项条件的 primitives。图 1 使用列式存储，但行式格式同样可行。为了最大化吞吐，数据库开发者会用 predicated evaluation（如 `*res=i; res+=cond`）或 SIMD 指令等方法高度优化这些函数。带着这些约束，我们进一步考察 Tectorwise 算子的实现细节。我们按图中方式实现选择，表达式也在算术运算符处分解成 primitives。即使是这些简单算子，Tectorwise 也必须改变算法结构并偏离 Typer 的数据访问模式；中间结果物化使快速 cache 对向量化引擎尤为重要。
...
图 2 给出两种哈希连接实现的部分伪代码。Typer 与 Tectorwise 都先消费一个输入的全部元组，并以行式格式放入哈希表以改善 cache 局部性；随后，我们用另一输入探测哈希表，并把找到的全部匹配组合交给父算子。Typer 生成的对应代码见图 2a。
...
我们对分组算子采用相似方法。聚合的两个阶段都使用保存分组键和聚合值的哈希表。我们首先像哈希连接一样，在表中为每个输入元组查找所属分组；若没有找到，就必须新增分组，但不能简单地为每个未找到分组的元组各插入一项，否则同一分组可能被重复添加。因此，我们先把所有未找到分组的元组按相同键重排成分区；复合键则逐组成部分处理。每个分区只向哈希表加入一个分组；待所有输入元组的分组都确定后，我们再运行聚合 primitives。转换成向量化形式后，分组算子的数据访问模式比 Typer 偏离得更大；对连接算子，这种转换带来更多相互独立的数据访问（见 4.1 节），而聚合则会产生额外工作。

实现 Tectorwise 算子时，我们必须偏离 Typer 的实现。这些差异不是任意实现选择，而是向量化的单类型、批处理约束所致。虽然每个算子因此有两份不同实现，但两者在核心上执行相同算法，并采用相同并行策略。
...
我们在同一个开源测试系统 [16] 中实现编译引擎 Typer 和向量化引擎 Tectorwise（TW）。两者使用相同算法、数据结构和物理计划；计时不包含 SQL 解析、优化、代码生成和编译，从而把唯一变量限制为“向量化执行”或“以数据为中心的编译执行”。

### 3.1 相关工作

Boncz 等人在 2005 年提出向量化 [7]，最先用于 MonetDB/X100；该系统后来演化为商业 OLAP 系统 VectorWise，随后 DB2 BLU [40]、SQL Server 列存 [21] 和 Quickstep [33] 等系统也采用了这一模型。2011 年，Neumann 提出用 LLVM 编译框架实现以数据为中心的代码生成，并把它作为内存混合 OLAP/OLTP 系统 HyPer 的查询处理模型 [28]；Peloton [26] 和 Spark [2] 也采用了这一模型。

据我们所知，本文是首次系统比较向量化与以数据为中心的编译。Sompolski 等人用若干微基准比较两种模型 [45]，但没有评估完整查询的端到端性能。OLTP 系统已有更详细的实验研究：Appuswamy 等人在同一原型中评估不同 OLTP 系统架构 [4]，Sirin 等人则对现有商业和开源 OLTP 系统作了细致的微架构分析 [43]；这些研究不能直接回答 OLAP 执行范式问题。

### 3.2 查询处理算法

我们在 Tectorwise 和 Typer 中都实现了五种关系算子：scan、select、project/map、join 和 group by。Scan 本质是可并行循环；selection 生成 `if`；projection 把表达式翻成对应 C 代码。与生产级系统不同，我们的实现刻意不做算术溢出检查。Join 使用同一张链式冲突哈希表，并把指针中未用的 16 位编码成类似 Bloom filter 的结构 [22]，使选择性 join 的探测未命中通常无需遍历链。Group by 分两阶段：预聚合处理 heavy hitter，并把 group spill 到分区；随后逐分区完成最终聚合。Typer 可直接把这些算法嵌入生成代码，Tectorwise 则必须按单类型、批量处理约束作出我们在第 2.1 节所述的调整。

### 3.3 工作负载

本文中，我们聚焦 OLAP 性能，因此多数实验使用著名的 TPC-H 基准。为了逐查询展示详细计数器而不只是汇总统计，我们选取五条能覆盖主要瓶颈的查询：Q1 是定点算术与 4 组聚合；Q6 是高选择性过滤；Q3 是 build 14.7 万、probe 320 万项的 join；Q9 是 build 32 万、probe 150 万项的 join；Q18 是 150 万组的高基数聚合（均为 SF=1，随规模线性增长）。剩余 17 条查询大多由 join 主导，因而类似 Q3/Q9；少数主要耗时在高基数聚合上，因而类似 Q18。最后，Q1 和 Q6 虽然是仅有的两条单表查询，行为却很不相同，所以我们同时展示两者的结果。五条查询合在一起覆盖了 TPC-H 最重要的性能挑战；能高效执行它们的执行引擎，很可能也能高效执行完整 TPC-H 套件 [6]。

### 3.4 实验设置

除非另有说明，我们的实验使用配备 10 核 Intel i9-7900X（Skylake X）的系统；该 CPU 的详细规格见硬件一节的表 4。我们使用 Linux，并以 GCC 7.2 编译代码；硬件计数器通过 Linux `perf events` 采集。全文中，我们把所有计数器除以该查询扫描的元组总数，使不同查询、规模和引擎间可直接比较，例如“Tectorwise 在 Q1 上每元组比 Typer 多执行 41 条指令”，也能比较数据规模增加 10 倍后每元组增加多少次 cache miss。

两套引擎的 join 都使用单表链式哈希连接。尽管近期研究主张分区哈希连接 [5,41]，单表连接在生产系统中仍很常见，HyPer 与 VectorWise 也都采用它。

实验随后扩展到 AMD Threadripper、Intel Knights Landing、多核、SSD 和不同 cache/内存带宽配置，以区分缓存驻留、带宽受限和高并行场景。

## 4. 微架构分析

为理解这两种查询处理范式，我们进行深入的微架构比较。我们首先聚焦顺序执行性能，把数据并行（SIMD）留到第 5 节讨论，把多核并行留到第 6 节讨论。

### 4.1 单线程性能

图 3 的 SF=1 结果从 Typer 在 Q1 快 74%，到 Tectorwise 在 Q9 快 32%。在考察原因之前，我们指出，这些差异并不大，尤其远小于现代引擎与 PostgreSQL 之间一到两个数量级的差距 [17]。这说明两种范式都可行，且不存在单方支配。因此，下文中我们分析这些性能差异，以理解两种模型各自的优势和弱点。
...
Tectorwise 把操作拆成 primitive，并在其间物化向量，因此最多执行 2.4 倍指令、产生 3.3 倍 L1 data miss；Typer 常把中间值留在寄存器，故 Q1 这类计算密集、cache-resident 聚合明显占优。我们还观察到，Q3/Q9 使用同一哈希表布局、LLC miss 近似相同，Tectorwise 却分别快 4% 和 32%，而且数据与哈希表增大后，join 优势最高可达 40%：它的 probe loop 很简单，乱序引擎能提前发出更多独立 load；Typer 的融合循环同时包含 scan、selection、probe 和 aggregation，更快填满乱序窗口，分支误预测也会丢弃更多推测工作。

哈希函数也与代码形态交互。尝试不同哈希函数后，我们为 Tectorwise 选择指令数约为 CRC 两倍、但吞吐更高的 Murmur2；为 Typer 选择把两个 32 位 CRC 合并为 64 位的低延迟哈希。后者在大规模上可快 40%，因为较短依赖链让连续迭代更易并行发起内存访问。两者使用不同哈希函数仍是公平的 join 性能比较，因为每套系统都使用了对自身更有利的函数。需要谨慎的是，IPC 不能单独代表性能：Q1 中 Tectorwise 的 IPC 高 40%，却因总指令近两倍而慢 74%。

### 4.2 解释和指令缓存

向量化仍采用 Volcano 式 pull 和解释，但一次调用处理约 1,000 个值，primitive 又按数据类型预专用。通过 profiler，我们确定 SF=10 的五条查询中，解释控制逻辑不足运行时间的 1.5%；换言之，我们的查询集合有 98.5% 的时间在 primitives 中完成查询处理工作。我们从表 1 看到，额外指令主要不是虚调用或类型分派，而是 primitive 之间物化结果的 load/store；由于 primitives 在编译时已知全部相关类型，我们得出结论，这些额外指令不是解释决策或虚函数调用所需的解释代码。既有研究曾指出 OLTP 负载中的 instruction cache miss 问题 [43]；为判断我们的两套查询引擎是否也有这一问题，我们测量了两者的 L1 instruction cache miss，发现两套引擎的热点代码都能放入 32 KB L1 instruction cache，本文 OLAP 实验中的 I-cache miss 可忽略。

### 4.3 向量大小

向量大小是所有向量化引擎的重要参数。到目前为止，我们的 Tectorwise 实验一直使用 1,000 个元组，这也是 VectorWise 的默认值。图 5 把运行时间归一化到 1K 向量，考察从大小 1 到最大值（即完全物化）的范围。我们观察到，很小（小于 64）和很大（大于 64K）的向量都会显著降低性能。

![图 5：Tectorwise vector size 对运行时间的影响](assets/precise-manual-figure-05-vector-sizes.png)

图 5：Tectorwise 的向量大小实验；各查询时间都归一化到向量大小为 1K 时的运行时间。

向量大小为 1 时，Tectorwise 退化为 Volcano 风格解释器，CPU 开销很大。大型向量放不进 CPU cache，因此会产生 cache miss；另一端的极限就是 MonetDB 使用的一次处理一整列 [9]。总体而言，我们发现 1,000 左右对所有查询都很合适；唯一明显例外是 Q3，使用 64K 向量时还能再快约 15%。

### 4.4 Star Schema Benchmark

到目前为止，我们研究的是精心选取的 TPC-H 子集。为了表明我们的发现具有更广泛的适用性，我们还在单线程 SSB SF=30 上实现并验证了四组查询模板。结果与 TPC-H Q3/Q9 一致：Tectorwise 指令更多，但 join-heavy 查询的 memory stall 更少，例如 Q4.1 每元组 stall 从 Typer 的 45.91 cycles 降至 19.48。总体而言，我们发现，就本文目的而言，TPC-H 已涵盖 SSB；为保持简洁，后文使用 TPC-H 展示我们的发现。
...
### 4.5 Tectorwise/Typer 与 VectorWise/HyPer 对比

最后，我们用生产系统 HyPer/VectorWise 与本文原型 Typer/Tectorwise 的单线程 TPC-H SF=1 时间结束本节比较：
...
表 2：生产系统与本文原型在 TPC-H SF=1、单线程下的运行时间。

这里比较的是 Actian Vector 5.0（VectorWise 当时的产品名）和 HyPer 的 TUM 研究版本；后者有一个相关系统后来归属 Tableau。HyPer 与 Typer、VectorWise 与 Tectorwise 的量级和相对趋势接近；除 Q6 外，我们的轻量原型略快于生产系统，因为后者还要处理我们的原型所忽略的溢出检查等完整语义。HyPer 在 Q6 上更快，是因为它直接在压缩列上用 SIMD 指令计算选择 [19]。这一对照支持原型对两类架构的代表性。

## 5. 数据并行执行（SIMD）

现在，我们把注意力转向使用 SIMD 操作的数据并行执行。数据库操作的 SIMD 加速已有大量研究 [51,50,36,37,38,35,46,44]，这些工作通常采用向量化执行模型：向量引擎的 primitive 是简单紧凑的循环，容易转成数据并行代码。也有研究把 SIMD 用于以数据为中心的代码 [34,26]，但生成代码更复杂。因此，我们以 Tectorwise 为平台，考察 SIMD 对内存 OLAP 工作负载的实际影响；与多数 SIMD 研究不同，我们使用 TPC-H 查询而非微基准。

本文使用的 Skylake X CPU 支持 AVX-512，每周期可执行两条 512-bit SIMD 操作，相比上一代微架构把寄存器宽度和吞吐翻倍：AVX-512 每周期可处理 32 个 32-bit 值，标量代码则只能处理 4 个。与 AVX2 相比，AVX-512 还提供更强且更正交的指令集：几乎全部操作支持 masking，新增 compress/expand，并为 8、16、32、64-bit 类型提供大多数操作。因此，人们会期待 SIMD 带来显著收益。下文中，我们重点研究选择与哈希表探测这两类常见且重要的操作。

### 5.1 数据并行选择

向量化选择 primitive 产生一个 selection vector，其中保存全部匹配元组的下标。AVX-512 比较指令生成 mask，我们再把该 mask 交给跨 SIMD lane 的 `COMPRESSSTORE`，将被选中的位置写入内存。AVX2 下的同一 primitive 并不简单，需要查找表 [35,19,26]，或基于 BMI2 [1] 的复杂置换逻辑。我们用一个 8,192 元素整数数组做微基准，比较无分支的标量 x86 实现与 SIMD 实现；微基准从该数组中选出所有小于某一常量的元素。最佳情形中，32-bit 输入连续且全部位于 L1 cache，图 6a 的加速为 8.4 倍。

![图 6：Tectorwise 中 scalar 与 SIMD selection 的对比](assets/precise-manual-figure-06-simd-selection.png)

图 6：Tectorwise 中标量与 SIMD selection 的对比。（a）连续输入，选择率 40%；（b）二级选择，输入 selection vector 和当前选择的选择率均为 40%；（c）TPC-H Q6、SF=1 的运行时间。
...
只有第一个选择 primitive 接收连续输入；从第二个开始，所有 primitive 都必须依据 selection vector 从不连续内存位置 gather 元素。Q6 包含一个无输入 selection vector 的初始选择，以及四个必须处理 selection vector 的后续选择，因此我们预期整体加速本就应更接近 3 倍而非 8 倍。图 7 又在 4 GB 数据集、输出选择率 40% 的条件下展示输入稀疏度的影响，并估计平均有多少 cycles 用于解决 cache miss：选择率低于 50% 时标量与 SIMD 几乎一样快，我们观察到平均时间大多用于解决 cache miss。Q6 的第一个选择留下 43% 的元组，后续选择因此都落在标量与 SIMD 速度相近的范围。
...
图 8a 中单独计算哈希可加速 2.3 倍；图 8b 的 gather 最佳也只有 1.1 倍，因为测试机内存系统每周期最多执行两次 load，无论使用 SIMD gather 还是标量 load；将 gather 等 SIMD 指令用于 Tectorwise probe primitive，图 8c 的最佳收益为 1.4 倍。但在完整 TPC-H join 中收益几乎消失，即使 Q3、Q9 分别有 55% 和 65% 的时间位于 SIMD 优化 primitive。图 9 解释了原因：working set 增长后，内存延迟主导成本，只有数据全部位于 cache 时 SIMD 才有明显收益。我们并不是最先观察到这一现象的人；Polychroniou 等人研究 SIMD 数据库算子时也有同样发现 [35]。
...
然而，自动 SIMD 优化没有带来显著运行时间改善；单独自动向量化几乎没有收益，甚至让部分代码变慢。这意味着，尽管 primitive 可以自动向量化，但这还不是可以一劳永逸的方案。

### 5.4 小结

我们发现，使用 AVX-512 常能直接把标量代码改写为数据并行代码，微基准最高可达 8.4 倍；但在更复杂的 TPC-H 查询中，收益很小，连接查询约为 10%。根本原因是大多数 OLAP 查询受数据访问而非计算限制，后者本来才是 SIMD 的强项。回到以数据为中心的编译与向量化比较，我们因此认为 SIMD 不会明显把天平转向向量化；需要指出，压缩数据 [19] 或 Knights Landing 等面向向量的 CPU 上收益可能更大。

## 6. 查询内并行

单线程性能长期停滞而 CPU 核数持续增长——Intel 当时已在单颗 Xeon 上提供 28 个核心、56 个超线程——现代查询引擎必须有效使用全部核心。下文中，我们讨论如何把查询内并行纳入两种执行模型。

### 6.1 Exchange 与 morsel-driven 并行

VectorWise 原本使用 exchange operator [3]；这一经典方法 [13] 让 join、aggregation 等查询算子基本无需感知并行。HyPer 使用 morsel-driven 并行，通过共享哈希表使 join 和 aggregation 显式感知并行，从而获得更好的局部性、负载均衡和扩展性 [22]；在 10 核 CPU 的 20 个硬件线程上，我们测得五条 TPC-H 查询在 HyPer 中平均加速 11.7 倍，而 VectorWise 为 7.2 倍。但并行框架与执行范式正交，因此我们为两套原型统一实现扩展性更好的 morsel-driven 并行 [22]。

Morsel-driven 并行最初为 HyPer 开发 [22]，所以 Typer 只需把 scan loop 换成并行循环，并同步共享 hash table。需要指出的是，在 Tectorwise 中如何使用 morsel-driven 并行并不那么直观。我们为每个 worker 建立一棵算子树和独占资源，每个算子另有共享状态用于结果交换和工作分配；pipeline breaker 使用 barrier，例如 hash join 必须等所有 worker 完成 build 后才能进入 probe。这样两者获得与 HyPer 实现相同的负载均衡行为 [22,23]。

### 6.2 多线程执行

我们在 TPC-H SF=100（约 100 GB 数据）上执行工作负载。表 3 给出 Skylake 上的多线程执行时间，以及相对于单线程执行的加速比：
...
表 3：Skylake 上 TPC-H SF=100 的多线程执行结果（10 个核心、20 个超线程）。

使用 Skylake 的 10 个物理核时，我们看到两套系统在 Q1/Q3/Q9 上都达到 8-9 倍加速；考虑现代 CPU 在多线程运行时会显著降频，这已接近理想扩展。Q6 受读带宽限制，Q18 接近写带宽。我们也在 AWS EC2 机器上执行了这些实验，并发现两套系统扩展性同样良好。不过，我们观察到，用更大实例加速查询会提高每条查询的价格：在 8 vCPU 的 `m5.2xlarge` 实例上，TPC-H 查询几何平均运行时间约为 2027 ms，每条查询 0.0002 美元；在 48 核实例上约为 534 ms，每条查询 0.00034 美元。也就是说，在这一案例中，查询运行快 4 倍，成本却高 1.7 倍。

Tectorwise 和 Typer 的扩展行为相近。不过，表 3 的 Ratio 列（两套系统运行时间之商）揭示了一个有趣现象：启用 20 个超线程后，除内存带宽受限的 Q6 外，两者差距普遍缩小；Q3/Q9 的 Tectorwise 优势约减半，Q1/Q18 中 Tectorwise 也更接近 Typer。这表明硬件多线程能隐藏一部分微架构上不理想的代码。Q6 是唯一例外：Typer 的无分支 selection 实现会消耗更多内存带宽，在高线程数下性能低 20%。

### 6.3 超出内存的实验

为以最高速度比较 Tectorwise 和 Typer，此前所有测量都在内存中执行。但大型 OLAP 数据库常会超出主存容量，因此我们也测量了从二级存储取数的影响。我们把表数据存入由三块 SATA SSD 组成、读带宽 1.4 GB/s 的 RAID 5，而非带宽 55 GB/s 的主存。SF=100、20 线程结果见表 5。与内存结果相比，我们可以看到两者比值更接近 1，但 join/aggregation 的差异仍可见；scan 主导的 Q1/Q6 受 I/O 影响最大。总体而言，我们发现，内存分析的结论仍适用于采用现代 I/O 设备的超内存场景。
...
表 5：从 SSD 读取数据时，TPC-H SF=100、20 线程的运行时间。

## 7. 硬件

此前实验中，我们只测量了 Intel 当时最新的 Skylake 微架构。为了判断我们的结果能否推广到其他硬件平台，现在我们还考察 AMD 当时的新一代 Zen 微架构和 Intel 的 Phi 产品线。

### 7.1 Intel Skylake X 与 AMD Threadripper

表 4 给出我们的 Intel 与 AMD CPU 规格。两台约 1,000 美元的平台内存带宽接近；Threadripper 有 16 核但每核频率较低、SIMD 吞吐仅 Skylake 的四分之一，Skylake 为 10 个更强的核。我们的实验表明，两种 CPU 的绝对查询处理性能总体相当。图 11 给出 SF=100 上两套系统在 Skylake 与 Threadripper 上的每秒查询数；由于两颗处理器核心数不同，横轴按已使用的可用核心百分比归一化。Q1 差异低于 20%，Q3 低于 25%，Q9 低于 40%，Q6/Q18 几乎相同；Typer/Tectorwise 的相对关系也保持一致。具体而言，Tectorwise 在 join 查询 Q3/Q9 和选择性扫描 Q6 上更快，Typer 在计算密集型 Q1 上占优。总体而言，两平台的性能特征非常相似，两种执行引擎的相对性能在两颗 CPU 上也几乎相同。主要差别是 Intel hyper-threading 对所有查询有益，AMD SMT 收益小，部分查询甚至退化。
...
| LLC | 14 MB | 32 MB | （16 GB 高带宽内存，可配置为 L3 cache） |
...
Knights Landing 有 64-72 个较慢核心，每核两组 512-bit 向量单元。对数据库系统而言，它还具有吸引力：主存可通过 6 个 DDR4 通道直接访问，不像 GPU 那样必须经 PCIe 复制数据；每个核心有 64 KB L1 cache，并与相邻核心共享 1 MB L2 cache；此外还有约 300 GB/s 的 16 GB 高带宽内存。可用内存、核心数和丰富 SIMD 资源使 Knights Landing 看起来很像理想的 OLAP 机器。自然地，我们希望探索这台机器的特性，并考察 Tectorwise 和 Typer 在这一场景中的表现。

在 Knights Landing 实验中，我们把高带宽内存设为硬件管理的 L3 cache，并在 Quadrant 模式下把所有 CPU 暴露为一个 NUMA 节点。图 12 将它的查询处理性能与 Skylake 比较。代码不作任何修改时，我们观察到 Q3/Q9 可比 Skylake 快 0-25%，两种 CPU 上 Tectorwise/Typer 的相对性能相近；Q18 慢约 20%，Q1 慢约 30%，Q6 因高带宽最多快 2 倍。由于我们重复执行每项测量，Q6 的整个工作集可放入 16 GB cache，这一结果偏乐观；实际负载中 cache 会与其他查询共享，并很可能逐出 Q6 的大量数据。作为参照，我们还在硬件不配置 L3 cache 时测量 Q6，此时 Knights Landing 相对 Skylake 的优势只有 10%。混合负载下的实际差距应落在这两个测量结果之间。到这一点为止，有些查询略快、有些略慢，Knights Landing 的总体表现并不突出。
...
到目前为止，我们聚焦 OLAP 工作负载，只发现两种模型之间较小且依查询而异的性能差异，尤其是在正确并行化之后。这些差异不足以让我们普遍建议采用向量化或编译。因此，从实践角度看，本节中我们讨论的 OLTP 性能、实现工作量等其他因素可能更重要。

### 8.1 OLTP 与多语言支持

向量化只有在一次处理许多值时才能摊薄开销，OLTP 查询常只访问一个 tuple，因此几乎没有收益。编译则能把整个存储过程的多条语句生成一个机器码片段，这对 OLTP/HTAP 是实质优势；SQL Server 即使已有向量化 Apollo [21,20]，仍加入编译式 Hekaton [12]。统一编译环境也便于融合 UDF 和多种语言 [11,32,31,42]。

### 8.2 编译时间

编译时间可能支配短 OLTP 查询 [48]，也可能因大 OLAP pipeline 而增长；LLVM 的优化编译时间往往随代码大小超线性增长。即使是 `SELECT * FROM T` 这样很小的 SQL，只要表 `T` 有数千列，也会因每列都生成代码而产生大量代码。存储过程可提前编译；HyPer 禁用部分不扩展的 LLVM pass、使用自有且扩展性更好的寄存器分配器，并先用 LLVM IR 解释器执行若干 morsel，若这些数据已足以回答查询，就省去完整 LLVM 编译 [17]；Spark 在单 pipeline 超过 8 KB Java bytecode 时回退到逐元组解释。这些缓解手段基本消除了编译时间的劣势，但增加了系统复杂度。
...
运行时重排合取谓词乃至 join 能抵消不可避免的基数估计误差。把适应性集成进编译执行非常困难；解释式系统可以在运行时改变解释方式，因而更适合自适应执行。向量化执行仍属于解释执行，结合细粒度 profiling 后，VectorWise 可以作出多种 micro-adaptive 决策 [39]。我们看到 VectorWise 在 TPC-H Q1 上快于 Tectorwise；这来自一个与 [15] 类似、但 Tectorwise 未实现的自适应聚合：按 group key 把当前向量拆成 selection vector；若当前向量的分组很少，分区成功，遍历每个 selection vector 就能把哈希聚合转成有序聚合，在寄存器中计算局部聚合值，并且每个向量只更新一次哈希表中的聚合值。若分组太多导致分区失败，系统会指数退避，减少后续向量尝试该优化的频率。

### 8.5 实现问题

向量化的难点是把算子拆成控制逻辑和 primitive，并保证绝大多数时间留在 primitive；编译引擎则是“生成代码的代码”，尤其直接生成 LLVM IR 时难理解、难调试，通常需要简化代码生成的抽象层 [42] 和面向多个后端的可移植接口 [32,31]。HyPer 的部分抽象已在文献 [29] 中介绍，另一些尚未发表。复合 `ORDER BY` 或 window sort key 对向量化尤其麻烦：primitive 只能按单类型专用，比较必须拆分并以布尔向量连接，产生额外物化；编译器可直接生成针对完整 record 与复合 key 专用的 sort。
...
两类技术大多正交，图 13 因而呈现连续设计空间。许多系统结合两种范式的思想，力求兼得两者优点。我们先说明向量化如何改善基于编译的 HyPer 和 Peloton，再讨论编译如何帮助向量化。HyPer 的压缩 Data Block scan 使用模板化 C++ 向量化而不在运行时生成代码：每个约 $2^{16}$ 值的 attribute chunk 可独立选择压缩格式，若为属性与格式的全部组合生成融合代码会指数膨胀；向量化还使 decompression/selection 更易用 SIMD。其他算子仍用以数据为中心的编译，因此实际 HyPer 已是混合系统。

Peloton 的 relaxed operator fusion 在 hash join 等位置有选择地打断 pipeline、批量物化，以提高乱序执行和预取能力，并便于加入 SIMD 与软件预取指令 [26]；如果优化器能正确决定是否打断 pipeline——这本身并不简单 [24]——它可能胜过两种基础模型。

反方向上，Sompolski 等观察到，把两个或更多相邻的 VectorWise primitive loop 用 JIT 编译融合成一个循环，有时可以省掉物化步骤 [45]；据我们所知，当时还没有任何系统集成这一想法。面向 UDF 的数据管理系统 Tupleware 则明确采用以数据为中心和向量化执行的混合方式，用代价模型与 UDF 分析选择最适合任务的执行方法 [11]。

Apache Impala 也采用一种混合式编译执行 [49]。它不融合关系算子，而让算子通过表示 tuple 批次的 vector 交互。算子是 C++ 模板，其中 tuple-specific 的数据移动、record 访问和比较表达式等函数作为参数；系统先提供较慢的通用 ADT 实现，编译时再把这些函数调用替换成生成的 LLVM IR。其优点是易于集成单元测试、调试和 profiling，缺点是缺少跨算子 pipeline 融合，代码效率较低。
...
向量化和以数据为中心的编译，是多数现代系统采用的两种先进查询处理范式，已经在很大程度上取代了本质上属于解释器的传统 pull-based iterator 模型。不过，还有其他或常见或少见的模型。在讨论这些替代方法的优势和弱点之前，我们先在表 6 中对它们作分类。我们按两个维度划分查询处理范式：（1）是否以及如何实现 pipelining；（2）如何执行。Pipelining 可以采用 pull（`next`）接口、push（`produce`/`consume`）接口，或完全不采用（即每个算子之后都完整物化）。与此正交，第二个分类维度是解释、向量化或编译执行。这样总共有 9 种组合，表 6 显示其中 8 种已经被使用或提出。

System R 以后，多数引擎用 pull 避免中间物化；push 后来常与编译结合，但也用于解释或向量化。Push 可让一个算子向多个 consumer 输出 DAG 计划 [27]，也适合分布式 exchange，这也是 Virtuoso 采用 push 的原因之一 [8]；缺点是控制流稍欠灵活：例如，merge-sort 必须完整物化一个输入关系。包括 MonetDB [10] 在内的一些系统完全物化各算子结果，简化实现却增加主存带宽。

编译也能结合三种 pipeline 策略，push 最常见且通常生成更高效代码 [47]。Hekaton 使用 pull 编译 [12]，其优点是算子多次产生结果时不会把 consumer 内联多份；push 编译的 full outer join 等算子会从源代码的两个不同位置产生结果，必须把 consumer 移入单独函数并调用两次，避免把 consumer 代码内联两份而导致指数代码增长。


## 10. 总结

令我们意外的是，在 OLAP 工作负载中，向量化执行与以数据为中心的编译查询执行性能非常接近。下面，我们总结若干主要结论：

- **计算。** 以数据为中心的编译代码更适合计算密集型查询，因为它能把数据保留在寄存器中，从而执行更少的指令。
- **并行数据访问。** 向量化执行更善于并行产生 cache miss，因此在访问大型聚合或连接哈希表的内存受限查询中略占优势。
- **SIMD。** 近年来，SIMD 是硬件架构师提高 CPU 性能的重要机制；理论上向量化执行更容易从中获益，但实践中我们发现收益较小，因为多数操作由内存访问成本主导。
- **并行化。** 我们发现，采用 morsel-driven 并行后，向量化引擎和编译引擎都能在多核 CPU 上良好扩展。
- **硬件平台。** 我们在 Intel Skylake、Intel Knights Landing 和 AMD Ryzen 上完成了全部实验；上述效应在所有机器上都出现，任何硬件平台上都没有一种模型全面支配另一种。
...
- [8] P. A. Boncz, O. Erling, and M. Pham. Advances in large-scale RDF data management. In Linked Open Data - Creating Knowledge Out of Interlinked Data - Results of the
  LOD2 Project, pages 21–44. Springer, 2014.
