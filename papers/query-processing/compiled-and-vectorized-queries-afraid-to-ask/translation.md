...
现在，我们把注意力转向使用 SIMD 操作的数据并行执行。数据库操作的 SIMD 加速已有大量研究 [51,50,36,37,38,35,46,44]，这些工作通常采用向量化执行模型：向量引擎的 primitive 是简单紧凑的循环，容易转成数据并行代码。也有研究把 SIMD 用于以数据为中心的代码 [34,26]，但生成代码更复杂。因此，我们以 Tectorwise 为平台，考察 SIMD 对内存 OLAP 工作负载的实际影响；与多数 SIMD 研究不同，我们使用完整 TPC-H 而非微基准作为最终依据。
...
System R 以后，多数引擎用 pull 避免中间物化；push 后来常与编译结合，但也用于解释或向量化。Push 可让一个算子向多个 consumer 输出 DAG 计划 [27]，也适合分布式 exchange，这也是 Virtuoso 采用 push 的原因之一 [8]；缺点是 merge-sort 等控制流较不灵活。包括 MonetDB [10] 在内的一些系统完全物化各算子结果，简化实现却增加主存带宽。
