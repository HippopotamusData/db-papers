...
本文实验中的 Typer 把以数据为中心的代码生成到 C++；HyPer 则生成 LLVM IR。论文不计编译时间，因此这一差别只影响被排除在测量之外的编译阶段，不影响本文比较的执行时间。
...
分组算子采用相似方法。聚合的两个阶段都使用保存分组键和聚合值的哈希表。每个输入元组首先像哈希连接一样在表中查找所属分组；若没有找到，就必须新增分组，但不能简单地为每个未找到分组的元组各插入一项，否则同一分组可能被重复添加。因此，系统先把所有未找到分组的元组按相同键重排成分区；复合键则逐组成部分处理。每个分区只向哈希表加入一个分组，待所有输入元组的分组都确定后，再运行聚合 primitives。转换成向量化形式后，分组算子的数据访问模式比 Typer 偏离得更大；对连接算子，这种转换带来更多相互独立的数据访问（见 4.1 节），而聚合则会产生额外工作。
...
两套引擎都实现 scan、select、project/map、join 和 group by。Scan 本质是可并行循环；selection 生成 `if`；projection 把表达式翻成对应 C 代码。原型刻意不做算术溢出检查。Join 使用同一张链式冲突哈希表，并把指针中未用的 16 位编码成类似 Bloom filter 的结构 [22]，使选择性 join 的探测未命中通常无需遍历链。Group by 分两阶段：预聚合处理 heavy hitter，并把 group spill 到分区；随后逐分区完成最终聚合。Typer 可直接把这些算法嵌入生成代码，Tectorwise 则必须按单类型、批量处理约束拆为 primitive。

### 3.3 工作负载

主要工作负载是 TPC-H。为逐查询展示计数器，我们选取五条能覆盖主要瓶颈的查询：Q1 是定点算术与 4 组聚合；Q6 是高选择性过滤；Q3 是 build 14.7 万、probe 320 万项的 join；Q9 是 build 32 万、probe 150 万项的 join；Q18 是 150 万组的高基数聚合（均为 SF=1，随规模线性增长）。剩余 17 条查询大多由 join 主导，因而类似 Q3/Q9；少数主要耗时在高基数聚合上，因而类似 Q18。Q1 和 Q6 虽然是仅有的两条单表查询，行为却很不相同，所以两者都被保留。五条查询合在一起覆盖了 TPC-H 最重要的性能挑战；能高效执行它们的执行引擎，很可能也能高效执行完整 TPC-H 套件 [6]。

### 3.4 实验设置

默认平台为 10 核 Intel i9-7900X（Skylake X），Linux，GCC 7.2；硬件计数器通过 Linux `perf events` 采集。所有计数器除以查询扫描的元组总数，使不同查询、规模和引擎间可直接比较，例如“Tectorwise 在 Q1 上每元组比 Typer 多执行 41 条指令”，也能比较数据规模增加 10 倍后每元组增加多少次 cache miss。
...
Figure 3 的 SF=1 结果从 Typer 在 Q1 快 74%，到 Tectorwise 在 Q9 快 32%。差异远小于现代引擎与 PostgreSQL 之间一到两个数量级的差距 [17]，说明两种范式都可行，且不存在单方支配。
...
Tectorwise 把操作拆成 primitive，并在其间物化向量，因此最多执行 2.4 倍指令、产生 3.3 倍 L1 data miss；Typer 常把中间值留在寄存器，故 Q1 这类计算密集、cache-resident 聚合明显占优。Q3/Q9 使用同一哈希表布局、LLC miss 近似相同，Tectorwise 却分别快 4% 和 32%，而且数据与哈希表增大后，join 优势最高可达 40%：它的 probe loop 很简单，乱序引擎能提前发出更多独立 load；Typer 的融合循环同时包含 scan、selection、probe 和 aggregation，更快填满乱序窗口，分支误预测也会丢弃更多推测工作。

哈希函数也与代码形态交互。Tectorwise 采用指令数约为 CRC 两倍、但吞吐更高的 Murmur2；Typer 采用把两个 32 位 CRC 合并为 64 位的低延迟哈希，在大规模上可快 40%，因为较短依赖链让连续迭代更易并行发起内存访问。两者使用不同哈希函数仍是公平的 join 性能比较，因为每套系统都使用了对自身更有利的函数。IPC 不能单独代表性能：Q1 中 Tectorwise 的 IPC 高 40%，却因总指令近两倍而慢 74%。

### 4.2 解释和指令缓存

向量化仍采用 Volcano 式 pull 和解释，但一次调用处理约 1,000 个值，primitive 又按数据类型预专用。Profiler 显示 SF=10 的五条查询中，解释控制逻辑不足运行时间的 1.5%；额外指令主要不是虚调用或类型分派，而是 primitive 之间物化结果的 load/store。既有研究曾指出 OLTP 负载中的 instruction cache miss 问题 [43]；但两套引擎的热点代码都能放入 32 KB L1 instruction cache，本文 OLAP 实验中的 I-cache miss 可忽略。
...
向量大小为 1 时，Tectorwise 退化为 Volcano 风格解释器，CPU 开销很大。大型向量放不进 CPU cache，因此会产生 cache miss；另一端的极限就是 MonetDB 使用的一次处理一整列 [9]。总体而言，1,000 左右对所有查询都很合适；唯一明显例外是 Q3，使用 64K 向量时还能再快约 15%。
...
这里比较的是 Actian Vector 5.0（VectorWise 当时的产品名）和 HyPer 的 TUM 研究版本；后者有一个相关系统后来归属 Tableau。HyPer 与 Typer、VectorWise 与 Tectorwise 的量级和相对趋势接近；除 Q6 外，轻量原型略快于生产系统，因为后者还要处理溢出检查等完整语义。HyPer 在 Q6 上更快，是因为它直接在压缩列上用 SIMD 指令计算选择 [19]。这一对照支持原型对两类架构的代表性。
...
向量化选择 primitive 产生一个 selection vector，其中保存全部匹配元组的下标。AVX-512 比较指令生成 mask，再把该 mask 交给跨 SIMD lane 的 `COMPRESSSTORE`，将被选中的位置写入内存。AVX2 下的同一 primitive 并不简单，需要查找表 [35,19,26]，或基于 BMI2 [1] 的复杂置换逻辑。我们用一个 8,192 元素整数数组做微基准，比较无分支的标量 x86 实现与 SIMD 实现；最佳情形中，32-bit 输入连续且全部位于 L1 cache，图 6a 的加速为 8.4 倍。
...
图 8a 中单独计算哈希可加速 2.3 倍；图 8b 的 gather 最佳也只有 1.1 倍，因为测试机内存系统每周期最多执行两次 load，无论使用 SIMD gather 还是标量 load；将 gather 等 SIMD 指令用于 Tectorwise probe primitive，图 8c 的最佳收益为 1.4 倍。但在完整 TPC-H join 中收益几乎消失，即使 Q3、Q9 分别有 55% 和 65% 的时间位于 SIMD 优化 primitive。图 9 解释了原因：working set 增长后，内存延迟主导成本，只有数据全部位于 cache 时 SIMD 才有明显收益。Polychroniou 等人研究 SIMD 数据库算子时也观察到了这一现象 [35]。
...
单线程性能长期停滞而 CPU 核数持续增长——Intel 当时已在单颗 Xeon 上提供 28 个核心、56 个超线程——现代查询引擎必须有效使用全部核心。本节比较如何把查询内并行纳入两种执行模型。

### 6.1 Exchange 与 morsel-driven 并行

VectorWise 原本使用 exchange operator [3]；这一经典方法 [13] 让 join、aggregation 等查询算子基本无需感知并行。HyPer 使用 morsel-driven 并行，通过共享哈希表使 join 和 aggregation 显式感知并行，从而获得更好的局部性、负载均衡和扩展性 [22]；后者在 20 个硬件线程上的五查询平均加速为 11.7 倍，前者为 7.2 倍。但并行框架与执行范式正交，因此本文为两套原型统一采用扩展性更好的 morsel-driven [22]。

Morsel-driven 并行最初为 HyPer 开发 [22]，所以 Typer 只需把 scan loop 换成并行循环，并同步共享 hash table。Tectorwise 为每个 worker 建立一棵算子树和独占资源，每个算子另有共享状态用于结果交换和工作分配；pipeline breaker 使用 barrier，例如 hash join 必须等所有 worker 完成 build 后才能进入 probe。这样两者获得与 HyPer 实现相同的负载均衡行为 [22,23]。
...
启用 20 个超线程后，除内存带宽受限的 Q6 外，两者差距普遍缩小；Q3/Q9 的 Tectorwise 优势约减半。硬件多线程能隐藏一部分微架构上不理想的代码。Q6 是唯一例外：Typer 的无分支 selection 实现会消耗更多内存带宽，在高线程数下性能低 20%。
...
前述实验只测量 Intel 当时最新的 Skylake 微架构。为判断结果能否推广，本节还考察 AMD 当时的新一代 Zen 微架构和 Intel 的 Phi 产品线。
...
Knights Landing 有 64-72 个较慢核心，每核两组 512-bit 向量单元，6 通道 DDR4，并提供约 300 GB/s 的 16 GB 高带宽内存。实验把它设为硬件管理的 L3 cache、Quadrant 单 NUMA 模式。未经 SIMD 调整时，Q3/Q9 可比 Skylake 快 0-25%，Q18 慢约 20%，Q1 慢约 30%，Q6 因高带宽最多快 2 倍。由于重复实验时 Q6 的整个工作集可放入 16 GB cache，这一结果偏乐观；把硬件配置为无 L3 cache 后，Q6 相对 Skylake 的优势只有 10%。混合负载下的实际差距应落在这两个测量结果之间。
...
运行时重排合取谓词乃至 join 能抵消不可避免的基数估计误差。把适应性集成进编译执行非常困难；解释式系统可以在运行时改变解释方式，因而更适合自适应执行。向量化执行仍属于解释执行，结合细粒度 profiling 后，VectorWise 可以作出多种 micro-adaptive 决策 [39]。VectorWise 的 Q1 优于 Tectorwise 就来自一个与 [15] 类似、但 Tectorwise 未实现的自适应聚合：按 group key 把当前向量拆成 selection vector；若当前向量的分组很少，分区成功，遍历每个 selection vector 就能把哈希聚合转成有序聚合，在寄存器中计算局部聚合值，并且每个向量只更新一次哈希表中的聚合值。若分组太多导致分区失败，系统会指数退避，减少后续向量尝试该优化的频率。
...
反方向上，Sompolski 等观察到，把两个或更多相邻的 VectorWise primitive loop 用 JIT 编译融合成一个循环，有时可以省掉物化步骤 [45]；据我们所知，当时还没有系统集成这一想法。面向 UDF 的数据管理系统 Tupleware 则明确采用以数据为中心和向量化执行的混合方式，用代价模型与 UDF 分析选择最适合任务的执行方法 [11]。
...
表 6 以“pipelining：pull/push/无”和“执行：解释/向量化/编译”构成 9 种组合，其中 8 种已有系统采用。System R 以后，多数引擎用 pull 避免中间物化；push 后来常与编译结合，但也用于解释或向量化。Push 可让一个算子向多个 consumer 输出 DAG 计划 [27]，也适合分布式 exchange，这也是 Virtuoso 采用 push 的原因之一 [8]；缺点是 merge-sort 等控制流较不灵活。包括 MonetDB [10] 在内的一些系统完全物化各算子结果，简化实现却增加主存带宽。
