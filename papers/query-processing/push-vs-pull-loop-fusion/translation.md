...
电子邮件：`{firstname}.{lastname}@epfl.ch`

**版本信息：** arXiv:1610.09166v1 [cs.DB]，2016 年 10 月 28 日。

## 摘要

数据库查询引擎使用 pull-based 或 push-based 方法，以避免在查询算子之间物化数据。我们深入研究这两类查询引擎，展示各自限制和优势。类似地，编程语言社区也发展了 loop fusion 技术，用于在 collection programming 中移除中间集合。我们通过展示 pipelined query engine 与 loop fusion 技术之间的联系，把数据库社区与编程语言社区联系起来。基于这种联系，我们提出一种新的 pull-based engine，它受 loop fusion 技术启发，结合两类方法的优点。

随后，我们首次在公平环境中，在 query compilation 上下文里实验评估多种 engine，消除过去只与某一种方法绑定使用的辅助优化所造成的偏差。我们表明，对于现实分析型 workload，两种 pipelined query engine 没有哪一种具有显著优势，这与近期研究的暗示不同。我们还通过 microbenchmark 展示，我们提出的 engine 通过结合双方优点，可以支配现有 engine。
...
关系数据库很早就提出了一个解决方案：Volcano Iterator model [19]。在该模型中，tuple 沿算子链向上被 pull。算子之间通过 iterator 链接，并以 lock-step 方式前进。中间结果不会累积；tuple 是按需、由概念上“后续”的算子请求产生。

近年又出现一种 operator chaining 模型，同样避免中间结果物化，但反转控制流：tuple 从 source relation 向最终结果算子 push。近期论文 [42, 30] 似乎暗示 push 模型总能比 pull 模型带来更好查询处理性能，尽管没有直接、公平的比较。本文的重要贡献之一就是破除这一迷思。我们表明，如果公平比较，push 和 pull 引擎性能非常相近，各有强项和弱项，没有明确赢家。push engine 本质上常只在 query compilation 上下文中被讨论，这把 push 范式的潜在优势与 code inlining 的优势混在一起。公平比较必须解耦这两个方面。
...
我们还展示编程语言社区如何遇到并解决同类挑战。具体地，我们研究 PL 社区针对 collection programming 的 stream fusion [11]，并展示它如何适配查询处理、结合 pull 和 push 的优点。此外，我们展示了我们如何使用 push 方法中的思想来解决 stream fusion 的已知限制。因此，我们构建出一种结合 push 与 pull 优点的查询引擎。本质上，该引擎在粗粒度层面是 pull-based，在细粒度层面则 push 单个数据 tuple。

本文贡献包括：

- 我们在第 2 节讨论 pipelined query engine；第 3 节介绍 collection programming 中的 loop fusion，我们在第 3.3 节展示二者联系，并进一步展示各方法的限制。
- 基于这种与 loop fusion 的联系，我们在第 4 节提出一种新的 pipelined query engine，其灵感来自 PL 社区为 collection programming 开发的 stream fusion [11]；我们还在第 5 节讨论实现问题和所需编译器优化。
- 我们在第 6 节实验评估多种 query engine 架构。通过 microbenchmark，我们讨论现有 engine 的弱点，以及新 engine 如何结合双方优点规避这些弱点；然后，我们用 TPC-H 查询展示，良好实现下各 engine 在现实分析型查询上没有显著优势差异。

全文中，我们对所有代码片段、接口和示例都使用 Scala，但概念不依赖 Scala。其他带有非纯函数式和面向对象特性的语言，如 OCaml、F#、C++11、C# 或 Java 8，也可使用这些思想。接下来，我们详细介绍 pull-based 和 push-based query engine。
...
Iterator model 是查询引擎中使用最广泛的 pipelining 技术。它最初在 XRM [37] 中提出，后来因 Volcano 系统 [19] 采用并加入并行化设施而流行。

简言之，iterator model 中每个算子通过向 source operator 请求下一个元素来 pipeline 数据。这样无需等待整个中间关系生成完毕，每个算子都可惰性生成数据。具体方式是 destination operator 调用 source operator 的 `next` 方法。pull-based engine 的设计直接对应面向对象编程中的 Iterator design pattern [54]。
...
Push-based engine 广泛用于流处理系统 [24]。Volcano 在实现 query engine 的 inter-operator parallelism 时也使用 data-driven pipelining。在 query compilation 上下文中，StreamBase [1]、Spade [16]、HyPer [42] 和 LegoBase [30] 都使用 push-based query engine。
...
Push engine 可用面向对象编程中的 Visitor design pattern [54] 实现。在 query engine 中，Visitor pattern 使我们能够把 query operator（数据处理算法）与元素 relation 分离。为此，每个算子定义为 visitor class，其中 `consume` 方法相当于 `visit`。算子链初始化通过 Visitor pattern 的 `accept` 方法完成，对应 push engine 中的 `produce` 方法。
...
但 push engine 在 `limit` 和 `merge join` 上有困难。对 `limit`，push engine 天生不允许提前终止 producer 的循环，因为 producer 控制迭代，因此还会产生永远不会使用的元素。对 merge join，两个有序输入之间存在一对多关系，而 merge join operator 无法指示哪个 source operator 应产生下一项；因此无法同时 pipeline 两个输入，至少必须打断一侧 pipeline，并物化该侧输入。该限制并非 merge join 独有：在 Spark RDD [57] 等 collection programming API 中，`zip` 与 merge join 行为相似，也难以在 push-based engine 中顺利 pipeline。

可以针对这两个算子增加特例：把 `limit` 与它的 source operator（多数情况下是 ordering operator）手工融合，以避免产生无用元素；也可以让 merge join 的两个 source operator 使用不同线程，并用同步构造控制两侧生产数据，但代价可能很高。不过，在本文中，我们所称的 push engine 是不含这类增强的纯 push engine。
...
图 3a 展示示例 SQL 在 pull engine 中内联后的代码。请注意，对 selection operator，我们需要额外的 `while` 循环；这增加分支并复杂化 CFG。图 3c 展示该 CFG：矩形表示语句块，菱形表示条件，边表示执行流，回边表示循环跳转。复杂 CFG 让优化编译器更难理解和优化，运行时主要因 branch prediction 变差而性能下降。
...
一些方法如 `map` 同时使用两者：消费给定 collection 并产生新 collection。另一些方法如 `sum` 只需 `foldr` 消费 collection 并产生聚合结果。我们采用该算法的 imperative 变体，用 `foreach` 代替 `foldr`；`foldr` 显式处理状态，而 `foreach` 把状态封装在内部、不暴露到接口。Scala 中 `foreach` 签名为：
...
> **原文脚注 1：**我们在这里给出的是 unfold fusion 的命令式版本；纯函数式版本使用 `unfold` 函数而不是 `generate`，该方法也由此得名。
...
图 4 为简洁起见，省略了用于检查 list 是否结束的 null-checking statements。Unfold fusion 对 `filter` 引入递归迭代，实践中可能有性能问题；但理论上仍能成功 deforest [23]。它也不能融合嵌套 collection 操作，本文不讨论这一点。

### 3.3 Loop Fusion 就是 Operator Pipelining

查询可通过串接 query operator 表达；collection program 也可通过 collection operator pipeline 表达。关系查询与 collection program 的关系已有充分研究；特别是，关系查询计划与一类 collection program 之间可以建立精确对应 [44]。算子可分三类：
...
Nested loop join 可用两个嵌套 `flatMap` 表达，但 hash-based join 没有直接等价物。此外，`flatMap` 可以表达嵌套 collection，而关系查询引擎中的每个 relation 都被视为扁平的。`zip` 与 merge join 行为相似，但语义不同。表示 Group By 的 aggregation 是 transformer，而折叠为单个结果的 aggregation 是 consumer。对于其余 join operator，我们扩展 collection programming，加入 join operator（例如 `hashJoin`、`semiHashJoin` 等）。LINQ [39] 算子与 Haskell list 之间的类似映射见 Steno [40]。请注意，我们在这里不考虑嵌套 collection；尽管 collection programming 可以直接支持它们，我们仍将其排除，以强调与关系查询引擎的相似性。
...
本节中，我们先介绍另一种 collection program loop fusion 技术。然后，我们基于 query 与 collection programming 的对应关系，提出一种受该 fusion 技术启发的新型 pull-based query engine。
...
图 4 中，stream fusion 把 `list.map(f).map(g)` 转换为 `list.map(f o g)`；对 `Step`，`step.map(f).map(g)` 与 `step.map(f o g)` 等价。Stream fusion 与 unfold fusion 思想相似，主要差异在 `filter`。Stream fusion 使用特殊值 `Skip` 实现 `filter`；unfold fusion 则用额外嵌套 `while` 循环跳过不必要元素。因此 stream fusion 解决了 unfold fusion 在 `filter` 上的实践问题。接下来，我们基于 stream fusion 的思想定义一种新的 pipelined query engine。

### 4.2 Stream-Fusion Engine

我们提出的 query engine 采用与 iterator model 相同设计，因此仍是 pull engine。但它不调用 `next`，而是调用 `stream`，返回类型为 `Step` 的 wrapper 对象。我们把我们提出的这种 engine 称为 stream-fusion engine。

正如我们在第 2.1 节所述，pull engine 的一个主要实践问题出现在 selection operator 上：目的算子要等待 selection operator 返回下一个满足谓词的元素。stream-fusion engine 用 `Skip` 对象解决这个问题：`Skip` 表示当前元素应被忽略。因此 selection 不再阻塞 destination operator。
...
例如，考虑一个包含两个元素的 relation，我们选择其中第一个元素，并过滤掉第二个元素：第一次调用 selection operator 的 `stream` 产生 `Yield(first)`；第二次产生 `Skip`；第三次产生 `Done`。
...
本节中，我们讨论上述 query engine 的实现。首先，我们讨论每种方法如何实现 fusion rule；然后，我们说明如何解决 stream-fusion engine 与中间对象相关的问题。

我们使用开源 DBLAB 框架 [48] 实现不同 query engine 及其优化。该框架使我们能够用 Scala 这种高级语言实现 engine。输入程序可表达为 QPlan 物理查询计划，或 QMonad collection program。此外，我们使用 DBLAB transformation framework 提供的 rewrite rule 实现优化。

> **原文脚注 2：**DBLAB 项目地址：<http://github.com/epfldata/dblab>。

我们实现了 collection programming 操作和对应 loop fusion 技术。由于第 3.3 节展示了 query engine 与 collection programming 的等价关系，collection fusion 的实验结果也对应不同 pipelined query engine 方法。接下来，我们讨论如何在该框架中表达不同 loop fusion algorithm 的 fusion rule。

### 5.1 通过 Inlining 实现 Fusion

Loop fusion rule 通常作为宿主语言编译器扩展的局部 transformation rule 实现；上述论文使用的宿主编译器是 GHC [26]。本节中，我们展示这些 fusion rule 如何只用 inlining 实现；Scala 中 fold fusion 的这种实现方式由文献 [27] 提出。对于其他 fusion 技术，我们采用类似方法。

以 fold fusion 为例，`build` 定义内联后创建一个 `QueryMonad` 对象。该对象的 `foreach` 方法把传给 `build` 的高阶函数 `f1` 应用到 `foreach` 输入参数 `f2`。通过内联这个 `foreach` 方法，我们得到第 3 节介绍的同一条 fold-fusion rule：
...
接下来，我们讨论 stream-fusion engine 创建中间对象的问题，以及我们的解决方案。
...
原始 stream fusion 依赖 GHC 编译器 [26] 提供的优化。若要在其他语言中有效实现，需要类似优化。

虚函数调用问题可通过把 `Step` 操作改写为枚举所有 `Step` 情况解决。因为该数据类型只有三种具体情况：`Yield`、`Skip`、`Done`。函数式语言可用 pattern matching；命令式实现可用 `if`。

heap allocation 问题可通过 escape analysis [10] 解决：这些对象没有逃逸出使用作用域，例如没有被复制进数组，也没有作为函数参数逃逸。因此 heap allocation 可转为 stack allocation。编译器还可进一步通过 scalar replacement 移除 stack allocation，把编码 `Step` 所需字段转换为局部变量，从而完全消除 `Step` 抽象。

换一个视角看：移除中间 `Step` 对象与移除中间 relation/collection 是同类问题，可借用类似思想。为此，我们使用 Visitor pattern 实现 `Step` 数据类型。正如我们在第 3.3 节所述，这类似数据类型的 Church encoding，会把 `Step` 对象 push 下 pipeline。因此 stream-fusion engine 在粗粒度层面是 pull engine，在细粒度层面则 push tuple。
...
在实验评估中，我们使用一台服务器级 x86 机器，配备两颗 Intel Xeon E5-2620 v2 CPU（2 GHz）、256 GB DDR3 RAM（1600 MHz）和两块 2 TB HDD。操作系统是 Red Hat Enterprise 6.7。

为公平比较，我们的 query compiler 对不同 pipelining 技术使用同一组 transformation：dead code elimination（DCE）、common subexpression elimination（CSE）/ global value numbering（GVN）、partial evaluation（inlining 和 constant propagation）。这些 transformation 由 DBLAB [48] 直接提供，我们将其用作我们的实验平台。除非明确说明，scalar replacement 总是应用。我们在这些实验中不使用数据结构 specialization transformation 或 inverted index。所有实验使用 DBLAB 的 in-memory row-store 表示。

在我们的评测中，我们使用 Clang 2.9 编译生成程序。我们采用 Clang 编译器提供的最激进优化选项 `-O3`。最后，对 C 数据结构，我们使用 GLib 2.42.1。

> **原文脚注 3：**尽管该优化选项会增加优化编译器的查询编译时间，我们使用 `-O1` 时仍观察到相似结果。`-O1` 包含 HyPer [42] 使用的全部转换 pass，但不包含 global value numbering（GVN）；我们的情形不需要该转换，因为 DBLAB [48] 已经提供。

我们的评测分两部分：首先，我们用 microbenchmark 更清楚地展示不同 query engine 的差异；然后，对更复杂的查询，我们使用 TPC-H benchmark [51]。我们展示不同 query engine 在这些更复杂场景中的行为。
...
本节中，我们讨论本文结果如何应用到并行查询引擎。Intra-operator parallelism 是实现查询引擎并行性的主要方式之一。在这种方法中，数据被分给不同线程，每个线程负责顺序处理自己的 chunk，最后把不同线程计算的结果 merge 成一个结果。split 和 merge 算子 [38] 被插入到 pipeline breaker（如 aggregation 和 hash join）之间。因此，每个线程顺序计算一串 pipelining operator 的结果。

如果使用相同 split 和 merge 算子，pull 与 push engine 的唯一区别就是它们多高效地计算一条 pipelining operator chain。因此，在 intra-operator parallelism 的公平环境中，我们在本文单线程场景中展示的实验结果预计也适用于多线程场景。

Intra-operator parallelism 的关键决策之一是何时做 partitioning decision。如果在查询编译时做，称为 plan-driven；如果推迟到运行期，称为 morsel-driven [36]。后者的优势是利用运行期信息，通过 work-stealing [5] 获得更好负载均衡。但 partitioning decision 的选择独立于 query engine 类型。所有 query engine 都可使用这两种方法，work-stealing 调度对它们的影响类似。

PL 社区也有类似并行 collection program 的工作。例如 morsel-driven parallelism [36] 与并行 collection programming library [46] 中的 work-stealing 调度有相似思想。

## 8. 结论

如果公平比较 push 与 pull-based query processing，特别是尽可能在两种方法中都内联代码，那么没有哪一种方法明显优于另一种。我们已经讨论了其中原因；实际上，细看二者的基本工作方式后，任何一方若在性能上完全支配另一方，反而会显得令人意外。
