...
Xiaoying Wang（`xiaoying_wang@sfu.ca`）、Changbo Qu（`changboq@sfu.ca`）、Weiyuan Wu（`youngw@sfu.ca`）[^equal-contribution]、Jiannan Wang（`jnwang@sfu.ca`），Simon Fraser University；Qingqing Zhou（`hewanzhou@tencent.com`），Tencent Inc.

[^equal-contribution]: 上述名单中的前三位成员对本研究贡献相同。
...
**许可：** 本文采用 [Creative Commons BY-NC-ND 4.0 International License](https://creativecommons.org/licenses/by-nc-nd/4.0/)；超出许可范围的使用须通过 `info@vldb.org` 获得授权。版权由权利人持有，出版权授予 VLDB Endowment。PVLDB 第 14 卷第 9 期，ISSN 2150-8097。
...
**研究机会。** 我们提出两个方向：控制学习方法的成本，以及让学习方法值得信赖，并公开代码与数据以支持后续研究。[^artifact-code]
...
按方法论可分为两组。[^method-names] 回归方法（亦称查询驱动方法）把基数估计建模为回归问题，通过特征向量学习“查询 → 特征向量 → 基数估计结果”的映射。联合分布方法（亦称数据驱动方法）先从表中构建 $P(A_1,A_2,\ldots,A_n)$，再估计基数。表中的“输入”表示建模所需输入：回归方法均需查询，联合分布方法只依赖数据。
...
**学习方法与实现。** 我们研究 Naru [95]、MSCN [34]、LW-XGB/NN [18] 与 DeepDB [30]。DQM [28] 被排除：其数据驱动版本与 Naru 表现相似，而查询驱动版本不能支持我们的负载，这一点已由 DQM 的作者确认。Naru[^naru-code] 和 DeepDB[^deepdb-code] 使用相应原论文团队提供的实现并作少量适配；Naru 选用兼顾效率与准确率的 ResMADE。MSCN 原本支持 join，会编码不同表上的 join 和谓词；为公平比较单表场景，我们只保留谓词和合格样本特征。[^mscn-code] LW-NN 基于 PyTorch [67]，LW-XGB 基于 XGBoost [10]，并依照原论文 [18] 实现 LW-XGB/NN；单列 CE 特征由 PostgreSQL 估计结果产生。数据处理、负载生成和评测代码均已公开。[^all-code]

**硬件。** 实验服务器有 16 个 2.00 GHz Intel Xeon E7-4830 v4 CPU；Naru、MSCN 与 LW-NN 还在 NVIDIA Tesla P100 GPU 上运行，以比较不同设置。

[^artifact-code]: <https://github.com/sfu-db/AreCELearnedYet>
[^method-names]: Naru、DeepDB 和 MSCN 沿用相应论文给出的名称。为方便讨论，本文把文献 [18] 的两个模型简称为 Lightweight Gradient Boosting Tree（LW-XGB）和 Lightweight Neural Network（LW-NN），把文献 [28] 的两个互补方法简称为 Data&Query Model - Data（DQM-D）和 Data&Query Model - Query（DQM-Q）。
[^naru-code]: <https://github.com/naru-project/naru>
[^deepdb-code]: <https://github.com/DataManagementLab/deepdb-public>
[^mscn-code]: <https://github.com/andreaskipf/learnedcardinalities>
[^all-code]: <https://github.com/sfu-db/AreCELearnedYet>
...
> 译者注：原文把第二个区间的中心写为 $(100-20)/2=40$，这里按原文保留；若按通常的区间中点定义， $[20,100]$ 的中点应为 60。
...
**超参数调优。** Naru、MSCN、LW-NN 的模型大小均限制在数据大小的 1.5% 内。每种方法选择四种层数、隐藏单元、embedding 等不同架构，并按原论文改变 batch size 和 learning rate。查询驱动的 MSCN、LW-NN 用 10K 验证查询；数据驱动的 Naru 以训练损失选参数。LW-XGB 按文献 [18] 改变树数（16、32、64……）并用 10K 验证查询。DeepDB 对 RDC 阈值和最小实例切片作网格搜索，只保留大小预算内的模型；它不输出训练损失，虽定位为数据驱动方法，调参仍需查询，因此仅用 100 条验证查询。MSCN、LW-XGB/NN 均用 100K 查询训练。
...
- **Sample-A 与 Sample-B。** Sample-A 使用 1.5% 的均匀随机样本；当样本中没有 tuple 同时满足全部谓词时，它会产生大误差。Sample-B 在这类零命中情形中改为假设各谓词彼此独立，也使用 1.5% 的样本。
- **MHIST [73]。** 在完整数据集上构建多维直方图；按文献 [74] 的最优配置，以 Maxdiff 为分区约束、Value 为排序参数、Area 为 source 参数，迭代运行 MHIST-2，直至达到数据大小的 1.5%。
- **QuickSel [66]。** 利用查询反馈，以均匀混合模型刻画数据分布，并用 10K 查询训练。选择它是因为文献 [66] 报告其准确率优于 STHoles [6] 和 ISOMER [81] 等查询驱动直方图。
- **Bayes [13]。** 代表概率图模型方法 [14, 24, 88]；采用文献 [95] 的同一实现，用 progressive sampling 估计区间查询，准确率很有希望。
- **KDE-FB [29]。** 代表以核密度模型刻画数据分布的方法，通过查询反馈优化朴素 KDE 的带宽；从每个数据集抽取 1.5% 的 tuple（最多 150K），并用 1K 查询训练。
...
推理方面，实验逐条发出 10K 查询并取平均。DBMS 的 1～2 ms 实际测量的是返回执行计划的延迟，已包含解析、绑定等额外工作，因此真实基数估计只会更快。查询驱动方法较有竞争力，特别是 LW-XGB/NN；DeepDB 约为 5～25 ms；Naru 在 GPU 上约 5～15 ms，而 CPU 最多再慢 20 倍。除特征化外，多数学习方法的瓶颈主要来自底层 NN、SPN、XGB 或 MSCN 模型；缩短推理时间可能需要模型压缩、蒸馏等技术。Naru 则是例外：为准确回答一次查询，它需要在 progressive sampling 中反复运行模型上千次，且逐属性选择率存在顺序依赖，不能仅靠一般模型压缩轻易消除。优化器可能在一次优化中调用估计器许多次，故这种延迟对短 OLTP 查询尤其可能成为阻塞问题。因而除轻量模型外，学习方法训练和推理通常都比 DBMS 慢 10～1000 倍。
...
学习方法内部也没有全局赢家。LW-XGB 在大多数情形至少与其他方法相当；Naru 在无更新时准确率很高，但更新频繁时不能胜过 LW-XGB。Census 与 Forest 上，一个 epoch 不足以让 Naru 得到好模型；DMV 上也能观察到文献 [18] 报告的现象：Naru 在一个 epoch 内表现良好。更新时间亦没有统一赢家：Census 上数据驱动的 DeepDB 最快，DMV 上查询驱动的 LW-XGB 最快。数据驱动模型要压缩整表联合分布，数据越大模型越复杂；查询驱动模型主要付出标签生成成本，固定训练查询数时模型复杂度未必随表大小增长。因此选择何种范式高度依赖应用。
...
为尽量发现困难查询，测试查询的每列中心均独立从列域抽取，即全部使用 OOD；其余设置与第 4 节相同。DeepDB 采用文献 [30] 推荐的默认超参数，即 RDC 阈值 0.3、最小实例切片 0.01；LW-XGB 固定 128 棵树；三个神经网络仍以数据 1% 为大小预算，并从随机超参数配置中选取稳定较好的模型。每个固定模型都在仅改变一个因素的数据集上测试同一组 10K 查询；为突出大误差，只展示最高 1% q-error 的分布。
...
**解释学习估计器。** 机器学习领域已有 surrogate model [75]、saliency map [79]、influence function [35]、decision set [40]、rule summary [76]、通用特征归因 [52, 83] 等方法。例如测试查询出现大误差时，可用 influence function [35] 定位最有影响的训练样本，或用 Shapley value [52] 检查各输入特征的重要性；原文此处误作“shapely value”。这些方法在基数估计场景的有效性仍是开放问题。
...
**基数估计的基准与经验研究。** Leis 等人 [42] 提出 Join Order Benchmark（JOB）：它基于真实 IMDB 数据集，合成查询包含 3 至 16 个连接 [42]；我们与之不同，聚焦单表基数估计。Ortiz 等人 [64] 对多种深度学习和机器学习架构的准确率、空间与时间权衡作经验分析。我们同时覆盖数据驱动和查询驱动方法（该工作聚焦查询驱动模型）、静态和动态环境，还通过受控合成数据研究模型何时出错，并提出简单逻辑规则。Harmouch 等人 [27] 也进行过基数估计实验综述，但目标是估计 distinct value 数量，与本文不同。
...
[10] T. Chen and C. Guestrin. XGBoost: A scalable tree boosting system. In *Proceedings of the 22nd ACM SIGKDD International Conference on Knowledge Discovery and Data Mining, KDD ’16*, pages 785–794, New York, NY, USA, 2016. ACM.
...
[50] D. López-Paz, P. Hennig, and B. Schölkopf. The randomized dependence coefficient. In *Advances in Neural Information Processing Systems 26: 27th Annual Conference on Neural Information Processing Systems 2013. Proceedings of a meeting held December 5–8, 2013, Lake Tahoe, Nevada, United States*, pages 1–9, 2013.
...
[70] M. Perron, Z. Shang, T. Kraska, and M. Stonebraker. How I learned to stop worrying and love re-optimization. In *35th IEEE International Conference on Data Engineering, ICDE 2019, Macao, China, April 8–11, 2019*, pages 1758–1761. IEEE, 2019.
