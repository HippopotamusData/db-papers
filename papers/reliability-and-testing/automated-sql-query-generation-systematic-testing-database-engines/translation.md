...
考虑图 1 所示的数据库模式。主键约束、外键约束等 SQL 语句，以及其他不影响 SQL 查询生成的语句，都与本文所解决的问题无关，因而被本方法忽略。图 1 中的 SQL 语句创建两个关系（亦称表）：（1）`students` 表有两个属性，即 `int` 类型的 `id` 和 `varchar` 类型的 `name`；（2）`grades` 表有三个属性，即表示学生 ID 号的 `int` 类型 `studetnID`、表示课程 ID 号的 `int` 类型 `courseID`，以及表示该学生在这门课中所得成绩的 `int` 类型 `grade`。
