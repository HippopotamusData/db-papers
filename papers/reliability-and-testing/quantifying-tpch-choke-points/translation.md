...
- **作者：** Markus Dreseler、Martin Boissier、Tilmann Rabl、Matthias Uflacker
- **单位：** Hasso Plattner Institute，University of Potsdam，Germany
- **邮箱：** firstname.lastname@hpi.de
- **出处：** *Proceedings of the VLDB Endowment*，13(8): 1206–1220，2020；ISSN 2150-8097
- **DOI：** <https://doi.org/10.14778/3389133.3389138>

## 来源与使用说明

论文采用 [CC BY-NC-ND 4.0](https://creativecommons.org/licenses/by-nc-nd/4.0/) 许可；超出许可范围的使用需通过 info@vldb.org 取得许可。著作权归原权利人所有，出版权授予 VLDB Endowment。本文仅作中文学习翻译，不改写原论文结论。
...
TPC-H 原本用于端到端比较数据库系统，但研究者也经常用它评估实现细节和算法。更高吞吐量常被视为某种方法更好的证据。如果所有其他组件相同，这种解释可能成立；但更常见情况是，不同论文使用不同数据库系统，因此系统中其他优化的影响被低估 [46]。
...
我们认为，在比较 TPC-H 结果时，应讨论相关优化。Boncz、Neumann 和 Erling 曾分析 TPC-H 中的“隐藏信息”和阻塞点，并定义阻塞点为：基准背后的技术挑战，其解决会显著提高产品性能 [11]。他们识别了 28 个阻塞点，分为聚合与连接、数据访问局部性、表达式求值、相关子查询和并行执行等挑战，但没有给出定量实验数据。本文补充这一缺口：新系统可据此决定有限开发资源应先投入哪些优化，跨系统比较可判断差异来自被测方法还是其它优化，基准设计者也可将阻塞点的重要性与真实负载比较并据此调整基准。
...
本文结构如下：第 2 节按受影响的查询执行阶段对阻塞点分类；第 3 节介绍实验系统 Hyrise、与其它内存 DBMS 的基线比较，以及隔离并评估阻塞点的方法；第 4 节评估改变查询计划结构的阻塞点，第 5 节讨论算子内部的逻辑阻塞点；第 6 节比较各阻塞点的相对影响并总结实现经验；第 7 节讨论 TPC-H 优化及 TPC-H 以外基准的相关工作。
...
评估基于研究型 DBMS Hyrise [17]。我们选择 Hyrise 是因为团队熟悉其实现，能够关闭特定优化并理解这些关闭操作对其他组件的影响。这比仅从外部比较多个 DBMS 更适合深入分析单个阻塞点。本节先介绍后文所需的 Hyrise 架构要点，再与其它内存 DBMS 比较以确认基线性能可比，最后说明隔离与评估阻塞点的方法；Hyrise 的更完整介绍见此前工作 [17]。

### 3.1 Hyrise 特征

Hyrise 是列式内存数据库，也是企业数据管理研究项目 [6, 22, 50] 的平台；这些项目的共同目标之一是在真实 DBMS 中评估新概念。为此，团队持续构建一个既能运行常见基准、又能运行企业负载 [7] 的系统，而不是依赖专门为 TPC-H 手工优化的计划。Hyrise 的 C++ 源码见 [项目仓库](https://github.com/hyrise/hyrise)。

表被水平划分为 chunk，每个 chunk 包含 100,000 行；该数值经实验确认适合 TPC-H [17]。chunk 是并行化、压缩和统计信息维护的单位，可类比 HyPer 用于动态放置和并行化的 morsel [28]；后者实验得到的最佳大小同样是 100,000 行。每个 chunk 每列有一个 segment，可使用字典、run-length、LZ4 和 frame-of-reference 压缩；本文使用字典编码。

SQL 到执行计划的流程是：SQL 字符串由基于 flex/bison 的 [SQL 解析器](https://github.com/hyrise/sql-parser) 解析，转换为逻辑查询计划（Logical Query Plan, LQP），优化器变换 LQP，再转换为包含实际算子的物理查询计划（Physical Query Plan, PQP）。

Hyrise 的优化既有纯规则优化，例如常量算术表达式的一次性求值；也有基数驱动优化，例如连接排序。后者使用表级等 distinct-count 直方图和 chunk 级 MinMax filter [31]。表扫描使用 AVX-512 向量化，连接主要使用单通道 radix-partitioned hash join [10]，聚合使用 hash-based aggregate。
...
我们将 Hyrise 与 DuckDB [47]、MonetDB [9]、Umbra [38] 和 HyPer [25] 进行单线程 TPC-H 对比。DuckDB、MonetDB、Umbra 在我们的硬件上实测；闭源 HyPer 的数字取自 Flare 论文 [19]，硬件可比；Umbra 使用预发布原型及随附 TPC-H demo，MonetDB 使用其 [TPC-H 脚本](https://github.com/MonetDBSolutions/tpch-scripts)，DuckDB 基准经小改以支持 SF 10。选择这些系统是因为仍活跃、实现路线多样且易取得数据，并非将其作为直接竞争者；单线程也不是多数系统默认模式，图 3 不能用来建立系统排名。目的只是确认 Hyrise 基线合理，避免阻塞点分析被基础性能瓶颈削弱。
...
对每个阻塞点，我们修改 Hyrise 并移除对应优化，然后重新执行基准；优化收益由启用与禁用优化时固定时间窗内完成的查询数之差衡量。实验使用单线程执行。虽然真实系统可能使用多线程，但并行性会引入调度、NUMA 和负载均衡等额外因素，使单个优化的隔离分析更困难。

无关优化保持开启；若某优化依赖被移除的优化、继续开启反而造成回退，则在相应实验中暂时一并关闭。基数缩减本身不随线程数改变，但并行运行会额外引入核心数、NUMA 布局、查询流数量，以及 scan/join sharing 等正交因素 [24]，因此论文只研究在单线程和多线程下同样成立的阻塞点。

实验机是 2017 Fujitsu Primergy RX4770 M4，含四颗 Xeon 8180（2.5 GHz base、3.8 GHz turbo、每颗 28 个物理核）。基准通过 `numactl -N 0 -m 0` 绑定单一 NUMA 节点；每节点有 512 GB DDR4-2666、8 条 DIMM，单核按 2:1 读写比测得合计 20,553.32 MB/s。Hyrise 用 GCC 9.2、`-O3 -march=native` 编译；对 Hyrise 而言，GCC 9.2 构建的 TPC-H 吞吐量始终比 clang 9 高 11%。

`hyriseBenchmarkTPCH` 自动生成数据与查询、执行并保存结果。每个查询运行 60 秒，时间到后允许最后一次执行完成；指标是优化使该时间窗内完成的查询数发生的相对变化。规模因子为 10，因为更大规模下关闭子查询扁平化或谓词下推，会使单条查询一次执行超过一小时。每项实验通过 patch 禁用源码中的相应优化；[复现实验仓库](https://github.com/hyrise/tpch_paper) 提供这些 patch、编译、执行和可视化说明，作为复现起点。

## 4. 计划级阻塞点

本组阻塞点通过尽早降低基数影响整个查询计划，对所有系统都有益：处理较少行的算子，即使实现不如另一个处理大量行的算子高度优化，通常仍会更快。以下依次讨论连接顺序、单表谓词的位置与顺序、聚簇带来的分区剪枝与访问规避、12 条含子查询查询的计划改写，以及跨计划和跨执行的结果复用。

### 4.1 连接顺序

除 Q1、Q6 外，所有查询都涉及多表，多数至少涉及三张表。TPC-H 常把连接写成 `FROM t1, t2 WHERE t1.a = t2.a`，而非显式 `JOIN ... ON`。连接排序器第一项职责是把无谓词 cross join 与对应谓词配对；若没有这一步，20 条连接查询中只有 Q4、Q13、Q15、Q18、Q20、Q21、Q22 能在一分钟内结束，原文称其余 14 条几乎无法执行。因此谓词配对是不可缺少的功能，不作为优化实验对象。

> 译注：原文同时写“20 条连接查询”“只有 7 条结束”和“其余 14 条”；前两项相减应为 13。这里保留原文的“14 条”，并明确标出这一处源文算术不一致。

第二项职责才是确定表的连接次序。Hyrise 把 LQP 转成 join graph [42]，在图上排序，再重建优化 LQP。图 4 以 SQL 原始次序为基线，对比动态规划 DPccp [32] 与自底向上的 greedy 算法 [20]。Q2、Q5、Q11 明显受益；只有 Q7 中两种算法生成不同次序：greedy 先连接未过滤的 `customer`、`orders`，破坏原本较优的次序；DPccp 先让 `customer` 与 `nation` 连接，把客户缩减为两个目标国家。DPccp 得到与原 SQL 等价的次序，仍有约 4% 损失，来自排序算法自身成本。
...
Q19 把 `l_quantity` 范围写成 `>=` 与 `<=`；Q4、Q5、Q6、Q10、Q12、Q14、Q15 使用 `>=` 与 `<` 表示日期范围，这是因为 SQL `BETWEEN` 只支持两端闭区间。优化器可识别并改写为单侧开区间，例如 Q5 的 `o_orderdate >= date AND o_orderdate < date + 1 year` 变为 $[\mathrm{date}, \mathrm{date} + 1\ \mathrm{year})$。
...
> 译注：原文在本段第二句写成 “In Q17”，但本节首句、谓词内容和图 7 均明确对应 Q7，故译文按 Q7 处理。
...
22 条查询中有 7 条按 `lineitem.l_shipdate` 过滤，5 条按 `orders.o_orderdate` 过滤。若表按这些列聚簇，scan 可从线性搜索变为二分搜索；系统还可借助分区条件、小型物化聚合 [31]、zone map [58] 或 Hyrise 的 chunk 级 MinMax，在 scan 前直接跳过不可能命中的范围。`l_shipdate` 跨 83 个月，Q6 只选 12 个月，因此最多可在不访问任何行的情况下排除 85% 数据。

图 8 以 `tpch-dbgen` 原始顺序为基线，比较三种设置：打乱 `lineitem`、`orders`；按日期聚簇但禁止 DBMS 利用该信息；按日期聚簇并允许 partition pruning。三种设置均排除相关性检测，后者作为下一阻塞点单独讨论。打乱后 12 条查询至少下降 10%，说明原始数据并非随机，而按主键排序；这种次序会降低 sort join 的排序成本，也改善 hash join 查表的 DRAM cache 局部性。
...
按 `l_shipdate` 聚簇 `lineitem` 不仅改善对该列的访问，还能利用列间相关性，把剪枝信息传播到没有显式聚簇的列。近期研究还建议跨 join 利用这种相关性。
...
综合物理局部性与相关列，TPC-H 极适合 range-based access avoidance：查询访问 `lineitem` 或 `orders` 时，平均约三分之一数据无需读取，与 Nica 等人的结果一致 [41]。真实 ERP 负载也常随时间递增，事务与分析偏向近期数据；我们在 Global 2000 企业 ERP 中观察到这种模式 [7]，SAP 还报告中央财务系统依靠 partition pruning 获得 2–3 个数量级提升 [41]。相关性也可跨 join 利用：本地 pruning 后用剩余分区统计构造 join 列新谓词，再推到另一侧；相关研究称约一半查询可避免三分之一访问 [43]。我们质疑 TPC-H 主键与日期缺少相关性：若订单大致按日期进入，自动生成主键也应近似递增，这可避免实现者在 join 局部性和谓词局部性之间二选一。

### 4.7 子查询扁平化

子查询扁平化将嵌套子查询改写为连接或其他更容易优化的计划结构。TPC-H 中部分查询包含相关子查询；若不扁平化，系统可能需要反复执行子查询，产生巨大开销。

TPC-H 有 6 条相关子查询：Q2、Q4、Q17、Q20、Q21、Q22。最直接实现是严格遵循 SQL 语义：对外表 $T$ 的每一行分别执行搜索条件中的每个子查询，再把结果用于该行的搜索条件；若子查询含有对 $T$ 某列的外部引用，该引用取当前行的列值 [2, p. 7.6]。我们认为先实现这个正确但缓慢的版本，再逐步优化很有帮助。常见的 “Simple Unnesting” [39] 会从子查询移除对外值的比较，让子查询一次返回所有外值对应结果，再用被移除谓词把它与外表连接。该方法适用于 `(NOT) IN`、`(NOT) EXISTS` 和 scalar subquery，只是 join predicate 不同，也能扁平化无相关的 `(NOT) IN`。Q21 还适合不同子查询类型的 coalescence [4]，但 Hyrise 尚未实现；支持 `ALL` 的方法 [23]，或处理外层暂不可见相关属性的更一般方法 [39]，对 TPC-H 不必要。
...
扁平化会把相关谓词从子查询移除。Q17 原本只对满足 `p_brand`、`p_container` 的每个 $\mathrm{part} \times \mathrm{lineitem}$ 组合计算 `AVG(l_quantity)` 的 0.2 倍；直接扁平化却在整张 `lineitem` 上聚合，最终使用不到 1% 的行。Hyrise 在聚合前加入 semi join [5]，先删除后续不可能匹配的 `lineitem`。图 11 中 aggregate 输入从 60M 降至 60K，缩小 1,000 倍。
...
Boncz 等人的替代方案是把外层谓词传播进子查询（CP5.2：Moving Predicates into a Subquery）[11]：Q17 需把过滤后的 `part` 再加入子查询、先与 `lineitem` 连接再聚合；Q2 可传播 `p_size`、`p_type`。我们人工改写 Q2、Q17、Q20，确认性能与 semi join reduction 无可区分。Hyrise 选择 semi join 有三点原因：只需加入一个已在去相关时识别的谓词；可像普通谓词一样估算和下推，不引入新连接排序问题；还能用于 Q4 这类非扁平化场景。不过，判断何时值得加入仍然困难。
...
除单次计划内复用外，DBMS 还可缓存整个查询结果。部分 TPC-H 查询参数空间很小：Q18 的 `l_quantity` 只从 312–315 四个整数中选择，执行四次并保存结果只需 7 KB；另一端 Q16 有 $1.6 \times 10^{16}$ 种变体，单次结果 28,232 B，完整缓存接近 400 EB。表 2 给出 SF=1 下参数数、变体数、验证结果、完整缓存、执行时间及估计收益。生产 DBMS 会使用结果缓存，但我们没有看到它影响研究界传播的 TPC-H 结果，因此这里只为完整性给出理论收益。

**表 2：TPC-H SF=1 查询的参数数、参数变体数、校验结果大小、完整缓存大小、执行时间与相对收益。** 相对收益按“执行时间（µs）除以完整缓存大小”计算。
...
> 译注：原文正文称 Q16 单次结果为 28,232 B，但表 2 的 `Result` 单元格写作 `4.5E20 B`；译表保留了源表数值。这是原文内部不一致。

## 5. 逻辑算子阻塞点

前一节中的优化会改变计划结构并尽早减少中间基数，从而降低后续算子的工作量。本节的阻塞点不改变计划结构，也不改变任何算子的输入输出基数，而是改变单个计划节点的逻辑属性；因此，其潜在影响受限于相应算子的成本。以下讨论移除函数依赖的 group-by 列，以及 `IN` 谓词的不同执行策略。

### 5.1 依赖 group-by 键

TPC-H 使用 SQL-92。该标准要求聚合查询的输出列要么是聚合函数，要么属于 group-by 定义 [2, §7.9(7)]。因此，“识别可能遇到问题的客户”并按收入降序返回的 Q10，不仅按足以唯一识别客户的客户键分组，还把客户姓名、地址等另外六列列入 group-by。根据主键和外键识别函数依赖后，可同时降低基于 hash 和基于 sort 的聚合成本。

该优化适用于 Q3、Q10、Q18，但只有 Q10 显著提高 24%。Q3 的主要成本是几乎未过滤的 `lineitem` 与 `orders` 连接；Q18 要计算两次聚合：先在未过滤 `lineitem` 上计算按 `l_partkey` 分组的 `SUM(l_quantity)`，之后包含 `lineitem` 与 `customer` 函数依赖列的第二个 aggregate 在 SF 10 下只处理不到 1,000 行（具体取决于随机参数），缩短键的收益很小。

> 译注：原文 §4.5 将 Q18 的未过滤分组键写作 `l_orderkey`，§5.1 此处则写作 `l_partkey`；译文在两处分别保留源文，明确标出这一内部不一致。

SQL-99 修改了查询规范的语法规则，允许输出除 group-by 列外、函数依赖于这些列的其它列 [3, §7.11(13)]。但 Oracle 11g 等系统仍不支持这一语法，仍要求所有非聚合列进入 group-by 列表，因此该模式及其优化仍有现实意义。

还可处理主键不在 group-by 中的情况：Q5、Q7、Q9、Q10 按唯一且非空的 `n_name` 分组，Q21 按 `s_name` 分组。TPC-H 规范 §1.5.6 禁止在这些列上显式创建 unique index [55]，但仍可利用隐式知识，例如列字典的基数。用整数 `s_suppkey` 替换 18 字符的 `s_name` 会缩小键。不过图 2 显示这些查询中 aggregate 占比低；即使 Q10 占 20%，`nation` 也只有 25 行，扩展优化没有明显收益。我们在其它负载见过收益，但把它作为 TPC-H 上的负面结果报告。

### 5.2 大 IN 子句

TPC-H 在 Q12 使用 2 个常量的 `IN`，Q16 使用 8 个，Q19 分别使用 2、4 个，Q22 使用 7 个。不采用 JIT 的 DBMS 有三种执行方式：通用解释器用双重循环逐输入值、逐列表项检查，最灵活但有虚函数等解释开销；把 `IN` 拆成多个析取谓词，使字典压缩 scan 等只能处理单谓词的优化生效，再合并 position list；或把列表哈希后用 semi join 探测，探测为 $O(1)$，但 join 的额外成本可能抵消收益，而且所有 `IN` 表达式必须类型一致。
...
本节汇总前述实验，比较不同阻塞点的相关性，并给出实现这些优化时获得的经验。

### 6.1 阻塞点相关性

表 3 汇总了各阻塞点对 TPC-H 查询的相对性能影响；`Mean` 列为描述该阻塞点对整个 TPC-H 基准相关性的几何平均数。空白单元表示查询不受该阻塞点影响，`*` 表示相对变化小于 10%。
...
| 4.8 | Semi Join Reduction | Q2 +1255; Q3 *; Q4 +288; Q5 -24; Q7 *; Q8 *; Q9 *; Q10 -14; Q11 *; Q12 *; Q16 *; Q17 +1656; Q19 -39; Q20 +1773; Q21 +604 | +62 | - |
...
> 译注：图 10 将 Q4 的子查询扁平化收益标为 `44,624%`，表 3 对应单元格则标为 `+446`；译文分别保留两处源文数值。这是原文内部不一致。
...
为 TPC-H 实现优化时必须警惕基准缺少 SQL 边界情况。除 Q13 外，TPC-H 没有 NULL 和 outer join，且所有 join 都是 equi-join，很容易诱使开发者写出在 non-inner 或 non-equi join 加入后不再正确的计划变换。SQLite [sqllogictest](https://www.sqlite.org/sqllogictest/doc/trunk/about.wiki) 与 [SQLsmith](https://github.com/anse1/sqlsmith) 提供更全面测试；理想方案是把查询计划变换的自动等价验证 [13, 14] 集成进开发流程。

即使优化对 TPC-H 正确且有益，也不保证适用于其它基准或真实应用，尤其 TPC-H 数据分布过于均匀。开发者应补充 TPC-DS、JCC-H [8]、Join Order Benchmark [29] 或带倾斜的 Star Schema Benchmark [48]，确认优化器能处理 skew。复杂优化还存在固定成本；分析型 TPC-H 中优化器成本很少占主导，不只面向分析负载的系统还应运行 TPC-C/TPC-E 等短查询事务负载，避免计划优化比执行本身更昂贵。

高效迭代要求尽量减少单轮 benchmark 的时间与步骤，包括缩短编译、便捷切换 scale factor、线程数、query stream 数，以及 isolated/mixed query execution。除 Hyrise 的 `hyriseBenchmarkTPCH` 外，我们认为 DuckDB benchmark suite 是良好范例；理想情况下，这些设施应由基准而非每个 DBMS 开发者重复提供 [34]。

还必须控制外部性能影响。CPU frequency scaling、后台进程等通常通过重复运行缓解，但另一些影响会跨多轮持续存在：一处代码修改会改变 binary 大小，让 hot loop 跨越页面边界；同一源码的增量重建与全新重建也可能因链接顺序变化而具有不同局部性和 cache 行为 [33]。在我们的 SIMD scan 极端案例中，这类差异让性能下降最多 10%。现有缓解方法 [12, 16] 已过时或难集成，因此我们呼吁编译器研究继续解决这一问题；当前可用 continuous benchmarking 区分离群点与真实回退，DuckDB 的 [逐查询基准结果](https://www.duckdb.org/benchmarks/individual_results/Q02) 同样给出良好示范。

## 7. 相关工作

据我们所知，这是首次系统比较多个计划级阻塞点的影响，但单项优化已有研究。Neumann、Radke 比较 14 种连接排序算法的优化成本 [40]，结论是“TPC-H 对连接排序算法没有挑战”，与本文发现其相关性低一致。Hellerstein、Stonebraker 很早指出除谓词下推外，谓词次序也越来越重要 [53]；当时原因是 UDF 成本不同，列式内存系统中还受 CPU cache、position list 写入和 branch prediction 影响 [49]。Nica 等人研究物理局部性、partition pruning 与列相关性 [41]，并用同样方式聚簇 `lineitem`、`orders` 模拟企业数据老化；其剪枝比例相近，但并行执行中 cold storage 访问延迟成为瓶颈，难以兑现收益。

执行引擎阻塞点更难隔离，因为同一问题有多种实现且彼此耦合。例如算术算子性能（CP4.1a [11]）可用向量化改善，解释器开销（CP4.1d）可用 JIT 改善，两者很难统一比较。Kersten 等人构建专用系统，在 Q1、Q3、Q6、Q9、Q18 上隔离比较编译和向量化，没有发现绝对占优方案 [27]。

数据库基准承担两个难兼容的目标：厂商比较需要尽量覆盖真实用例并防止投机捷径；研究比较则要求易实现、易复现。TPC-DS 几乎在每个维度都比 TPC-H 复杂 [35, 44]，对四个匿名系统的资源分析显示其覆盖 CPU、内存、网络的广泛需求 [45]，但完整实现很难，少有论文跑全套；其它工作会因 SQL 特性缺失或优化不完整只选部分查询，被批评为以营销之名做不科学比较 [21]。

TPC 也承认其基准已经极难开发和运行 [34]，因而推出较易执行的 TPC Express 系列；TPCx-HS [36] 在研究界仍不如 TPC-H/TPC-DS 普及。CH-benCHmark [15] 结合 TPC-C 事务与 TPC-H 分析挑战。DBmbench [52] 则用简化的 µTPC-C、µTPC-H 降低复杂度；它只保留三条查询，反映微架构挑战，却也移除了大多数计划级阻塞点，更适合直接比较执行引擎。

Vogelsgesang 等人首次把 TPC-H 阻塞点与真实数据集比较 [56]，发现列相关性等部分阻塞点较为 TPC-H 特有，同时确认大 `IN` 列表及另外两个执行侧阻塞点的重要性。本文的新增贡献是在统一实验设置中隔离多个逻辑阻塞点并量化影响。
