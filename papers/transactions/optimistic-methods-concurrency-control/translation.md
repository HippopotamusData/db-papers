...
我们考虑如何为一个组织成对象集合的数据库提供共享访问。假定其中始终存在一些特别指定的根对象；非根对象只能先访问某个根，再沿对象中的指针访问。任何能够保持数据完整性约束的一系列数据库访问，都称为事务（参见 [4]）。

若目标是最大化数据库访问吞吐量，至少在两种情况下需要高度并发。第一，数据库可能大到无法全部装入主存，因此访问过程中必须在主存和辅存之间换入、换出对象。一个事务等待辅存访问时，其他事务应能继续执行。第二，即使数据库完全位于主存，也可能存在多个处理器。在这两种情况下，并发度过低都会使硬件得不到充分利用。
...
显式分配事务号时，首先要决定如何分配。事务号必须按顺序分配：如果 $T_i$ 在 $T_j$ 开始以前已经完成，就必须有 $t(i)\lt{}t(j)$。我们使用全局整数事务号计数器 `tnc`；需要事务号时先将它加一，再返回新值。事务号还必须在验证之前取得，因为验证条件需要知道正在验证事务的事务号。
...
乐观方法希望尽可能立即验证事务，以改善响应时间。因此，我们在读阶段末尾分配事务号。这样，只要 $t(i)\lt{}t(j)$，条件 3 中“ $T_i$ 的读阶段先于 $T_j$ 的读阶段完成”这一要求就会自动成立。
