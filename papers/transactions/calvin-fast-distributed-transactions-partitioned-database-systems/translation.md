...
Calvin 的第二种检查点模式紧密基于 Cao 等人的 Zig-Zag 算法 [10]。对于给定数据存储中的每条记录，Zig-Zag 保存两个副本 $AS[K]\relax_0$ 和 $AS[K]\relax_1$，另为每条记录保存两个位 $M_R[K]$ 与 $M_W[K]$，其中 $K$ 是记录键。 $M_R[K]$ 指定读取记录 $K$ 时使用哪个版本， $M_W[K]$ 指定更新记录 $K$ 时覆盖哪个版本。因此，记录 $K$ 的新值始终写入 $AS[K]\relax_{M_W[K]}$；每次更新 $K$ 时，再令 $M_R[K] = M_W[K]$。
...
因此， $AS[K]\relax_{M_W[K]}$ 始终保存记录最新版本，而 $AS[K]\relax_{\neg M_W[K]}$ 始终保存最近检查点周期开始之前写入的最后一个值。异步检查点线程可以遍历每个键 $K$，把 $AS[K]\relax_{\neg M_W[K]}$ 写入磁盘，无须担心记录在此期间被覆盖。
...
$$
\frac{1}{C \cdot D_{2PC}}
$$

其中 $D_{2PC}$ 是两阶段提交协议的持续时间。
