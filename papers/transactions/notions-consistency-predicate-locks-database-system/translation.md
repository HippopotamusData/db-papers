...
发表于 *Communications of the ACM*，1976 年 11 月，第 19 卷第 11 期，第 624–633 页。
...
在上述动作模型中，动作通过给出元组全部字段的值来指定元组。形式上没有问题，但前面的例子表明它不适合当前语境。第一个例子想读取账户 3123 的余额，并不关心该账户的位置；但该模型除了账户号，还要求动作指定账户余额和位置。类似地，第二个事务想读取账户 32123 的余额和位置，再把该账户余额以及账户所在地的资产都增加 50 美元。

> **原文一致性说明：** 本段第一个账户号写作 `3123`，而图 7、上下文及相邻公式均写作 `32123`；译文分别按源 PDF 保留。
...
它忽略 `Location` 字段，读取 `Number` 字段并更新 `Balance` 字段。

> **原文一致性说明：** 原文将该示例称为“读取账户余额”，但公式把 `Balance` 标为 `write`，紧随其后的文字也称该访问更新 `Balance`；译文按源 PDF 保留。
