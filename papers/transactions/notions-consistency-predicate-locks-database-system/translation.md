...
$$
T = ((T,a_i,e_i))\relax_{i=1}^{n},
$$

其中， $T$ 是事务名， $a_i$ 是第 $i$ 步的动作， $e_i$ 是第 $i$ 步作用的实体。（注：序列 $S=s_1,\ldots,s_n$ 记作 $(s_i)\relax_{i=1}^{n}$。满足条件 $C$ 的元素子序列，仿照集合记法记作 $(s_i\in S\mid C(s_i))\relax_{i=1}^{n}$。 $S$ 的第 $i$ 个元素记作 $S(i)$。）
...
$$
S = ((T_i,a_i,e_i))\relax_{i=1}^{m},
$$

使得对每个 $j=1,\ldots,n$，

$$
T_j = ((T_i,a_i,e_i) \in S \mid T_i = T_j)\relax_{i=1}^{m},
$$
...
为证明这一点，考虑任意非良构事务 $T_1=((T_1,a_i,e_i))\relax_{i=1}^{n}$。那么对某个步骤 $k$， $T_1$ 在第 $k$ 步之前没有一直锁定 $e_k$。考虑一个良构的两阶段事务
...
$$
S=(T_1(i))\relax_{i=1}^{k-1}T_2(1),T_2(2),T_1(k),T_2(3),T_2(4),(T_1(i))\relax_{i=k+1}^{n}
$$
