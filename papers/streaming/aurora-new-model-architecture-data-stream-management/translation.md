...
**作者：** Daniel J. Abadi¹，Don Carney²，Uğur Çetintemel²，Mitch Cherniack¹，Christian Convey²，Sangdon Lee²，Michael Stonebraker³，Nesime Tatbul²，Stan Zdonik²

**机构与邮箱：**

- ¹ Department of Computer Science, Brandeis University, Waltham, MA 02254, USA；`{dna,mfc}@cs.brandeis.edu`
- ² Department of Computer Science, Brown University, Providence, RI 02912, USA；`{dpc,ugur,cjc,sdlee,tatbul,sbz}@cs.brown.edu`
- ³ Department of EECS and Laboratory of Computer Science, M.I.T., Cambridge, MA 02139, USA；`stonebreaker@lcs.mit.edu`

**出版信息：** *The VLDB Journal*（2003）；DOI：10.1007/s00778-003-0095-z；© Springer-Verlag 2003。

**编辑与日期栏：** 原文照印为 “Edited by ♣. Received: ♣/ Accepted: ♣” 和 “Published online: ♣ 2003”，没有给出可核实的人名或具体日期。
...
底部路径代表临时查询，可随时接到连接点。系统从连接点持久化规范所确定的最早时刻 $T$ 开始处理数据并交付答案，直到显式断开查询分支。因此临时查询等价于一条从 $t _ {now}-T$ 开始执行、直到显式终止的连续查询。
...
这类队列通常放在主存，但 ASM 为任意扩展采用不同方式。磁盘划分为长度可调的固定块 `block_size`，典型环境预计使用 128KB 或更大的块。每个队列先分配一个块，只要不溢出就作为环形缓冲区使用；溢出时，ASM 寻找两个块（尽量连续），把队列动态扩展到 $2\times block\verb0_0size$ 并继续环形管理。队列收缩可类似处理。
...
迭代计算每个方框的输出速率和计算需求，即可得到稳态稳定运行每单位时间所需的最小总计算资源 $min\verb0_0cap$。容量为 $C$ 的系统若 $C\lt{}min\verb0_0cap$，就无法处理期望稳态负载；若

$$
C\times H\lt{}min\verb0_0cap,
$$
...
其中 $A,B_1,\ldots,B_m$ 是属性， $n$ 为非负整数。[^4] 当 $Slack=0$ 且 `GroupBy` 为空时，元组预期按 $A$ 升序到达；凡 $A$ 小于此前某个元组的值，就视为乱序并忽略。[^5]
...
\begin{aligned}
&Join(P,Size\ s,Left\ Assuming\ O_1,Right\ Assuming\ O_2)(S_1,S_2)\\
={}&Join(P,Size\ s,Left\ Assuming\ O' _ 1,Right\ Assuming\ O' _ 2)\\
...
$$
\begin{aligned}
O_1&=Order(On\ A,Slack\ n_1,GroupBy\ A_1,\ldots,A _ {m_1}),\\
O_2&=Order(On\ B,Slack\ n_2,GroupBy\ B_1,\ldots,B _ {m_2}),\\
O' _ 1&=Order(On\ A,GroupBy\ A_1,\ldots,A _ {m_1}),\\
O' _ 2&=Order(On\ B,GroupBy\ B_1,\ldots,B _ {m_2}).
\end{aligned}
$$
...
图 16 给出表达式。第一个方框按 `Time` 假定顺序，用用户定义函数计算全部位置报告的质心；`Timeout` 保证从第一份到最后一份士兵报告的等待不超过 $t$ 秒。第二个 `Aggregate` 维护大小为 2 的窗口，比较最近两次质心计算是否相差至少 100。用户定义窗口函数 $F$ 执行比较，并输出标志 `Diff`；若最新质心比前一个相差至少 10 米，标志设为 `TRUE`。[^source-threshold] 最后的 `Filter` 返回标志为真的质心及其时间戳。
...
[^4]: 属性 A 可以是虚拟属性 `Tup#`，它把流中连续元组映射成连续整数，从而可按到达次序指定顺序，适用于基于元组数定义窗口。
...
[^source-threshold]: 译者注：原文“Query 2”把告警条件写为相差 100 m，但紧随其后的说明把 `Diff` 置真阈值写为 10 m；两处数值不一致，此处均按原文保留。
