...
作者：Yanif Ahmad；Oliver Kennedy、Christoph Koch、Milos Nikolic<br>
单位：约翰斯·霍普金斯大学；洛桑联邦理工学院（École Polytechnique Fédérale de Lausanne）<br>
联系邮箱：yanif@jhu.edu；{christoph.koch, oliver.kennedy, milos.nikolic}@epfl.ch
...
**示例 1** 考虑查询 $Q$，它统计关系 $R$ 与 $S$ 的乘积中的元组数。暂时只考虑在插入操作下维护 $Q$ 的视图。用 $\Delta_R$（相应地，$\Delta_S$）表示向 $R$（相应地，$S$）插入一个元组时视图发生的变化。假设我们同时物化以下四个视图：

- $Q$（0 阶）；
- $\Delta_RQ=\operatorname{count}(S)$ 和 $\Delta_SQ=\operatorname{count}(R)$（一阶）；
...
\begin{cases}
R(\pi_{\operatorname{sch}(R)}\vec t)\cdot S(\pi_{\operatorname{sch}(S)}\vec t),
& \vec t=\pi_{\operatorname{sch}(R)\cup\operatorname{sch}(S)}(\vec t)\\
0,& \text{其他情况}
\end{cases}
$$
...
$$
\operatorname{Sum}_{\vec A;f}R:\quad
\vec a\mapsto\sum_{\pi_{\vec A}(\vec t)=\vec a}R(\vec t)\cdot f(\vec t).
$$

这里 $\pi$ 投影的是记录而不是关系，即从 $\vec t$ 中删除标签不属于列名集合 $\vec A$ 的字段；$\operatorname{sch}(R)$ 表示 GMR $R$ 的列名列表。聚合 $\operatorname{Sum}_{\vec A;f}R$ 与 SQL 查询 `select A, sum(f) from R group by A` 几乎相同，区别是 SQL 把聚合值放入新列，而 $\operatorname{Sum}_{\vec A;f}R$ 把它放入分组元组的重数。聚合还可作为保留重数的投影；查询 $\operatorname{Sum}_{\vec A;1}(R)$ 同时等价于 SQL 查询 `select A from R` 和 `select A, sum(1) from R group by A`。
...
| $\operatorname{Sum}_{AC;1/2}(R\bowtie(S_1+S_2))\langle A,C\rangle$ | 重数 |
| --- | ---: |
...
$$
Q ::= R\mid\{\vec A:\vec a\mapsto c\}\mid Q\bowtie Q\mid Q+Q\mid
\sigma_\phi Q\mid\operatorname{Sum}_{\vec A;f}Q\mid\rho_{\vec A}Q,
$$

其中 $c$ 是有理数，$f$ 是项，$\phi$ 是项上的条件。项使用有理常数和列名上的算术定义。此外，非分组聚合也可以作为项使用（其值即重数），尤其可用在选择条件中。由此可以表达带嵌套聚合的查询。嵌套聚合可以像 SQL 中常见的那样与外部相关。例如，$\sigma_{C<\operatorname{Sum}_{A;B}R}S$ 表示以下 SQL 查询：
...
可以把删除写作 $R-S$；这并非本质上的新操作，因为可以定义 $R-S:=R+(S\bowtie\{\langle\rangle\mapsto-1\})$。这里 $\{\langle\rangle\mapsto-1\}$ 是单例 GMR 构造 $\{\vec A:\vec a\mapsto c\}$ 的零元特例。
...
\Delta(\operatorname{Sum}_{\vec A;f}Q)&:=\operatorname{Sum}_{\vec A;f}(\Delta Q),\\
...
在代数中写作 $\operatorname{Sum}_{\langle\rangle;A*D}(\sigma_{B=C}(R\bowtie S))$。忽略命名差异，这就是示例 2 的查询。对关系 $R$ 应用变化 $\Delta R$、而保持 $S$ 不变（$\Delta S$ 为空）时，该查询的增量为：

$$
\begin{aligned}
\Delta\operatorname{Sum}_{\langle\rangle;A*D}(\sigma_{B=C}(R\bowtie S))
&=\operatorname{Sum}_{\langle\rangle;A*D}(\Delta\sigma_{B=C}(R\bowtie S))\\
&=\operatorname{Sum}_{\langle\rangle;A*D}(\sigma_{B=C}\Delta(R\bowtie S)).
\end{aligned}
$$
...
因此，增量查询为 $\operatorname{Sum}_{\langle\rangle;A*D}(\sigma_{B=C}((\Delta R)\bowtie S))$。□

增量规则适用于批量更新。单元组更新这一特例很有意义，因为它让我们可以进一步简化增量查询，并生成特别高效的视图刷新代码。

**示例 4** 继续示例 3，但现在假设 $\Delta R$ 是插入单个元组 $\langle A:x,B:y\rangle$。增量查询 $\operatorname{Sum}_{\langle\rangle;A*D}(\sigma_{B=C}(\{\langle A:x,B:y\rangle\}\bowtie S))$ 可简化为 $\operatorname{Sum}_{\langle\rangle;x*D}(\sigma_{y=C}S)$。□
...
在代数中等价于 $\operatorname{Sum}_{*;1}(\sigma_{B<\operatorname{Sum}_{\langle\rangle;D}(\sigma_{A>C}(S))}R)$。这里，$R$ 模式中的所有列都是输出变量。在子表达式 $\operatorname{Sum}_{\langle\rangle;D}(\sigma_{A>C}(S))$ 中，$A$ 是输入变量；由于聚合不分组，所以没有输出变量。□
...
**示例 7** 回到示例 4 中带单元组更新的查询 $Q$。该查询的度为 2。二阶增量 $(\Delta_{sgn_R R(x,y)}\Delta_{sgn_S S(z,u)}Q)[x,y,z,u]$ 的值为 $sgn_Rsgn_S\operatorname{Sum}_{\langle\rangle;x*u}(\sigma_{y=z}\{\langle\rangle\})$；这等价于：若 $y=z$，则 $\langle\rangle\mapsto sgn_Rsgn_Sx*u$，否则为 0；其中 $sgn_R,sgn_S\in\{+,-\}$。变量 $x,y$ 是触发器参数，在运行时已绑定，但必须在变量 $z,u$ 上循环。另一方面，只有在 $y=z$ 时触发器右侧才非零。因此可以处处用 $y$ 替换 $z$，消去 $z$。根据视图元变换，使用这一简化后的 $R$ 插入触发器 $+R(x,y)$ 为：
...
**图 2：部分物化的重写规则。双向箭头表示以启发式方式或使用第 5.1 节代价模型来应用的规则。**

1. 查询分解

   $$
   \mathcal M(\operatorname{Sum}_{\vec A\vec B;f_1*f_2}(Q_1\bowtie Q_2))
   \Rightarrow
   \mathcal M(\operatorname{Sum}_{\vec A;f_1}(Q_1))\bowtie
   \mathcal M(\operatorname{Sum}_{\vec B;f_2}(Q_2))
   \tag{1}
   $$
...
3. 输入变量
   ...
   $$
   \mathcal M(\operatorname{Sum}_{\vec A;f(\vec B,\vec C)}
   (\sigma_{\theta(\vec B,\vec C)}(Q)))
   \Rightarrow
   \operatorname{Sum}_{\vec A;f(\vec B,\vec C)}
   (\sigma_{\theta(\vec B,\vec C)}
   (\mathcal M(\operatorname{Sum}_{\vec A\vec B;1}(Q))))
   \tag{3}
   $$
...
4. 嵌套聚合与去相关
   ...
   $$
   \mathcal M(\operatorname{Sum}_{\vec A;f}
   (\sigma_{\theta(Q_N,\vec B)}(Q_O)))
   \Rightarrow
   \operatorname{Sum}_{\vec A;1}
   (\sigma_{\theta(\mathcal M(Q_N),\vec B)}
   (\mathcal M(\operatorname{Sum}_{\vec A\vec B;f}(Q_O))))
   \tag{4}
   $$
...
\equiv\langle(M_{Q,1}\bowtie M_{Q,2}),\{M_{Q,i}:=Q_i\}\rangle.
$$
...
**多项式展开与因式分解。** 如上所述，查询分解只作用于合取查询。为了跨越并进行分解，我们注意到并操作可以穿过聚合求和（即 $\operatorname{Sum}(Q_1+Q_2)=\operatorname{Sum}(Q_1)+\operatorname{Sum}(Q_2)$），并能在连接上分配。

任何查询表达式都可以展开成扁平多项式表示，即一组纯合取查询的并。随后分别对每个合取查询应用查询分解。这一重写规则见图 2 的规则 2。

该重写规则也能反向应用。可以通过识别公共子表达式（规则中的 $Q_L$ 和 $Q_R$）并将其提到并之外，把多项式表达式因式分解为更小的表示。基于代价的优化器大量使用因式分解，以完整探索可能的物化决策空间。启发式优化器在作出物化决策时不尝试因式分解；不过，物化决策 $\langle M_Q,\{\ldots\}\rangle$ 最终确定后，物化查询 $M_Q$ 自身会通过因式分解得到简化。
...
$$
Q:=\operatorname{Sum}_{\langle\rangle;1}
(\sigma_{\operatorname{Sum}_{\langle\rangle;1}(S)=A}(R)).
$$
...
$$
\Delta_{+S(B')}Q:=
\operatorname{Sum}_{\langle\rangle;1}(\sigma_{\operatorname{Sum}_{\langle\rangle;1}(S)+1=A}(R))-
\operatorname{Sum}_{\langle\rangle;1}(\sigma_{\operatorname{Sum}_{\langle\rangle;1}(S)=A}(R)).
$$
...
**示例 9** 继续示例 8，$\Delta_{+S(B')}Q$ 的物化决策使用两个物化视图：$M_{Q,1}:=\operatorname{Sum}_{\langle\rangle;1}(S)$ 和 $M_{Q,2}:=R$，并各自使用两次。然而，$\Delta_{+R(A')}Q$ 的度自然比 $Q$ 低，因此会整体物化。□

另外我们观察到，对某些查询，在某些更新下完全重新计算 $Q$ 更高效。考虑嵌套聚合的一般形式：

$$
Q:=\operatorname{Sum}_{\vec A;f_1}
(\sigma_{(\operatorname{Sum}_{\vec A\vec B;f_2}(Q_O))}).
$$
...
$$
cost_e(Q)=|dom(Q)|+
\sum_{\{Q_i\mid\operatorname{Sum}(Q_i)\in\mathbb Q\}}|dom(Q_i)|.
$$

$Q$ 的维护代价以维护 $\vec M_Q$ 中所有 $M_{Q,i}$ 的代价为基础。对出现在 $M_{Q,i}$ 中的每个关系 $R_j$ 的每次变化，都必须更新 $M_{Q,i}$。若 $M_{Q,i}$ 的增量查询使用物化决策 $\langle Q'_{i,j},\vec M_{Q_{i,j}}\rangle$ 物化，且向 $R_j$ 的插入速率为 $rate_{R_j}$，则维护 $M_{Q,i}$ 的代价为：

$$
cost_m(M_{Q,i})=
\sum_{R_j}rate_{R_j}\cdot cost_e(Q'_{i,j})+
...
$$
\operatorname{Sum}_{CK;QTY}
(\sigma_{100<\operatorname{Sum}_{\langle\rangle;QTY'}
...
$$
Q_{ns}:=(100<\operatorname{Sum}_{\langle\rangle;QTY'}
...
$$
\mathcal M(\operatorname{Sum}_{\langle CK\rangle;1}(\sigma_{CK=ck}C))\bowtie
\mathcal M(\operatorname{Sum}_{\langle\rangle;QTY}
(\sigma_{OK=ok\land(100<\operatorname{Sum}_{\langle\rangle;QTY'}
(\sigma_{ok=OK'}(\rho_{OK',QTY'}LI)))}LI)).
$$

第二个物化映射还可通过规则 1 和 4 进一步简化。$OK$ 绑定到触发器参数 $ok$，这会断开选择谓词与 $LI$ 之间的连接图。随后，因为选择谓词应用于单例，可以安全地只物化谓词中的聚合。应用这些优化可得到以下物化决策（$1:=\{\langle\rangle\mapsto1\}$）：

$$
\mathcal M(\operatorname{Sum}_{\langle CK\rangle;1}(\sigma_{CK=ck}C))\bowtie
\mathcal M(\operatorname{Sum}_{\langle\rangle;QTY}(\sigma_{OK=ok}LI))\bowtie
\operatorname{Sum}_{\langle\rangle;1}
(\sigma_{100<\mathcal M(\operatorname{Sum}_{\langle\rangle;QTY}
(\sigma_{ok=OK}LI))}(1)).
$$
...
$$
Q_{O1}:=\operatorname{Sum}_{CK;1}(C),\qquad
Q_{O2}:=\operatorname{Sum}_{OK;QTY}(LI).
$$

$Q_{O1}[][CK]$ 在插入 $C$ 时由下式维护：

$$
\Delta_{+C\langle ck\rangle}Q_{O1}:=\{\langle CK:ck\rangle\mapsto1\},
$$
...
$$
\begin{aligned}
\Delta_{+LI\langle ok,qty\rangle}Q:=\operatorname{Sum}_{CK;QTY}\big(&
\sigma_{OK=ok\land100<qty+Q_{ns}}
(C\bowtie O\bowtie(LI+\{\langle OK:ok,QTY:qty\rangle\mapsto1\}))\\
&-\sigma_{OK=ok\land100<Q_{ns}}(C\bowtie O\bowtie LI)\big).
\end{aligned}
$$
...
$$
\mathcal M(\operatorname{Sum}_{CK;1}(\sigma_{OK=ok}(C\bowtie O)))\bowtie
\Big((\mathcal M(Q_2)+\{\langle\rangle\mapsto qty\})
\bowtie\sigma_{100<qty+\mathcal M(Q_2)}(1)
-\mathcal M(Q_2)\bowtie\sigma_{100<\mathcal M(Q_2)}(1)\Big),
$$

其中 $Q_2=\operatorname{Sum}_{\langle\rangle;QTY}(\sigma_{OK=ok}LI)$。

除最外层的物化（$C\bowtie O$）外，该表达式中其余四次物化不仅等价，而且都与已在维护的 $Q_{O2}$ 相同。只需物化一个视图：$Q_{LI}:=\operatorname{Sum}_{CK;1}(\sigma_{OK=ok}(C\bowtie O))$。重写物化决策会产生触发器语句 08。
...
DBToaster 编译器把触发器程序生成为 C++ 代码，其中的视图用 Boost MultiIndex 实现；它是一种灵活的主存集合数据结构，支持多种辅助索引类型。我们的编译器内部仍在研究中，超出了本文范围。我们在 Red Hat Enterprise Linux 上评估 DBToaster 的实验性能，机器配有 16 GB 内存和 Intel Xeon E5620 2.4 GHz 处理器（使用单核）。
...
| --- | --- | --- | --- | :---: | :---: |
| Finance | AXF | 2, = | $\lor,<$ | 是 | 0 / 0 |
| Finance | BSP | 2, = | $\land,<$ | 是 | 0 / 0 |
| Finance | BSV | 2, = | 无 | 否 | 0 / 0 |
| Finance | MST | 2, x | $\land,<$ | 是 | 2 / 1 |
| Finance | PSP | 2, x | $\land,<$ | 否 | 2 / 1 |
| Finance | VWAP | 1 | $<$ | 否 | 2 / 1 |
| TPCH | Q3 | 3, = | $\land,<$ | 是 | 0 / 0 |
| TPCH | Q11 | 2, = | 无 | 是 | 0 / 0 |
| TPCH | Q17 | 2, = | $<$ | 否 | 1 / 1 |
| TPCH | Q18 | 3, = | $<$ | 是 | 1 / 2 |
| TPCH | Q22 | 1 | $=,<$ | 是 | 2 / 1 |
...
**图 7：DBToaster 与两个商业查询引擎的比较，单位为每秒刷新次数。DBMS（DBX）和流系统（SPY）两列均给出在每次更新时完整刷新的成本。**

![Q3、Q11、Q17 和 Q18 的运行时间、刷新率与内存曲线](assets/figure-8-tpch-q3-q11-q17-q18.png)
