...
- 滑动窗口算子 $\texttt{seq-window} _ {S2}$ 从队列 $q_2$ 处理输入流 $S_2$ 的元组。它维护概要结构 $\mathrm{Syn} _ 2$，其中包含 $S_2$ 中时间戳位于最近 600 秒内的全部元组；并在队列 $q_5$ 上输出该窗口（一个关系）的插入与删除。具体而言，对于时间戳为 $\tau$ 的 $S_2$ 元组 $s$， $\texttt{seq-window} _ {S2}$ 在时间戳 $\tau$ 把 $s$ 插入该窗口，并在时间戳 $\tau+600$ 秒从窗口删除 $s$。回顾第 11.1 节，算子按时间戳非递减顺序发出全部插入与删除。
