...
首先讨论零样本文本到 SQL 中的问题表示。给定数据库 $D$ 上的自然语言目标问题 $q$，问题表示的目标是最大化 LLM $M$ 生成正确 SQL $s^\ast{}$ 的概率：

$$
\max_{\sigma}\thickspace{} P_M\bigl(s^\ast{} \mid \sigma(q,D)\bigr),
$$
...
| --- | --- | --- | --- | --- | --- | ---: |
| BSₚ | ✗ | ✗ | ✗ | [35] | — | — |
| TRₚ | ✓ | ✗ | ✗ | [27] | CODE-DAVINCI-002 | 69.0 |
| ODₚ | ✓ | ✓ | ✗ | [24] | GPT-3.5-TURBO | 70.1 |
| ODₚ | ✓ | ✓ | ✗ | [35] | GPT-4 | 64.9 |
| CRₚ | ✓ | ✗ | ✓ | [27] | CODE-DAVINCI-002 | 75.6 |
| CRₚ | ✓ | ✗ | ✓ | [6] | CODE-DAVINCI-002 | 71.8 |
| CRₚ | ✓ | ✗ | ✓ | [6] | GPT-3.5-TURBO | 70.7 |
| ASₚ | ✓ | ✗ | ✗ | [44] | — | — |
...
- **基本提示（BSₚ）。** 该表示 [35] 如清单 1 所示，由表模式、带 `Q:` 前缀的自然语言问题和响应前缀 `A: SELECT` 组成。由于没有指令，我们称其为基本提示。
- **文本表示提示（TRₚ）。** 该表示 [27] 如清单 2 所示，用自然语言表示模式与问题，并在开头加入指令；配合 CODE-DAVINCI-002 时在 Spider-dev 零样本场景取得 69.0% 执行准确率。
- **OpenAI 演示提示（ODₚ）。** 它最早用于 OpenAI 官方文本到 SQL 演示 [31]，并在 [24, 35] 中评估。指令、表模式和问题均以 `#` 注释；相比 TRₚ，它额外要求“只补全 SQLite SQL 查询，不要解释”。
- **代码表示提示（CRₚ）。** 该表示 [6, 27] 直接给出 `CREATE TABLE` SQL，并以注释提出问题，能够完整表达列类型、主键和外键等建库信息；[27] 使用 CODE-DAVINCI-002 得到约 75.6% 的正确 SQL。
- **Alpaca SFT 提示（ASₚ）。** 这是为监督微调设计的 Markdown 格式提示 [44]，要求模型依照指令与输入上下文完成任务。我们同时考察它在提示工程和 SFT 中的效果与效率。
...
$$
\max_{Q',\sigma}\thickspace{} P_M\bigl(s^\ast{}\mid\sigma(q,D,Q')\bigr),
\qquad
\text{s.t. } |Q'|=k,\thickspace{} Q'\subset Q.
$$

函数 $\sigma(\cdot,\cdot,\cdot)$ 决定如何表示目标问题 $q$、数据库 $D$ 的模式信息及从 $Q$ 中选出的 $k$ 个样例。我们研究跨领域文本到 SQL，即目标数据库不出现在候选样例数据库中： $D\notin\lbrace{}D_i\mid(q_i,s_i,D_i)\in Q\rbrace{}$。因此，上下文学习包含两个子任务：选择最有帮助的 $Q'$，以及把所选样例的信息组织进提示。
...
- **问题相似度选择（QTSₛ）。** 用预训练语言模型分别嵌入候选问题与目标问题，再以欧氏距离或负余弦相似度度量，并用 $k\text{-NN}$ 选出最相近的 $k$ 个样例 [26]。
- **掩码问题相似度选择（MQSₛ）。** 针对跨领域场景，把所有问题中的表名、列名和值替换为掩码 token，消除领域信息的负面影响，再通过嵌入与 $k\text{-NN}$ 计算相似度 [14]。
- **查询相似度选择（QRSₛ）。** 不直接依赖目标问题，而是用初步模型根据 $q$ 与 $D$ 生成 SQL $s'$，把它视为 $s^\ast{}$ 的近似；随后按 SQL 关键字把候选查询编码成二值句法向量，在兼顾与 $s'$ 的相似度及样例多样性的条件下选出 $k$ 个样例 [27]。
...
- **完整信息组织（FIₒ）。** FIₒ [6, 27] 使用与目标问题相同的表示组织样例。它与目标问题的唯一区别是：样例在末尾 `SELECT` 之后给出完整 SQL。
- **仅 SQL 组织（SOₒ）。** SOₒ [14] 只保留所选样例的 SQL，并在提示前添加说明。它力图在 token 长度有限时容纳更多样例，但删除了问题与 SQL 之间可能有用的映射信息。

FIₒ 保留完整信息，重视样例质量；SOₒ 只保留 SQL，偏重样例数量。由此产生一个问题：是否存在能在样例质量与数量间取得更好平衡的组织方式？

### 3.3 DAIL-SQL

为解决上述样例选择与组织问题，我们提出 DAIL-SQL。受篇幅限制，伪代码见完整版本 [13]。

在样例选择方面，受 MQSₛ 和 QRSₛ 启发，我们提出同时考虑问题与查询的 DAIL Selection（DAILₛ）。它首先屏蔽目标问题 $q$ 与候选集 $Q$ 中问题 $q_i$ 的领域专用词，再按掩码后 $q$ 与 $q_i$ 的嵌入欧氏距离对候选样例排序。同时，它计算预预测 SQL $s'$ 与候选 SQL $s_i$ 的查询相似度。最终，选择准则优先保留问题相似度排序靠前、且查询相似度超过阈值 $\tau$ 的候选项。这样选出的前 $k$ 个样例在问题和查询两方面都具有较高相似度。

为保留问题—SQL 映射并提高 token 效率，我们提出 DAIL Organization（DAILₒ），在质量与数量之间折中。DAILₒ 同时给出问题 $q_i$ 及对应 SQL $s_i$（清单 8）。作为 FIₒ 与 SOₒ 的折中，它保留问题—SQL 映射，同时删除消耗 token 的数据库模式，从而缩短样例。
...
DAIL-SQL 使用 CRₚ 表示目标问题。相比其他表示，CRₚ 包含主键、外键等完整数据库信息，能够为预测 `JOIN` 子句提供有用线索；此外，LLM 在大量代码语料上预训练，可以更自然地理解 CRₚ。

综上，DAIL-SQL 以 CRₚ 表示问题，根据问题和查询两方面的信息选择样例，并以保留问题—SQL 映射的方式组织样例。借助这种提示设计，DAIL-SQL 在 Spider 排行榜取得 86.2% 的执行准确率。它也是灵活、易扩展的方案，例如加入自洽性（self-consistency）[50] 后可达到 86.6%。不过，自洽性只提升 0.4 个百分点，却显著增加耗时和成本，故我们仍聚焦 DAIL-SQL 本身。
...
$$
\min_{\sigma,M^\ast{}}\thickspace{} \sum_{i=1}^{|T|}
\mathcal{L}\relax_{M^\ast{}}\bigl(\sigma(q_i,D_i),s_i\bigr),
$$
...
在一般领域，监督数据的每个条目 $T=\lbrace{}(p_i,r_i)\rbrace{}$ 包含输入提示 $p_i$ 与 LLM 的期望响应 $r_i$。为使训练与推理一致，我们从文本到 SQL 数据生成提示—响应对：给定 $T=\lbrace{}(q_i,s_i,D_i)\rbrace{}$，构造 $T=\lbrace{}(p_i=\sigma(q_i,D_i),r_i=s_i)\rbrace{}$，即以目标问题和数据库表示为提示，以目标 SQL 为响应。数据就绪后，可按算力选择全量微调 [32] 或参数高效微调 [16]。微调得到的 $M^\ast{}$ 随后根据自然语言问题生成查询；训练和推理使用相同的 $\sigma$。后文实验将展示 SFT 的潜力。
...
图 1 表明，ODₚ 适配四种 LLM，并在 GPT-3.5-TURBO 上取得 75.5% 执行准确率；ASₚ 在 GPT-3.5-TURBO、TEXT-DAVINCI-003 和 Vicuna-33B 上表现较差，需要合适的模型才能奏效。出人意料的是，GPT-4 偏好 Din-SQL [35] 的简单 BSₚ，说明强模型可降低表示设计的复杂性。从平均表现看，GPT-4 与 GPT-3.5-TURBO 的零样本能力更强。考虑 GPT-4 成本较高，GPT-3.5-TURBO 配合 ODₚ 可能是更好的零样本选择；对较弱的 TEXT-DAVINCI-003 和 Vicuna-33B，则更适合 ODₚ 与 CRₚ。
...
**外键（FK）。** 外键暗示关系表之间的联系，可能有助于文本到 SQL；原始表示中只有 CRₚ 包含外键。图 2 给其他表示加入外键后发现：在 OpenAI LLM 上，除 GPT-4 的 TRₚ（−0.2%）和 TEXT-DAVINCI-003 的 ASₚ（−0.4%）外，外键均使执行准确率提高 0.6%–2.9%。但它对 Vicuna-33B 的影响不稳定：BSₚ 意外提升 5.0%，ODₚ 和 ASₚ 却下降。
...
**规则暗示（RI）。** 受 ODₚ 优异表现启发，我们考察“不要解释”这一规则。图 3 显示，加入规则后，所有 LLM 的 EM 与 EX 均稳定提高，最大提升分别超过 6% 和 3%；从 ODₚ 中移除规则则使 EM 下降约 2.4%–6.2%，EX 下降 1.3%–2.4%，说明规则暗示很重要。

总之，外键与“不要解释”规则都对文本到 SQL 有益。本实验中，带外键的 ODₚ 配合 GPT-3.5-TURBO 最有效且经济，EM 为 51.5%，EX 为 78.4%。

### 4.3 文本到 SQL 的上下文学习

在少样本场景下，我们用 GPT-4、GPT-3.5-TURBO、TEXT-DAVINCI-003 和 Vicuna-33B 比较不同样例选择与组织策略。根据完整版本 [13] 的单样本预实验，所有实验固定使用表现更好的 CRₚ。

#### 4.3.1 样例选择

为验证问题与查询都很重要，我们计算所选样例与目标实例之间的问题和查询 Jaccard 相似度，并在表 2 中报告平均值。具体做法是去除问题 [48] 与查询 [21] 中的数据库专用信息，再计算余下 token 的 Jaccard 相似度。表中还加入“上限”：它与 DAIL Selection 相似，但用真值查询 $s^\ast{}$ 代替初步预测器生成的查询。真值不会直接交给 LLM，只用于样例选择参考，因此该结果大致代表基于相似度方法的性能上界。
...
表 2 比较 Spider-dev 上 1、3、5 样本场景的选择策略。DAILₛ 总体优于其他策略；5 样本、GPT-4 场景下，DAIL-SQL 的执行准确率达到 82.4%。问题与查询相似度提高通常对应更高 EX，说明二者都很重要。DAILₛ 仍低于“上限”，原因是预预测查询与真值查询之间存在差距，导致查询相似度较低。
...
总体来看，样例选择必须重视问题到 SQL 的映射；同时考虑问题与查询相似度的 DAILₛ 优于其他策略。样例组织方面，DAILₒ 有效但要求较强 LLM。最终，GPT-4 版 DAIL-SQL 在 Spider-dev 与 Spider-Realistic 分别达到 83.5% 和 76.0% 的最高执行准确率。
...
**问题表示的影响。** 最佳 EX 为 CRₚ 的 68.5%。可能原因是 CRₚ 更能激发 LLM 的编码能力；这一现象在 CodeLLaMA-34B 上尤为明显，它使用基于自然语言的 TRₚ 时只有 40.3% EX。
...
图 5：开源 LLM 在 Spider-dev 上的少样本评估。左图为 EM，右图为 EX；问题表示固定为 CRₚ，样例选择固定为 DAIL Selection。
...
**零样本场景。** 图 6 显示，SFT 后各模型与表示相较表 3 均大幅提升。为 SFT 设计的 Alpaca SFT Prompt 优势明显；不同表示及模型规模之间的差距也缩小。可能原因是微调后模型无需任务指令与外键信息也能回答新问题。最佳组合为 LLaMA-13B 加 Alpaca SFT Prompt，EX 为 68.6%；更大模型中，LLaMA-33B 加 CRₚ 达到 69.1%。因此，SFT 对开源 LLM 很有帮助，微调后的 LLaMA-13B 与 30B 在零样本场景已可与 TEXT-DAVINCI-003 相比；此处“30B”沿用原文，前文实验模型及图 6 标注为 33B。

![表 4：监督微调 LLM 在 Spider-dev 上的少样本结果](assets/table-04-finetuned-few-shot.png)

表 4：监督微调 LLM 在 Spider-dev 上的少样本结果。FIₒ、SOₒ 与 DAILₒ 分别表示完整信息、仅 SQL 与 DAIL 组织；EM 与 EX 均为百分比。
...
OpenAI LLM 按 token 数计费，模型运行时间也与 token 长度成正比，因此我们强调提示工程的 token 效率：用更少 token 获得更高准确率。我们回顾 Spider-dev 实验，比较 OpenAI 与开源 LLM 在准确率与 token 数之间的权衡。token 数主要受问题表示与样例组织影响；样例选择统一固定为 DAILₛ。对比还包括 DIN-SQL [35]、STRIKE [27] 和 CBR-ApSQL [14] 等 SOTA。更多效率分析见完整版本 [13]。
...
图 7 显示，在零样本场景中，相比只用规则暗示，加入外键通常以更多 token 换取更高 EX。在少样本场景中，FIₒ 的 token 数是 DAILₒ 和 SOₒ 的数倍，效率很低；DAILₒ 与 SOₒ token 成本相近，但前者配合 GPT-4 达到最高 83.5% EX，因此 DAILₒ 更高效。

与 SOTA 相比，DAIL-SQL 在准确率和效率两方面都优于 DIN-SQL 与 STRIKE。CBR-ApSQL 配合 TEXT-DAVINCI-003 达到 78.2%，仍低于 DAILₛ + FIₒ 的最佳结果。图 7(d) 中，经过文本到 SQL 微调的开源模型效率更高；但如 4.4 节所述，增加样例对开源模型无益，反而降低 token 效率。
