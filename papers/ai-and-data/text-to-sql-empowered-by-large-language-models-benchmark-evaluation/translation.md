...
- **文本表示提示($\mathrm{TR} _ P$)。** 该表示 [27] 如清单 2 所示，用自然语言表示模式与问题，并在开头加入指令；配合 CODE-DAVINCI-002 时在 Spider-dev 零样本场景取得 69.0% 执行准确率。
- **OpenAI 演示提示($\mathrm{OD} _ P$)。** 它最早用于 OpenAI 官方文本到 SQL 演示 [31]，并在 [24, 35] 中评估。指令、表模式和问题均以 `#` 注释；相比 $\mathrm{TR} _ P$，它额外要求“只补全 SQLite SQL 查询，不要解释”。
...
- **仅 SQL 组织($\mathrm{SO} _ O$)。** $\mathrm{SO} _ O$ [14] 只保留所选样例的 SQL，并在提示前添加说明。它力图在 token 长度有限时容纳更多样例，但删除了问题与 SQL 之间可能有用的映射信息。

$\mathrm{FI} _ O$ 保留完整信息，重视样例质量； $\mathrm{SO} _ O$ 只保留 SQL，偏重样例数量。由此产生一个问题：是否存在能在样例质量与数量间取得更好平衡的组织方式？

### 3.3 DAIL-SQL

为解决上述样例选择与组织问题，我们提出 DAIL-SQL。受篇幅限制，伪代码见完整版本 [13]。

在样例选择方面，受 $\mathrm{MQS} _ S$ 和 $\mathrm{QRS} _ S$ 启发，我们提出同时考虑问题与查询的 DAIL Selection($\mathrm{DAIL} _ S$)。它首先屏蔽目标问题 $q$ 与候选集 $Q$ 中问题 $q_i$ 的领域专用词，再按掩码后 $q$ 与 $q_i$ 的嵌入欧氏距离对候选样例排序。同时，它计算预预测 SQL $s'$ 与候选 SQL $s_i$ 的查询相似度。最终，选择准则优先保留问题相似度排序靠前、且查询相似度超过阈值 $\tau$ 的候选项。这样选出的前 $k$ 个样例在问题和查询两方面都具有较高相似度。
...
DAIL-SQL 使用 $\mathrm{CR} _ P$ 表示目标问题。相比其他表示， $\mathrm{CR} _ P$ 包含主键、外键等完整数据库信息，能够为预测 `JOIN` 子句提供有用线索；此外，LLM 在大量代码语料上预训练，可以更自然地理解 $\mathrm{CR} _ P$。

综上，DAIL-SQL 以 $\mathrm{CR} _ P$ 表示问题，根据问题和查询两方面的信息选择样例，并以保留问题—SQL 映射的方式组织样例。借助这种提示设计，DAIL-SQL 在 Spider 排行榜取得 86.2% 的执行准确率。它也是灵活、易扩展的方案，例如加入自洽性（self-consistency）[50] 后可达到 86.6%。不过，自洽性只提升 0.4 个百分点，却显著增加耗时和成本，故我们仍聚焦 DAIL-SQL 本身。
...
**外键（FK）。** 外键暗示关系表之间的联系，可能有助于文本到 SQL；原始表示中只有 $\mathrm{CR} _ P$ 包含外键。图 2 给其他表示加入外键后发现：在 OpenAI LLM 上，除 GPT-4 的 $\mathrm{TR} _ P$（−0.2%）和 TEXT-DAVINCI-003 的 $\mathrm{AS} _ P$（−0.4%）外，外键均使执行准确率提高 0.6%–2.9%。但它对 Vicuna-33B 的影响不稳定： $\mathrm{BS} _ P$ 意外提升 5.0%， $\mathrm{OD} _ P$ 和 $\mathrm{AS} _ P$ 却下降。
...
**规则暗示（RI）。** 受 $\mathrm{OD} _ P$ 优异表现启发，我们考察“不要解释”这一规则。图 3 显示，加入规则后，所有 LLM 的 EM 与 EX 均稳定提高，最大提升分别超过 6% 和 3%；从 $\mathrm{OD} _ P$ 中移除规则则使 EM 下降约 2.4%–6.2%，EX 下降 1.3%–2.4%，说明规则暗示很重要。

总之，外键与“不要解释”规则都对文本到 SQL 有益。本实验中，带外键的 $\mathrm{OD} _ P$ 配合 GPT-3.5-TURBO 最有效且经济，EM 为 51.5%，EX 为 78.4%。

### 4.3 文本到 SQL 的上下文学习

在少样本场景下，我们用 GPT-4、GPT-3.5-TURBO、TEXT-DAVINCI-003 和 Vicuna-33B 比较不同样例选择与组织策略。根据完整版本 [13] 的单样本预实验，所有实验固定使用表现更好的 $\mathrm{CR} _ P$。
...
表 2 比较 Spider-dev 上 1、3、5 样本场景的选择策略。 $\mathrm{DAIL} _ S$ 总体优于其他策略；5 样本、GPT-4 场景下，DAIL-SQL 的执行准确率达到 82.4%。问题与查询相似度提高通常对应更高 EX，说明二者都很重要。 $\mathrm{DAIL} _ S$ 仍低于“上限”，原因是预预测查询与真值查询之间存在差距，导致查询相似度较低。
...
总体来看，样例选择必须重视问题到 SQL 的映射；同时考虑问题与查询相似度的 $\mathrm{DAIL} _ S$ 优于其他策略。样例组织方面， $\mathrm{DAIL} _ O$ 有效但要求较强 LLM。最终，GPT-4 版 DAIL-SQL 在 Spider-dev 与 Spider-Realistic 分别达到 83.5% 和 76.0% 的最高执行准确率。
...
**问题表示的影响。** 最佳 EX 为 $\mathrm{CR} _ P$ 的 68.5%。可能原因是 $\mathrm{CR} _ P$ 更能激发 LLM 的编码能力；这一现象在 CodeLLaMA-34B 上尤为明显，它使用基于自然语言的 $\mathrm{TR} _ P$ 时只有 40.3% EX。
...
OpenAI LLM 按 token 数计费，模型运行时间也与 token 长度成正比，因此我们强调提示工程的 token 效率：用更少 token 获得更高准确率。我们回顾 Spider-dev 实验，比较 OpenAI 与开源 LLM 在准确率与 token 数之间的权衡。token 数主要受问题表示与样例组织影响；样例选择统一固定为 $\mathrm{DAIL} _ S$。对比还包括 DIN-SQL [35]、STRIKE [27] 和 CBR-ApSQL [14] 等 SOTA。更多效率分析见完整版本 [13]。
