...
Dawei Gao\*、Haibin Wang\*、Yaliang Li、Xiuyu Sun、Yichen Qian、Bolin Ding、Jingren Zhou（阿里巴巴集团；\*共同第一作者）

## 摘要

大语言模型（large language model，LLM）已经成为文本到 SQL（Text-to-SQL）任务的新范式。然而，缺乏系统化基准，阻碍了有效、高效且经济的 LLM 文本到 SQL 方案的发展。为应对这一挑战，我们首先对现有提示工程方法开展系统、广泛的比较，覆盖问题表示、样例选择和样例组织，并据实验结果分析各自优缺点。基于这些发现，我们提出集成方案 DAIL-SQL，以 86.6% 的执行准确率刷新 Spider 排行榜并建立新的标杆。

为探索开源 LLM 的潜力，我们在多种场景下考察它们，并通过监督微调进一步提升其性能。结果既表明开源 LLM 在文本到 SQL 上具有潜力，也揭示了监督微调的优势与不足。此外，为构建高效、经济的 LLM 文本到 SQL 方案，我们强调提示工程中的 token 效率，并按这一指标比较以往工作。我们希望我们的研究能加深对 LLM 文本到 SQL 的理解，并启发后续研究和广泛应用。
...
针对上述挑战，我们旨在为基于 LLM 的文本到 SQL 提供全面、系统且公平的基准。基准同时讨论各种提示工程策略的效果与效率，以及使用开源 LLM 的可行性。

为系统、深入地理解文本到 SQL 提示工程，我们首先实证评估既有研究中的多种策略。我们在零样本场景下用不同 LLM 比较典型问题表示，识别各自优缺点；随后在少样本场景下研究样例选择与组织。例如，在选择方面，我们比较不同策略并检验一个假设：LLM 从问题骨架到 SQL 骨架的映射中学习；在组织方面，则比较完整信息、仅 SQL 以及问题—SQL 对三种方案。

随后，我们从上下文学习与监督微调两方面展示开源 LLM 的潜力。实验覆盖不同规模、不同提示工程策略的开源模型，观察到模型规模增大以及良好对齐 [32] 带来的显著收益。为进一步提升性能，我们用多种问题表示微调并评估开源模型，证明表示策略在监督微调中同样关键。我们也发现微调后上下文学习能力会下降，这仍需进一步研究。

为了得到更经济、高效的方案，我们还以 token 效率评估不同策略，目标是在更少 token 下获得可观性能。分析覆盖问题表示、样例选择和样例组织的整个提示工程过程。

最后，我们提出集成方案 DAIL-SQL。它把结构知识编码为 SQL 语句，根据骨架相似度选择样例，并从样例中移除跨领域知识以节约 token。DAIL-SQL 以 86.6% 的执行准确率刷新 Spider 排行榜并位列第一；此前最佳结果为 85.3% [35]。我们的主要贡献如下：

- 系统研究 LLM 文本到 SQL 提示工程：在四个 LLM 上比较五种问题表示、两个提示组件、四种样例选择和三种样例组织方式，为如何选择表示及利用上下文学习能力提供依据。
- 据我们所知，我们首次同时探索开源 LLM 在文本到 SQL 上的上下文学习和监督微调，说明 SFT 的潜力并给出相应洞见。
- 从成本效率角度实证比较不同提示，为真实文本到 SQL 应用提供实践指导。
- 提出 DAIL-SQL，在性能与 token 效率之间取得平衡；其 86.6% 执行准确率超过此前最佳方案 1.3 个百分点，且 token 成本更低。
...
按提示中提供的样例数量，提示工程可分为零样本与少样本场景。零样本场景不提供样例，主要挑战是如何有效表示自然语言问题，并纳入对应数据库模式等相关信息 [6, 12, 24, 47]。我们把表示自然语言问题及相关信息的过程称为**问题表示**。少样本场景还需研究如何选择最有帮助的样例，并在提示中妥善组织。LLM 从上下文样例中学习的过程称为**上下文学习** [11]：模型可识别输入提示中的显式或隐式模式，在推理阶段无需任务专用训练即可完成新任务。近期研究 [14, 26, 35] 证实了样例对上下文学习的重要作用。
...
综上，问题表示、上下文学习与监督微调是基于 LLM 的文本到 SQL 的三个关键旋钮。下面将对三者进行系统研究与讨论。

## 3 方法

本节为问题表示、上下文学习与监督微调给出形式化定义，系统梳理现有方案并指出潜在问题。针对这些问题，我们提出 DAIL-SQL，并以 86.6% 的执行准确率刷新 Spider 排行榜。

### 3.1 问题表示

首先讨论零样本文本到 SQL 中的问题表示。给定数据库 $D$ 上的自然语言目标问题 $q$，问题表示的目标是最大化 LLM $M$ 生成正确 SQL $s^*$ 的概率：

$$
\max_{\sigma}\; P_M\bigl(s^* \mid \sigma(q,D)\bigr),
$$

其中，函数 $\sigma(\cdot,\cdot)$ 决定如何表示问题 $q$ 及数据库 $D$ 模式中的有用信息，也可以包含指令、规则暗示与外键。按照这一定义，我们从文献中选取四种代表性零样本表示，并加入监督微调中常用的 Alpaca [44] 表示。表 1 汇总五种表示及原论文报告的信息。
...
| CR$_P$ | ✓ | ✗ | ✓ | [6] | CODE-DAVINCI-002 | 71.8 |
| CR$_P$ | ✓ | ✗ | ✓ | [6] | GPT-3.5-TURBO | 70.7 |
| AS$_P$ | ✓ | ✗ | ✗ | [44] | — | — |
...
- **基本提示（BS$_P$）。** 该表示 [35] 如清单 1 所示，由表模式、带 `Q:` 前缀的自然语言问题和响应前缀 `A: SELECT` 组成。由于没有指令，我们称其为基本提示。
- **文本表示提示（TR$_P$）。** 该表示 [27] 如清单 2 所示，用自然语言表示模式与问题，并在开头加入指令；配合 CODE-DAVINCI-002 时在 Spider-dev 零样本场景取得 69.0% 执行准确率。
- **OpenAI 演示提示（OD$_P$）。** 它最早用于 OpenAI 官方文本到 SQL 演示 [31]，并在 [24, 35] 中评估。指令、表模式和问题均以 `#` 注释；相比 TR$_P$，它额外要求“只补全 SQLite SQL 查询，不要解释”。
- **代码表示提示（CR$_P$）。** 该表示 [6, 27] 直接给出 `CREATE TABLE` SQL，并以注释提出问题，能够完整表达列类型、主键和外键等建库信息；[27] 使用 CODE-DAVINCI-002 得到约 75.6% 的正确 SQL。
- **Alpaca SFT 提示（AS$_P$）。** 这是为监督微调设计的 Markdown 格式提示 [44]，要求模型依照指令与输入上下文完成任务。我们同时考察它在提示工程和 SFT 中的效果与效率。

表 1 表明，不同表示常在不同 LLM、不同框架中评测，难以公平比较；外键、规则暗示等单独组件的作用也不明确。因此，有必要系统研究这些表示，并通过公平比较揭示其利弊。

### 3.2 文本到 SQL 的上下文学习

上述问题表示使 LLM 能在零样本条件下直接输出 SQL，但加入少量样例后，上下文学习可以进一步提升效果。本节讨论两个关键问题：样例选择和样例组织。先给出形式化定义。

给定三元组集合 $Q=\{(q_i,s_i,D_i)\}$，其中 $q_i$ 是自然语言问题，$s_i$ 是数据库 $D_i$ 上对应的 SQL，文本到 SQL 上下文学习的目标是针对目标问题 $q$ 与数据库 $D$，最大化 LLM $M$ 生成正确 SQL $s^*$ 的概率：

$$
\max_{Q',\sigma}\; P_M\bigl(s^*\mid\sigma(q,D,Q')\bigr),
\qquad
\text{s.t. } |Q'|=k,\; Q'\subset Q.
$$

函数 $\sigma(\cdot,\cdot,\cdot)$ 决定如何表示目标问题 $q$、数据库 $D$ 的模式信息及从 $Q$ 中选出的 $k$ 个样例。我们研究跨领域文本到 SQL，即目标数据库不出现在候选样例数据库中：$D\notin\{D_i\mid(q_i,s_i,D_i)\in Q\}$。因此，上下文学习包含两个子任务：选择最有帮助的 $Q'$，以及把所选样例的信息组织进提示。
...
- **问题相似度选择（QTS$_S$）。** 用预训练语言模型分别嵌入候选问题与目标问题，再以欧氏距离或负余弦相似度度量，并用 $k$NN 选出最相近的 $k$ 个样例 [26]。
- **掩码问题相似度选择（MQS$_S$）。** 针对跨领域场景，把所有问题中的表名、列名和值替换为掩码 token，消除领域信息的负面影响，再通过嵌入与 $k$NN 计算相似度 [14]。
- **查询相似度选择（QRS$_S$）。** 不直接依赖目标问题，而是用初步模型根据 $q$ 与 $D$ 生成 SQL $s'$，把它视为 $s^*$ 的近似；随后按 SQL 关键字把候选查询编码成二值句法向量，在兼顾与 $s'$ 的相似度及样例多样性的条件下选出 $k$ 个样例 [27]。

这些策略只使用目标问题或目标查询的一侧信息。然而，上下文学习本质上是类比学习 [11]；在文本到 SQL 中，LLM 要学习问题与 SQL 之间的映射。因此，选择样例时同时考虑问题与查询可能更有利，3.3 节将继续讨论。

#### 3.2.2 样例组织

样例组织决定把哪些所选样例信息放进提示。现有策略主要分为完整信息组织和仅 SQL 组织，分别见清单 6 与清单 7。`${DATABASE_SCHEMA}` 表示数据库模式，`${TARGET_QUESTION}` 表示清单 4 的问题表示。
...
- **完整信息组织（FI$_O$）。** FI$_O$ [6, 27] 使用与目标问题相同的表示组织样例。它与目标问题的唯一区别是：样例在末尾 `SELECT` 之后给出完整 SQL。
- **仅 SQL 组织（SO$_O$）。** SO$_O$ [14] 只保留所选样例的 SQL，并在提示前添加说明。它力图在 token 长度有限时容纳更多样例，但删除了问题与 SQL 之间可能有用的映射信息。

FI$_O$ 保留完整信息，重视样例质量；SO$_O$ 只保留 SQL，偏重样例数量。由此产生一个问题：是否存在能在样例质量与数量间取得更好平衡的组织方式？

### 3.3 DAIL-SQL

为解决上述样例选择与组织问题，我们提出 DAIL-SQL。受篇幅限制，伪代码见完整版本 [13]。

在样例选择方面，受 MQS$_S$ 和 QRS$_S$ 启发，我们提出同时考虑问题与查询的 DAIL Selection（DAIL$_S$）。它首先屏蔽目标问题 $q$ 与候选集 $Q$ 中问题 $q_i$ 的领域专用词，再按掩码后 $q$ 与 $q_i$ 的嵌入欧氏距离对候选样例排序。同时，它计算预预测 SQL $s'$ 与候选 SQL $s_i$ 的查询相似度。最终，选择准则优先保留问题相似度排序靠前、且查询相似度超过阈值 $\tau$ 的候选项。这样选出的前 $k$ 个样例在问题和查询两方面都具有较高相似度。

为保留问题—SQL 映射并提高 token 效率，我们提出 DAIL Organization（DAIL$_O$），在质量与数量之间折中。DAIL$_O$ 同时给出问题 $q_i$ 及对应 SQL $s_i$（清单 8）。作为 FI$_O$ 与 SO$_O$ 的折中，它保留问题—SQL 映射，同时删除消耗 token 的数据库模式，从而缩短样例。
...
DAIL-SQL 使用 CR$_P$ 表示目标问题。相比其他表示，CR$_P$ 包含主键、外键等完整数据库信息，能够为预测 `JOIN` 子句提供有用线索；此外，LLM 在大量代码语料上预训练，可以更自然地理解 CR$_P$。

综上，DAIL-SQL 以 CR$_P$ 表示问题，根据问题和查询两方面的信息选择样例，并以保留问题—SQL 映射的方式组织样例。借助这种提示设计，DAIL-SQL 在 Spider 排行榜取得 86.2% 的执行准确率。它也是灵活、易扩展的方案，例如加入自洽性（self-consistency）[50] 后可达到 86.6%。不过，自洽性只提升 0.4 个百分点，却显著增加耗时和成本，故我们仍聚焦 DAIL-SQL 本身。

### 3.4 文本到 SQL 的监督微调

在零样本场景中，常见增强方式是前述上下文学习；监督微调则是较少探索但很有前景的替代方案。与其他语言任务类似，我们可用它提升 LLM 在文本到 SQL 下游任务上的性能。

给定 LLM $M$ 和文本到 SQL 训练集 $T=\{(q_i,s_i,D_i)\}$，其中 $q_i$ 与 $s_i$ 分别是数据库 $D_i$ 上的自然语言问题和对应查询，监督微调的目标是最小化经验损失：

$$
\min_{\sigma,M^*}\; \sum_{i=1}^{|T|}
\mathcal{L}_{M^*}\bigl(\sigma(q_i,D_i),s_i\bigr),
$$

其中 $\mathcal{L}$ 衡量生成查询与真值查询之间的差异，$\sigma$ 决定如何表示问题及数据库模式中的有用信息。因此，监督微调包含两个子任务：用监督数据 $T$ 微调 $M$ 得到最优模型 $M^*$，以及寻找最优问题表示 $\sigma$。3.1 节已讨论表示，本节重点讨论数据准备与微调。

在一般领域，监督数据的每个条目 $T=\{(p_i,r_i)\}$ 包含输入提示 $p_i$ 与 LLM 的期望响应 $r_i$。为使训练与推理一致，我们从文本到 SQL 数据生成提示—响应对：给定 $T=\{(q_i,s_i,D_i)\}$，构造 $T=\{(p_i=\sigma(q_i,D_i),r_i=s_i)\}$，即以目标问题和数据库表示为提示，以目标 SQL 为响应。数据就绪后，可按算力选择全量微调 [32] 或参数高效微调 [16]。微调得到的 $M^*$ 随后根据自然语言问题生成查询；训练和推理使用相同的 $\sigma$。后文实验将展示 SFT 的潜力。

## 4 实验

本节先介绍实验设置，再分别比较问题表示、上下文学习和监督微调方案，最后从 token 效率角度比较它们。

### 4.1 设置

**数据集。** 实验使用 Spider [53] 与 Spider-Realistic [9]。Spider 是大型跨领域文本到 SQL 数据集，覆盖 200 个数据库，训练集含 8,659 个样例，开发集含 1,034 个样例；每个样例包含针对某个数据库的自然语言问题及对应 SQL。测试集未公开，因此我们以 Spider-dev 评测。Spider-Realistic 是更具挑战的变体：它从 Spider-dev 选取 508 个样例，人工改写问题但保持 SQL 不变。在少样本场景下，无论测试 Spider-dev 还是 Spider-Realistic，都使用 Spider 训练集作为候选样例。

**指标。** 遵循 [57]，使用精确集合匹配准确率（exact-set-match accuracy，EM）与执行准确率（execution accuracy，EX）。EM 比较预测 SQL 与真值 SQL 的关键字集合；EX 比较二者在若干数据库实例上的执行结果。一个问题可能存在多条有效 SQL，因此 EX 通常能更准确地估计模型性能。评测脚本来自 <https://github.com/taoyds/test-suite-sql-eval>。

**LLM。** 为公平比较，所有方法使用相同最大上下文长度：OpenAI LLM 为 4,096，开源 LLM 为 2,048；另预留 200 token 用于生成响应。默认温度设为 0，以消除随机性影响。后处理遵循既有工作，从响应中提取第一条 SQL 并移除额外输出。更多实现细节见完整版本 [13]。

### 4.2 问题表示

本节在零样本场景下用 GPT-4、GPT-3.5-TURBO、TEXT-DAVINCI-003 和 Vicuna-33B 评估 3.1 节的问题表示。

![图 1：Spider-dev 零样本场景下不同问题表示的结果](assets/figure-01-question-representations.png)

*图 1：Spider-dev 零样本场景下不同问题表示的结果。左图为 EM，右图为 EX。*

图 1 表明，OD$_P$ 适配四种 LLM，并在 GPT-3.5-TURBO 上取得 75.5% 执行准确率；AS$_P$ 在 GPT-3.5-TURBO、TEXT-DAVINCI-003 和 Vicuna-33B 上表现较差，需要合适的模型才能奏效。出人意料的是，GPT-4 偏好 Din-SQL [35] 的简单 BS$_P$，说明强模型可降低表示设计的复杂性。从平均表现看，GPT-4 与 GPT-3.5-TURBO 的零样本能力更强。考虑 GPT-4 成本较高，GPT-3.5-TURBO 配合 OD$_P$ 可能是更好的零样本选择；对较弱的 TEXT-DAVINCI-003 和 Vicuna-33B，则更适合 OD$_P$ 与 CR$_P$。
...
**外键（FK）。** 外键暗示关系表之间的联系，可能有助于文本到 SQL；原始表示中只有 CR$_P$ 包含外键。图 2 给其他表示加入外键后发现：在 OpenAI LLM 上，除 GPT-4 的 TR$_P$（−0.2%）和 TEXT-DAVINCI-003 的 AS$_P$（−0.4%）外，外键均使执行准确率提高 0.6%–2.9%。但它对 Vicuna-33B 的影响不稳定：BS$_P$ 意外提升 5.0%，OD$_P$ 和 AS$_P$ 却下降。
...
**规则暗示（RI）。** 受 OD$_P$ 优异表现启发，我们考察“不要解释”这一规则。图 3 显示，加入规则后，所有 LLM 的 EM 与 EX 均稳定提高，最大提升分别超过 6% 和 3%；从 OD$_P$ 中移除规则则使 EM 下降约 2.4%–6.2%，EX 下降 1.3%–2.4%，说明规则暗示很重要。

总之，外键与“不要解释”规则都对文本到 SQL 有益。本实验中，带外键的 OD$_P$ 配合 GPT-3.5-TURBO 最有效且经济，EM 为 51.5%，EX 为 78.4%。

### 4.3 文本到 SQL 的上下文学习

在少样本场景下，我们用 GPT-4、GPT-3.5-TURBO、TEXT-DAVINCI-003 和 Vicuna-33B 比较不同样例选择与组织策略。根据完整版本 [13] 的单样本预实验，所有实验固定使用表现更好的 CR$_P$。

#### 4.3.1 样例选择

为验证问题与查询都很重要，我们计算所选样例与目标实例之间的问题和查询 Jaccard 相似度，并在表 2 中报告平均值。具体做法是去除问题 [48] 与查询 [21] 中的数据库专用信息，再计算余下 token 的 Jaccard 相似度。表中还加入“上限”：它与 DAIL Selection 相似，但用真值查询 $s^*$ 代替初步预测器生成的查询。真值不会直接交给 LLM，只用于样例选择参考，因此该结果大致代表基于相似度方法的性能上界。
...
表 2 比较 Spider-dev 上 1、3、5 样本场景的选择策略。DAIL$_S$ 总体优于其他策略；5 样本、GPT-4 场景下，DAIL-SQL 的执行准确率达到 82.4%。问题与查询相似度提高通常对应更高 EX，说明二者都很重要。DAIL$_S$ 仍低于“上限”，原因是预预测查询与真值查询之间存在差距，导致查询相似度较低。
...
图 4(a) 与 4(e) 显示，GPT-4 在两个数据集上都随上下文样例增加而稳定受益。采用 DAIL 组织后，其 Spider-dev EX 从 72.3% 升至 83.5%，Spider-Realistic 从 66.5% 升至 76.0%。GPT-3.5-TURBO 与 TEXT-DAVINCI-003 的上下文学习能力有限，增加样例有时反而降低 EX；Vicuna-33B 使用 DAIL 组织时则随样例数持续提升。

不同组织方式体现出模型偏好：GPT-4 在两个数据集上都偏好 DAIL 组织，说明它能有效学习问题—SQL 映射；GPT-3.5-TURBO 的上下文学习增益在四种模型中最小；TEXT-DAVINCI-003 明显偏好完整信息组织，且样例越多差距越大；Vicuna-33B 上，DAIL 组织优于仅 SQL，却不及完整信息。由此可推断，上下文学习能力强的模型（如 GPT-4）最能受益于 DAIL 组织，而较弱模型需要样例提供更多信息。尽管如此，DAIL 组织仍是在高性能下的良好选择，实验最佳 EX 由 GPT-4 配合 DAIL 组织取得。

总体来看，样例选择必须重视问题到 SQL 的映射；同时考虑问题与查询相似度的 DAIL$_S$ 优于其他策略。样例组织方面，DAIL$_O$ 有效但要求较强 LLM。最终，GPT-4 版 DAIL-SQL 在 Spider-dev 与 Spider-Realistic 分别达到 83.5% 和 76.0% 的最高执行准确率。

### 4.4 文本到 SQL 的监督微调

由于微调 OpenAI LLM 成本过高，本节聚焦开源 LLM。已有文本到 SQL 工作很少采用开源模型，其性能也不清楚，因此我们先用不同问题表示、样例选择和组织策略全面评估开源 LLM，再对其监督微调，观察零样本与少样本场景的改善。

#### 4.4.1 开源 LLM

实验选择 LLaMA [45] 及不同规模的对齐变体。这里“对齐”指模型经过训练而变得更有帮助、更无害、更诚实 [1]；后缀 `-7B`、`-13B`、`-33B` 分别表示约 70 亿、130 亿、330 亿参数。
...
表 3 给出 Spider-dev 结果；受篇幅限制，Spider-Realistic 结果见完整版本 [13]。

**问题表示的影响。** 最佳 EX 为 CR$_P$ 的 68.5%。可能原因是 CR$_P$ 更能激发 LLM 的编码能力；这一现象在 CodeLLaMA-34B 上尤为明显，它使用基于自然语言的 TR$_P$ 时只有 40.3% EX。

**模型规模的影响。** LLaMA 与 Vicuna 的规模都与文本到 SQL 性能正相关。LLaMA 的平均 EX 从 9.1% 提升到 34.0%，Vicuna 从 15.3% 提升到 36.6%；参数最多的 LLaMA-2-CHAT-70B 将平均结果提高到 39.6%。

**对齐的影响。** 同规模下，Vicuna 在 Spider-dev 和 Spider-Realistic 上的 EX 均比 LLaMA 高约 5%，表明对齐有益。Falcon-40B 在所有表示上表现都很差，可能是训练数据中缺少专门的代码数据。相反，对齐阶段使用精心收集代码数据的 CodeLLaMA-34B 在相近规模下显著改善；尽管参数只有 LLaMA-2-CHAT-70B 的一半，其平均 EX 仍高 20.6%。这说明模型规模有潜在收益，但训练语料——尤其是任务专用数据——更关键。

#### 4.4.3 开源 LLM 的少样本场景

![图 5：开源 LLM 在 Spider-dev 上的少样本评估](assets/figure-05-open-source-few-shot.png)

*图 5：开源 LLM 在 Spider-dev 上的少样本评估。左图为 EM，右图为 EX；问题表示固定为 CR$_P$，样例选择固定为 DAIL Selection。*

图 5 比较 LLaMA-33B 与 Vicuna-33B。LLaMA-33B 从样例中获益更多，在 5 样本完整信息组织下达到 36.4% EM。就 EX 而言，增加样例在多数情况下有益；完整信息组织在各个 $k$ 上都优于其他策略，Vicuna-33B 的最高 EX 为 51.5%。更多分析见完整版本 [13]。不过，无论零样本还是少样本，开源 LLM 仍明显落后于 OpenAI LLM，下面尝试用监督微调增强它们。

#### 4.4.4 开源 LLM 的监督微调

与上下文学习一样，SFT 可能偏好不同表示。因此，我们先用不同表示构造零样本训练样例，对开源 LLM 进行微调。遵循 [32, 44]，训练时屏蔽提示部分的梯度，只用响应（SQL）更新参数；训练集为 Spider 的 8,659 个样例。更多训练细节见完整版本 [13]。

![图 6：不同微调开源 LLM 在 Spider-dev 上的零样本结果](assets/figure-06-finetuned-zero-shot.png)

*图 6：不同微调开源 LLM 与问题表示在 Spider-dev 上的零样本执行准确率。*

**零样本场景。** 图 6 显示，SFT 后各模型与表示相较表 3 均大幅提升。为 SFT 设计的 Alpaca SFT Prompt 优势明显；不同表示及模型规模之间的差距也缩小。可能原因是微调后模型无需任务指令与外键信息也能回答新问题。最佳组合为 LLaMA-13B 加 Alpaca SFT Prompt，EX 为 68.6%；更大模型中，LLaMA-33B 加 CR$_P$ 达到 69.1%。因此，SFT 对开源 LLM 很有帮助，微调后的 LLaMA-13B 与 30B 在零样本场景已可与 TEXT-DAVINCI-003 相比；此处“30B”沿用原文，前文实验模型及图 6 标注为 33B。

![表 4：监督微调 LLM 在 Spider-dev 上的少样本结果](assets/table-04-finetuned-few-shot.png)

*表 4：监督微调 LLM 在 Spider-dev 上的少样本结果。FI$_O$、SO$_O$ 与 DAIL$_O$ 分别表示完整信息、仅 SQL 与 DAIL 组织；EM 与 EX 均为百分比。*

**少样本场景。** 一个重要问题是：监督微调后，能否继续通过上下文样例提升开源 LLM？表 4 比较微调前后的 LLaMA-7B 与 13B，在 0、1、3、5 样本提示上的表现。出人意料的是，微调模型无法从样例中学习：加入上下文样例使 EM 和 EX 突然下降，加入更多样例也无帮助。可能原因是 LLM 过拟合了零样本提示，导致样例失去作用。
...
OpenAI LLM 按 token 数计费，模型运行时间也与 token 长度成正比，因此我们强调提示工程的 token 效率：用更少 token 获得更高准确率。我们回顾 Spider-dev 实验，比较 OpenAI 与开源 LLM 在准确率与 token 数之间的权衡。token 数主要受问题表示与样例组织影响；样例选择统一固定为 DAIL$_S$。对比还包括 DIN-SQL [35]、STRIKE [27] 和 CBR-ApSQL [14] 等 SOTA。更多效率分析见完整版本 [13]。

![图 7：不同表示在 Spider-dev 上的 token 效率](assets/figure-07-token-efficiency.png)

*图 7：不同表示在 Spider-dev 上的 token 效率。颜色区分问题表示，形状区分样例组织及是否使用外键/规则暗示；形状重叠表示同时使用外键与规则。圆环表示零样本提示，星形表示既有 LLM 少样本 SOTA。*

图 7 显示，在零样本场景中，相比只用规则暗示，加入外键通常以更多 token 换取更高 EX。在少样本场景中，FI$_O$ 的 token 数是 DAIL$_O$ 和 SO$_O$ 的数倍，效率很低；DAIL$_O$ 与 SO$_O$ token 成本相近，但前者配合 GPT-4 达到最高 83.5% EX，因此 DAIL$_O$ 更高效。

与 SOTA 相比，DAIL-SQL 在准确率和效率两方面都优于 DIN-SQL 与 STRIKE。CBR-ApSQL 配合 TEXT-DAVINCI-003 达到 78.2%，仍低于 DAIL$_S$ + FI$_O$ 的最佳结果。图 7(d) 中，经过文本到 SQL 微调的开源模型效率更高；但如 4.4 节所述，增加样例对开源模型无益，反而降低 token 效率。

综上，token 效率是 LLM 文本到 SQL 实际应用中的关键指标。DAIL-SQL 同时提供较高执行准确率和更好的 token 效率，因而更适合现实应用。
...
我们的工作仍有一些局限。开源 LLM 只用 Spider 训练集微调，更多文本到 SQL 数据可能继续提升性能；Spider 与 Spider-Realistic 的数据库也可能不够大，当数据库含大量表时，效果与效率方面会出现新挑战。此外，现有指标优先衡量正确性而非效率，如何促使 LLM 在多个正确候选中生成更高效的 SQL 仍是重要而未解决的问题。我们将继续研究这些局限与开放问题。

## 6 结论

我们从提示工程与监督微调两方面系统研究基于 LLM 的文本到 SQL。我们指出，现有上下文学习方法忽视了问题与查询之间的映射，以及样例质量与数量之间的权衡。为此提出 DAIL-SQL，以 86.6% 执行准确率刷新 Spider 排行榜并位列第一。在监督微调方面，实验展示了开源 LLM 的潜力，强调训练语料与模型规模的重要性，也指出微调后上下文学习能力退化。我们还从效率角度考察现有方案。上述问题都为未来研究带来挑战与机遇；我们希望我们的工作能提供全面认识，为实际应用给出指引，并推动文本到 SQL 继续发展。
