...
$$
e_i = \{e_{i,j}\}_{1 \le j \le m}, \qquad e_{i,j}=\{\mathrm{attr}_j,\mathrm{val}_j\}.
$$
...
**错误检测。** ED 是数据清洗流水线的重要步骤。给定条目 $e$，目标是找出值 $\mathrm{val}_j$ 存在错误的属性 $j$。该任务被表述为分类问题：对给定的 $e$，预测 $\mathrm{val}_j$ 是否正确。
...
**数据插补。** DI 是修复脏数据源的关键步骤。给定一个缺少属性值 $\{\mathrm{attr}_j,\mathrm{NULL}\}$ 的条目 $e$，目标是推断缺失值。缺失值的全部可能取值事先未知。
...
$$
\mathrm{serialize}(e) \coloneqq \mathrm{attr}_1:\mathrm{val}_1\ \ldots\ \mathrm{attr}_m:\mathrm{val}_m
$$
