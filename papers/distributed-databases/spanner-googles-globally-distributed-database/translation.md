...
![图 6：TrueTime 的 $\epsilon$ 分布，在 timeslave daemon 刚轮询完 time master 后采样。左图绘制第 90、99 和 99.9 百分位；右图展示 4 月 13 日维护造成的尖峰。](assets/spanner-fig06-truetime-epsilon.png)
...
因此 $lease_i.end < lease_{i+1}.start$，两个 leader lease interval 不相交。
