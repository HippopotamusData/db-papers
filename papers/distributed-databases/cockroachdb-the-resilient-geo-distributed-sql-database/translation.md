...
作者：Rebecca Taft、Irfan Sharif、Andrei Matei、Nathan VanBenschoten、Jordan Lewis、Tobias Grieger、Kai Niemi、Andy Woods、Anne Birzin、Raphael Poss、Paul Bardea、Amruta Ranade、Ben Darnell、Bram Gruneir、Justin Jaffray、Lucy Zhang、Peter Mattis。

机构：Cockroach Labs, Inc.；联系邮箱：`sigmod2020@cockroachlabs.com`。

本文发表于 SIGMOD ’20（2020 年 6 月 14–19 日，美国俄勒冈州波特兰），DOI：<https://doi.org/10.1145/3318464.3386134>。

会议分类：Industry 3: Cloud and Distributed Databases（工业赛道 3：云与分布式数据库）。
...
现代事务处理工作负载正日益走向地理分布。这一趋势既源于全球企业构建可扩展应用的需要，也源于它们出于性能和监管原因对数据驻留位置进行细粒度控制的需要。
...
本节先在第 2.1 节概述 CRDB 的架构；第 2.2、2.3 节再说明系统如何通过复制与数据分布提供容错、高可用和地理分布式分区。

### 2.1 CockroachDB 架构

CRDB 采用标准无共享架构 [62]，任意数量节点同时承担数据存储和计算。节点可以共置于一个数据中心，也可以分散到全球各地；客户端可连接集群中任意节点。单节点内部自上而下分为 SQL、事务 KV、分布、复制与存储五层。
...
CRDB 通过数据复制（第 2.2.1 节）、故障时的自动恢复机制（第 2.2.2 节）和战略性数据放置（第 2.2.3 节）保证容错与高可用。

#### 2.2.1 使用 Raft 复制

CRDB 使用 Raft 共识算法 [46] 进行一致复制。一个 Range 的副本构成 Raft 组，其中长期 leader 协调写入，其他副本为 follower。复制单位是 command，即将应用到底层存储引擎的一组低级编辑。Raft 在副本间维护一致、有序的日志；日志项提交后，各副本分别把命令应用到本地存储。
...
CRDB 的副本与 leaseholder 放置机制支持广泛的数据放置策略，使用户既能满足数据驻留要求，也能在性能与容错之间作出取舍。系统支持的若干多区域模式如下：

- Geo-partitioned replicas：按地理访问位置把表分区，并将每个分区（一组 Range）固定在特定区域。该策略支持低延迟的区域内读写，也能承受可用区（AZ）故障；但整个区域故障会导致固定在该区域的数据不可用。该策略还可用于满足数据驻留要求。
- Geo-partitioned leaseholders：leaseholder 固定到访问区域，其他副本分布在剩余区域。该策略提供本地读和区域故障生存能力，但跨区域写入更慢。
- Duplicated indexes：在不同区域复制索引副本，并将每个索引的 leaseholder 固定到对应区域。该策略可服务快速本地读并保持区域故障能力，但写放大和跨区域写入成本更高；它适合更新不频繁或无法绑定到特定地理位置的数据。
...
第 3.1 节先概述事务模型，第 3.2 节说明如何保证事务原子性，第 3.3、3.4 节介绍保证可串行化隔离的并发控制机制，第 3.5 节则概述 follower 副本如何服务一致的历史读。

### 3.1 概览

SQL 事务从 SQL 连接所到达的 gateway 节点开始。该节点与 SQL 客户端交互式收发请求，并充当事务协调器，编排关联事务并最终提交或中止它。应用通常连接到地理位置较近的 gateway，以尽量降低延迟。协调器为事务分配时间戳、跟踪写集合和读集合，并把每个 KV 操作路由到相应 Range 的 leaseholder。
...
这些 latch 为并发且重叠的请求提供互斥。Evaluation 会生成描述所需修改的底层 command，以及返回客户端的 response（例如写成功，或读出的值）。读操作不需要复制；写操作需要通过 Raft 达成共识并由每个副本应用命令。提交操作则在写入复制后通知协调器。算法没有展开 evaluation 阶段，因为事务可能在这里遇到其他事务的未提交写，或时间戳过于接近、暂时无法判断正确事务顺序的写；以下各节会讨论这些情形以及 CRDB 如何保证原子性和可串行化隔离。
...
若 key 已在时间戳 $t_b \ge t_a$ 被读取，则不能再于 $t_a$ 写入。CRDB 把写事务的提交时间戳推进到 $t_b$ 之后。
...
提交时间戳从 $t_a$ 推进到 $t_b\gt{}t_a$ 后，为维持可串行化，读时间戳必须同步推进。只有能证明事务在 $t_a$ 读过的数据在区间 $(t_a, t_b]$ 没有更新时，推进才安全；若发生变化，事务必须重启。尚未向客户端交付结果时，CRDB 可在内部重试；为了提高重试成功率，系统会推迟重启，让原事务先在打算写入的 key 上放置作为写锁的 intent。结果已经交付时，则通知客户端丢弃结果并重启。

协调器在内存预算范围内保存读集合。Read refresh 会重新扫描这些 key，检查区间内是否出现 MVCC 版本；这等价于检测 PostgreSQL SSI [8, 49] 跟踪的 rw 反依赖。为避免维护完整依赖图，CRDB 与 PostgreSQL 一样可能产生假阳性，即中止一个理论上可继续的事务。扫描遇到不确定值时也会尝试 refresh；成功推进时间戳后，该值就能作为过去值返回。

### 3.5 Follower 读取

只读查询可通过 `AS OF SYSTEM TIME` 在非 leaseholder 副本上读取足够久远的历史时间戳。副本要安全服务时间戳 $T$，必须知道 leaseholder 已不再接受时间戳满足 $T' \le T$ 的新写，并且自己已应用影响该 MVCC 快照的全部 Raft 日志前缀。
...
本节先介绍 CRDB 用于描述时间戳顺序的混合逻辑时钟（第 4.1 节），再说明该时钟方案如何在宽松同步条件下高效提供事务间的单 key 线性一致性（第 4.2 节），最后讨论系统违反可配置时钟同步边界时的行为（第 4.3 节）。
...
事务创建时，从协调器本地 HLC 取得暂定提交时间 $commit\verb0_0ts$，不确定区间为：

$$
[commit\verb0_0ts,\ commit\verb0_0ts + max\verb0_0offset].
$$

时间戳低于 $commit\verb0_0ts$ 的值显然属于过去，读时可见、写时可覆盖。没有全局同步时，一个真实时间上更晚的事务，其暂定时间戳可能比因果前驱早至多 $max\verb0_0offset$。若事务遇到时间戳高于 $commit\verb0_0ts$ 但落在不确定区间内的值，就执行 uncertainty restart：把暂定提交时间推进到该值之后，同时保持不确定区间上界不变。这样区间内所有值都被当作过去写，单 key 操作顺序因而与真实时间一致。
...
CRDB 使用两道保护维持隔离。其一，lease 带起止时间；leaseholder 不能服务 MVCC 时间戳高于其 lease 区间上界的读请求，也不能服务 MVCC 时间戳位于该 lease 区间之外的写请求，并且各 lease 区间互不重叠。其二，每次写入 Raft 日志都携带提议时所依据的 lease 序号；复制成功后再与当前有效 lease 核对，不一致就拒绝。Lease 变更本身也写入同一 Raft 日志，因此任一时刻只有一个 leaseholder 能真正修改 Range。第一道保护防止新 leaseholder 的写使旧 leaseholder 已服务的读失效，第二道保护防止旧 leaseholder 的写使新 leaseholder 的读写失效。即使严重偏斜，CRDB 仍保持可串行化隔离。

超过偏移上限仍可能破坏有因果关系事务之间的单 key 线性一致性。若两个事务通过不同 gateway 发起，第二个节点的时钟比第一个慢超过 $max\verb0_0offset$，第一个事务写入的时间戳可能落在第二个事务不确定区间之外，造成第二个事务读不到已完成写，即陈旧读。节点会周期测量彼此偏移；若某节点相对多数节点超过最大偏移的 80%，它会主动终止，以降低这种情况的概率。

## 5. 分布式 SQL

所有用户操作都经过 SQL 层。CRDB 支持 ANSI SQL 的大部分 PostgreSQL 方言 [51]，并加入地理分布式数据库所需的扩展。

本节先说明 SQL 数据模型及其如何映射到下层（第 5.1 节），再介绍 SQL 规划与执行的技术细节（第 5.2、5.3 节）以及 schema change（第 5.4 节）。

### 5.1 SQL 数据模型

每个 SQL 表和索引被存储在一个或多个 Range 中。所有用户数据存储在一个或多个有序索引中，其中一个是主索引。主索引以主键编码，其他列存储在值中；如果 schema 没有显式指定主键，系统会自动生成主键。二级索引以索引键编码，并存储主键列以及索引 schema 中指定的任意数量额外列。CRDB 也支持 hash index，用于避免热点并把负载分散到多个 Range。

### 5.2 查询优化器

CRDB 使用 Cascades 风格 [27] 查询优化器。转换规则用名为 Optgen 的 DSL 编写，Optgen 会编译成 Go 代码并集成到 CRDB 代码库。规则包括 normalization rule 和 exploration rule：前者重写为逻辑等价的规范形式，后者探索 join 重排、算法选择和分布式执行方式等替代计划。

#### 5.2.1 Optgen：查询变换 DSL

优化器用 200 多条变换规则搜索执行计划空间。Optgen 提供定义、匹配和替换计划树算子的语法，并编译为 Go，从而与 CRDB 的其余代码无缝集成；除存储层外，CRDB 全部使用 Go 实现。例如消除双重否定的规则为：
...
在一条数据流内部，CRDB 会依据输入基数和计划复杂度，在逐行执行引擎与向量化执行引擎之间选择。

#### 5.3.1 逐行执行引擎

主执行引擎采用 Volcano [26] 迭代器模型，每次处理一行。CRDB 所有 SQL 功能都在此实现，包括 join、聚合、排序、窗口函数等。
...
所有向量化算子都能处理 selection vector，即紧凑存放尚未被前序过滤移除的行索引，避免每次过滤都物理搬移数据。Merge join 等复杂算子会按是否存在 selection vector 生成不同内部循环。这些优化使单算子加速超过两个数量级，TPC-H [69] 查询最高约加速 4 倍。
...
| 系统 / 指标 | 1,000 warehouses | 10,000 warehouses | 100,000 warehouses |
| --- | ---: | ---: | ---: |
| **CockroachDB** |  |  |  |
...
本节详述过去五年构建 CRDB、并将其打磨为生产级系统过程中获得的经验。

### 7.1 让 Raft 在生产中运行

CRDB 最初选择 Raft 作为共识算法，因为其实现描述精确且被认为易于使用 [46]；生产实践表明，把它嵌入复杂数据库仍有挑战。
...
CRDB 因此实现 Raft 论文 [46] 所述的原子成员变更 Joint Consensus。它仍有中间配置，但写入必须同时获得旧多数派和新多数派，只有旧或新多数派本身失效时才不可用。ZooKeeper [59] 的重配置协议类似。团队发现其实现复杂度并未明显高于默认协议，因此建议生产级 Raft 系统直接采用 Joint Consensus。
...
CRDB 采用 PostgreSQL SQL 方言和网络协议，以复用成熟客户端驱动生态。这一选择显著促进了早期采用，也持续帮助工程团队聚焦决策 [50]。但 CRDB 某些行为不同于 PostgreSQL，客户端代码必须介入，例如 MVCC 冲突后重试事务、配置结果分页。
...
CockroachDB 是一个源代码可用、可扩展的 SQL 数据库，设计使命是“让数据变得简单”。我们新颖的事务协议无需专用硬件，就能在规模化部署中实现可串行化隔离。基于共识的复制提供容错和高可用，也允许从 leaseholder（leader）和 follower 副本进行本地读取优化。地理分区与 Follow the Workload 功能确保数据位于最靠近访问者的位置，尽量减少 WAN 往返造成的延迟。最后，CockroachDB 的 SQL 层既为用户提供 SQL 的灵活性和熟悉体验，又利用 CockroachDB 的分布式特性获得可扩展性与性能。

CockroachDB 已经为数千家组织创造价值，但我们仍在持续迭代其设计，并在每个版本中改进软件。后续版本计划重写存储层、加入地理感知查询优化，并继续改进其他组件。更长期目标是加强运维自动化，使数据库从用户视角真正 serverless。CRDB 已提供全托管服务 [11]，但要隐藏全部运维细节，仍需发展分离式存储、按需扩缩和按使用量计费；如何让地理分布式数据库在这种环境下保持高性能，也是适合独立研究的开放问题。我们期待支持并参与这类研究，继续推进“让数据变得简单”的目标。
...
- [56] Marco Serafini, Essam Mansour, Ashraf Aboulnaga, Kenneth Salem, Taha Rafiq, and Umar Farooq Minhas. 2014. Accordion: Elastic scalability for database systems supporting distributed transactions. Proceedings of the VLDB Endowment 7, 12 (2014), 1035-1046.
...
- [60] Jeff Shute, Radek Vingralek, Bart Samwel, Ben Handy, Chad Whipkey, Eric Rollins, Mircea Oancea, Kyle Littlefield, David Menestrina, Stephan Ellner, et al. 2013. F1: A distributed SQL database that scales. Proceedings of the VLDB Endowment 6, 11 (2013), 1068-1079.
...
- [65] Rebecca Taft, Essam Mansour, Marco Serafini, Jennie Duggan, Aaron J Elmore, Ashraf Aboulnaga, Andrew Pavlo, and Michael Stonebraker. 2014. E-store: Fine-grained elastic partitioning for distributed transaction processing systems. Proceedings of the VLDB Endowment 8, 3 (2014), 245-256.
