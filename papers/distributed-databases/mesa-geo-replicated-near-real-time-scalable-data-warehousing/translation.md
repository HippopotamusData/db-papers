...
![图 1](assets/mesa-fig01-related-tables.png)
...
![图 2](assets/mesa-fig02-updates.png)
...
![图 3](assets/mesa-fig03-delta-compaction.png)
...
![图 4](assets/mesa-fig04-controller-worker.png)
...
![图 5](assets/mesa-fig05-query-processing.png)
...
![图 6](assets/mesa-fig06-multidatacenter-update.png)

图 6：多数据中心 Mesa 部署中的更新处理。
...
![图 7](assets/mesa-fig07-update-performance.png)

图 7：单个数据源连续七天的更新性能。左图是每秒更新行数，右图是各更新批耗时区间所占百分比。
...
![图 8](assets/mesa-fig08-query-performance.png)

图 8：同一数据源中各表连续七天的查询性能指标，依次为每日查询数、返回行数、平均延迟和第 99 百分位延迟。
...
![图 9](assets/mesa-fig09-rows-read-skipped-returned.png)

图 9：查询处理期间读取、跳过和返回的行数。
...
![图 10](assets/mesa-fig10-query-throughput-scalability.png)
...
![图 11](assets/mesa-fig11-growth-latency.png)

图 11：连续 24 个月的增长与延迟指标。
