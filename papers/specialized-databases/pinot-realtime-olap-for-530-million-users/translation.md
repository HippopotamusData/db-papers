...
全部署名者均来自 LinkedIn Corp.（美国加利福尼亚州山景城）：

| 署名 | 联系邮箱 |
| --- | --- |
| Jean-François Im | jfim@linkedin.com |
| Kishore Gopalakrishna | kgopalak@linkedin.com |
| Subbu Subramaniam | ssubrama@linkedin.com |
| Mayank Shrivastava | mshrivas@linkedin.com |
| Adwait Tumbde | atumbde@linkedin.com |
| Xiaotian Jiang | xajiang@linkedin.com |
| Jennifer Dai | jdai@linkedin.com |
| Seunghyun Lee | snlee@linkedin.com |
| Neha Pawar | npawar@linkedin.com |
| Jialiang Li | jlli@linkedin.com |
| Ravi Aringunram | raringun@linkedin.com |
...
我们介绍 Pinot：LinkedIn 生产环境使用的一套统一系统。它每秒可服务数万次分析查询，能从流式数据源近实时摄取数据，并能满足大型 Web 业务的运维要求。我们还将 Pinot 与架构相近的 Druid 做性能比较。
...
基于上述标准，我们介绍 Pinot：一套为 LinkedIn 全体用户提供近实时分析的统一系统。它既支持新闻流定制、“谁查看了我的主页”（Who Viewed My Profile，WVMP）等面向大量终端用户的简单高吞吐查询，也支持广告主分析和内部分析所需的复杂仪表盘。我们总结我们在大规模运行 Pinot 过程中得到的经验，比较 Pinot 不同索引技术在生产工作负载上的性能和可扩展性。我们还将 Pinot 与架构相近的分析系统 Druid [30] 比较。
...
Pinot 是 LinkedIn 开发的可扩展分布式 OLAP 数据存储，用来以低延迟提供实时分析。它面向不可变、仅追加数据的分析用例做了优化，数据新鲜度可达数秒量级。我们多年来一直在 LinkedIn 的生产环境中运行 Pinot，部署覆盖数百台服务器和数百张表，每秒处理数千次查询。Pinot 尤其支撑 WVMP、新闻流定制等要求极低延迟的用户侧应用，以及允许内部业务分析人员切片和切块数据的仪表盘。
...
**ZooKeeper 与对象存储。** ZooKeeper 既是持久元数据存储，也是集群节点间的通信机制。集群状态、segment 分配和元数据都经 Helix 存入 ZooKeeper；segment 数据本身则位于持久对象存储。在 LinkedIn 内部，Pinot 使用本地 NFS 挂载点存储数据；在 LinkedIn 数据中心之外运行时，我们也使用过 Azure Disk Storage。

### 3.3 常见操作

下面我们说明 Pinot 中几种常见操作的实现。
...
这套云友好架构使我们很容易把 Pinot 迁移到现成的容器执行服务；所需代码改动仅限于适配云供应商的对象存储。借助 Kubernetes 等容器管理器，部署和扩容也很方便。

## 4. 扩展 Pinot

Pinot 的若干特性对于在大规模场景下获得可接受性能至关重要。下面我们说明这些特性，并解释它们如何支持面向 LinkedIn 用户的分析查询服务。

### 4.1 查询执行

Pinot 的查询执行模型可以容纳新的算子和查询形态。例如，早期 Pinot 不支持为 `SELECT COUNT(*)` 这类查询生成纯元数据计划。加入这项能力只需修改查询规划器并新增一个基于元数据的物理算子，无需改变整体架构。

Pinot 针对每种数据表示提供专用物理算子，不同数据编码都有相应算子。这种灵活性允许我们加入新的索引类型和用于查询优化的专用数据结构。由于我们可以在 server 本身或通过 minion 子系统在线重建索引，因此我们可以在 Pinot 用户无感知的情况下部署新的索引类型和编码。

### 4.2 索引与物理记录存储

与 Druid 类似，我们支持基于 bitmap 的倒排索引。不过，我们观察到，按主列和次列对数据做物理重排，可以显著加速某些查询。
...
无法向量化时，我们观察到：退回到迭代器式扫描，扫描列中的一个范围，通常比在大型 bitmap 索引上执行 bitmap 运算更快。因此，构造物理过滤算子时，先执行作用于物理有序列的算子，并把得到的列范围传给后续算子。后续算子只需评估列的一部分，性能因而大幅提高。
...
在 Pinot 中，对于未分区表，我们会预生成路由表。路由表把 server 映射到执行查询时它应处理的一组 segment。形式化地，设 segment 全集为 $U = \lbrace{}S_1, S_2, \ldots, S_n\rbrace{}$，集合族 $A$ 含有 $m$ 个集合，分别表示各 server 被分配的 segment。生成一个路由表条目，就是从 $A$ 的元素中选择若干子集，使这些子集的并集恰好覆盖 $U$。
...
均衡策略适用于小型和中型集群，但在更大集群上很快就不再实用。直观地说，我们可以推测：集群越大，至少一台主机不可用或因异常拖慢处理的概率就越高。[24] 的图 14 和图 16 也用实验表明其他系统中同样存在这种拖尾节点（straggler）。

因此，Pinot 为大型集群提供专用路由策略，尽量减少单次查询联系的主机数。这样可以降低任一异常主机的不利影响，并减少大集群的尾延迟。

寻找 $A$ 的最小子集使其覆盖 $U$ 是 NP-hard 问题。我们实现了一种随机贪心策略，求得近似最小的子集，同时让所有 server 的负载保持均衡。算法 1 和算法 2 分别说明大型集群路由表的生成与筛选：系统先随机选一组 server，继续加入 server 直到完全覆盖 $U$，再把 segment 尽可能均匀地分给所选 server。每张候选路由表都用指标衡量适应度；实测表明，各 server 所分 segment 数量的方差效果很好。最终只保留指标最低的一批路由表。
...
输入：
  ...
  I：分配给该表的实例集合
  T：每次查询的目标 server 数
  Sorphan：尚无任何 server 与之关联的 segment 集合
  Iused：当前使用的实例集合
  IS：instance -> 该 instance 服务的 segment 列表
  SI：segment -> 服务该 segment 的 instance 列表
  Qsi：按候选实例列表长度升序排列的 segment 优先队列

过程 GenerateRoutingTable
  Sorphan <- S  # 初始时，所有 segment 都没有关联实例
  Iused <- 空集

  if length(I) <= T then
    Iused <- I  # 若实例数少于 T，则全部使用
    Sorphan <- 空集
  else
    while length(Iused) <= T do  # 随机选取 T 个实例
  ...
  while Sorphan 非空 do  # 添加 server，为尚无 server 的 segment 提供服务
  ...
  for Scurrent in S do
    Iseg <- SI.get(Scurrent) intersect Iused  # 取得该 segment 的实例
    Qsi.put(Scurrent, Iseg)
  end for

  R <- 空映射
  while Qsi 非空 do  # 按副本实例数升序遍历 segment
...
> **译者注：** 算法 1 的源伪代码条件写作 `length(I) <= T`，紧随其后的原文注释则写作“If there are less than T instances, use all instances”。二者的边界条件在原文中不一致，此处分别按原文保留。

算法 1 生成候选路由表后，系统再用下述过程保留指标最优的一批候选。
...
过程 FilterRoutingTables
  ...
  for i <- C..G do
    ...
    M <- ComputeMetric(R)
    (Rtop, Mtop) <- H.top()
    if M <= Mtop then  # 新路由表是否优于当前最差者？
      H.pop()  # 是：移除最差者并加入新路由表
      H.put(R, M)
    end if
  end for
end procedure
...
在 LinkedIn，Pinot 运行于 3,000 多台分布在不同地理位置的主机上，服务 1,500 多张表、100 多万个 segment；不计数据副本，总压缩数据量接近 30 TiB。所有数据中心合计的生产查询率超过每秒 50,000 次。下面我们总结在 LinkedIn 大规模运行 Pinot 得到的实践经验。

### 5.1 用例类型

在 LinkedIn，我们观察到 Pinot 用户大致分为两类：
...
LinkedIn 以服务模式运行 Pinot：开发用户功能的团队像使用其他服务一样使用 Pinot，Pinot 团队则持续提供支持并负责日常运行。随着使用 Pinot 的团队数量增长，如果为每个团队配备专人支持和调优，人员需求也会持续增长。因此，我们在设计 Pinot 时尽量支持自助服务。

例如，Pinot 允许在线修改 schema、添加新列而不停机。向既有 schema 加入新列时，所有已有 segment 都会自动以默认值补上该列，并在几分钟内可供查询。我们还会持续解析查询日志和执行统计，自动为确实有收益的列添加倒排索引。

跨多个数据中心和环境（测试、生产）复制表配置一直是一个常见难题。目前，我们的方案是把表配置保存在源码控制中，再通过 Pinot REST API 持续同步。这样让我们既能保留完整的变更审计轨迹，也能对全部 schema、索引和配置变更复用搜索、校验与代码审查工具。

## 6. 性能

本节我们在从低吞吐到高吞吐的多种场景下评测 Pinot。由于 Druid 与 Pinot 架构相近，我们也会在这些场景中比较 Druid 与 Pinot。为保证评测真实，数据集和查询都取自生产系统：每个场景使用该场景的完整数据集，并从生产查询中抽样出数万条不同查询，以模拟生产环境。由于很难为两套系统建立可重复的共同环境，Pinot 与 Druid 均关闭了实时摄取。
...
我们评测 Pinot 与 Druid 的第一个场景，复现了 LinkedIn 对多维关键业务指标做即席报告和异常检测的环境。我们的异常检测系统包含用户可见组件，因此查询集既有监控所用的自动生成查询，也有用户对检测到的异常做根因分析时发出的即席查询。Pinot 必须同时为自动查询提供高吞吐，也为交互式用户查询提供低延迟。查询会聚合指标，并根据具体下钻请求包含数量不定的过滤谓词和分组子句。
...
我们介绍了一套生产级系统的架构及其规模化经验。Pinot 在严苛的 Web 环境中每秒服务数万次查询。我们还表明，同一套系统能够处理大型网站常见的多类分析查询。

最后，我们还使用来自大型职业社交网络的生产数据与查询，将 Pinot 与相近系统 Druid 比较，并评估 Pinot 中不同索引技术和优化的影响。
...
- [4] Marcel Kornacker, Alexander Behm, Victor Bittorf, Taras Bobrovytsky, Casey Ching, Alan Choi, Justin Erickson, Martin Grund, Daniel Hecht, Matthew Jacobs, Ishaan Joshi, Lenni Kuff, Dileep Kumar, Alex Leblang, Nong Li, Ippokratis Pandis, Henry Robinson, David Rorke, Silvius Rus, John Russell, Dimitris Tsirogiannis, Skye Wanderman-Milne, and Michael Yoder. 2015. Impala: A modern, open-source SQL engine for Hadoop. In *Proceedings of the 7th Biennial Conference on Innovative Data Systems Research*.（目标 PDF 的该署名行排版已损坏；此处依据同仓所收录 Impala 论文 `source.pdf` 首页的署名列表校正。）
...
- [8] C. Chen. 2005. Top 10 unsolved information visualization problems. *IEEE Computer Graphics and Applications* 25(4) (July–Aug. 2005), 12–16. https://doi.org/10.1109/MCG.2005.91
