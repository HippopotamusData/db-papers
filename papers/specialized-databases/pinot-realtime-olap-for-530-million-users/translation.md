...
对于未分区表，Pinot 会预生成路由表。路由表把 server 映射到执行查询时它应处理的一组 segment。形式化地，设 segment 全集为 $U = \lbrace{}S_1, S_2, \ldots, S_n\rbrace{}$，集合族 $A$ 含有 $m$ 个集合，分别表示各 server 被分配的 segment。生成一个路由表条目，就是从 $A$ 的元素中选择若干子集，使这些子集的并集恰好覆盖 $U$。
