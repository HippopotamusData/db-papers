...
![图 1](assets/gorilla-fig01-ods-overview.png)

**图 1：** ODS 监控与告警系统的高层概览；Gorilla 充当最近 26 小时时序数据的直写式缓存。
...
![图 2](assets/gorilla-fig02-compression-overview.png)

**图 2：** 完整压缩算法示意。在此示例中，48 字节的数值与时间戳被压缩到略少于 21 字节，即 167 位。
...
> **译注：**原文此处用“从前一个时间戳值中减去当前值”描述相减方向，但同页步骤 2(a) 的公式及示例采用“当前差值减前一个差值”。本段按该公式和示例解释二阶差分。
...
![图 3](assets/gorilla-fig03-timestamp-buckets.png)

**图 3：** 不同取值范围桶中的时间戳压缩分布；样本来自 Gorilla 中 440,000 个真实时间戳。
...
![图 4](assets/gorilla-fig04-xor-patterns.png)

**图 4：** 与前一个值做 XOR 后，结果往往带有前导零和尾随零；对许多时序而言，非零元素集中在相近位置。

![图 5](assets/gorilla-fig05-value-buckets.png)

**图 5：** 不同 XOR 桶中压缩数值的分布；样本来自 Gorilla 中 160 万个真实数值。
...
![图 6](assets/gorilla-fig06-window-size.png)

**图 6：** 压缩桶从 0（不压缩）变化到 240 分钟时，每个 ODS 数据点平均占用的字节数。对我们的数据集而言，桶大小超过两小时不会带来显著的额外压缩收益。结果覆盖整个生产 Gorilla 数据集（约 20 亿条时序）。
...
![图 7](assets/gorilla-fig07-memory-structures.png)

**图 7：** Gorilla 的内存数据结构。查询时，首先在 a）检查 TSmap 指针。若 b）指针为空，表示此 Gorilla 主机不拥有该分片；若非空，则在 c）对 TSmap 加读锁，在无序映射中找到并复制时序结构（TS）的指针。此时可以释放两个读写锁。接着在 d）锁住 TS 自旋锁，直接复制查询时间范围内的数据。
...
![图 8](assets/gorilla-fig08-query-latency.png)

**图 8：** 使用不同 ODS TSDB 方案时的查询总延迟分解。与 HBase 相比，根据查询规模不同，Gorilla 带来 73 至 350 倍提升。图中还包含另两种方案的初步结果：用闪存保存 26 小时以上数据的 Gorilla，以及带 ODS 缓存的 HBase。
...
![图 9](assets/gorilla-fig09-query-volume.png)

**图 9：** Gorilla 引入后查询总量的增长；它降低了数据探索和开发新分析工具的门槛。
...
![图 10](assets/gorilla-fig10-root-cause.png)

**图 10：** 搜索全站错误率上升的根因时，Gorilla 的时序相关性分析找到了时间上相关的异常事件，即复制新发布的二进制文件时，内存用量出现下降。
