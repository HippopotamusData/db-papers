...
**以 TopK 临时索引消除分歧。** ANNS 系统仅暴露 TopK 接口；当前方案先取 K 个向量并按距离排序，创建保持单调性的临时索引，再执行数据库查询 [76, 80, 86]。但为 TopK 加过滤、向量范围过滤等查询预测恰当的 $\widetilde K$ 很困难，甚至可能做不到。保守的大 K [80, 86] 或不断尝试不同 K [76] 都产生过多数据访问和计算。
...
**通用性。** 我们考察的 ANN Benchmarks [2] 主流向量索引都包含索引遍历、终止检查、单调性检查和保存当前 K 个最近向量的优先队列。在 ANNS 索引中，单调性检查通常是终止检查的必要条件。不同实现的单调性检查都满足定义 1；下面我们说明代表性索引如何设置相关参数：
...
**通用终止条件。** VBASE 在原终止条件上增加松弛单调性检查。因为索引保证该性质，公式 3 可化为检查 $M_q^s\gt{}R_q$，这里假设步骤 s 之后的松弛单调性检查均为真。只有原终止条件和此检查都通过，查询才停止；对传统索引，后者恒真，退化为原迭代模型。下面我们说明 VBASE 的终止条件如何用于向量查询中的带限制 OrderBy 和距离范围过滤。
...
统一引擎也支持向量 Join。虽然这不是在线查询，但向量 Join 在文档自动标注等场景中很有用。文档自动标注 [26, 72] 可把小型已标注文档集与大型未标注文档集按嵌入距离连接，为每个未标注文档找到最近标签。VBASE 用范围过滤驱动索引嵌套循环连接；既有方案只能全表扫描，实验中 VBASE 快 7,000 倍。
...
- **HNSW。** 该索引从上层粗粒度邻域图按 best-first 方式导航到下层细粒度图。上层图中找到的近似最近点是下层图的入口点。在 VBASE 中，我们移除 TopK 结果队列，只保留遍历所需状态：已访问位图、当前向量、后续候选向量。`Open` 搜索高层图，`Next` 返回当前最近的未访问节点、记录它并展开其邻居，`Close` 清理状态。总体上，我们只修改了不到 200 行代码来集成 HNSW。
