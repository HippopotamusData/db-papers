...
$$
R_q=\operatorname{Max}\left(\operatorname{TopE}\left(\{\operatorname{Distance}(q,v_j)\mid j\in[1,s-1]\}\right)\right). \tag{1}
$$
...
$$
M_q^s=\operatorname{Median}\left(\{\operatorname{Distance}(q,v_i)\mid i\in[s-w+1,s]\}\right). \tag{2}
$$
...
**通用终止条件。** VBASE 在原终止条件上增加松弛单调性检查。因为索引保证该性质，公式 3 可化为检查 $M_q^s>R_q$。只有原终止条件和此检查都通过，查询才停止；对传统索引，后者恒真，退化为原迭代模型。
...
$$
r_1=\operatorname{Limit}_K(\operatorname{Filter}(\operatorname{Limit}_{K'}(\operatorname{Sort}(R_1)))). \tag{4}
$$
...
$$
\begin{aligned}
r_1&=\operatorname{Limit}_{K''}(\operatorname{Filter}(\operatorname{Sort}(R_1))),\\
K''&=\min(K,K'\times\text{filter\_selectivity}).
\end{aligned} \tag{5}
$$

若能预测最优 $K'=\widetilde K=K/\text{filter\_selectivity}$，执行恰得 K 条结果，公式 5 化为：

$$
r_1=\operatorname{Limit}_K(\operatorname{Filter}(\operatorname{Sort}(R_1))). \tag{6}
$$

VBASE 用同一索引遍历 $R_2$：

$$
r_2=\operatorname{Limit}_K(\operatorname{Sort}(\operatorname{Filter}(R_2))). \tag{7}
$$

两种系统使用相同向量索引、遍历算法和松弛单调性终止检查，因此 $R_1=R_2$[^1]；Filter 与 Sort 可交换，故 $r_1=r_2$。若 TopK 系统猜得过小，会结果不足、准确率低；猜得过大或反复试探，则浪费遍历。VBASE 在执行中确定 $\widetilde K\times\text{filter\_selectivity}=K$，兼得准确率和性能。

**范围过滤。** 仅 TopK 方案可写为：

$$
r_1=\operatorname{Filter}(\operatorname{Limit}_{K'}(\operatorname{Sort}(R_1))). \tag{8}
$$
...
\begin{aligned}
r_1&=\operatorname{Limit}_{K''}(\operatorname{Filter}(\operatorname{Sort}(R_1))),\\
K''&=K'\times\text{filter\_selectivity}.
\end{aligned} \tag{9}
$$

若查询应产生 T 个向量且 $K'=\widetilde K=T/\text{filter\_selectivity}$，则：

$$
r_1=\operatorname{Limit}_T(\operatorname{Filter}(\operatorname{Sort}(R_1)))=\operatorname{Filter}(\operatorname{Sort}(R_1)). \tag{10}
$$

VBASE 的结果为：

$$
r_2=\operatorname{Filter}(R_2). \tag{11}
$$
...
$$
\operatorname{Sel}_{sample}(q)\approx\operatorname{Sel}_{full\_data}(q).
$$

实验中采样率 0.001 通常使 q-error 小于 1.1，额外延迟不足 1 ms。

**索引扫描成本。** 每个遍历步骤包含磁盘读取 $t_{IO}$ 与距离计算：

索引扫描成本分为启动成本与遍历成本。启动阶段负责在返回首条向量前定位目标附近区域；遍历阶段迭代访问匹配元组。向量索引的单步工作既可能从磁盘读取索引块，也必须计算向量距离，因此不能沿用标量索引的固定单步常数。

$$
C_{step}=t_v(\mathrm{dim})+t_{IO}.
$$

对 IVFFlat、SPANN 等分区索引：

$$
C_p=N_cC_{step}+\max\left(\left\lceil\operatorname{Sel}(q)N/N_p\right\rceil,m\right)N_pC_{step},
$$
...
$$
C_g=N_{start}C_{step}+\max(\operatorname{Sel}(q)N,N_E)R_{iter}C_{step},
$$

其中 $N_{start}$ 是 `Open` 遍历上层图的步数，$N_E$ 是满足松弛单调性所需步数，$R_{iter}$ 是每个 `Next` 步骤调用距离函数的平均次数。这些量依赖索引超参数与数据分布，可通过采样自动估计并嵌入数据库 `Analyze`。

分区索引公式的第一项是比较 $N_c$ 个质心的启动成本；第二项估计为覆盖过滤结果需要扫描的分区数，但绝不能少于满足松弛单调性所需的 m 个分区。图索引公式同理：先付出上层导航的 $N_{start}$ 步，再在细粒度图中遍历选择率预期所需的向量数；即使预期结果很少，也至少执行 $N_E$ 步以获得可靠终止信号。`Analyze` 通过采样记录这些统计量，让 PostgreSQL 能把向量路径与 B-tree、顺序扫描等传统路径放在同一成本空间比较。
...
$$
W_i=\left\lceil n_2\times\frac{1/avg_i}{\sum_{j=1}^{m}1/avg_j}\right\rceil,
$$
