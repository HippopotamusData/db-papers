...
$$
R_q=\mathrm{Max}\left(\mathrm{TopE}\left(\{\mathrm{Distance}(q,v_j)\mid j\in[1,s-1]\}\right)\right). \tag{1}
$$
...
$$
M_q^s=\mathrm{Median}\left(\{\mathrm{Distance}(q,v_i)\mid i\in[s-w+1,s]\}\right). \tag{2}
$$
...
$$
r_1=\mathrm{Limit}_K(\mathrm{Filter}(\mathrm{Limit}_{K'}(\mathrm{Sort}(R_1)))). \tag{4}
$$
...
$$
r_1=\mathrm{Limit}_K(\mathrm{Filter}(\mathrm{Sort}(R_1))). \tag{6}
$$

VBASE 用同一索引遍历 $R_2$：

$$
r_2=\mathrm{Limit}_K(\mathrm{Sort}(\mathrm{Filter}(R_2))). \tag{7}
$$
...
$$
r_1=\mathrm{Limit}_T(\mathrm{Filter}(\mathrm{Sort}(R_1)))=\mathrm{Filter}(\mathrm{Sort}(R_1)). \tag{10}
$$
...
$$
\mathrm{Sel}_{sample}(q)\approx\mathrm{Sel}_{full\_data}(q).
$$
...
$$
C_p=N_cC_{step}+\max\left(\left\lceil\mathrm{Sel}(q)N/N_p\right\rceil,m\right)N_pC_{step},
$$
...
$$
C_g=N_{start}C_{step}+\max(\mathrm{Sel}(q)N,N_E)R_{iter}C_{step},
$$
