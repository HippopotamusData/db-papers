...
默认页数上限、单篇例外和 skipped 原因集中在 `config/policy.yaml`。单篇页数例外必须包含高于默认值的 `max_source_pages` 和用户授权依据 `authorization`；跳过原因使用代码定义的受控值。验收账本只保存当前文件哈希、一个受控 `review_action` 和机械候选确需人工豁免时的受控 `waivers`。源文或译文发生实质变化时先迁移到 `draft`，旧账本哈希不得继续支持 `translated`。
