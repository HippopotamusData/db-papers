...
| 代码正确，但选型解释把 `A and B` 改成“A 或 B”，或把前提下的例子写成独立适用条件 | 按改变方法适用边界的实质错误处理；连同优化必要条件和结论段回源核对，不能因代码正确降级 |
| 原文写“完整物化一个输入”，译文只说“控制流不灵活”，又增添无来源的量化解释 | 分别记录具体约束漏译与补写，双向核对来源，不以推断合理代替忠实翻译 |
| 用户要求重大问题优先、小问题暂记且仅在无重大问题时修复 | 先完成约定抽查；存在重大问题时交付分级证据及已授权规范变更，不自行触发小问题修复 |
