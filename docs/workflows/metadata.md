...
| `topics` | string list | 一个或多个 `config/taxonomy.yaml` 受控主题 |
| `reading_status` | enum | `unavailable`、`source_only`、`draft`、`translated` 或 `skipped` |

论文 ID 和一级领域由目录表达；原文、译文和资源可用性由 `source.pdf`、`translation.md`、`assets/` 表达。目标语言、文件名和页数上限属于 `config/project.yaml`，单篇例外与 skipped 原因属于 `config/paper-policy.yaml`，验收哈希和风险处置属于 `config/acceptance.yaml`。这些信息不得在 `paper.yaml` 中重复。
...
标题、作者和年份只来自论文首页或可靠原文入口；证据冲突且无法解决时保持未知。分类依据核心研究问题和主要贡献，不依据标题关键词、来源清单小节、处理批次或工作优先级。
