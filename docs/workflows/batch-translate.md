...
- **根代理**：维护队列和基线，检查写入范围，串行执行 accept、catalog、检查点和最终集成。只有根代理可以修改共享配置和生成目录或执行 Git 写操作。
...
根代理在任务状态中记录 paper ID、译者、审阅者，以及 `queued`、`translating`、`draft-ready`、`reviewing`、`accepted`、`blocked` 之一。版本库真相仍以文件和验收账本为准。
...
8. 若已获本地检查点授权且本轮全绿，根代理创建一个只包含本轮成果的 checkpoint commit；不在轮次边界合并 `main`。
...
报告批次基线、处理和交付模式、轮次数、accepted/draft/blocked paper ID、译者与审阅者分工、检查点提交、单篇 warning、未解决阅读缺口、实际门禁，以及最终分支是否已提交、合并或推送。只有账本哈希匹配且状态为 `translated` 的论文计入验收完成数。
