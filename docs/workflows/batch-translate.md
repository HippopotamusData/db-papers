...
- **审阅子代理**：不得审阅自己的译文；读取 review workflow 与 translation policy；只核对或修复当前论文，不执行 accept、Git 写操作或共享文件更新。
...
从 checkpoint、队列、`paper.yaml`、工作区差异和验收账本恢复；不从其他任务复制 `acceptance.yaml`，不手工合并哈希。单篇失败可用同一 paper ID 重新分派，不阻塞其他论文。
